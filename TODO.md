...
> - **Now mechanical (reconciled — was mis-filed as GM-narrated):** the *enter-the-area-mid-move* re-trigger for radius damaging AoEs shipped v2.567.0–v2.568.0 (`_trigger_persistent_aoe_on_move` in the move endpoint **and** `_force_move`, `_tick_persistent_aoe_start_of_turn` on turn advance, per-turn dedupe via `_aoe_enter_triggered`, save-for-half + caster/affects filter). A creature takes a persistent radius AoE's save+damage whether it walks in, is forced in, or starts its turn there. Cube/cone/line shapes are N/A (no SRD damaging AoE uses them). See [`aoe-enter-trigger.md`](plans/aoe-enter-trigger.md). Harness: `test_aoe_enter_trigger.py` (9).
> - **Genuinely still GM-narrated** (confirmed no substrate): Forbiddance's 5d10 ward damage + 40,000-sq-ft geometry (flag-buff), the remote scrying **views** (Clairvoyance / Arcane Eye / Project Image — no camera surface), and the object **triggers** (Magic Mouth / Illusory Script — no trigger engine). These remain counted done under the rubric; mechanizing them needs Maps-2.0 geometry / a remote-sensor surface.
...
1. ✅ **DONE — spatial/object/scrying resolution** (mechanical where SimpleVTT models it, GM-narrated otherwise; see the verify-pass note above). Already **mechanical**: Globe of Invulnerability spell-block (`_target_globe_blocks_spell`, geometry-aware), Antilife Shell movement-barrier (`_move_crosses_antilife_shell`), emanation start-of-turn damage (`_tick_auras` — Spirit Guardians / Holy Aura / aura family), and **the persistent-AoE enter-trigger** — a creature that walks / is forced into a radius damaging AoE (Spirit Guardians / Moonbeam / Cloudkill / Insect Plague …) takes its save+damage, shipped v2.567.0–v2.568.0 (`_trigger_persistent_aoe_on_move` + `_tick_persistent_aoe_start_of_turn`, [`aoe-enter-trigger.md`](plans/aoe-enter-trigger.md), `test_aoe_enter_trigger.py`). Still **GM-narrated** (no substrate): Forbiddance's ward damage + 40k-sq-ft geometry, the remote scrying views (Clairvoyance / Arcane Eye / Project Image), and object triggers (Magic Mouth / Illusory Script). **Counted complete.** Mechanizing the GM-narrated slice (Maps 2.0 geometry + a remote-sensor surface) is an **optional future enhancement**, not missing SRD coverage.
...
### Unified Monster Sheet in Initiative Tracker (reuse character sheet UI) — ✅ SHIPPED (reconciled 2026-07-14)
**Done — shipped v2.3.10 (route) + v2.99.7 (click-through wiring); verified against the code + `tests/harness/test_monster_sheet_init.py` (3 tests, green).** The `monster_template_sheet_page` route (`GET /campaign/{cid}/monster-template/{tid}/sheet`, `tabletop_routes.py:~125927`) renders the **full interactive `sheet_dnd5e.html`** for a monster `TokenTemplate` — the block is projected via `_monster_template_to_sheet` (abilities/mods/skills/attacks/spells), GM-only, `can_edit=False` (read-mostly, save buttons suppressed). `wireDnd5eRollButtons` gives PC-parity click-to-roll: ability/save/skill clicks route through `/roll` with monster attribution (`actor_name` + `no_char_attribution`, so the roll log shows the monster name, not the GM's first PC), and `.atk-strike` Strike attacks route through `/npc_attack`. Live combatant HP overlays via `?combatant_id=`. Both entry points are wired: the init-tracker "📋 Sheet" button (`tabletop.js:~10080`) and the map-token click (`_openSheetForToken`, `tabletop.js:~4399`). The three v2.99.7 JS globals (`IS_MONSTER_SHEET` / `MONSTER_NAME` / `MONSTER_COMBATANT_ID`) drive the routing. **Remaining polish (optional):** first-class Legendary/Lair-action buttons on the sheet (currently narrative text). ✅ **Row-click access shipped v2.1044.1** ("The Second Look") — but deliberately *not* as filed. The filed note said to take the row's **primary** click; that click already toggles the inline mini-sheet, which monsters normally have (`_hydrateMonsterCard` puts them in the card pool), so taking it would have removed a faster affordance and broken symmetry with PC rows. Shipped instead: **double-click** a monster row opens the stat-block drawer (matching the map token), with the dblclick handler undoing the toggle the first click applied so expand state is preserved; **plus** orphan rows — whose expanded card is only the "Template missing…" placeholder — open the sheet on the **first** click, since expanding them is a dead end. Harness: `tests/harness_ui/test_init_monster_row_sheet.py` (4).

---

## Combat

_Advantage & Disadvantage Tracking, Death Saving Throws, and Combat 2.0 Action Economy all shipped — see [`TODONE.md`](TODONE.md) → Combat (reconciled v2.745.1). **NPC death saves (Phase 4) complete — 4a v2.1053.0 "The Boss's Last Breath" + 4b v2.1054.0 "The Rattling Dice":** a boss flagged via `POST /set_npc_death_saves` enters *dying* at 0 HP and **auto-rolls** its death save each turn (`_npc_death_save_transition` + `_npc_death_save_roll`), broadcasting `npc_death_save`; harness `test_npc_death_saves.py` (9). Only remaining death-saves slice, tracked in [`death-saves.md`](docs/plans/death-saves.md): **Phase 3c** (stable-at-0 countdown — session-time-blocked, shared PC+NPC)._

### Summon Spells — Creature Picker + Auto-Control + Init Placement ✅ (shipped v2.746.0–v2.751.0)
Shipped end-to-end — see [`TODONE.md`](TODONE.md) → Combat. Creature picker for all six conjure spells + Find Familiar (`/summon-options` + `window.openSummonPicker`), auto-control (summon token `controller_user_id`), and auto-init behind the caster (`_summon_initiative_for_body`). Design record: [`movement-and-summons.md`](docs/plans/movement-and-summons.md).
...
> **Partial:** **Combat movement locking** shipped (the `/token/{id}/move` speed cap — `over_speed_cap` 409 + `_effective_speed_walk`, integrated with the action economy). **Fog of war** shipped at a token-visibility level (v2.64.0 F2 per-user hidden filter + the v2.704.0+ Vision & Light line-of-sight overlay). **Walls & doors — in progress:** the storage substrate shipped v2.752.0 (`Map.walls` JSON + `GET/PUT /map/{id}/walls` + `walls_update` broadcast + `GET /active-map`); **server-side vision occlusion** shipped v2.753.0 (`_visibility_between` returns `unseen`/`blocked_by_wall` when a solid wall or closed door crosses the sight line; an open door passes — `_walls_block_sight` + `_segments_intersect`). **GM wall-editor UI** shipped v2.754.0 (the 🧱 Walls toggle + the `#wall-overlay` SVG: click two points to add a wall, click a wall to delete, "door" box for openable doors; renders walls/doors for all clients, live over `walls_update`). **Door open/close toggle** shipped v2.755.0 (`POST /map/{id}/door/{id}/toggle`, any member; clicking a door in play mode flips it + updates LOS live). **Clickable map items / hotspots** shipped v2.756.0 (`Map.hotspots` JSON + `GET/PUT /map/{id}/hotspots` + the `📍 Spots` GM editor: click to place a labelled description marker, click a marker in play mode for the popup). **Wall-shadow rendering** shipped v2.757.0 (the client lighting overlay casts per-light wall shadows — a point behind a wall stays veiled). **Roll-prompt hotspots** shipped v2.758.0 (a hotspot can carry a dice expression; its popup shows a 🎲 Roll button that fires through `/roll`). **Segment drag-to-move** shipped v2.759.0 (drag a wall/door in edit mode to reposition it; a tap deletes). ✅ **Maps 2.0 is complete** — see [`TODONE.md`](TODONE.md).
...
Shipped as the Vision & Light engine — light emitters (radius/colour/falloff), magical Darkness/Daylight/Fog, per-token vision senses (darkvision/blindsight/truesight), ambient levels, and the client dynamic-lighting overlay. See [`TODONE.md`](TODONE.md) → Maps & Map Editor + [`vision-and-light.md`](docs/plans/vision-and-light.md). ✅ **Fully closed (reconciled v2.1043.2)** — the two items this line long carried as *"not done"* both shipped and were never struck: **wall-occlusion shadows** at v2.757.0 (the client lighting overlay casts per-light wall shadows; `tabletop.js:~233` punches map lights into the overlay "with wall shadows", and the Maps 2.0 bullet above already recorded the ship) and **fire flicker animation** at v2.849.0 (the colored torch-flicker glow canvas + rAF loop in `tabletop.html:~2565/3643`, extended v2.852.0 to carried token lights and v2.935.0 to a per-light `flicker` speed 0–4).
...
✅ **The access gate closed at v2.1046.0 "The Locked Reading Room"** — handout media (images *and* documents) is no longer public static bytes. `serve_handout_media` is registered ahead of the `/static` mount and authorizes every request through the same `_can_see_handout` rule, so hiding a handout now revokes access to its media and a leaked URL is useless without a session. The URL shape was kept deliberately (the bytes stay on the `uploads_data` volume, so backups + admin-center storage accounting keep working, and every pre-existing handout is retroactively protected). Harness: `test_handout_media_gate.py` (12).

*Remaining tails:* (1) **handout audio** — still filed on [`notes-and-handouts.md`](docs/plans/notes-and-handouts.md); (2) **non-PDF document types** — deliberately out of scope, since the feature's promise is inline rendering with no download and PDF is the only format every browser does that with natively; ✅ (3) **the same gate for other uploaded media — shipped v2.1047.0 "The Closed Stacks".** `serve_upload` covers `maps` / `thumbnails` / `tokens` / `portraits` / `token_templates` / `encounter_bg` / `audio` with a campaign-membership gate (handouts keep their stricter per-player reveal rules). Resolution is by exact URL match across every column that can hold one — not per-bucket, since a `maps/` file can be referenced by `Campaign.thumbnail_url`. Orphans fail closed. Harness: `test_media_gate.py` (11).
...
### SRD Rules in Full Text — ✅ Phases 1–3 shipped (complete as of v2.1032.0)
...
- 🟠 **Phase 3 (in progress)** — contextual links from the character sheet / encounter panels (click a condition name → its reference entry); the non-content SRD rules sections (actions in combat, resting, cover, etc. — not in the current per-type JSON). **Substrate shipped v2.1025.0 "The Marginal Gloss":** `GET /api/reference/entry?type=&slug=` returns one shipped-SRD record (offline, SRD-only, six player-safe types), the reusable read surface for inline rule popovers. Harness: `test_srd_reference.py` (+4). **Consumers wired:** (1) v2.1026.0 "The Severed Cord" — the sheet's condition-chip popover now reads from the offline `/api/reference/entry` tier instead of the network-capable `/api/open5e/conditions` proxy; (2) v2.1027.0 "The Table Ruling" — the initiative tracker's `⚠ Conditions` pill (PC + NPC mini-sheet cards) is clickable, opening a popover with each active condition's full SRD rule text. (3) v2.1029.0 "The Combat Codex" — the **non-content SRD rules sections** started: a new `rules` reference type + `Rule` model ships 16 combat rules (actions in combat, cover, grappling, opportunity attacks, difficult terrain, being prone, …) as rephrased SRD 5.1 entries under `app/data/local/dnd5e/rules/`, flowing through the existing search / entry / pin plumbing. *(Spell/feat name-links were found redundant — the sheet spell rows, feat cards, and roll-log spell card all already show their full description inline.)* v2.1030.0 "The Wanderer's Almanac" extended the `rules` type with 10 Adventuring + Environment rules (short/long rest, travel pace, carrying capacity, jumping, climbing/swimming, vision & light, falling, suffocating, food & water) — 26 rules total. v2.1031.0 "The Salvager's Ledger" added the final filed batch — 8 more covering **Objects** (interacting with objects, breaking objects + the material-AC table, improvised weapons), situational combat (mounted combat, ranged attacks in close combat, two-weapon fighting), underwater combat, and madness — **34 rules total, and the plan's filed rules-section list is now closed.** ✅ **Phase 3 complete (v2.1032.0 "The Rules Lawyer's Thumb")** — the last slice, contextual rules links from the tabletop, shipped: a 📖 chip in the initiative tracker's action-economy strip (Act/Bns/Rxn/Mov) opens the SRD "actions in combat" rules in the same popover the ⚠ Conditions pill uses. The enabler was generalizing `_openCondRefPopover` / `_condRefEntry` from conditions-only to type-aware (`data-ref-type` + `data-ref-slugs`, cache keyed `type/slug`); the legacy `data-cond-slugs` path still works and is regression-tested in the browser. Harness: `test_econ_rule_chip.py` (3) + `test_econ_rule_chip_popover.py` (4 UI).
...
### Frosted-glass treatment across the whole tabletop interface ✅ (shipped)
The frosted-glass look is now applied across the tabletop — see [`TODONE.md`](TODONE.md). The v2.759.2 sweep confirmed/closed the last per-surface gaps (the ruler + AoE-picker hints). All backings use theme-coherent `color-mix` + `var(--accent)` so they adapt across themes. _Optional ops follow-up if a long session feels heavy: gate the blur behind a "low-detail" toggle (compositor-layer count)._
...
- 🟢 **P3 — Per-feature Phase-2 finishers (reconciled 2026-07-14 — most already shipped; verified against `app/routes/tabletop_routes.py`):**
    ...
    - 🟠 **AP (Assassinate) Phase 3** — the /attack auto-crit-vs-surprised + advantage-vs-not-acted is mechanized (v2.131.0–v2.132.0). ✅ **Assassinate Phase 3 complete.** **Surprise-state model shipped v2.1056.0 "The Unseen Blade":** a server-side `surprised` combatant flag (set via `POST /set_surprised`, GM), read by `/attack` (`_target_is_surprised`) for the auto-crit, auto-cleared on the target's first turn — the client `target_surprised` flag is no longer required. **Auto-detection shipped v2.1057.0 "The Silent Approach":** `POST /detect_surprise` rolls each ambusher's Stealth vs every defender's passive Perception (RAW PHB p.189) and sets the flag on surprised defenders. Harness: `test_set_surprised.py` (6) + `test_detect_surprise.py` (4). Loop closed end-to-end: name ambushers → compute surprise → Assassin auto-crits → clears on the target's turn.
- ✅ **DONE (v2.1031.2) — Classifier rerun for `docs/automation-coverage.md`.** *(Struck v2.1043.2 — this line claimed the counts "still pin v2.99.460" long after the rerun landed; [`BUGS.md`](BUGS.md) B5 had already recorded it FIXED.)* The rerun refreshed the "Full classification" counts **289/35/8 → 306/31/9** (endpoint total 332 → 346) and fixed a real classifier blind spot: `_MUTATORS` in `scripts/classify_feature_endpoints.py` listed `_install_buff` but not `_install_buff_on_combatant_id`, so every endpoint buffing a *target* combatant was mis-tagged announce-only. See `docs/automation-coverage.md:31`.
...
- [`advantage-disadvantage.md`](docs/plans/advantage-disadvantage.md) — ✅ **effectively complete (v2.1001.0)**: Phases 1, 2a–2f (v2.2.0–v2.157.0), 4a/4b (v2.252.0–v2.253.0), and **Phase 3 (positional prone edge) shipped v2.1001.0** once Maps 2.0 dissolved the blocker — `_target_prone_positional_edge` rides `_combatant_token` + `_distance_ft_between_points` across `/attack` + `/npc_attack`. **Both cloak tails now closed:** Cloak of Elvenkind perception-disadvantage half shipped v2.1055.0 (`imposes_perception_disadvantage_to_see` + `/roll` `vs_character_id`); Cloak of Displacement suppress-after-damage was already shipped v2.645.0 (`cloak_disp_suppressed_round`) — the "still filed" note was stale, reconciled v2.1055.0. Only remaining tail: Elven Accuracy (out of scope by design).
