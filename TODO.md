...
> **Re-verified 2026-06-22 (v2.565.1) — no change.** Denominators confirmed directly against `app/data/local/dnd5e/`: **319 spells, 322 monsters, 9 races, 15 conditions, 294 equipment (239 magic items), 12 class-feature + 13 subclass-feature files (222 per-row entries)**. A `git diff` of `app/data/local/dnd5e/` **and** `app/routes/tabletop_routes.py` from the v2.553.1 audit (78ea2cc3) to HEAD is **empty** — the entire interim (v2.554.0–v2.565.1) was the Notes & Handouts feature, which touches no SRD content or spell engine. So every number below still stands: **Spells ~93%, Conditions ~92%, Races/Monsters/Class-features/Magic-items ~100%, overall ~98–99%.** The remaining gap is unchanged: deliberately GM-narrated spatial/object/scrying resolution + the filed PC-save-or-suck install hook (gaps #1–#2), both future-3.x engine depth rather than un-wired surfaces.
