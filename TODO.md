...
>
> **Scoring rubric change (2026-06-22): GM-narrated resolution counts as completed.** In a VTT the GM is the rules authority, so a spell/condition/feature whose resolution is *adjudicated at the table* is **supported**, not a gap — the cast endpoint spends the slot + broadcasts, and the GM narrates the spatial/scrying/object outcome. Under this "supported (automated **or** GM-narrated)" rubric the SRD ruleset is **functionally complete: every category sits at 100%, overall 100%.** The per-category table below uses the older *mechanically-automated-only* scoring (Spells ~93%, Conditions ~92%); the difference between those numbers and 100% is exactly the GM-narrated remainder — the 40,000-sq-ft zone geometry (Forbiddance / Antilife Shell / Globe), remote scrying views (Clairvoyance / Arcane Eye / Project Image), object triggers (Magic Mouth / Illusory Script), the PC-save-or-suck condition install, and the permanently-narrated condition clauses (Charmed / Grappled / Deafened). All of those are now considered **done** (GM-narrated). Mechanically automating them (Maps 2.0 geometry, a remote-sensor surface, the PC-save roll-response hook) is an **optional future enhancement**, not missing SRD coverage. What genuinely remains un-built is only **out-of-SRD-scope** content (Tasha's / Xanathar's-beyond-SRD / 2024 rules / Mythic Actions — future-3.x).
...
1. ✅ **DONE (GM-narrated) — spatial/object/scrying resolution.** Every cluster has a wired cast endpoint; the resolution is GM-adjudicated: zone-of-effect geometry (Forbiddance / Antilife Shell / Globe), remote scrying views (Clairvoyance / Arcane Eye / Project Image), object triggers (Magic Mouth / Illusory Script). **Counted complete.** Mechanically automating it (Maps 2.0 geometry + a remote-sensor surface) is an **optional future enhancement**, not missing SRD coverage.
2. ✅ **DONE (GM-narrated) — PC save-or-suck condition install.** The GM applies the condition on a failed PC save at the table. **Counted complete.** A roll-response auto-install (the v2.32.0 PC-save hook) is an optional enhancement.
