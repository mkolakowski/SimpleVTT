...
*Remaining tails:* (1) **handout audio** — still filed on [`notes-and-handouts.md`](docs/plans/notes-and-handouts.md); (2) **non-PDF document types** — deliberately out of scope, since the feature's promise is inline rendering with no download and PDF is the only format every browser does that with natively; (3) **the same gate for other uploaded media** — map images, portraits, tokens, and audio are still served as public static bytes at unguessable paths. Handouts were the case with an explicit per-player reveal rule, so they went first; the others have weaker confidentiality expectations but the same posture.
