...
- 🟢 **DONE (root cause) — v2.99.465** — Bug: NPCs unable to use action buttons, IE strike button on Dagger for Vex
    ...
    - **Remaining (lower-priority follow-ups):** (1) true AoE for save breath weapons (pass `aoe_target_combatant_ids` from the strike button); (2) the unified mini-sheet `.mini-strike-btn` NPC path could get the same save routing if it surfaces save actions; (3) a runtime normalization (parse `desc` to-hit / save-DC) for Open5e-API-imported monsters so an import without `attack_bonus`/`save_dc` doesn't re-introduce the gap.
