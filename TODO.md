...
> **Progress (v2.746.0–v2.748.0):** Behavior **#3 (auto-init behind caster)** shipped (`_summon_initiative_for_body`, all 7 conjure endpoints). Behavior **#1 (creature picker)** shipped for all six conjure spells (count-based + single-summon Elemental/Fey/Celestial, v2.749.0) — backend (`beast_slug`/`creature_slug`), `GET /summon-options`, and the `window.openSummonPicker` modal wired into the mini-sheet cast flow (v2.748.0). Behavior **#2 (auto-assign control to the casting player)** is still open. Remaining: the picker for single-summon conjures (Elemental/Fey/Celestial) + Find Familiar's `form` list, and #2. See `docs/plans/movement-and-summons.md`.
