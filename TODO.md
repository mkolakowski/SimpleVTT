...
- 🟢 **P3 — Per-feature Phase-2 finishers (reconciled 2026-07-14 — most already shipped; verified against `app/routes/tabletop_routes.py`):**
    ...
    - 🟠 **AP (Assassinate) Phase 3** — the /attack auto-crit-vs-surprised + advantage-vs-not-acted is mechanized (v2.131.0–v2.132.0). **Surprise-state model shipped v2.1056.0 "The Unseen Blade":** a server-side `surprised` combatant flag (set via `POST /set_surprised`, GM), read by `/attack` (`_target_is_surprised`) for the auto-crit, auto-cleared on the target's first turn — the client `target_surprised` flag is no longer required. Harness: `test_set_surprised.py` (6). **Remaining:** auto-*detection* of surprise (Stealth vs passive Perception → sets the flag) — the `detect_surprise` compute endpoint (next commit).
