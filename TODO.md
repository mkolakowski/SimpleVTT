...
### SRD Rules in Full Text — 🟠 Phases 1–2b shipped · Phase 3 in progress (v2.1026.0)
...
- 🟠 **Phase 3 (in progress)** — contextual links from the character sheet / encounter panels (click a condition name → its reference entry); the non-content SRD rules sections (actions in combat, resting, cover, etc. — not in the current per-type JSON). **Substrate shipped v2.1025.0 "The Marginal Gloss":** `GET /api/reference/entry?type=&slug=` returns one shipped-SRD record (offline, SRD-only, six player-safe types), the reusable read surface for inline rule popovers. Harness: `test_srd_reference.py` (+4). **First consumer wired v2.1026.0 "The Severed Cord":** the sheet's condition-chip popover now reads from the offline `/api/reference/entry` tier instead of the network-capable `/api/open5e/conditions` proxy. *Remaining:* add contextual links on the encounter panel / mini-sheet (and to spell / feat names); then author the non-content SRD rules sections.
