...
### Migrate monster action_charges to the unified partial
**Filed 2026-05-23 (v2.49.203 Phase 2.5b regression).** Monster action_charges (per-combatant charge tracking for limited-use abilities — Frightful Howl, Soren the Cult Acolyte's Inflict Wounds at 2/2 ↻, etc.) are rendered by `buildMonsterInitSheet` via `_renderActionRow` (`tabletop.html:~5230`) into a `.monster-action-charges` widget + an `${spent ? ' disabled' : ''}` flag on the strike button. The unified `_mini_sheet_card.html` partial + the extracted `_tab_actions.html` / `_tab_spells.html` per-tab partials do NOT carry equivalent charge markup. So once Phase 2.5b's hydration switches monster mini-sheets to the unified partial, the per-combatant charge counter widget vanishes for monsters that have charge-tracked actions; their cast/strike buttons no longer show the spent-disabled state either. Server-side state (`combatant.action_charges` map) is untouched — the underlying tracking still works; only the visual readout is gone. To migrate: (1) extend the `_tab_actions.html` and `_tab_spells.html` partials to render a charge-counter chip + ↻ recharge button when an action has `charges_max > 0` (gated on `is_monster` + `is_gm` since these are GM-only affordances); (2) the server projection needs to pass per-combatant `action_charges` into the partial scope (today the partial sees the TokenTemplate's default sheet — combatant state isn't visible there); (3) wire the `.monster-charge-reset` click handler (already in the JS) to the new partial-rendered button. Option (2) is the design pull — combatant state lives in `battle.combatants` (client-side) not in the server's render context, so the migration likely needs the hydration step to also patch in the per-combatant charge values via a similar pattern to HP patching. Until this migration ships, the workaround is: `buildMonsterInitSheet` is still in the codebase as the orphan-fallback path (post-2.5c), so a future "is this a charge-tracked monster?" check could route those specific combatants through the legacy path. But cleaner long-term: get the partial to render charges.

### Re-investigate demo NPC spells (Cult Acolyte / Soren)
**Filed 2026-05-23.** User reported that the demo NPC spells "don't seem to work right" but didn't capture an exact symptom. Context: the demo seed (`app/demo_seed.py`) gives Soren the Cult Acolyte action entries that reference shared spells via `spell_slug` (added v2.49.182). The server-side `_resolve_spell_slug_action` (v2.49.174) overlays the spell catalog onto each action so range / damage / save_dc / save_ability come from the canonical spell definition rather than being hand-typed per monster. The NPC mini-sheet's Spells tab then renders these via `buildMonsterInitSheet`, with `✨ Cast` buttons that hit `/npc_attack` for attack-roll spells (Inflict Wounds) or fire the `/roll` announce flow for save-DC spells (Sacred Flame). Several gotchas have already landed in this surface: v2.49.176 fixed Pydantic stripping the `spell_slug` field from action JSON; v2.49.180 added `_enrich_pc_spell_from_catalog` for PC parity; v2.49.181 made the chips caster-dependent (DC + atk bonus). So when re-investigating: (1) what's the exact symptom — does the Spells tab fail to render, does the Cast button no-op, does the roll fire but with wrong values, does the chip layout look broken? (2) Check Soren's `actions` array in the demo seed has the `spell_slug` references the catalog can resolve. (3) Confirm the v2.49.176 Pydantic fix is still in `action_schema.py` (the field can vanish silently if a future Pydantic-Model refactor doesn't re-declare it). (4) Spot-check `_resolve_spell_slug_action` against the resolved sheet — `prof_saving_throws` parsing in `buildMonsterInitSheet` doesn't influence spell DCs but the spell's `save_dc` comes from `_resolve_spell_slug_action`'s lookup; if the lookup returns a bare catalog dict without a DC, the chip reads weird (DC 0 or missing entirely). (5) Verify the `/npc_attack` flow handles spell actions — it was originally written for weapon attacks; v2.49.177 extended the picker UX but the server-side path may not have grown spell-specific handling (caster save DC, area templates for AoE spells like Hold Person, etc.).
