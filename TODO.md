...
- 🟡 **P2** — **GDPR Article 15/20 user data export endpoint.** Today a user exercising their right of access (Article 15) or data portability (Article 20) gets served by the operator running SQL queries against postgres (recipe in the [privacy policy](docs/wiki/privacy.md) Section 5.1). That's defensible but high-friction. Filed: add an authenticated `GET /api/users/me/export` endpoint that returns the user's full record set as a single JSON archive (account fields, campaigns owned, characters owned, chat messages authored, rolls made, recent audit-log lines about them). Per Article 20 the format must be "structured, commonly used, machine-readable" — JSON satisfies that. Pairs with the v2.478.0 privacy policy → Section 5.1 currently documents the manual SQL path; this TODO replaces it with a self-serve flow. Should rate-limit per user (one export per 24 h) and ship a corresponding harness test asserting the archive contains the canonical record set.
