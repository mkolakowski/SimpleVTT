...
- [`pending-resolution-state-machine.md`](docs/plans/pending-resolution-state-machine.md) — the biggest remaining reactions-v3 item: make the save-or-suck failure resolution reusable so a reaction that flips an outcome re-resolves the effect. **Phases 0–3a + 3c-1/3c-2 all shipped** (v2.610.1–v2.612.0 save path; v2.658.0 3c-1 logs NPC weapon on-hit-save installs under `attack_id`; v2.659.0 3c-2 auto-reverts the condition on an NPC-Parry hit→miss flip via `_revert_attack_buff_installs`). **Only 3c-3 remains** — the PC-defender held "pending" window (defer the on-hit save until the reaction window closes; its own async arc).
- [`reactions-automation.md`](docs/plans/reactions-automation.md) — v3 auto-resolution arc: AC-bump negation family **complete** for PCs + NPCs (Shield v2.600.0 / Defensive Duelist v2.601.0 / Form of the Beast Tail v2.602.0 / Combat Inspiration v2.607.0 / NPC Parry v2.608.0); d20-reroll family done (Lucky v2.609.0; Silvery Barbs v2.610.0–v2.612.0 re-resolves the flip). The remaining work is **only 3c-3** of the pending-resolution state machine (above) — every other slice has shipped.
...
- [`pending-resolution-state-machine.md`](docs/plans/pending-resolution-state-machine.md) — **3c-3 (PC-defender held pending window)** is the one substantial remaining slice of the reactions-v3 auto-resolution arc; everything else (the save path + NPC-defender flip revert) shipped through v2.659.0. See the IN PROGRESS section above for the full breakdown.
