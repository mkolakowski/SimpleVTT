...
_Advantage & Disadvantage Tracking, Death Saving Throws, and Combat 2.0 Action Economy all shipped — see [`TODONE.md`](TODONE.md) → Combat (reconciled v2.745.1). **NPC death saves Phase 4a shipped v2.1053.0** ("The Boss's Last Breath") — a boss flagged via `POST /set_npc_death_saves` enters *dying* at 0 HP (`_npc_death_save_transition`), state carried on the combatant; harness `test_npc_death_saves.py`. Remaining death-saves slices, tracked in [`death-saves.md`](docs/plans/death-saves.md): **Phase 4b** (turn-start auto-roll of the NPC's death save) + **Phase 3c** (stable-at-0 countdown, session-time-blocked)._
