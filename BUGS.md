...
| **Test helper's derivation ≠ engine's** | `test_spell_catalog_attack::test_every_attack_spell_bonus_and_hit` + `::test_attack_crit_doubles_damage_dice`, `test_spell_catalog_save::test_every_save_spell_dc_and_ability` (3) | "15 attack-bonus mismatches (expected uniform +4)" — engine derives +6; "116 save-DC mismatches (expected uniform 12)" | **Stale test.** `_expected_attack_bonus` defaults `proficiency_bonus` to 2 and reads a possibly-unset `spellcasting_ability`, while the engine derives proficiency from level (Lv5 → +3). Confirm whether the helper or a demo-sheet field drifted, then fix the helper. |
...
**Fix path:** work the four confirmed clusters, each its own commit. **Cluster 1 (vision) ✅ done v2.1033.2.** Remaining: the derivation-helper cluster (another pure test fix), then the catalog + RNG clusters, then re-run CI and triage whatever's left in the `harness` / `encounter-sim` jobs against a no-longer-red baseline. Until the suite is green, treat any single red test as suspect-until-bisected (memory: reseed + re-run isolated before blaming a change).
