...
| **RNG-dependent** | `test_cast_confusion_npc::test_thalindra_casts_confusion_on_bandit_npc` (1) | "no failed bandit Wis save in 30 tries; Confused didn't install" | **Flaky.** Needs a forced-fail save (seed the roll) instead of relying on 30 random tries clearing a low DC. |
...
**Fix path:** work the four confirmed clusters, each its own commit. **Cluster 1 (vision) ✅ done v2.1033.2; cluster 2 (derivation helper) ✅ done v2.1033.3; cluster 3 (catalog anchor) ✅ done v2.1033.4.** Remaining: the RNG cluster (1: confusion-npc), then re-run CI and triage whatever's left in the `harness` / `encounter-sim` jobs against a no-longer-red baseline. Until the suite is green, treat any single red test as suspect-until-bisected (memory: reseed + re-run isolated before blaming a change).
