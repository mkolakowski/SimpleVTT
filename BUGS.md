...
|---|---|---|---|
| **Vision/LOS cancels the effect under test** | `test_cast_blur`, `test_cast_holy_aura`, `test_cast_mislead`, `test_cast_true_seeing`, `test_cast_foresight` (5) | roll-state comes back `canceled_unseen_attacker_vs_blur` / `…_vs_cant_see` / `canceled_rage_vs_cant_see` instead of the expected advantage/disadvantage/invisible | **Test-setup**, not a product regression. The attacker can't *see* the target on the demo map's dim ambient, so the can't-see cancellation pre-empts the adv/dis the test is asserting. Same root cause as the known vision/lighting flake note in memory — the tests don't establish LOS/light before the attack. |
...
**Fix path:** work the four confirmed clusters first (2 are pure test fixes — vision-setup + the derivation helper — that turn most of the `spell-catalog` job green), then re-run CI and triage whatever's left in the `harness` / `encounter-sim` jobs against a no-longer-red baseline. Each cluster is its own commit. Until the suite is green, treat any single red test as suspect-until-bisected (memory: reseed + re-run isolated before blaming a change).
