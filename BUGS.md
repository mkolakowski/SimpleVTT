...
### B16 — CI harness suite has been red since ≥2026-07-12; a real regression would be invisible in the noise · 🔴 P1 · OPEN
**Source:** the `test-harness.yml` workflow (`gh run list --workflow=test-harness.yml`). **All four CI jobs — `harness`, `spell-catalog`, `encounter-sim`, `harness-ui` — are failing**, and have been since at least the 2026-07-12 run (`29213958047`, the most recent). The CLAUDE.md rule "the CI workflow runs the harness suite on every push … a regression fails the workflow before merge" is therefore **not currently protecting `main`** — the suite is already red, so a genuinely new regression lands in a sea of pre-existing red and is missed. This is exactly the failure mode that cost time in the v2.1033.0 session (every change gets diffed against a red baseline by hand).
...
**Fix path:** **all four confirmed `spell-catalog` clusters are now ✅ FIXED** — cluster 1 (vision) v2.1033.2, cluster 2 (derivation helper) v2.1033.3, cluster 3 (catalog anchor) v2.1033.4, cluster 4 (index drift) v2.1033.5. **Three of the four turned out to be the *same underlying issue* — stale hardcoded assumptions against a drifted demo sheet (spell indices, caster level/ability, corrected catalog data) — not product regressions.** Remaining B16 work is only the **un-reproduced-locally CI remainder**: the `harness` and `encounter-sim` jobs fail a larger set on CI that passes locally in isolation (likely CI-ordering/seed-specific — the same order-dependence class as cluster 1's leaked ambient). Next step: push these fixes, let CI re-run, and triage the `harness`/`encounter-sim` jobs against a much-greener `spell-catalog` baseline. Until the whole suite is green, treat any single red test as suspect-until-bisected (memory: reseed + re-run isolated before blaming a change).
