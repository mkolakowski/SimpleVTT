...
| **Catalog anchor drift** | `test_cast_conjure_family::test_conjure_family_present_in_catalog` (asserts `conjure-celestial`), `test_cast_polymorph::test_polymorph_present_in_catalog` + `::test_cast_binds_concentration_despite_catalog_flag` (2) | `AssertionError: conjure-celestial`; "catalog should flag Polymorph concentration:false — the divergence test depends on this anchor" | **Verify** whether the shipped catalog changed (real) or the test's expected anchor is stale (test). |
...
**Fix path:** work the four confirmed clusters, each its own commit. **Cluster 1 (vision) ✅ done v2.1033.2; cluster 2 (derivation helper) ✅ done v2.1033.3.** Remaining: the catalog-anchor + RNG clusters, then re-run CI and triage whatever's left in the `harness` / `encounter-sim` jobs against a no-longer-red baseline. Until the suite is green, treat any single red test as suspect-until-bisected (memory: reseed + re-run isolated before blaming a change).
