...
def test_action_surge_refunds_action_chip(
    ...
    # v2.1006.0 — un-skipped (was B2 in BUGS.md, filed v2.49.236). The
    # init-tracker orphan-cleanup (tabletop.html:4807) drops any
    # combatant whose char_id isn't tokenized, and the demo's
    # tokenized-six lineup has no Fighter — so the test tokenizes
    # Garrik itself via place-token and removes the token on teardown
    # (no demo-seed change, no extra token left on the public map).
