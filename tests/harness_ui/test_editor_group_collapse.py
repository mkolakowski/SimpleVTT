...
def test_draw_zone_collapse_includes_lair_and_label_is_vertical(gm_page: Page) -> None:
    ...
    # v2.924.0 — Theme A group names; v2.936.0 — Notes+Tokens moved to Map.
    for label in ("Walls", "Terrain", "Lighting", "Environment"):
...
def test_zone_collapse_persists_across_reload(gm_page: Page) -> None:
    ...
    expect(gm_page.locator("#me-overlay")).to_be_visible()
    gm_page.wait_for_timeout(400)
    for label in ("Walls", "Terrain", "Lighting", "Environment", "Lair zones"):  # v2.936.0 — Tokens now in Map
        g = gm_page.locator(f'.me-group[aria-label="{label}"]')
