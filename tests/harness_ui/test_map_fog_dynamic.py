...
def test_dynamic_fog_three_states(alice_page: Page) -> None:
    ...
    alice_page.wait_for_function("() => window.__testDrawFog", timeout=8000)

    # Grid 70px. Hero token at cell (2,2). A solid vertical wall at x=5*g
    # occludes everything to its right on that band. Cell (3,2) is in front
    # (visible); cells (7,2)/(7,3) are behind the wall (not currently visible).
    # Pre-mark (7,2) as explored → it should render as dim memory; (7,3) is
    # never seen → opaque.
    res = alice_page.evaluate("""() => {
        ...
        return {
            visible: out.visible,
            frontAlpha: cell(3, 2),     // in front of wall → currently visible
            memoryAlpha: cell(7, 2),    // behind wall, explored → dim memory
            unseenAlpha: cell(7, 3),    // behind wall, never seen → opaque
        };
    }""")

    assert "3,2" in res["visible"], res         # front cell is in the live set
    assert "7,2" not in res["visible"], res      # wall occludes the memory cell
