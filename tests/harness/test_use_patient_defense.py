...
async def test_patient_defense_happy_path(gm_client, gm_ws, kael_rested):
    ...
    # Ki decremented by 1. v2.49.229: Kael bumped to Lv 7 Monk → Ki max 7.
    assert body["max"] == 7
...
async def test_patient_defense_no_ki(gm_client, kael_rested):
    ...
    # otherwise gate on second call). v2.49.229: Kael bumped to Lv 7 →
    # Ki max 7, so 7 successful drains → ki=0; 8th → 409 no_ki.
    for _ in range(7):
        r = await gm_client.post(
            f"/api/campaign/{CAMPAIGN_ID}/use_patient_defense",
