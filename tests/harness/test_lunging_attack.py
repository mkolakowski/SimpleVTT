"""v2.99.257 — Battle Master maneuver 7: Lunging Attack.

Phase E.1 Phase 3 (maneuver 7 of 16). RAW PHB p.74: melee
attack on your turn — expend 1 die; +5 ft reach for that
attack + die added to damage on hit.

Tests:
  - Happy d8 → extra_reach_ft 5, extra_damage_on_hit 1..8, dice 4→3.
  - Out of dice → 409.
  - Wrong subclass → 409.
"""
import asyncio
...
def _la_broadcasts(gm_ws, character_id):
    return [
        m for m in gm_ws.buffered("feature_used")
        if (m.get("data") or {}).get("source") == "lunging-attack"
...
async def test_use_la_happy(
    gm_client, gm_ws, garrik_battle_master,
):
    """Lv 9 Garrik d8 → +5 ft reach, +1..8 damage on hit."""
    garrik = garrik_battle_master
    gm_ws.mark()
    r = await gm_client.post(
        f"/api/campaign/{CAMPAIGN_ID}/use_lunging_attack",
        json={"character_id": garrik["id"]},
    )
    assert r.status_code == 200, r.text
    data = r.json()
    assert data["extra_reach_ft"] == 5
    assert 1 <= data["extra_damage_on_hit"] <= 8
    assert data["die_size"] == "d8"
    assert data["dice_remaining"] == 3
    await asyncio.sleep(0.3)
    feats = _la_broadcasts(gm_ws, garrik["id"])
    assert feats


async def test_use_la_out_of_dice(
...
async def test_use_la_wrong_subclass(
    gm_client, roster,
):
