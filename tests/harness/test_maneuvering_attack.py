"""v2.99.265 — Battle Master maneuver 15: Maneuvering Attack.

Phase E.1 Phase 3 (maneuver 15 of 16). RAW PHB p.74: on hit,
+die damage; chosen ally can use reaction to move half speed
without provoking OAs from the target.

Tests:
  - Happy d8 → extra_damage 1..8, ally + target names mirrored.
  - Out of dice → 409.
  - Wrong subclass → 409.
"""
import asyncio
...
def _ma_broadcasts(gm_ws, character_id):
    return [
        ...
        if (m.get("data") or {}).get("source") == "maneuvering-attack"
...
async def test_use_ma_happy(
    ...
    """Lv 9 Garrik d8 → ally + target names mirrored."""
    garrik = garrik_battle_master
    gm_ws.mark()
    r = await gm_client.post(
        f"/api/campaign/{CAMPAIGN_ID}/use_maneuvering_attack",
        json={
