...
- **Creature picker — shipped (backend v2.539.0, options endpoint v2.747.0, picker UI v2.748.0).** The conjure family accepts a per-cast creature choice (`beast_slug` / `creature_slug`, CR-gated via `_conjure_catalog_summon_template`; `cast_find_familiar` takes `form`). `GET /summon-options` lists the valid creatures per spell + CR tier. The tabletop's `window.openSummonPicker` modal opens after a count-based conjure spell is cast from the mini-sheet (`window._SUMMON_SPELLS` hook in the PC cast path), lets the player choose the summoning option (count↔CR tier) + the creature, and POSTs to the matching `/cast_conjure_*` endpoint. Tests: `test_summon_options.py` + `test_summon_picker.py`. **v2.749.0** extended both the endpoint + picker to the single-summon conjures (Elemental / Fey / Celestial — `mode: single`, CR scales with the slot). *Remaining:* Find Familiar's `form` list (a fixed RAW creature list, not catalog-CR-based).
