...
- 🔄 **Phase 4 — NPC death saves with per-combatant toggle** — **Phase 4a shipped v2.1053.0.** A boss flagged `rolls_death_saves` (via `POST /api/campaign/{cid}/set_npc_death_saves`, GM-only) enters the *dying* state at 0 HP instead of dropping inert — `_npc_death_save_transition` mirrors the PC `_apply_hp_change` transitions on the combatant dict (dying/stable/dead, massive-damage instant-death, damage-while-dying failure ticks). State (`rolls_death_saves` + `death_saves`) lives on the combatant in hub state, carried forward across the `/battle` PUT; the transition broadcasts `npc_death_save`. Harness: `test_npc_death_saves.py` (7). **Phase 4b (turn-start auto-roll of the death save) is the remaining slice.**
