...
- ✅ **Phase 4 — NPC death saves with per-combatant toggle** — **complete (4a v2.1053.0 + 4b v2.1054.0).** A boss flagged `rolls_death_saves` (via `POST /api/campaign/{cid}/set_npc_death_saves`, GM-only) enters the *dying* state at 0 HP instead of dropping inert — `_npc_death_save_transition` mirrors the PC `_apply_hp_change` transitions on the combatant dict (dying/stable/dead, massive-damage instant-death, damage-while-dying failure ticks). State (`rolls_death_saves` + `death_saves`) lives on the combatant in hub state, carried forward across the `/battle` PUT. **Phase 4b:** a dying flagged boss **auto-rolls** its death save at the start of its turn (`_npc_death_save_roll` in `update_battle`, flat d20 vs 10, PC-parity outcomes); transitions broadcast `npc_death_save` (`source: damage` / `turn_start`). Harness: `test_npc_death_saves.py` (9). Only the shared **Phase 3c** (stable-at-0 countdown, session-time-blocked) remains across the whole plan.
