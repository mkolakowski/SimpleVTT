...
>
> **Status (rider/bonus scaling):** ✅ **PHASE 4 CLOSED** at v2.434.0 (2026-06-18). Scales a flat numeric *rider* a spell grants its target (an attack/damage bonus, a temp-HP bump) as the cast slot climbs. Tier-walk substrate (`_SPELL_BONUS_MAP` + `_spell_bonus_for_slot()`): **Magic Weapon** (v2.421.0) + **Elemental Weapon** (v2.422.0). Linear-additive sibling substrate (`_SPELL_BONUS_ADDITIVE_MAP` + `_spell_bonus_additive_for_slot()`): **False Life** (v2.423.0, `step_size: 1` implicit) + **Aid** (v2.432.0 — refactor) + **Spiritual Weapon** (v2.433.0 — opens the **step-N additive** sub-shape via the new `step_size: 2` field; refactor). 5 consumers shipped end-to-end across the three sub-shapes (tier-walk / step-1 additive / step-N additive). The v2.434.0 closure audit confirmed no remaining SRD candidates fit the Phase 4 contract (everything else lives on Phase 1 / 2 / 3 substrates or has no per-slot scaling RAW). See the [Phase 4 section](#phase-4--riderbonus-scaling-substrate-v24210) below.
>
...
### Phase 4 closure audit (v2.434.0)

**Phase 4 surface is effectively closed at 5 consumers.** A v2.434.0 audit walked the bespoke `/cast_*` and `/use_*` endpoints looking for a sixth Phase 4 candidate; the SRD doesn't have one. Every remaining rider/bonus-like scaling either rides a different substrate already or has no per-slot scaling RAW:

| Candidate | Endpoint | Why not Phase 4 |
|---|---|---|
| **Heal** | `/cast_spell` (generic dispatcher) | The v2.130.0 `_scale_flat_for_upcast()` prose-parser path already handles it: `parse_upcast_dice("the amount of healing increases by 10 for each slot level above 6th")` returns `flat_healing_per_slot: 10`, which the generic upcast scaler applies before `/cast_spell` registers the heal claim. No bespoke endpoint, no inline scaling to refactor — moving Heal onto `_SPELL_BONUS_ADDITIVE_MAP` would be redundant. |
| **Hunter's Mark** | `/cast_hunters_mark` | The 1d6 damage rider is fixed RAW; only **duration** scales per slot → Phase 1 (✅ shipped v2.405.0). |
| **Hex** | `/cast_hex` | Same as Hunter's Mark — duration only → Phase 1 (✅ shipped v2.405.1). |
| **Bane** | (`_SPELL_TARGET_CAPS`) | Target-count scaling, not rider → Phase 3 (✅ shipped v2.404.6). |
| **Animate Dead** | `/cast_animate_dead` | "+2 undead per slot above 3rd" is COUNT scaling, not rider → Phase 3 (✅ shipped via the `_SPELL_SUMMON_COUNT_MAP` family). |
| **Create or Destroy Water** | `/cast_create_or_destroy_water` | Cube edge scaling → Phase 2 (✅ shipped via `_SPELL_AOE_RADIUS_MAP`). |
| **Private Sanctum / Confusion / Fog Cloud** | bespoke | All AoE-radius scaling → Phase 2 (✅ shipped). |
| **Heroes' Feast** | (catalog) | Fixed 24h RAW; no per-slot scaling. Filed at v2.404.x. |
| **Aura of Vitality** | (catalog) | Fixed 2d6 heal per bonus action; no per-slot scaling. Filed at v2.423.1. |

**The 5 shipped consumers cover all three sub-shapes the substrate exposes** — tier-walk (Magic Weapon, Elemental Weapon), step-1 additive (False Life, Aid), step-N additive (Spiritual Weapon). Any future Phase 4 ship would need a NEW spell shape (e.g. exponential scaling, multi-axis riders, time-bound riders) that the SRD doesn't have. Closing Phase 4 doesn't preclude a sixth consumer landing in a future SRD update — the substrate is ready when a candidate appears.
