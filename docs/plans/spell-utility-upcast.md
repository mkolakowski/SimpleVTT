...
>
> **Status (duration-scaling):** 🟠 **PHASE 1 IN FLIGHT** as of v2.406.0 (2026-06-17). Substrate (`_SPELL_DURATION_MAP` + `_spell_duration_rounds_for_slot()` helper) + Hunter's Mark retrofit (v2.405.0) + Hex retrofit (v2.405.1) + Bestow Curse retrofit (v2.405.2, first `"permanent"` marker) + Geas (v2.406.0, first NEW endpoint + day/year markers) shipped. **Scope correction:** the remaining 2 spells (Mass Suggestion, Modify Memory) are catalog-only with no cast endpoint, so they're endpoint-*builds* like Geas, not one-line retrofits. See the [Phase 1 section](#phase-1--duration-scaling-substrate-v24050) below.
...
**Real Phase 1 scope (revised):** Hunter's Mark ✅ + Hex ✅ + Bestow Curse ✅ + Geas ✅ + Mass Suggestion + Modify Memory = **6 spells** with genuine duration scaling. The others get filed out of scope (no per-slot duration ladder) or moved to Phase 2/4. **Scope note:** only the first three were true one-line retrofits (they had endpoints with inline ladders). Geas, Mass Suggestion, and Modify Memory are catalog-only — making them substrate consumers means *building* a `/cast_<spell>` endpoint (MINOR), as Geas demonstrated in v2.406.0. Phase 1 closure: ~2 more endpoint-build commits (Mass Suggestion, Modify Memory).
