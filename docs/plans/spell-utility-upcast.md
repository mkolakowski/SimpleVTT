...
>
> **Status (summon-count scaling):** 🟠 **PHASE 3 IN PROGRESS** as of v2.417.0 (2026-06-17). Multiplier substrate (`_SPELL_SUMMON_MAP` + `_spell_summon_multiplier_for_slot()`): Conjure Animals retrofit (v2.414.0, ×1/×2/×3/×4) + Conjure Woodland Beings (v2.415.0, `/cast_conjure_woodland_beings` + `fey-spirit` template) + Conjure Minor Elementals (v2.416.0, `/cast_conjure_minor_elementals` + `elemental-spirit` template; Druid/Wizard gate) — **count-multiplier family complete**. Additive substrate (`_SPELL_SUMMON_ADDITIVE_MAP` + `_spell_summon_additive_for_slot()`): Animate Dead (v2.417.0, `/cast_animate_dead` + `undead-servant` template; 1 + 2/slot above 3rd) — **count-additive family open**. Only the CR-increase family (Conjure Fey / Elemental / Celestial) remains. See the [Phase 3 section](#phase-3--summon-count-scaling-substrate-v24140) below.
