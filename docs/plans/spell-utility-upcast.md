...
>
> **Status (summon-count scaling):** 🟠 **PHASE 3 IN PROGRESS** as of v2.418.0 (2026-06-17). Multiplier substrate (`_SPELL_SUMMON_MAP` + `_spell_summon_multiplier_for_slot()`): Conjure Animals retrofit (v2.414.0, ×1/×2/×3/×4) + Conjure Woodland Beings (v2.415.0, `/cast_conjure_woodland_beings` + `fey-spirit` template) + Conjure Minor Elementals (v2.416.0, `/cast_conjure_minor_elementals` + `elemental-spirit` template; Druid/Wizard gate) — **count-multiplier family complete**. Additive substrate (`_SPELL_SUMMON_ADDITIVE_MAP` + `_spell_summon_additive_for_slot()`): Animate Dead (v2.417.0, `/cast_animate_dead` + `undead-servant` template; 1 + 2/slot above 3rd) — **count-additive family complete**. CR-increase substrate (`_SPELL_SUMMON_CR_MAP` + `_spell_summon_cr_for_slot()`): Conjure Elemental (v2.418.0, `/cast_conjure_elemental`; count fixed at 1, CR 5 base +1/slot above 5th) + Conjure Fey (v2.419.0, `/cast_conjure_fey`; CR 6 base +1/slot above 6th, Druid/Warlock gate, reuses `fey-spirit`). Only Conjure Celestial (L7, a two-tier CR ladder) remains. See the [Phase 3 section](#phase-3--summon-count-scaling-substrate-v24140) below.
...
- **`_SPELL_SUMMON_CR_MAP`** + **`_spell_summon_cr_for_slot(spell_slug, slot_level, default_cr=0)`** (v2.418.0) — sibling map for the CR-increase family. Entries carry `{base_level, base_cr, per_slot}`; the helper returns `base_cr + per_slot × max(0, slot − base_level)`. Count stays fixed at 1; only the summoned creature's CR scales. First consumer: Conjure Elemental.
