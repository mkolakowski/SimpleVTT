...
- **Per-item / per-spell charge tracking:** **mostly ✅ — reconciled.** The named items are already charge-tracked through the separate [magic-items-automation](magic-items-automation.md) arc, not via the reaction layer: **Pearl of Power** decrements its `pearl-of-power` resource in `_use_item_action_pearl`, and **Wand of Lightning Bolts** (+ Magic Missiles / Fireballs / Web / Polymorph / Binding) decrement charges in `_use_item_action_charge_wand` — both behind `POST /api/campaign/{cid}/character/{cid}/use_item_action`. The Lucky feat itself tracks `sheet.resources["lucky"].current` (v2.77.0). The reaction-layer piece is now also ✅ **shipped v2.647.0**: `_decrement_item_reaction_charge` spends an item reaction's `cost_charges` from the item's `charges` field in **both** the `/use_reaction` `item-*` dispatch and the GM-panel `/spend_reaction_manual` — refusing with `409 out_of_charges` once depleted and surfacing `charges_remaining`. A **Ring of Spell Turning** (3 charges, `cost_charges: 1`) was baked onto Lyra Sunstrider as the fixture. Charge-less item reactions (the informational Cloak shape) pass through unchanged.
...
- ~~**Lucky-vs-Lucky cancel mechanic**~~ ✅ **shipped v2.648.0** (RAW PHB p.167: if two creatures both spend luck on the same roll, the points cancel — no extra die). When the resolving client sets `params.cancel: true` on `use-lucky`, the attacker (from `context.attacker_char_id`) must also have Lucky available; on success both points spend (via the new shared `_spend_lucky_charge`), the reroll is voided, no heal-back fires, and a `feature_used(source=lucky-cancel)` broadcasts.
- **Range gates:** Silvery Barbs and Mage Slayer use per-trigger context.distance_ft; Counterspell walker uses 60 ft. **Shared range-gate primitives ✅ v2.647.1–v2.647.2.** Two halves now standardized: `_combatant_token_on_map(db, combatant, map_id)` (the `source_token_id`→`char_id` token resolution, the map-id-cached sibling of the older campaign-based `_combatant_token`) and `_battle_grid_context(db, campaign)` (the active-map / grid-size / grid-type setup block). Both adopted across all three distance-gated reaction walkers — Counterspell (+ the Mage Slayer 5-ft gate riding it), Protective Field, and Sentinel — which now share `_battle_grid_context` + `_combatant_token_on_map` + `_distance_ft_between_points` instead of inlining each. A pairwise `distance_between_combatants(c1, c2)` wrapper was **evaluated and deferred**: every current walker either caches its anchor token once across the loop or needs the watcher `Token` object for the prompt payload, so a pure pairwise function (which re-resolves both tokens per call) doesn't fit any existing caller — defer until a genuine single-pair caller appears. The remaining standardization is the per-trigger *range constant* (60 / 30 / 5 ft) living in each walker rather than a shared map.
