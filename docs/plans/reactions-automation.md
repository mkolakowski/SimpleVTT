...
- **Range gates:** Silvery Barbs and Mage Slayer use per-trigger context.distance_ft; Counterspell walker uses 60 ft. **First shared primitive ✅ v2.647.1** — `_combatant_token_on_map(db, combatant, map_id)` consolidates the `source_token_id`→`char_id` token-resolution the Counterspell / Mage Slayer range gate + Protective Field walkers each repeated inline before measuring distance with `_distance_ft_between_points`. Remaining walkers (Sentinel, OA) can adopt it incrementally; a full `distance_between_combatants` wrapper is the next step.
