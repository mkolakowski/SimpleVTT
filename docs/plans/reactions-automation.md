...
**Status:** 🟠 Reactions framework is broadly shipped. Phases 1–6 all have at least a partial v1; Phase 3e (Feather Fall) is the only entirely-unstarted slice. v2.78.0 closed Phase 5 (item reactions) and v2.77.0 closed Phase 4 (Lucky), so the user-facing surface for class features + feats + spells + items + monster reactions is now in place. v3 work is uniformly filed: a pending-damage state machine that lets reactions retroactively negate the triggering event, auto-cast routing for the "click Cast Spell to resolve" advisories, charge tracking for items, and the own-roll Lucky/Silvery-Barbs surfaces (need new `attack_resolved` + `check_resolved` events). The full 566-test harness suite passes against the v2.78.0 reactions surface.
...
- **Auto-resolution of advisory reactions:** ~~Shield AC negation~~ (✅ v2.600.0), ~~Defensive Duelist AC negation~~ (✅ v2.601.0), ~~Form of the Beast Tail AC negation~~ (✅ v2.602.0), ~~HR damage-to-attacker~~ (✅ v2.446.0), Mage Slayer melee attack, War Caster spell cast, Lucky d20 reroll, SB d20 reroll, NPC Parry AC bump, Cloak of Displacement disadvantage, ~~Counterspell undo of the original cast~~ (✅ — `/undo_attack_damage` refund pipeline). The v3 auto-resolution backlog is being picked off one reaction at a time via a shared "AC-bump negation" recipe (read the attack context → if the boosted AC turns a non-crit hit into a miss, heal back the full applied damage). The remaining AC-bump reaction is **Combat Inspiration** (Valor Bard, +BI-die AC — same recipe); after that the distinct-mechanic items are the **NPC Parry AC bump** (GM-side NPC HP path) and the **Lucky / Silvery Barbs d20 reroll** surfaces.
...
- **Passive feat effects:** Mage Slayer save advantage vs nearby casters + concentration-disadvantage on damage; War Caster concentration advantage on damage; Cloak of Displacement "suppressed for 1 round on damage."
