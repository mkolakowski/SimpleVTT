...
**Status:** 🟠 Phases 1a (v2.67.0) + 1b (v2.67.1) + 2a-partial (v2.67.2) shipped. Phase 1a: server-side foundation — `reaction_prompt` broadcast + `/use_reaction` endpoint + `_eligible_reactions` + `_emit_reaction_prompt` + schema v60 `users.reaction_prompt_mode` + OA exit-reach retrofit. Phase 1b: client popup UI (`reaction_prompt.js`) + `/api/settings/reaction_prompt_mode` endpoint + radio-group toggle on `/settings`. Phase 2a-partial: Uncanny Dodge surfaces through the prompt pipeline (ack-only — auto-fire preserved, prompt-first flip deferred to Phase 2a.1). v2.67.3 closes the two-way audit gap: NPC watchers now consume their reaction slot via `_mark_battle_economy_by_combatant_id`, so the GM experience mirrors the player experience end-to-end. **v2.68.0 ("The GM's Console") ships the GM Reactions Panel** — a new GM Tools drawer section listing every combatant's available reactions (class features, feats, spells, monster stat-block reactions) as one-click spend buttons. Bypasses the trigger-event pipeline so the GM can spend any reaction at any time. Phases 2b-7 still ⚪ for the trigger-event automation side.
...
- **Phase 3a:** Shield (highest gameplay impact).
- **Phase 3b:** Counterspell (needs `spell_cast_near` event).
- **Phase 3c:** Hellish Rebuke + Absorb Elements (both `damage_taken` listeners).
