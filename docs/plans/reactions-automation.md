...
**Status:** 🟠 Phases 1a (v2.67.0) + 1b (v2.67.1) + 2a-partial (v2.67.2) + 2b-partial (v2.68.5) + 2 catalog (v2.68.6–v2.68.11) + 3a-partial (v2.69.0) shipped. Phase 1a: server-side foundation — `reaction_prompt` broadcast + `/use_reaction` endpoint + `_eligible_reactions` + `_emit_reaction_prompt` + schema v60 `users.reaction_prompt_mode` + OA exit-reach retrofit. Phase 1b: client popup UI (`reaction_prompt.js`) + `/api/settings/reaction_prompt_mode` endpoint + radio-group toggle on `/settings`. Phase 2a-partial: Uncanny Dodge surfaces through the prompt pipeline (ack-only — auto-fire preserved). v2.67.3: NPC watchers consume their reaction slot via `_mark_battle_economy_by_combatant_id`. **v2.68.0 ("The GM's Console") ships the GM Reactions Panel** — manual-spend bypass for any reaction. v2.68.1: Polearm Master + Sentinel emit `reaction_prompt` alongside their legacy `feature_used` advisories. **v2.68.5 ("Bards and Bulwarks") — Phase 2b partial:** Cutting Words + Indomitable endpoints now emit `reaction_prompt(reaction_used)` acks. **v2.68.6 ("Stance and Sting") — Phase 2b continuation:** Battle Master maneuvers (Riposte / Parry / Brace) + Deflect Missiles added to the GM Reactions Panel catalog. **v2.68.7 ("Stand and Shield") — Phase 2c:** Protection + Interception fighting styles added to the catalog. **v2.68.8 ("Faith and Fey") — Phase 2d:** Warding Flare (Cleric Light) + Wrath of the Storm (Cleric Tempest) + Misty Escape (Warlock Archfey Lv 6+) added to the catalog with a shared subclass-slug normalization pipeline. **v2.68.9 ("Crown and Chorus") — Phase 2e:** Mantle of Inspiration (Bard Valor Lv 3+) + Rebuke the Violent (Paladin Crown Lv 7+ Channel Divinity) added to the catalog. **Phase 2 is now ✅ shipped on the catalog side** — every class-feature reaction in the plan doc's category A is surfaced (or filed with explicit per-mechanic notes). **v2.69.0 ("The Arcane Aegis") — Phase 3a partial:** Shield spell is the first reaction *spell* wired to a runtime trigger event (`attack_targeted`). `/attack` against a PC target now emits `reaction_prompt` with a `cast-shield` option (AC math preview in label, lowest available 1st+ slot); `/use_reaction` consumes the slot, marks the reaction, and installs a `shield-active` buff (`ac_bonus=5` + `immune_magic_missile` + `duration_rounds=1`). Retroactive negation of the triggering hit is filed as v1 limitation — popup tells the player whether the new AC would have made the hit miss; the v3 pending-damage state machine is the long-term fix. **v2.70.0 ("The Silence") — Phase 3b partial:** Counterspell is the second reaction spell wired to a runtime trigger. New `spell_cast_near` event fires from `/cast_spell` + `/npc_cast_spell` for every PC watcher within 60 ft of the caster who has Counterspell prepared + a 3rd+ slot available. `/use_reaction` consumes the slot, marks reaction, broadcasts `feature_used(source=counterspell-cast)` with `outcome_hint` (`auto` when slot ≥ incoming level, `check` with `arcana_check_dc` otherwise). RAW arcana-check roll + auto-undo of the original cast filed for v3 alongside the pending-damage state machine. **v2.78.0 ("The Displaced Bard") — Phase 5 partial:** Item reactions shipped as a generic catalog mechanism. New `_pc_item_reactions_for_trigger(char, trigger)` helper walks `sheet.inventory[*]._reactions[]` for items with `equipped: True`. SRD/homebrew items declare reactions by editing the inventory entry (no new helper code needed). Generic `item-*` dispatch in `/use_reaction` marks reaction + broadcasts `feature_used(source="item-reaction")` with item identity. Demo: Cloak of Displacement equipped on Lyra with a `_reactions[]` entry binding to `attack_targeted`. v1 limitations filed for v3: per-item charge tracking, auto-effect resolution, GM Reactions Panel integration, "suppressed for 1 round on damage" Cloak state tracker. Ring of Spell Turning + Spell-Storing Ring + Periapt of Wound Closure still ⚪. **v2.77.0 ("Three Luck Points") — Phase 4b partial / Phase 4 COMPLETE:** Lucky feat shipped, completing all four Phase 4 feats. First reaction with charge-tracked resource (3 charges / long rest, not per-turn or per-trigger). Helper reads `sheet.resources["lucky"].current` alongside the feat detection. Dispatch decrements via in-place `sheet.resources[*].current` mutation. v1 only wires the "attack roll against you" trigger (reuses v2.69.0 attack_targeted); the three own-roll surfaces (attack/check/save) need new trigger events. Auto-reroll + Lucky-vs-Lucky cancel mechanic filed for v3. Garrik (Fighter) got the feat + 3-charge resource. **v2.76.0 ("Spell in Hand") — Phase 4c partial:** War Caster feat shipped as the third PC-feat reaction. Extends both OA branches (`creature_exits_reach` + Polearm Master `creature_enters_reach`) with a second option per prompt — `take-war-caster-cast`, a 1-action spell cast at the provoker. Helper gates on the feat AND at least one prepared/known 1-action spell. Tavik got the feat (Cleric, Sacred Flame + Guiding Bolt + Inflict Wounds satisfy the spell-list gate). v1 filed for v3: auto-cast routing through `/cast_spell` against the provoker, spell-list filtering in the UI, the two passive War Caster benefits (concentration-save advantage on damage, somatic-while-holding-weapons). **v2.75.0 ("Mage Slayer") — Phase 4d partial:** Mage Slayer feat shipped as the second PC-feat reaction. Reuses the v2.70.0 `spell_cast_near` trigger + 60-ft walker; the walker's eligibility check broadened from CS-only to `cs_eligible OR ms_eligible`. `_eligible_reactions[spell_cast_near]` builds a list with Counterspell (60 ft) + Mage Slayer (gated on `context.distance_ft <= 5.0`). Dispatch marks reaction + broadcasts `feature_used(source=mage-slayer)` naming caster + spell. Krieger got the feat (Greataxe-wielding Barbarian, melee-only). v1 limitations filed: auto-roll the melee attack against the caster, passive save advantage vs spells from nearby casters, concentration-check disadvantage on damage Krieger deals. **v2.74.0 ("The Finesse Parry") — Phase 4a partial:** Defensive Duelist feat shipped as the first PC-feat reaction wired to a runtime trigger. Reuses the v2.69.0 `attack_targeted` trigger event; new helper `_pc_has_defensive_duelist_available` gates on feat + equipped finesse weapon. PC branch of `_eligible_reactions[attack_targeted]` now appends a `use-defensive-duelist` option alongside Shield. Dispatch marks reaction + broadcasts `feature_used(source=defensive-duelist, pb_bonus=N)`. Lyra got the feat (Bard, Rapier-equipped, no UD to compete with the reaction slot). v1 filed for v3: retroactive AC negation of triggering attack, melee-only gate, Pip + UD interaction. **v2.73.0 ("The Parried Blade") — Phase 6 partial:** NPC stat-block reactions (Parry, etc.) now surface through the `attack_targeted` trigger pipeline. `/attack` emit gate extended from PC-only to "any combatant with reaction unused"; `_eligible_reactions[attack_targeted]` branches on `watcher_char_id` (PC → Shield, NPC → walks `_monster_template_to_sheet(tmpl).actions` for `category == "reaction"` entries). New `monster-*` dispatch in `/use_reaction` marks the NPC reaction via `_mark_battle_economy_by_combatant_id` and broadcasts `feature_used(source="monster-reaction")` with action_name + monster_name + desc. Filed for v3: per-reaction effect resolution (Parry's +2 AC retroactive bump, Snake Form's transformation, etc.); NPC reactions on other triggers (damage_taken, spell_cast_near, ally_attacked_near). **v2.72.0 ("Argent Words") — Phase 3d partial:** Silvery Barbs wired to a new `save_resolved` trigger event. Emit fires from the `/roll_request/{id}/respond` save-resolution path when `roll_req.dc is not None AND result.total >= dc` (RAW: SB triggers on SUCCESS). Walker iterates every PC combatant in the battle (excluding the rolling character themselves), checks `_pc_has_silvery_barbs_available`. `/use_reaction` `cast-silvery-barbs` dispatch consumes the 1st+ slot, marks reaction, broadcasts `feature_used(source=silvery-barbs-cast)` with `rerolled_target_*` fields + `original_rolled`/`original_dc` for the GM/player to re-roll manually. Demo: Silvery Barbs added to Thalindra's spell list. v1 limitations filed for v3: auto-reroll the d20, advantage-on-next-roll buff for an ally, 60-ft range gate, extension to `attack_resolved` / `check_resolved` triggers. **v2.71.0 ("The Devil's Bargain") — Phase 3c partial:** Hellish Rebuke + Absorb Elements wired to the existing `damage_taken` event. Second `_emit_reaction_prompt(damage_taken)` site in `_apply_damage_to_combatant`'s PC branch (after `_log_damage_entry`) gated on `applied > 0 AND not uncanny_dodge_used AND not reaction_used`; `_eligible_reactions[damage_taken]` now returns a LIST so UD ack + HR + AE can coexist. Hellish Rebuke dispatch consumes lowest 1st+ slot, marks reaction, broadcasts `feature_used(source=hellish-rebuke-cast)` with `damage_expr` + `damage_type=fire` + `rebuke_target_*` fields. Absorb Elements dispatch installs `absorb-elements-active` buff with `effects.resistance_damage_type`, `effects.next_melee_bonus_dice`, `effects.next_melee_bonus_type`. v1 limitations filed: auto-damage to rebuked attacker, resistance pipeline reading buff effects, next-melee bonus damage pipeline reading buff effects, demo PC fixture for Absorb Elements. Phase 3d (Silvery Barbs + `save_failed`) / 3e (Feather Fall + `falling`) still ⚪.
