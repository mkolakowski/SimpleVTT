...
- ✅ **Bundled demo assets** — done in v2.3.0. `app/static/demo/maps/tavern.png`, `tokens/rogue.png`, `tokens/wizard.png` (no audio shipped — `tavern.ogg`/`battle.ogg` were skipped at the original landing per the v2.3.0 release notes). **Update (v2.689.x):** all demo NPC/PC tokens plus every battle map — the five leveled maps and the Sundered Vault's `tavern.png` — now ship final art (generated via the ImagineArt MCP from `docs/demo/image-prompts.md`).
...
- ✅ **Upload lockdown** — done in **v2.1034.0** ("The Velvet Rope"). `DEMO_DISABLE_UPLOADS` (default `true`) makes every user-facing file-upload endpoint (token/portrait/template/map/handout/encounter-background images, audio tracks, character/campaign import) return `403 {"detail": "Uploads are disabled on this demo instance"}` — **only when `DEMO_MODE` is also true**, so normal deploys are untouched. Implemented as the shared FastAPI dependency `app.auth.require_uploads_enabled` (predicate `uploads_disabled_in_demo`) attached to the 11 upload handlers in `tabletop_routes.py` / `notes_routes.py` / `audio_routes.py`. Keeps anonymous demo visitors from filling the shared `uploads_data` volume with arbitrary files between hourly reseeds. Set `DEMO_DISABLE_UPLOADS=false` to re-enable uploads on a demo you control. Covered by `tests/harness/test_demo_disable_uploads.py`. **Widened in v2.1034.1 ("The Closed Loophole")** to seal the remaining write paths: the admin `POST /campaign/{id}/thumbnail` + `/maps` routes and the admin-center `POST /campaigns/{id}/maps` map upload now respect the same flag, and the *optional* thumbnail attached to campaign create / settings-save is skipped on a locked demo (the endpoints stay functional — only the file write is dropped).
