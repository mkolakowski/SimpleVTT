...
- ⏸ **Phase 2b — `/api/campaign/{id}/roll` parity** — still open. Conditions only auto-fire on `/attack` today; saves and ability checks made via the roll endpoint don't yet read the same source set. Same helpers should fold into `_apply_roll_state` (or sit next to it) as a v2.152.x follow-up.
- ⏸ **Phase 2c — NPC attacker condition automation** — still open. `_attacker_has_condition_disadvantage` reads PC `_buffs_active`; NPC attackers carrying poisoned would not auto-disadvantage. One-helper extension reading the attacker's combatant.buffs as a fallback when no PC sheet is present.
