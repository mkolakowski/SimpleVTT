...
**Phase 3 shipped in v2.1001.0** — the positional prone edge. RAW PHB p.292: an attack roll against a prone creature has advantage when the attacker is within 5 ft, disadvantage otherwise. The long-standing "needs Maps 2.0" blocker dissolved once the Maps 2.0 arc landed (v2.752.0–v2.763.0): the new `_target_prone_positional_edge` helper rides the existing `_combatant_token` (v2.99.432, resolves PC + NPC combatants to tokens) + `_distance_ft_between_points` (v2.61.0 grid math) substrate with zero new geometry code, folding into all three attack sites (`/attack` bonused + bonusless branches, `/npc_attack`). Off-grid (either side has no token / no gridded map) → no edge, GM adjudicates as before. See [Implementation status](#implementation-status) below for the per-phase breakdown.
...
- ✅ **Phase 3 — Context-aware rolls (positional prone edge)** — shipped in **v2.1001.0**. `_target_prone_positional_edge(db, campaign_id, attacker_combatant, target_combatant_id)` returns `"advantage"` (target prone + attacker token within 5 ft), `"disadvantage"` (target prone + attacker farther), or `None` (target not prone, or positions unresolvable — off-grid stays GM-adjudicated). The attacker side accepts a real hub combatant dict (NPC path via `source_token_id`) or a `{"char_id": id}` pseudo-dict (PC path); both resolve through `_combatant_token`. Folded into the `/attack` bonused + bonusless source sets (labels `target_prone_within_5ft` / `target_prone_beyond_5ft`) and the `/npc_attack` mirror; Elusive suppression + PHB p.173 cancel compose for free. Note the RAW rule is distance-based, not melee/ranged-based — a reach weapon swung from 10 ft rolls at disadvantage. Harness: `tests/harness/test_attack_prone_positional.py` (within-5ft advantage, beyond-5ft disadvantage, off-grid fallback, attacker-prone-vs-target-prone cancel). The prone-attacker disadvantage half had already shipped in Phase 2a.
...
- 🟢 **Phase 4 — Item-granted adv/dis** — Phase 4a shipped v2.252.0 (Cloak of Displacement: attacks against the wearer have disadvantage) + its suppress-after-damage clause v2.645.0 (`cloak_disp_suppressed_round`). Phase 4b shipped v2.253.0 (Cloak of Elvenkind Stealth advantage) + its perception-disadvantage half v2.1055.0 (`vs_character_id` target-side read). Eyes of the Eagle shipped v2.254.0. **Both cloaks fully modeled;** no filed follow-ups remain here. See [Phase 4 — Item-granted adv/dis](#phase-4--item-granted-advdis).
...
### Phase 3 — Context-aware rolls — ✅ shipped v2.1001.0

Attack rolls from within 5 ft of a prone target → auto-advantage; from farther away → auto-disadvantage (RAW PHB p.292 — the split is distance-based, not melee/ranged-based). Shipped once Maps 2.0 (v2.752.0–v2.763.0) made token positions authoritative: `_target_prone_positional_edge` composes `_combatant_token` + `_distance_ft_between_points` and folds into `/attack` (both branches) + `/npc_attack`. Off-grid → no edge (GM adjudicates). See the implementation-status bullet above for details + test coverage.
...
**Status:** 🟢 Phase 4a shipped v2.252.0 (Cloak of Displacement) + suppress-after-damage v2.645.0. Phase 4b shipped v2.253.0 (Cloak of Elvenkind Stealth) + perception-disadvantage half v2.1055.0. Both cloaks fully modeled; no filed follow-ups remain.
...
**RAW (DMG p.158, rare, attunement):** "While you wear this cloak, it projects an illusion that makes you appear to be standing in a place near your actual location, causing any creature to have disadvantage on attack rolls against you. If you take damage, this property ceases to function until the start of your next turn. This property is suppressed while you are incapacitated, restrained, or otherwise unable to move." (v1 modeled the always-on disadvantage; the **"suppressed after damage" clause shipped v2.645.0** — `_apply_damage_to_combatant` stamps `cloak_disp_suppressed_round` on the wearer's combatant and `_target_wearer_imposes_attack_disadvantage` suppresses the disadvantage for that round. The "while incapacitated/restrained" clause remains GM-narrated.)
...
- ✅ **Stealth-advantage items** — **shipped v2.253.0** (Cloak of Elvenkind) + **v2.255.0** (Boots of Elvenkind). The `check_advantage_on: ["stealth"]` item-effect feeds `/roll`'s adv source set via `_roll_item_check_advantage`. Boots of Elvenkind (no attunement) dropped in on the same substrate by adding a `boots-of-elvenkind` `_MAGIC_ITEM_PASSIVES` row (no `requires_attunement`) + a demo seed on Quan Reelstep — riding free of the attunement gate alongside his full 3/3 loadout.
...
- ✅ **Eyes of the Eagle** — **shipped v2.254.0**. Advantage on Wisdom (Perception) checks via the same `/roll` substrate as Phase 4b (a `check_advantage_on: ["perception"]` `_MAGIC_ITEM_PASSIVES` row + demo seed on Mira Greenleaf). No new helper/`/roll` code — the substrate generalizes over the skill key.
