...
- ✅ **End-to-end happy-path regression test (v2.428.0).** Resolved by monkeypatching `httpx.AsyncClient` in-process instead of depending on the wiremock container. 9 new tests cover the exact request shape (URL + Bearer header + Content-Type + body), the response parsing (`rule_id` extraction), the error paths (non-200, `success=false`, env-unset), the idempotent delete (404 treated as success), the list shape (array + IP filter), and the client-side notes truncation. The wiremock service stays in `docker-compose.yml` under the `dev` profile for operator-side dev convenience.
