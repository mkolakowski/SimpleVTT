...
- Ioun Stone ability variants (+2, max 20) ✅ v2.224.0 — shipped on a NEW capped-additive substrate (`ability_bonus` / `ability_bonus_cap`) layered onto this engine. Unlike the set-based items (Belt/Amulet/Headband/Gauntlets), the Ioun Stones *add* a bonus with a ceiling rather than setting a fixed value; `effective_ability_score` folds the bonus AFTER any set override (raises toward the cap, never lowers). The six ability variants share the single `ioun-stone` slug with the variant riding the item via `_ability_bonus`. Seed on Magnus Hexbinder (Warlock, INT 10 → 12) + `test_item_ioun_stone.py`.
