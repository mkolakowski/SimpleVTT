...
**Authors:** rolling
**Last updated:** 2026-06-12 (Phase 3a complete — exact attack-roll buff-effect uplifts: Bless +1d4 / Bane −1d4)
...
**Status:** 🟠 partial — Phase 3a shipped (v2.183.6). The two auto-applied
attack-roll uplifts (Bless +1d4 / Bane −1d4) are validated *exactly* in
`tests/harness/test_spell_catalog_buff_effects.py`: a same-seed
with/without-buff attack pair isolates the buff die and asserts the
attack-total delta equals the printed d4 value × the registry sign, so a
`1d4`→`1d6` content edit, a dropped uplift, or a sign flip fails the gate.
Filed for later 3b+ slices: save-side Bless/Bane (+1d4/−1d4 on saves),
Shield of Faith (+2 AC), Mage Armor (13+DEX), Haste / Slow (AC + speed),
Hex / Hunter's Mark (+1d6 on hits). The per-spell spot tests in
`test_buff_attack_hooks.py` already cover several of these at
"token appears" granularity; 3b promotes them to exact-contribution gates.
