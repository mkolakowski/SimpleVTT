...
**Authors:** rolling
**Last updated:** 2026-06-12 (Phase 2A multi-beam backfill)
...
damage type. **Still filed:** save-for-half (`auto_save_damage_rolled`)
and auto-hit (`auto_hit_targets`, Magic Missile) — both gate on
`campaign.auto_apply_damage`, which the multi-beam/attack-roll paths
don't require, so they need the settings-form toggle wired into the
test first.
...
Filed follow-up (not in v2.182.6): the save-gated condition installs
(`_SPELL_CONDITION_MAP` — Hold Person → Paralyzed, Bane → Baned,
Faerie Fire, Confusion, Banishment, …) need a forced-fail save (via
`/api/test/dice/seed`) to install deterministically; several already
...
- [🟠] Phase 2A — Damage assertions (v1 v2.49.108: Fire Bolt attack-roll; v2.182.8: multi-beam backfill (`test_spell_catalog_multibeam.py`) — Scorching Ray 3×2d6 + Eldritch Blast 2×1d10 beam-count/per-beam-range/aggregate-sum/type; filed: save-for-half + auto-hit (both need the `auto_apply_damage` toggle))
