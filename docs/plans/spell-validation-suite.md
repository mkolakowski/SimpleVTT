...
Phase 3d adds the weapon-hit damage riders (Hunter's Mark / Hex, +1d6 on
hits): a real cast through the dedicated endpoint installs a
`weapon_hit_bonus_dice: "1d6"` rider on the caster keyed to the target, and
...
- Wish — ✅ shipped v2.183.20 (`test_cast_wish.py`): the L9 narration-only capstone — no dedicated endpoint, no `_SPELL_BUFF_MAP` entry, effect is pure DM adjudication; rides the generic `/cast_spell` path. Pins the catalog shape (L9 Conjuration / Self / Instantaneous / `concentration: false` / Sorcerer+Wizard) + an SRD-build quirk (the stress clause folded into the cast action's `damage: "1d10"` necrotic, but no `attack_roll`/`save_ability` — narration-grade backlash), the cast contract (L9 slot decrement + `spell_cast` broadcast names it; "doesn't 500"), and the non-concentration fact (no `concentration-wish` anchor). No L17 demo PC, so the fixture patches Wish + a wizard L9 slot onto Thalindra, restored in finally.
- Mage Armor — ✅ shipped v2.183.19 (`test_ac_buff_spells.py`, 2 tests appended): the suite's reference case for a *documented RAW-vs-engine simplification* — RAW sets base AC to 13 + Dex, the engine installs a flat +3 `ac_bonus` buff (non-concentration, 8 hours, applied unconditionally; the unarmored gate / 13+Dex recompute are intentionally not enforced). Pins the catalog shape (L1 Abjuration / Touch / 8 hours / `concentration: false` / Sorcerer+Wizard / no-save-attack-damage action) and the buff-contract simplification (flat `ac_bonus: 3`, no `ac_set`/`ac_base`/`ac_override`, non-concentration, 8-hour duration). Complements the existing `test_mage_armor_installs_ac_buff_and_raises_ac` (+3 into `/attack` `target_ac`). Caster: Magnus Hexbinder (Warlock), self-cast.
- Mirror Image — ✅ shipped v2.183.18 (`test_cast_mirror_image.py`): the suite's reference case for a *deliberately narration-only* spell — no dedicated endpoint, no `_SPELL_BUFF_MAP` entry, single no-save/attack/damage catalog action. Pins the catalog shape (L2 Illusion / Self / 1 minute / `concentration: false`), the generic `/cast_spell` cast contract (L2 slot decrement + `spell_cast` broadcast), the RAW non-concentration fact (no `concentration-mirror-image` anchor — inverse of Polymorph), and the narration-only contract (no duplicate/AC buff installed; the 3-duplicate misdirection is GM-narrated). Caster: Zara Emberfire (Sorcerer, spell_index 9).
- Bless of Bahamut (homebrew) — ✅ shipped v2.183.22 (`test_cast_homebrew_spell.py`, 3 tests): the homebrew-handling proof. A fictional "Bless of Bahamut" (deliberately absent from the catalog) casts entirely from its inline sheet definition via the generic `/cast_spell` path (tabletop_routes.py:18164-18177 reads the sheet dict, enriches from the catalog only when `_slug` resolves, `setdefault` so sheet wins). Pins (1) catalog absence guard, (2) the fully-homebrew inline cast (slot decrement + `spell_cast` broadcast echoing the homebrew name/level/school/casting-time/save/damage + inline `actions` verbatim), and (3) sheet-override-beats-catalog precedence (a renamed `bless` broadcasts the sheet name while the catalog Enchantment school bleeds through). Closes the Phase 0 complex list.
...
- Spiritual Weapon — ✅ shipped v2.183.15 (`test_cast_spiritual_weapon.py`): upcast dice scaling (base 2nd-level hit rolls a single d8; a 4th-level hit exceeds a single d8's ceiling → proves the +1d8-per-two-levels tier added a die) + the catalog-vs-runtime concentration divergence (catalog `concentration: false`, runtime binds concentration + broadcasts `concentration_update`). Complements the endpoint-named `test_use_spiritual_weapon.py` (summon / concentration-drop / single-attack lifecycle).
...
- Hold Person — ✅ shipped v2.183.16 (`test_cast_hold_person.py`, 2 tests appended to the existing endpoint-contract file): the installed buff is the canonical Paralyzed condition (`key == "paralyzed"`, concentration, `hold-person-spell` source, WIS re-save stamps, speed→0, full RAW `raw_effects` narration), and the RAW end-of-turn WIS re-save via `/use_repeated_save` drops the buff on a pass / keeps it on a fail (seed-loop observing both outcomes). Auto-fail-saves / melee-auto-crit remain GM-narrated `raw_effects`, not engine-enforced.
- Polymorph — ✅ shipped v2.183.17 (`test_cast_polymorph.py`, 3 tests appended): the catalog-vs-runtime concentration divergence (catalog `concentration: false`, `/cast_polymorph` returns `concentration: true` + installs the `concentration-polymorph` anchor) + the full six-ability stat-block replace distinguishing Polymorph from Wild Shape (all six abilities become the beast's, beast HP pool, mental stats changed) with `/revert` restoring the prior form exactly (asserted structurally off the transform response's `form_sheet`; Open5e-gated). Complements `test_transform.py` (Wild-Shape path), `test_polymorph_*_wis_save.py`, and `test_polymorph_*_concentration_revert.py`.
- Conjure X / Summon X — ✅ shipped v2.183.21 (`test_cast_conjure_family.py`, 3 tests): the narration-only contrast to Conjure Animals. The five higher conjures (Celestial L7, Elemental L5, Fey L6, Minor Elementals L4, Woodland Beings L4) have no bespoke endpoint and no `_SPELL_BUFF_MAP` entry — they ride the generic `/cast_spell` path, which spends a slot + broadcasts `spell_cast` but creates no combatant. Pins (1) the catalog shape across all five (Conjuration, level, spell-list membership, no-save/attack/damage action, and the `concentration: false` vs "Up to" duration divergence) and (2) the generic-cast narration contract for Conjure Minor Elementals (L4 slot decrement + named broadcast, NO `combatants`/`token_ids` summon payload, no buff/`concentration-conjure-*` anchor). Conjure Animals' real summon-engine path stays covered by `test_cast_conjure_animals.py`.
