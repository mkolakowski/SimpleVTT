...
**Status:** 🟢 substantially shipped (re-audited 2026-06-10, v2.158.68) — Phase 0 shipped (v2.49.120-123): both Font of Magic endpoints + multiclass `class_slug` + ephemeral slot creation, 13 harness tests. Phase 1 shipped (v2.49.124-125): `/use_metamagic_empowered_spell` endpoint + `/cast_spell` integration on save-for-half single-target NPC path AND multi-beam attack-roll spells (Scorching Ray / Eldritch Blast / Fire Bolt) with pool reroll across all beams, 7 harness tests. **All 8 PHB metamagics now shipped end-to-end.** Empowered (v2.49.124), Twinned (v2.99.33/.167/.174/.181/.183/.184/.187/.189), Distant (v2.99.34/.159), Heightened (v2.99.35-.36/.41), Careful (v2.99.38/.42), Extended (v2.99.37/.161/.165), Subtle (v2.99.162/.173/.186), and **Quickened ✅ v2.649.0** — the final metamagic: `POST /use_metamagic_quickened_spell` (2 SP → `metamagic-quickened-pending` buff) + a `/cast_spell` read (`_caster_has_quickened_pending`) that retargets a 1-action cast's economy slot to `bonus` and consumes the buff, built on the v2.643.0–v2.648.8 economy-slot-retarget plumbing (`as_reaction` / `as_war_magic_bonus`); harness `test_use_metamagic_quickened.py`. **Sorcerous Restoration ✅** (v2.99.39) closes the Lv 20 capstone. **AoE multi-target Empowered loop integration (Phase 1.5) ✅ shipped v2.661.0** — Empowered's reroll now fires on the `/place_aoe` placement path (first-target-wins, once per cast). With Quickened (v2.649.0) + this, **all 8 metamagics are fully shipped end-to-end with no remaining scope finishers.** The only GM-tracked edge left is the Quickened "can't pair two leveled spells in one turn" rule. The plan body below pre-dates the v2.99.x shipping arc — treat it as historical context.
**Authors:** rolling
...
- [✅] Phase 1 — Picker + pending-buff machinery + Empowered Spell (v2.49.124 walking skeleton: endpoint + sheet button + save-for-half single-target integration; v2.49.125 multi-beam pool reroll for Scorching Ray / Eldritch Blast / Fire Bolt. **Phase 1.5 AoE multi-target `/place_aoe` loop integration ✅ v2.661.0** — first-target-wins reroll on the placement path. Styled picker modal deferred to Phase 4.)
