...
| 3 | Metamagic | 🟢 | **Empowered Spell ✅** v2.49.124 — `/use_metamagic_empowered_spell` arms a `metamagic-empowered-pending` buff on the caster; the next `/cast_spell` damage roll reads the buff via `_roll_with_empowered_reroll` and rerolls up to CHA-mod lowest dice. Costs 1 SP. Zara Emberfire (Tiefling Sorcerer Lv 5) is the demo fixture. Harness: `test_use_metamagic_empowered.py` (9 tests covering arm, consume on Fireball, pool reroll on Scorching Ray multi-beam, slot-scaling, error paths). **Quickened Spell 🟡** v2.6.0 — curated `_FEATURE_ECONOMY['quickened-spell']` entry with `slot: 'bonus'`; clicking the class-features button announces + flips the Bns chip but the player still casts the actual spell separately (no automatic action-time swap — Quickened RAW changes the spell's casting time from 1 action to 1 bonus action; the demo simulates this via the chip flip). **Other metamagics ⚪** — Twinned Spell, Subtle Spell, Distant Spell, Heightened Spell, Careful Spell still need per-cast intercepts. Picker UI for selecting metamagic at cast time is filed. |
| 4 / 8 / 12 / 16 / 19 | Ability Score Improvement | ✅ | |
