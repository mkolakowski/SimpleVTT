...
| 3 | Metamagic | 🟢 | **Empowered Spell ✅** v2.49.124 — `/use_metamagic_empowered_spell` arms a `metamagic-empowered-pending` buff; the next `/cast_spell` damage roll reads the buff and rerolls up to CHA-mod lowest dice. Costs 1 SP. 9 tests. **Twinned Spell ✅** v2.99.33 — `/use_metamagic_twinned_spell` decrements SP by `max(1, spell_level)` + broadcasts trigger. v1 announce-only. 6 tests. **Distant Spell ✅** v2.99.34 — `/use_metamagic_distant_spell` decrements 1 SP + broadcasts trigger. v1 announce-only — GM applies the doubled range / 30 ft Touch-extension at cast time. 3 tests. **Quickened Spell 🟡** v2.6.0 — curated `_FEATURE_ECONOMY['quickened-spell']` entry with `slot: 'bonus'`; clicking the class-features button announces + flips the Bns chip. Zara Emberfire (Tiefling Sorcerer Lv 5) is the demo fixture for all four. **Other metamagics ⚪** — Subtle Spell (blocked on F7 components), Heightened Spell, Careful Spell, Extended Spell still need per-cast intercepts. Picker UI for selecting metamagic at cast time is filed. |
| 4 / 8 / 12 / 16 / 19 | Ability Score Improvement | ✅ | |
