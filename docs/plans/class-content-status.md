...
| 3 | Martial Archetype | ✅ | Subclass system shipped — see Subclasses table. Champion: Improved Critical ✅ (v2.49.231); Remarkable Athlete still descriptive. Battle Master has Superiority Dice counter. |
...
| Fighter | **Champion** | ✅ | n/a | 🟢 | **Improved Critical ✅** (v2.49.231 — `_attacker_crit_threshold(sheet)` returns 19 for Champion Fighter Lv 3+, used by `use_attack`'s crit detection; harness in `test_use_attack_improved_critical.py`). Remarkable Athlete / Survivor still descriptive — Survivor (Lv 18) waits on a Lv 18+ fixture; Remarkable Athlete needs the ability-check intercept that the Lv 9 Indomitable save-reroll work will also need. |
