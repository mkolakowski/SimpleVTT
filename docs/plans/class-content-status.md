...
|---|---|---|---|
| 1 | Rage | 🟢 | `/use_rage` endpoint installs the rage buff with `effects.melee_str_damage_bonus`, `advantage_on=["str_check","str_save","str_attack"]`, `resistance_to=["bludgeoning","piercing","slashing"]`. **Damage bonus ✅** (v2.20.0 via `_compute_attack_auto_uplifts`), **STR attack advantage ✅** (v2.49.238 via `_attacker_has_str_attack_advantage`), **B/P/S resistance ✅** (v2.20.0 via `_resistance_halve` buff-reading), **STR save advantage ✅** (v2.99.26 via `_pc_has_rage_str_save_advantage` at all 3 save-roll construction sites). Krieger Stonefist (Half-Orc Berserker Lv 7) is the demo fixture. Harness coverage: `test_use_rage.py` + `test_rage_str_save.py` (3 tests with Gust of Wind STR save). **STR check advantage** is the last descriptive-only half — the `/roll` endpoint doesn't carry an ability-tag in its body, so STR-vs-other check disambiguation is filed for a follow-up. |
