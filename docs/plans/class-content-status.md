...
| High Elf | Darkvision, Keen Senses, Fey Ancestry, Trance, Elf Weapon Training, Cantrip, Extra Language | 🟢 | Cantrip choice + Elf Weapon Training proficiency wire through existing systems. **Fey Ancestry charm-save advantage ✅** v2.99.11 — Thalindra (Elf Wizard), Mira (Wood Elf Druid), Kael (Wood Elf Monk) all benefit; saves vs charm install swap d20 → 2d20kh1. **Trance** is NOT a save-advantage trait — RAW it just means "elves meditate 4 hours instead of sleeping 8" (no combat / save effect). The v2.60.1 plan conflated this with the Fey Ancestry "magically put to sleep" immunity clause (corrected v2.99.14). Fey Ancestry's sleep-immunity half is a condition-install IMMUNITY gate (Phase 3 territory, mirror of v2.55.0 Aura of Devotion shape) — filed there. |
...
Phase 2 follow-ups still ⚪: Halfling Lucky attack-roll + ability-check surfaces; Stout Halfling Stout Resilience (needs subrace-aware slug normalizer to differentiate from Lightfoot). Phases 3–5 still ⚪ (condition-install immunity gate keyed off race slug — would ship Fey Ancestry sleep-immunity + Half-Orc Relentless Endurance HP-pinned-to-1 + Tiefling Infernal Legacy per-day cantrip). Poison damage RESISTANCE half of Dwarven Resilience filed — needs v2.63.0 resistance pipeline to read a per-PC race trait list.
