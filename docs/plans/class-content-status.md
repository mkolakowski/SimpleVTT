...
> Missiles flipped to 🟢 (reactions-panel entry, v2.68.6). **Aura of
> Courage shipped v2.368.0** (`_ally_has_aura_of_courage` + pre-install
> `_install_buff` gate + 3 harness tests via Sir Caelan PATCH'd to Lv
> 10) — the last genuine ⚪ row from the v2.344.3 reconciliation is now
> ✅. The headline class-feature coverage is **~99%** (the one remaining
> ⚪ would be a new SRD addition, not a v2.344.3 holdover).
>
...
|---|---|---|---|
| 1 | Rage | ✅ | `/use_rage` endpoint installs the rage buff with `effects.melee_str_damage_bonus`, `advantage_on=["str_check","str_save","str_attack"]`, `resistance_to=["bludgeoning","piercing","slashing"]`. **Damage bonus ✅** (v2.20.0 via `_compute_attack_auto_uplifts`), **STR attack advantage ✅** (v2.49.238 via `_attacker_has_str_attack_advantage`), **B/P/S resistance ✅** (v2.20.0 via `_resistance_halve` buff-reading), **STR save advantage ✅** (v2.99.26 via `_pc_has_rage_str_save_advantage` at all 3 save-roll construction sites), **STR check advantage ✅** (v2.99.28 via `_pc_has_rage_str_check_advantage` + optional `stat_key` body field on `/roll`). Krieger Stonefist (Half-Orc Berserker Lv 7) is the demo fixture. Harness: `test_use_rage.py` + `test_rage_str_save.py` (3 tests) + `test_rage_str_check.py` (3 tests). Client-side skill / ability buttons still need to send `stat_key` for STR-keyed rolls to fully exercise the check advantage end-to-end; filed. |
| 1 | Unarmored Defense | ✅ | **Shipped v2.369.0** (`_pc_unarmored_defense_ac` helper + `_read_target_ac` hook). RAW: AC = 10 + DEX mod + CON mod while no armor (shield OK). Engine takes max(stored, computed) so PATCHing CON or DEX auto-flows to attack-time AC reads. Harness `test_unarmored_defense.py::test_barbarian_unarmored_defense_*`. |
...
| 20 | Superior Inspiration | ✅ | v2.99.44 — `/battle` PUT hook detects the inactive → active transition (canonical "initiative rolled" moment), walks combatants for PC Bards Lv 20+ whose `bardic-inspiration` resource is at 0, refunds 1 use, broadcasts `resource_update` + `feature_used(source=superior-inspiration)`. Skips quietly when BI > 0 (RAW gate), level < 20, non-Bard, or battle already active (e.g. GM tweaks turn_index mid-combat — no re-trigger). Lyra Sunstrider is the demo fixture; tests bump her Lv 6 → Lv 20 via the v2.99.39 capstone-test pattern. 5 tests cover happy + 4 gate cases. |
...
| 2 | Channel Divinity | ✅ | Resource counter (`key: 'channel-divinity'`) + v2.9.0 option-picker (`showResourceOptionPicker`). **All 12 canon Cleric domains now have a curated Lv 2 CD option** (v2.56.1 added Knowledge / Tempest / Trickery / Forge / Grave / Order / Nature / Twilight; **v2.56.2** added Death / Arcana / Peace, plus rolled Destroy Undead into Turn Undead's desc). Life Domain shipped v2.14.0 with Turn Undead + Preserve Life; Light + War options shipped v2.14.3. Each option is announce-only — the GM applies the mechanical effect manually after the chip flip + broadcast. **Lv 6 options shipped v2.99.80** (Knowledge: Read Thoughts; Trickery: Cloak of Shadows) along with a picker `min_level` gate that hides Lv 6+ options from clerics under the required level (filter in `sheet_dnd5e.html`'s `classEntries` walker reads `cc.level` per entry, options without `min_level` stay always-on per the legacy Lv 2 behavior). Harness in `test_use_feature.py` covers both new options (`test_channel_divinity_read_thoughts`, `test_channel_divinity_cloak_of_shadows`). Curated table also serves Paladin Oath of Devotion (Sacred Weapon + Turn the Unholy). **Use scaling** (Lv 2-5: 1/short rest; Lv 6-17: 2; Lv 18+: 3) lives on the sheet's `resources.channel-divinity.max` field — bumped via demo seed when the cleric levels (Tavik 1/1 at Lv 5 → 2/2 at Lv 6 in v2.57.1). |
...
| 10 | Divine Intervention | ✅ | v2.99.47 — `/use_divine_intervention` endpoint. Validates Cleric + level >= 10 + daily counter. At Lv 10-19 rolls `1d100` (success when ≤ cleric level); at Lv 20 auto-succeeds (no roll). Atomic decrement + `resource_update` + `feature_used(source=divine-intervention)` carrying `{success, auto_success, rolled, threshold}`. Brother Tavik Stonebrow is the demo fixture; tests bump Lv 8 → Lv 10/20 via the v2.99.39 capstone-test pattern + use the v2.49.12 `/api/test/dice/seed` for d100 reproducibility. 6 tests cover both level paths + 4 gate cases. v1 simplification: 1/long-rest cooldown regardless of outcome (RAW says 7-day cooldown on success — filed for a multi-rest tracker). |
...
| Lv | Feature | Status | Notes |
|---|---|---|---|
| 1 | Fighting Style | 🟢 | **Four of six options auto-apply at /attack time, gated on `sheet.fighting_style`.** **Archery ✅** v2.99.83 — `_pc_archery_bonus(sheet, attack)` returns +2 when the attacker has Archery + the attack is a ranged weapon (range contains `/` or > 15 ft); appended to the d20 expression. Rowan Quickbow demo. **Dueling ✅** v2.99.83 — `_pc_dueling_bonus(sheet, attack)` returns +2 when Dueling + 1H melee; appended to damage. **Great Weapon Fighting ✅** v2.99.85 — `_apply_great_weapon_fighting_reroll(damage_expr)` rolls each die individually + rerolls 1s/2s once (RAW PHB p.72 "must use new result"); breakdown shows `original→kept` markers. Garrik Ironside demo. **Two-Weapon Fighting ✅** v2.99.87 — `_pc_two_weapon_fighting_eligible` + `_two_weapon_fighting_ability_mod`; off-hand attacks (`attack.off_hand: True`) gain `+CHA/+DEX/+STR` mod derived from `attack_bonus − proficiency_bonus`. Rowan's Shortsword gets `off_hand: True` flag. **Protection ✅** wired via the v2.68.10 reactions panel (reaction-triggered AC/disadvantage). **Defense 🟡** — +1 AC when wearing armor still needs the auto-AC engine (`sheet.ac` is hand-authored today). 7 harness tests across `test_fighting_style.py`, `test_great_weapon_fighting.py`, `test_two_weapon_fighting.py`. |
...
| Lv | Feature | Status | Notes |
|---|---|---|---|
| 1 | Unarmored Defense | ✅ | **Shipped v2.369.0** (`_pc_unarmored_defense_ac` helper + `_read_target_ac` hook). RAW: AC = 10 + DEX mod + WIS mod while no armor + no shield. Engine takes max(stored, computed). Harness `test_unarmored_defense.py::test_monk_unarmored_defense_*` + `test_monk_shield_disables_unarmored_defense`. |
| 1 | Martial Arts | ✅ | v2.99.81 — RAW (PHB p.78) unarmed-strike + monk-weapon die progression auto-substitutes at /attack time. `_monk_martial_arts_die(sheet)` returns "1d4"/"1d6"/"1d8"/"1d10" based on monk level (Lv 1-4/5-10/11-16/17+); `_apply_monk_martial_arts_die(sheet, attack_name, damage_expr)` regex-swaps the leading die when the MA die is larger than the sheet's authored die (never downgrades — sheet-authored larger dice win). Name gate matches "unarmed strike" or "martial arts" in the attack name (case-insensitive); Kael's "Quarterstaff (Martial Arts)" picks up the swap. Kael Brightleaf (Way of the Open Hand Lv 7) demo fixture; harness `test_monk_martial_arts.py` (4 tests, capstone-test pattern via PATCH for Lv 5/11/17). |
...
| 3 | Deflect Missiles | ✅ | **Shipped v2.370.0** — auto-fire in `_apply_damage_to_combatant` mirrors the v2.49.243 Uncanny Dodge pattern, gated on the v2.366.0 `is_ranged_weapon_attack` kwarg. Rolls `1d10 + DEX + Monk Lv` and subtracts (floor 0), marks the Monk's reaction, broadcasts `feature_used(source="deflect-missiles")` with `caught: True` when reduction zeros the damage (catch+ki-throw sub-feature remains GM-narrated v1). Harness `test_deflect_missiles.py`. The v2.68.6 reactions-panel entry stays as a manual fallback. |
| 4 / 8 / 12 / 16 / 19 | Ability Score Improvement | ✅ | |
...
| 2 | Fighting Style | ✅ | All six SRD styles auto-applied: Archery / Dueling / Great Weapon Fighting / Two-Weapon Fighting (v2.99.83/85/87 helpers fire at /attack time); Protection (v2.68.10 reactions panel entry); Defense (+1 AC) via `_pc_defense_ac_bonus` hooked into `_read_target_ac` since v2.99.95 (stale doc — flipped ✅ in the v2.369.0 reconciliation). Caelan demo PATCHes through any style. |
| 2 | Spellcasting | ✅ | |
...
| 10 / 18 | Aura of Courage | ✅ | **Shipped v2.368.0** (`_ally_has_aura_of_courage` + pre-install `_install_buff` gate). RAW: you + friendly creatures within 10 ft (30 ft at Lv 18) can't be frightened while you're conscious. Mirror of the v2.55.0 Aura of Devotion gate but oath-agnostic + keyed on Frightened; gate lives in `_install_buff` so every install path (failed-save respond, Demon Slayer on_hit_save, Wand of Fear cone, future fear effects) is covered uniformly. 3 harness tests via Sir Caelan PATCH'd to Lv 10. |
...
| 14 | Cleansing Touch | ✅ | **Shipped v2.99.157** (`/use_cleansing_touch` endpoint) — full mechanical wiring: Lv 14+ gate, `cleansing-touch-uses` resource decrement, target combatant + buff lookup, `_remove_buff`-style strip, `battle_update` + `feature_used(source="cleansing-touch")` broadcasts. Harness `test_use_cleansing_touch.py` (5 tests). Caelan's seed carries the resource at CHA-mod 3/3; the harness PATCHes him to Lv 14 for the happy-path test. **v1 simplifications (GM-narrated):** the "willing creature" gate + the touch-range check. Doc flipped ✅ in v2.371.0. |
...
| 2 | Fighting Style | ✅ | Same framework as Fighter / Paladin rows above — all SRD styles auto-applied (Archery v2.99.83 / Dueling / Great Weapon Fighting / Two-Weapon Fighting / Defense via `_pc_defense_ac_bonus` v2.99.95 / Protection reactions panel). Rowan Quickbow is the demo fixture (`fighting_style: "archery"`). Doc reconciliation flipped ✅ in v2.369.0. |
| 2 | Spellcasting | ✅ | |
...
| 3 | Metamagic | 🟢 | **Empowered Spell ✅** v2.49.124 — full mechanical (CHA-mod lowest-dice reroll). 9 tests. **Twinned Spell ✅** v2.99.33 — announce-only. 6 tests. **Distant Spell ✅** v2.99.34 — announce-only. 3 tests. **Heightened Spell ✅** v2.99.35 + v2.99.36 — FIRST save-roll-mechanical metamagic; all 3 PC save sites wired. v2.99.41 — RAW PHB p.173 cancellation fix (advantage + disadvantage → 1d20 normal, not the v2.99.35 "advantage wins" simplification). 5 tests. **Extended Spell ✅** v2.99.37 — announce-only. 3 tests. **Careful Spell ✅** v2.99.38 — SECOND save-roll-mechanical metamagic. `/use_metamagic_careful_spell` arms a `metamagic-careful-pending` buff on the caster (1 SP) carrying `effects.protected_combatant_ids` (length ≤ CHA-mod). All 3 PC save sites read the buff + swap protected target's d20 → "1d20+99" (auto-pass). Multiple protected targets all benefit (buff stays armed across cast). 5 tests. **Quickened Spell 🟡** v2.6.0 — announce-only via the curated `_FEATURE_ECONOMY` entry. Zara Emberfire (Tiefling Sorcerer Lv 5) is the demo fixture for all seven. v2.99.42 — NPC saver wires for both Careful + Heightened landed at the 3 NPC save sites (`/cast_spell` single + AoE loop + `/place_aoe`); closes the v1 "PC paths only" filed notes. **Remaining metamagics ⚪** — Subtle Spell (blocked on F7 components). Other RAW metamagics (Seeking / Transmuted from Tasha's) are out-of-scope for PHB-only coverage. Picker UI for selecting metamagic at cast time is filed. |
| 4 / 8 / 12 / 16 / 19 | Ability Score Improvement | ✅ | |
| 20 | Sorcerous Restoration | ✅ | v2.99.39 — short-rest path in `/character/{id}/rest` gates on class==sorcerer AND level>=20, adds `min(4, max-current)` to the `sorcery-points` resource, broadcasts `resource_update` + `feature_used(source=sorcerous-restoration)`. Response carries `sorcerous_restoration_sp: int` so tests can assert the refund amount. `level` added to `_SHEET_PATCH_KEYS` so capstone-level harness tests can bump Zara to Lv 20 via the lightweight PATCH. 4 tests (happy, level gate at Lv 19, class gate Krieger Barbarian, cap-at-max). **Sorcerer class now fully ✅** — every RAW class feature L1–L20 shipped. Multiclass note: gate reads top-level `class`/`level` (primary class); a Sorc 18 / Wiz 2 wouldn't qualify even though primary class is Sorcerer. Filed when a multiclass Sorcerer fixture lands. |
...
| Sorcerer | **Draconic Bloodline** | ✅ | n/a | 🟢 | **Elemental Affinity ✅** v2.99.43 + v2.99.48 — Lv 6 auto-fire +CHA mod damage bonus on spells matching ancestor's damage type (Zara=Red→fire). v2.99.43 wired the single-target NPC save-for-half damage site; v2.99.48 added the `/cast_spell` AoE NPC loop + `/place_aoe` NPC wires with a `_ea_fired` flag so the bonus applies to ONE damage roll per cast (RAW). 1 SP `/use_elemental_affinity` arms a 1-hour resistance buff. 6 tests. Dragon Ancestor / Draconic Resilience (built into AC) descriptive; Dragon Wings (Lv 14) + Draconic Presence (Lv 18) still descriptive. |
...
| Sentinel | PHB feat | 🟢 | v2.66.5–v2.66.6 ("The Sentinel's Watch") — `_combatant_has_sentinel(db, combatant)` reads the sheet's `feats` for slug "sentinel". **Effect 3 ✅** (ally-attacked-near-you advisory) wired into `/attack` (v2.66.5) + `/npc_attack` (v2.66.6) — when a Sentinel-feated combatant is within 5 ft of the target of a melee attack, the response emits an OA reaction prompt for the Sentinel. **Caelan (Paladin Devotion Lv 5, Variant Human) is the demo fixture** (v2.99.24 — sheet-driven detection via the `feats` list). **Effect 1** (OA-hit reduces target speed to 0) + **Effect 2** (Disengage bypass denial) still filed pending the OA auto-fire stack + Disengage modeling. |
| Polearm Master | PHB feat | 🟢 | v2.66.4 ("The Quarterstaff") — `_combatant_has_polearm_master(db, combatant)` checks `feats` for slug "polearm_master". An inverse-transition (from > reach, to ≤ reach) OA trigger so a creature ENTERING a Polearm Master's reach provokes an OA. **Wielding-a-polearm enforcement ✅** v2.99.27 — `_pc_wields_polearm(sheet)` helper scans inventory for an `equipped: True` weapon in `_POLEARM_WEAPON_NAMES = {glaive, halberd, quarterstaff, spear, pike}`; the sheet-driven path of `_combatant_has_polearm_master` now gates on both feat slug AND wielded polearm. **Garrik Ironside (Variant Human Champion Fighter Lv 9) is the demo fixture** — his Lv 4 ASI replaced with the Polearm Master feat + Glaive added to attacks/inventory (default `equipped: False`). Tests in `test_polearm_master_wielding.py` flip the Glaive's equipped state via sheet-fields PATCH. The 1d4 bonus-action butt-end attack is filed as a future class-features-shaped action button. |
...
|---|---|---|---|
| Dragonborn | Draconic Ancestry, Breath Weapon, Damage Resistance | 🟢 | **Damage Resistance ✅** v2.99.20 — `damage_resistances: ["lightning"]` on Rowan's Bronze Dragonborn sheet, read by the v2.99.18 `_resistance_halve` extension. RAW (PHB p.34): each ancestry maps to a damage type (Bronze → lightning, Red → fire, Green → poison, etc.); future Dragonborn PCs ship the resistance type per their ancestry choice. Rowan Ashstride (Warlock The Fiend Lv 5, Bronze Dragonborn) is the demo fixture. Breath weapon needs a "click to fire" + save-DC challenge action (his `breath-weapon` resource counter exists since v2.18.4; mechanical wiring would route through the v2.9.0 picker primitive + a save-prompt-on-feature flow). Draconic Ancestry beyond the resistance type is descriptive (cosmetic + Persuasion / Intimidation flavor). |
...
| Half-Orc | Darkvision, Menacing, Relentless Endurance, Savage Attacks | 🟢 | **Relentless Endurance ✅** v2.99.17 — `_apply_hp_change` hook clamps `new_current = 1` (not 0) on the alive→0 transition when (a) massive-damage rule didn't fire AND (b) `relentless-endurance` resource is available (1/long rest). **Savage Attacks ✅** v2.99.23 — `_compute_attack_auto_uplifts` extended with `is_crit` + `weapon_damage_expr` kwargs; on a melee crit (physical-damage proxy) the helper adds one extra weapon damage die to the uplift list with `source="savage-attacks"`. Krieger Stonefist (Half-Orc Berserker Lv 7, Greataxe 1d12) is the demo fixture for both. Decrement + `feature_used(source=relentless-endurance)` broadcast + `resource_update` event for RE; uplift entry surfaces in the chat card for Savage Attacks. Darkvision + Menacing (Intimidation proficiency) descriptive / sheet-level. |
...
| Hill Dwarf | Darkvision, Dwarven Resilience (poison adv/resistance), Dwarven Combat Training, Tool Proficiency, Stonecunning, Speed Not Reduced by Heavy Armor, Dwarven Toughness | 🟢 | Demo Tavik benefits from Dwarven Toughness (already in the cleric sheet's HP). **Dwarven Resilience save advantage ✅** v2.99.12 — saves vs poison-damage spells OR poisoned-condition installs swap d20 → 2d20kh1. **Dwarven Resilience poison resistance ✅** v2.99.19 — `damage_resistances: ["poison"]` on Tavik's sheet, read by the v2.99.18 `_resistance_halve` extension; poison damage halved on the way in. Tavik (Hill Dwarf Cleric Lv 8) is the demo fixture. Dwarven Combat Training proficiencies + Stonecunning skill-context intercept are sheet-level / (B) tasks. |
...
| Lightfoot Halfling | Lucky, Brave, Halfling Nimbleness, Naturally Stealthy | 🟢 | **Halfling Lucky ✅ full RAW coverage** (v2.99.13 saves + v2.99.21 attacks + v2.99.22 ability/skill/initiative checks) — `_pc_has_halfling_lucky(sheet)` + `_extract_kept_d20_from_breakdown(breakdown)` + post-result intercepts in `/roll_request/respond`, `/attack`, AND `/roll`. When the kept d20 lands on 1, the server rerolls the full expression once + broadcasts `feature_used(source=halfling-lucky)`. Pip (Halfling Rogue Lv 7) is the demo fixture; harness coverage in `test_halfling_lucky.py` (7 tests across all three surfaces) using TEST_MODE dice seeding. **Halfling Brave ✅** (v2.99.14) — `_RACE_SAVE_ADVANTAGES["halfling"]` rule with `condition_keys=["frightened"]`; saves vs Frightened install swap d20 → 2d20kh1. Halfling Nimbleness + Naturally Stealthy remain descriptive (positional / move-through traits). |
...
| Tiefling | Darkvision, Hellish Resistance (fire resist), Infernal Legacy (Thaumaturgy + Hellish Rebuke 1/day + Darkness 1/day) | 🟢 | **Hellish Resistance ✅** v2.99.18 — sheet-level `damage_resistances: ["fire"]` read by the extended `_resistance_halve` helper. Fire damage halved on the way in. Zara Emberfire (Tiefling Sorcerer Lv 5) is the demo fixture. **Infernal Legacy spells ✅ resource-tracked** (v2.18.1) — `hellish-rebuke` (1/long-rest reaction at Lv 3+) + `darkness-racial` (1/long-rest at Lv 5+) counters on the sheet; Thaumaturgy cantrip on her spell list. Hellish Rebuke casts route through the v2.71.0 Reactions framework's `damage_taken` trigger; Darkness routes through `/cast_spell` without consuming a slot. Darkvision descriptive (no light-aware engine). |
...
Phase 2 follow-ups still ⚪: Halfling Lucky attack-roll + ability-check surfaces; Stout Halfling Stout Resilience (needs subrace-aware slug normalizer to differentiate from Lightfoot). **Phase 3 ✅ (first / second / third ships)** — v2.99.15 Fey Ancestry magical sleep immunity via `_is_sleep_immune` extension; **v2.99.17 Half-Orc Relentless Endurance** HP-clamp-to-1 via `_apply_hp_change` hook; **v2.99.18 Tiefling Hellish Resistance** sheet-level `damage_resistances` field via `_resistance_halve` extension. Phase 3 follow-ups still ⚪: Tiefling Infernal Legacy spell cast bookkeeping (resource counters exist; mechanical wiring works via existing Reactions / spell-cast paths). Phases 4–5 still ⚪. Poison damage RESISTANCE half of Dwarven Resilience now unblocked (just add `damage_resistances: ["poison"]` to Tavik's sheet) — filed for a follow-up bump. Dragonborn ancestry damage resistance unblocked — same shape.
