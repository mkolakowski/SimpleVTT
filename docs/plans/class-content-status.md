...
Phase 2 follow-ups still ⚪: Halfling Lucky attack-roll + ability-check surfaces; Stout Halfling Stout Resilience (needs subrace-aware slug normalizer to differentiate from Lightfoot). **Phase 3 ✅ (first + second ships)** — v2.99.15 Fey Ancestry magical sleep immunity via `_is_sleep_immune` extension; **v2.99.17 Half-Orc Relentless Endurance** HP-clamp-to-1 via `_apply_hp_change` hook. Phase 3 follow-ups still ⚪: Tiefling Infernal Legacy per-day cantrip + Hellish Rebuke / Darkness counters. Phases 4–5 still ⚪. Poison damage RESISTANCE half of Dwarven Resilience filed — needs v2.63.0 resistance pipeline to read a per-PC race trait list.
