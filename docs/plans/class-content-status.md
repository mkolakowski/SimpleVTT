...
| Half-Orc | Darkvision, Menacing, Relentless Endurance, Savage Attacks | 🟢 | **Relentless Endurance ✅** v2.99.17 — `_apply_hp_change` hook clamps `new_current = 1` (not 0) on the alive→0 transition when (a) massive-damage rule didn't fire AND (b) `relentless-endurance` resource is available (1/long rest). Krieger Stonefist (Half-Orc Berserker Lv 7) is the demo fixture. Decrement + `feature_used(source=relentless-endurance)` broadcast + `resource_update` event fire from `_apply_damage_to_combatant`. Resource auto-refills on long rest via existing `/rest` walker. Darkvision + Menacing (Intimidation proficiency) descriptive / sheet-level. Savage Attacks (extra crit die) filed under (B) crit-intercept work. |
