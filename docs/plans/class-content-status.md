...
| 3 | Metamagic | 🟢 | **Empowered Spell ✅** v2.49.124 — `/use_metamagic_empowered_spell` arms a `metamagic-empowered-pending` buff on the caster; the next `/cast_spell` damage roll reads the buff via `_roll_with_empowered_reroll` and rerolls up to CHA-mod lowest dice. Costs 1 SP. 9 tests in `test_use_metamagic_empowered.py`. **Twinned Spell ✅** v2.99.33 — `/use_metamagic_twinned_spell` decrements SP by `max(1, spell_level)` + broadcasts `feature_used(source=metamagic-twinned-spell)`. v1 announce-only — player makes the second-target cast manually via a follow-up Cast. 6 tests in `test_use_metamagic_twinned.py`. **Quickened Spell 🟡** v2.6.0 — curated `_FEATURE_ECONOMY['quickened-spell']` entry with `slot: 'bonus'`; clicking the class-features button announces + flips the Bns chip but the player still casts the actual spell separately. Zara Emberfire (Tiefling Sorcerer Lv 5) is the demo fixture for all three. **Other metamagics ⚪** — Subtle Spell (blocked on F7 components), Distant Spell, Heightened Spell, Careful Spell, Extended Spell still need per-cast intercepts. Picker UI for selecting metamagic at cast time is filed. |
| 4 / 8 / 12 / 16 / 19 | Ability Score Improvement | ✅ | |
