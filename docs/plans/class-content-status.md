...
| Cleric | **Life Domain** | ✅ | ✅ | 🟢 | Domain spells auto-grant (demo Tavik post-v2.4.15); Channel Divinity: Preserve Life is data only. **Blessed Healer (Lv 6+) — descriptive** (v2.57.1 — Tavik Lv 5 → 6 bump): outgoing heals heal Tavik for 2 + spell level; pure-descriptive class_features row today, would need an outgoing-heal interception on `/cast_spell` to auto-apply. Disciple of Life (Lv 1) + Divine Strike (Lv 8) + Supreme Healing (Lv 17) still descriptive. |
