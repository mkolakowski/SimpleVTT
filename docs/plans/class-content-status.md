...
| 3 | Metamagic | 🟢 | **Empowered Spell ✅** v2.49.124 — full mechanical (CHA-mod lowest-dice reroll). 9 tests. **Twinned Spell ✅** v2.99.33 — announce-only. 6 tests. **Distant Spell ✅** v2.99.34 — announce-only. 3 tests. **Heightened Spell ✅** v2.99.35 — FIRST save-roll-mechanical metamagic. `/use_metamagic_heightened_spell` arms a `metamagic-heightened-pending` buff on the caster (3 SP); the next `/cast_spell` single-target save site reads the buff, swaps target's d20 → 2d20kl1 (disadvantage), drops the buff (one-use). Hold Person on Zara's spell list as the test trigger. 3 tests. **Quickened Spell 🟡** v2.6.0 — announce-only via the curated `_FEATURE_ECONOMY` entry. Zara Emberfire (Tiefling Sorcerer Lv 5) is the demo fixture for all five. **Other metamagics ⚪** — Subtle Spell (blocked on F7 components), Careful Spell, Extended Spell still need per-cast intercepts. Heightened AoE / `/place_aoe` site wires filed. Picker UI for selecting metamagic at cast time is filed. |
| 4 / 8 / 12 / 16 / 19 | Ability Score Improvement | ✅ | |
