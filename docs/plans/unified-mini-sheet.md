...
| 2.5b | `renderBattle()` switches to clone-and-patch from `#monster-card-pool`; `buildMonsterInitSheet` becomes the orphan-fallback for combatants without a template card. | `app/templates/tabletop.html` (~line 5665+) | Clones the source template (`cloneNode(true)`), rewrites every `data-char-id="monster-template-{tid}"` to the per-combatant `slotKey`, patches HP / charge / status from per-combatant state, then injects into `#char-detail-{slotKey}` for the existing hoist logic to find. The clone-and-patch step is what makes per-combatant state work for the 3-bandits-share-one-template case. |
