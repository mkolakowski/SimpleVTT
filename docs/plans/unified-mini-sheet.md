...
| 2.5c | Delete `buildMonsterInitSheet` body (keep the orphan-fallback stub for combatants whose template was removed). | `app/templates/tabletop.html` | Once 2.5b is stable, the bulk of `buildMonsterInitSheet`'s ~466-line HTML-building can go. The fallback path (when no template card exists in the pool) can stay as a 1-line "Open full sheet" link since orphans are an edge case. |
