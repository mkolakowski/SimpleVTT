...
| 2.5a | ✅ **Done v2.49.202.** Server pre-renders monster mini-sheets into a hidden `#monster-card-pool` div via the unified partial with `is_monster=True`. | `app/routes/tabletop_routes.py` (new `monster_template_cards` projection), `app/templates/tabletop.html` (new hidden pool block), `tests/harness/test_mini_sheet_partials.py` (3 new tests). | Pure additive. Pool ids use `char-detail-monster-template-{tid}` (note the `-template-` infix) deliberately so the existing `hasCharDetail` lookup at `tabletop.html:~5668` (which matches `char-detail-monster-{tid}`, no infix) does NOT activate. `buildMonsterInitSheet` keeps driving every NPC init-card today; the pool is dormant. Phase 2.5b will switch `renderBattle()` to clone-and-patch from the pool per combatant. |
| 2.5b | ✅ **Done v2.49.203.** `renderBattle()` switches to clone-and-patch from `#monster-card-pool`; `buildMonsterInitSheet` becomes the orphan-fallback for combatants without a template card. | `app/templates/tabletop.html` (new `_hydrateMonsterCard` helper + slotId computation now prefers `c.id` for monsters). | Clones the source template (`cloneNode(true)`), rewrites every `data-char-id="monster-template-{tid}"` to the per-combatant `slotKey` (= `c.id`), patches HP from per-combatant state (the template carries default HP; combatant carries actual current HP), then leaves the clone in the pool's parent for the existing hoist logic to find. The clone-and-patch step makes per-combatant state work for the 3-bandits-share-one-template case — each gets its own clone keyed by `combatant.id`. **Known regression filed in TODO:** monster `action_charges` (per-combatant charge tracking widgets — Soren's "2/2 ↻" Inflict Wounds counter, Frightful Howl's spent-disabled state) are lost — the unified partial doesn't carry charge markup. Underlying state tracking is intact; only the visual readout is gone until the "Migrate monster action_charges to the unified partial" TODO ships. |
| 2.5c | ✅ **Done v2.49.221.** Delete `buildMonsterInitSheet` body (keep the orphan-fallback stub for combatants whose template was removed). | `app/templates/tabletop.html` | Body shrunk from ~440 lines to ~30 lines. Stub preserves the v2.3.39 name-substring heal (so demo-reseed token_template_id cycling still works) and re-attempts `_hydrateMonsterCard` after the heal; true orphans get a placeholder message that defers to the existing `📋 Sheet` link in the row above. v2.49.220 (action_charges migration) unblocked this commit by removing the last GM-only affordance that lived only in the legacy renderer. |
