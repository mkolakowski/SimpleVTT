...
- **HP / death-save effect refund.** Drinking a Potion of Healing → undo → the potion comes back, but the HP gain stays. Same for `/use_lay_on_hands` (pool refunded; target's HP unchanged) and `/cast_spell` heals. A separate "effect rewind" audit pass would stamp `heal` / `damage` entries alongside the resource leg so the existing pre-v2.97.0 damage/heal-undo branch reverses them too. Decided to keep v2.97.x scoped to resource refund only because (a) the HP refund interacts with concurrent damage from other sources and needs more thought, (b) the resource refund is the higher-leverage fix (resources are the strictly-limited currency; HP is fluid and easy to manually re-apply).
