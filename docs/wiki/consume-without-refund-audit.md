# Undo refund audit (v2.97.0 – v2.97.32)

> **Status:** ✅ shipped. Two stacked audits closed: **consume-without-refund** (resources + slots + inventory + HP, v2.97.0 – v2.97.18) and **buff teardown** (buffs from feature use + reaction casts + spell save-or-suck + catalog-driven /use_feature + Bardic Inspiration target buff + no-save spell buffs via `_SPELL_BUFF_MAP` + dedicated concentration-spell endpoints (Hunter's Mark + Hex), v2.97.20 – v2.97.32). Filed follow-ups documented at the bottom.
...
- **Other `/cast_spell` buff-installing spells beyond save-or-suck.** ✅ Bless shipped in v2.97.31 via `_SPELL_BUFF_MAP`; **Hex + Hunter's Mark shipped in v2.97.32** (via direct edits to their dedicated `/cast_hex` and `/cast_hunters_mark` endpoints rather than `_SPELL_BUFF_MAP`, since those spells don't route through `/cast_spell`). Remaining filed: Bane / Faerie Fire / Haste / Aid / Heroism / Protection from Evil and Good / Shield of Faith / Sanctuary — all one-line additions to `_SPELL_BUFF_MAP` for the /cast_spell-routed ones, or the same v2.97.32 patch for any future dedicated endpoint. The catalog + the patch pattern both exist; this is a content-population follow-up, not an architecture task.
