...
- ~~**HP / death-save effect refund.**~~ ✅ Shipped across **v2.97.16 + v2.97.17.** v2.97.16 stamped heal entries for the four dedicated HP-applying endpoints (`/use_lay_on_hands`, `/use_second_wind`, `/use_wholeness_of_body`, `/use_item` heal kind). v2.97.17 closed the `/cast_spell` heal-claim path (`/apply_healing` now stamps too). The auto-applied `/cast_spell` heal path was already covered via `_apply_heal_to_combatant`'s pre-existing stamp. Mass heals (Mass Cure Wounds / Mass Healing Word) inherit the auto-applied coverage per-target. The only remaining gap is the Life Domain Blessed Healer self-heal that fires inside `/apply_healing` with `cast_id=None` — filed for a follow-up commit.
