...
| [Admin Center TOTP MFA + recovery code](../plans/admin-center-mfa.md) | Markdown (design) | Operators + Contributors | ⚪ proposed · design only — opt-in TOTP second factor on the admin-center login + env-set recovery code (blank accepts nothing); fails closed if MFA on with no secret |
