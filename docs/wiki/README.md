...
How much of the SRD 5.1 ruleset SimpleVTT mechanically automates, by content category, as of the **v2.553.0 audit refresh** (races denominator updated v2.654.0). "Automated" means the engine derives or enforces the rule (saves, damage, passive item effects, class features, conditions) rather than leaving it to GM narration; "supported" additionally counts deliberately GM-narrated resolution per the 2026-06-22 rubric. Recomputed each audit pass; see [TODO](../../TODO.md) for the breakdown and gaps.

| Category | Count | Automated |
|----------|-------|-----------|
| Races | 14 | ✅ ~100% |
| Monsters | 322 | ✅ ~100% |
| Conditions | 15 | ~92% · 100% supported |
| Class features | 222 rows | ✅ 100% |
| Spells | 319 | ~93% · 100% supported |
| Magic items | 239 / 239 wired | ✅ 100% |

**Overall ~98–99% mechanically automated · 100% supported — the SRD ruleset is functionally complete.** Under the 2026-06-22 rubric (in a VTT the GM is the rules authority, so GM-adjudicated resolution counts as supported) every SRD spell, condition, feature, monster, race, and magic item is either mechanically automated or deliberately GM-narrated. The two big movers since the v2.404.10 snapshot this table previously pinned: the v2.437.0–v2.553.0 [cast-and-broadcast utility-spell tail](../plans/cast-and-broadcast-tail.md) (Phase 1 + Phase 2 #1–#67) wired a cast endpoint for every previously substrate-blocked spell cluster — invisibility-detection (`sees_invisible` attack-edge), Mirror Image's deflection mechanic, scrying-sensor markers, AoE-shape zone markers (Globe's spell-block and Antilife Shell's movement barrier are fully mechanical), and the conjure-catalog summon override — lifting Spells ~85% → ~93% automated; and v2.654.0 shipped the 5 SRD subraces missing from the shipped tier (Mountain Dwarf, Wood Elf, Forest Gnome, Stout Halfling, Drow), lifting the Races denominator 9 → 14. The residual automated-only gap is GM-narrated by design: 40,000-sq-ft zone geometry (Forbiddance), remote scrying views (Clairvoyance / Arcane Eye / Project Image), object triggers (Magic Mouth / Illusory Script), and the permanently-narrated condition clauses (Charmed clause 2 social-check, Grappled clause 3 out-of-reach, Deafened hearing-checks). Mechanizing that slice (Maps 2.0 zone geometry, a remote-sensor surface) is an optional future enhancement, not missing coverage; the only genuinely un-built remainder is out-of-SRD-scope content (Tasha's / Xanathar's / 2024 rules — future 3.x). Known defects are tracked in [BUGS](../../BUGS.md).
...
|------|--------|----------|--------|
| [Map-editor control reorganization (review & proposals)](../plans/editor-controls-reorg.md) | Markdown (design) | Contributors | ✅ all 4 themes shipped (A · D · B · C, v2.924.0–v2.927.0) |
...
| [Vision & light (sight/obscurement engine)](../plans/vision-and-light.md) | Markdown (design) | Contributors | ✅ all phases shipped (v2.704.0–v2.710.0 · server engine + client lighting) |
| [Exploration-tracking fog of war](../plans/exploration-fog.md) | Markdown (design) | Contributors | ✅ all phases shipped (v2.843.0–v2.845.0 · engine + client render + editor/demo) |
...
| [Encounter-sim test suite](../plans/encounter-sim-test-suite.md) | Markdown (design) | Contributors | 🟠 Phases 1–3 shipped (17/17 sim tests pass); Phase 4 pending |
...
| [Reactions automation](../plans/reactions-automation.md) | Markdown (design) | Contributors | ✅ shipped (Phases 1–6 v2.67.0–v2.78.0; v3 auto-resolution closed via the pending-resolution arc v2.600.0–v2.664.0); minor filed tail (auto-cast routing + own-roll reroll surfaces) |
...
| [Spell-validation test suite](../plans/spell-validation-suite.md) | Markdown (design) | Contributors | ✅ all 5 phases shipped (v2.183.23 · catalog smoke + per-spell deep-dives + spell_catalog CI gate) |
...
| [Magic-item automation](../plans/magic-items-automation.md) | Markdown (design) | Contributors | ✅ all phases shipped — framework Phases 1–9.1 (v2.158.74–v2.367.0) + Phase 9.2 charge tracking + Phase 9.3 umbrella slugs (v2.403.0–v2.404.0); 239/239 = 100%; Bucket D mechanization filed for 3.x |
...
| [Ability-score override engine](../plans/str-override.md) | Markdown (design) | Contributors | ✅ complete · Phases 0–4 shipped (v2.211.0–v2.217.0) — override substrate + STR saves/checks/carry + attack/damage + Belt/Amulet/Potion item tiers |
...
| [Demo magic-link login](../plans/demo-magic-link.md) | Markdown (design) | Operators + Contributors | ✅ Phases 1 + 1B shipped (v2.425.0 + v2.430.0) — mint/verify/table/admin UI/double-gate + app-demo compose overlay |
| [fail2ban / CrowdSec log integration](../plans/fail2ban-crowdsec-integration.md) | Markdown (design) | Operators + Contributors | ✅ Phases 1 + 2 shipped (v2.426.0 + v2.429.0) — emission module + canonical auth/API events + in-repo filter.d/jail.d configs; Phase 4 (active enforcement) open |
| [Cloudflare edge-banning integration](../plans/cloudflare-edge-banning.md) | Markdown (design) | Operators + Contributors | ✅ Phase 1 shipped (v2.427.0) — Cloudflare client + admin_audit_log table + admin endpoints + GM-facing edge-ban (extended through v2.531.0) |
| [Admin Center TOTP MFA + recovery code](../plans/admin-center-mfa.md) | Markdown (design) | Operators + Contributors | 🟠 partial · core shipped v2.486.0–2.487.0 — opt-in TOTP second factor + env recovery code (blank accepts nothing), fail-closed if MFA on with no secret; QR provisioning page deferred |
| [Cast-and-broadcast utility-spell tail](../plans/cast-and-broadcast-tail.md) | Markdown (design) | Contributors | ✅ complete — Phase 1 (v2.441.0) + Phase 2 #1–#67 (through v2.553.0, Forbiddance); Bucket B permanently GM-narrated by design |
...
| [Self-test SRD coverage & demo evaluation](../self-test-srd-coverage.md) | Markdown (audit) | Contributors | ✅ shipped (v2.987.0) |
...
- [x] **Building an encounter.** Encounters panel CRUD, token templates, monsters from the SRD bestiary vs homebrew, spawn-point layout, default-encounter wiring on a campaign. → [building-an-encounter.md](building-an-encounter.md) (illustrated, 4 screenshots).
- [x] **Maps + grids + tokens.** Uploading maps, scaling, grid overlay (`show_grid`), placing PC and monster tokens, the token-tracker side panel. → [maps-grids-tokens.md](maps-grids-tokens.md) (illustrated, 4 screenshots).
...
- [x] **Backups + restore.** The `simplevtt-backup` container, daily `pg_dump` cycle, restoring from a backup, exporting / importing a campaign. → [backups.md](backups.md) (shipped v2.628.0; offsite uploads + disaster-recovery pull v2.998.0–v2.1000.0).
