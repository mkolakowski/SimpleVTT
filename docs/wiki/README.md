...
|------|--------|----------|--------|
| [Notes & handouts (GM prep + E2E-encrypted player notes)](../plans/notes-and-handouts.md) | Markdown (design) | Contributors | 🟠 Phases 1–3 + Phase 4 server-side shipped (v2.554.0–v2.557.0); Phase 4 browser crypto + Phase 5 UI pending |
...
| [Demo magic-link login](../plans/demo-magic-link.md) | Markdown (design) | Operators + Contributors | ⚪ proposed · Phase 1 unstarted — URL-based passwordless login for the demo instance, double-env-var gated; sibling plans for fail2ban/CrowdSec log integration and Cloudflare edge banning live in TODO.md |
| [fail2ban / CrowdSec log integration](../plans/fail2ban-crowdsec-integration.md) | Markdown (design) | Operators + Contributors | ⚪ proposed · Phase 1 unstarted — canonical structured log lines + reference filter.d/jail.d (fail2ban) and parsers/scenarios (CrowdSec) configs ship in-repo so operators get drop-in banning detection; pairs with the demo magic-link plan and the Cloudflare edge-banning TODO |
| [Cloudflare edge-banning integration](../plans/cloudflare-edge-banning.md) | Markdown (design) | Operators + Contributors | ⚪ proposed · Phase 1 unstarted — outbound API client + env-var config + GM-only "Ban IP at edge" button + admin-audit log, wiremock service in compose for dev; closes the three-piece security spine (demo magic-link + fail2ban/CrowdSec + this) |
...
| [Cast-and-broadcast utility-spell tail](../plans/cast-and-broadcast-tail.md) | Markdown (design) | Contributors | ⚪ proposed · Phase 1 unstarted — mechanizes the Bucket A utility spells (True Strike, Find Steed, Speak with Animals, Pass Without Trace, Spider Climb) that currently cast + broadcast without a server-side effect; Bucket B filed as permanently GM-narrated |
