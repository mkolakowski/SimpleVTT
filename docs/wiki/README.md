...
| [Race features (close the SRD races gap)](../plans/race-features.md) | Markdown (design) | Contributors | 🟢 partial (v2.395.0) — Phase 1 shipped (Tiefling racial Hellish Rebuke routes through the `hellish-rebuke` resource not a slot); plus pre-existing Savage Attacks (v2.99.23, reconciled v2.394.0). 5 phase ships remaining (Stonecunning / heavy-armor speed / Halfling Nimbleness + Naturally Stealthy / Artificer's Lore) |
