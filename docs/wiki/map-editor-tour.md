...
| **Markers → Lights** | Light | A light source with a **bright** and **dim** radius (in feet) and a colour. Pick a preset (candle / torch / lantern / lamp / bullseye / daylight). Lights matter most on a **dark**-ambient map. | Shadowfell Spire (coloured braziers in ambient dark); Caldera Throne (fire-glow) |
...
| **Environment → Fog** | Fog | Fog of war. Toggle it on, then paint **revealed** rectangles over the areas the party can see; the rest stays hidden. | Tide-Wracked Catacombs; Shadowfell Spire (both revealed generously so the play area stays visible) |
| **Environment → Ambient / Weather** | *(selects)* | Map-wide **ambient light** (bright / dim / dark — governs how much the light sources matter) and a **weather** overlay (rain / snow / fog particles). | Shadowfell Spire ships `ambient: dark` |
...
4. Two **lights** — a warm `torch` by the hearth and a dimmer `candle` on the bar.
