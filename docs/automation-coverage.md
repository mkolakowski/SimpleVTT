...
| `use_tempestuous_magic` | ✅ tracked | v2.697.0 — installs a `tempestuous-magic-fly` free-move buff (`free_movement_remaining_ft: 10` + `oa_immune_during_move`) riding the generalized free-move read in `/token/move`; next ≤10-ft move is cap-exempt + OA-free, then consumed; on-ground/recent-cast prereq GM-narrated |
...
| `use_overchannel` | ✅ tracked | v2.1010.0 — Evocation Wizard Lv 14+. Arms a one-shot `overchannel-armed` buff; the next damaging 1st-5th level spell has every roll maxed (via `_max_dice_total`, gated on `_caster_overchannel_buff`), then the buff drops + the escalating necrotic self-damage (`use_number × spell_level` d12, untyped/ignores resistance) hits the caster on the 2nd+ use since a long rest (`overchannel_uses_since_rest` counter, reset by the long-rest flow). **v2.1019.0 — Phase 2:** the max now covers all damage paths — `/cast_spell` single-target-save + AoE-loop + spell-attack-roll (per-beam, crit-doubled) + `/place_aoe`; the consume block moved to function scope so attack-roll casts also drop the buff + self-damage |
