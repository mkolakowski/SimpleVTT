...
**Total tests:** 5022 in `tests/harness/` + 321 in `tests/harness_ui/` (as of v2.1057.0; +4 — `test_detect_surprise.py` covers `POST /detect_surprise`: a stealthy rogue flags a low-Perception bandit surprised (seeded), consistency (ambushers never surprised, full partition, threshold == min roll), GM-only, 400 no-ambushers; prior line: as of v2.1056.0; +6 — `test_set_surprised.py` covers the surprise-state model: the `set_surprised` toggle (mark/clear, GM-only, 400 no-ids, unknown-skipped), an Assassin auto-crit driven by the server `surprised` flag (no client `target_surprised`), and the flag auto-clearing on the target's first turn; prior line: as of v2.1055.0; +4 — `test_item_cloak_of_elvenkind.py` gains the perception-disadvantage half: a Perception check with `vs_character_id` = a cloak wearer rolls 2d20kl1 (+ 3 controls); prior line: as of v2.1054.0; +2 — `test_npc_death_saves.py::{test_turn_start_auto_rolls_death_save,test_turn_start_no_roll_for_alive_boss}` cover Phase 4b: a dying boss auto-rolls its death save on its turn (`npc_death_save` `source: turn_start`, counters match the raw d20), and an alive boss doesn't; prior line: as of v2.1053.0; +7 — `test_npc_death_saves.py` covers Phase 4a NPC death saves: the `set_npc_death_saves` toggle (set/clear, GM-only, PC-reject, 404), a flagged boss entering `dying` at 0 HP (+ `npc_death_save` broadcast), the unflagged-NPC inert control, and a damage-while-dying failure tick; prior line: as of v2.1052.0; +1 — `test_generate_map.py::test_generate_custom_size` asserts a `size=custom` 50×40 map's served PNG is exactly `50*70×40*70` px; prior line: as of v2.1051.0; +1 — `test_generate_map.py::test_generate_density_changes_map` covers the new density slider on `POST …/maps/generate` (sparse≠dense image + out-of-range clamp); prior line: as of v2.1050.0; +2 — `test_generate_map.py::{test_generate_all_biomes,test_generate_bad_biome}` cover the new dungeon/cave/wilderness/tavern biome picker on `POST …/maps/generate`; prior line: as of v2.1049.0; +1 — `test_generate_map.py::test_generate_populates_functional_walls` asserts a generated map exposes Maps 2.0 walls via `GET …/map/{id}/walls` and a generated door toggles `open`; prior line: as of v2.1048.0; +5 — `test_generate_map.py` covers the new procedural map-generator endpoint `POST /campaign/{cid}/settings/maps/generate` (happy-path + served-PNG, default name, seed determinism, 400 bad-size, 403 player); prior line: as of v2.1047.8; no count change — `helpers.AttemptLog` instruments the attack-until-hit loops in `test_relentless_endurance` / `test_relentless_rage` / `test_cast_death_ward` / `test_npc_concentration`. Those loops do `if r.status_code != 200: continue`, so when they exhaust they report only *that* nothing landed, never *why* — a 409 range gate, a 400, a target already at 0 HP, and genuine misses all look identical. CI run 31126181450 had **12** such failures and none were diagnosable from the report. The assertion now carries the status distribution, the hit/miss/resisted/immune outcome spread, and the last non-200 body (v2.1047.8 "The Autopsy Note"); prior line: as of v2.1047.7; no count change — `test_battle_line_targets` / `test_battle_sphere_cone_targets` now intersect the hub battle state's `source_token_id`s with the **live** token ids (`helpers.live_token_ids`) before picking combatants. The hub's battle state is in-memory and nothing prunes a combatant when its token row disappears, so CI run 31126181450 had the state pointing at token **1** while the DB held a different generation — all 9 tests in those two files died in setup on `404 {"detail":"Token not found"}`. Verified by simulation: with healthy state the filter is a no-op (4 picks), with fully-stale ids it yields 0 and the fixture asserts with a message naming the drift instead of 404ing on a move (v2.1047.7 "The Roll Call"); prior line: as of v2.1047.6; `test_ci_uploads_enabled.py` renamed to **`test_ci_env_invariants.py`** (+3) — the demo-reset invariant: `DEMO_MODE=true` spawns an unconditional `while True: sleep(interval); reset_and_reseed()` scheduler that **wipes and reseeds the dataset**, and at `DEMO_RESET_INTERVAL_MINUTES=60` the 272-minute harness job absorbed ~4 wipes mid-suite (CI run 31070630004). Empirically confirmed locally at the 5-minute minimum: an uploaded map vanished and Thalindra's token changed **id 10371 → 10430** and position (700,100) → (420,560) — which is exactly the `"Token not found"` 404s, the distance mismatches, and (7 users reseeded) the `"Login required"` 401s seen in CI. Now pinned to 1440, the config clamp's maximum, with guards that DEMO_RESET_ON_BOOT stays on and the value can't exceed the clamp (which would be silently rewritten) (v2.1047.5 "The Undisturbed Table"); prior line: as of v2.1047.4; `tests/harness/test_no_blocked_external_assets.py` (+9) — no served page may reference a subresource its own CSP forbids. `base.html` loaded webfonts from `fonts.googleapis.com` and htmx from `unpkg.com` while the app sends `style-src 'self' 'unsafe-inline'` / `script-src 'self' 'unsafe-inline'` / `font-src 'self' data:` — so both were refused on **every page load** and the fonts silently never applied (readers saw the Georgia fallbacks). The tests derive the rule from the CSP the app actually sends rather than hardcoding the two origins, so the next CDN tag is caught too; plus fonts.css serves real `wOF2` bytes for all three families and the font files sit outside the gated `/static/uploads/` tree. Fixes the 2 harness_ui console-error failures from CI run 31070630004 (v2.1047.4 "The Local Press"); prior line: as of v2.1047.3; no count change — `helpers.ensure_token_at()` replaces the fire-and-forget setup move in `test_cast_spell_range` / `test_cast_attack_range` / `test_place_aoe_range`, which measured distance from wherever the token happened to be when the move silently 409'd on the over-speed gate (which, unlike the movement-lock and off-turn gates, does **not** bypass for the GM). CI run 31070630004 showed this as `assert 307.1 == 350.0` / `332.1 == 350.0` / `32.1 == 50.0`. The helper waives the gate, asserts the move returned 200, and asserts the landed coordinates (v2.1047.3 "The Surveyor's Stake"); prior line: as of v2.1047.2; `tests/harness_ui/test_gif_token_src_stable.py` (+1) — an animated-GIF token's `<img>` `src` is not reassigned on every render: `_updateGifOverlay`'s guard compared `img.src` (the *property*, which resolves to an absolute URL) against the root-relative `t.image_url`, so it was **always true** and rewrote `src` at the tail of every `render()` (60+ call sites), restarting the GIF animation. The test instruments the element's `src` setter, drives six real token-move WS updates, and asserts **zero** writes — plus a vacuity guard asserting the overlay actually repositioned, because an earlier draft drove renders with synthetic wheel events that never reached `render()` and so passed against the unfixed code. Verified in both directions (6 writes broken / 0 fixed) (v2.1047.2 "The Steady Flame"); prior line: as of v2.1047.1; `tests/harness/test_ci_uploads_enabled.py` (+4) — CI must actually exercise the upload endpoints: every workflow env block that sets `DEMO_MODE=true` must also set `DEMO_DISABLE_UPLOADS=false`, or compose's `:-true` default makes all 4 jobs answer 403 and every upload test goes vacuously red; plus a guard-the-guard test (the first draft's heredoc parser matched **zero** blocks because the workflow uses a quoted `<<'EOF'` delimiter, so the invariant passed vacuously), a workflow-YAML-parses test, and a check that compose still defaults the demo lockdown ON (v2.1047.1 "The Unlocked Turnstile"); prior line: as of v2.1047.0; `tests/harness/test_media_gate.py` (+11) — the rest of the uploads tree (maps / thumbnails / tokens / portraits / token_templates / encounter_bg / audio) is authorization-gated like handouts: a member reads their campaign's map, an **anonymous request with the exact URL gets 404**, a campaign *player* can read (membership, not GM-only), **deleting the map row revokes access to its file**, orphans fail closed, percent-encoded traversal / nested path / dotfile → 404 with no stylesheet bytes leaked, `/static/style.css` + the bundled `/static/demo/` art still serve, responses are `private` but NOT `no-store` (ordinary art must stay browser-cacheable) with an ETag, a conditional request returns a bodyless 304, and handouts keep their own stricter `no-store` handler; plus a coverage guard that harvests the media URLs the settings page actually renders and asserts each still serves (v2.1047.0 "The Closed Stacks"); prior line: as of v2.1046.0; `tests/harness/test_handout_media_gate.py` (+12) — handout media is authorization-gated rather than public static bytes: an anonymous request holding the exact URL gets 404; a player is blocked before reveal, reads after reveal, and is **revoked again on hide** (the leg the old static-bytes posture could not do); a scoped reveal admits alice and refuses bob; the PDF path is gated identically; an unattached upload is readable by the GM (composer preview, via the campaign id in the filename) but not a player; deleting the handout revokes player access; unknown name / path traversal / nested path / dotfile → 404; **other `/static` paths are still served**; served bytes carry `Cache-Control: private, no-store` (v2.1046.0 "The Locked Reading Room"); prior line: as of v2.1045.0; `tests/harness/test_handout_file.py` (+13) — handout document attachments, the "Resources" tier: GM PDF upload → 200 + a `/static/uploads/handouts/…pdf` URL/original filename/byte count (and the URL actually serves `%PDF-` bytes); the `file_url`/`file_name`/`file_size` trio round-trips through create + PATCH and detaches cleanly; **the access gate** — an un-revealed document 404s for a player and appears only on reveal, a scoped reveal excludes the other player, and hiding withdraws it again; `has_file` on the `handout_revealed` broadcast (true and false); 403 / bad-extension / bad-magic-bytes / junk-size error paths. Verified by `--collect-only`: **4950** (v2.1045.0 "The Sealed Envelope"); prior line: as of v2.1044.1; `tests/harness_ui/test_init_monster_row_sheet.py` (+4) — init-tracker monster row → full stat block: the row is stamped `data-monster-tid` and carries the 📋 anchor; a single click still expands the inline mini-sheet and leaves the drawer closed; a double-click opens the drawer *and* restores the expand state the first click of the pair toggled; an orphan row (`data-monster-orphan`) opens the sheet on the first click instead of expanding a dead-end placeholder (v2.1044.1 "The Second Look"); prior line: as of v2.1044.0; `test_presence_idle.py` (+4) — presence idle state on the `presence_update` broadcast: every user row carries `idle` (bool) + `idle_seconds` (numeric, ≥0) alongside the pre-existing `user_id`/`display_name`/`is_gm`, the payload carries `idle_after_seconds`, and a freshly-connected GM reads active; an inbound `{"type":"activity"}` frame is accepted and keeps the sender active; two tabs for one user still dedupe to one row; a non-JSON frame neither kills the socket nor drops the user from presence (v2.1044.0 "The Empty Chair"). **Count note:** a full `--collect-only` at v2.1044.0 reports **4937**, i.e. the prior header's 4923 was ~10 behind reality — this line is the re-measured figure, not 4923+4; prior line: as of v2.1043.0; `test_map_as_thumbnail.py` (+3) — reuse a map image as the campaign thumbnail: GM `POST .../settings/maps/{id}/use-as-thumbnail` → 200 with the map's static URL, persisted on the settings page; unknown map → 404; non-GM → 403 (v2.1043.0 "Double Duty"); prior line: as of v2.1042.4; `test_battle_master_rest_refill.py` (+2) — Battle Master Phase 2 refill contract: deplete the demo Fighter's `superiority-dice` pool (`reset: "short"`) and assert a short rest refills 1/4→4/4 and a long rest refills 0/4→4/4 (v2.1042.4 "The Whetstone"); prior line: as of v2.1042.3; `test_medicine_stabilize.py` (+2 → 5) — Medicine-check stabilize now honors the healer's roll_state: advantage upgrades the d20 to `2d20kh1` (`auto_advantage`) and disadvantage to `2d20kl1` (`auto_disadvantage`), asserted via the response `roll_state` + `roll_breakdown` (v2.1042.3 "The Steady Hand"); prior line: as of v2.1042.0; `test_wiki_sanitize.py` (+2) — wiki markdown HTML sanitization: `nh3.clean` strips script/event-handlers/js-urls and preserves markdown structure tags (headings/tables/code/links) (v2.1042.0 "The Sieve"); prior line: as of v2.1041.1; `test_open5e_monsters_auth.py` (+1) — `/api/open5e/monsters` now requires login: unauthenticated → 401 (v2.1041.1 "The Warded Menagerie"); prior line: as of v2.1040.2; `test_open5e_items_auth.py` (+1) — `/api/open5e/items` now requires login: unauthenticated → 401 (v2.1040.2 "The Members' Entrance"); prior line: as of v2.1040.0; `test_import_zip_guard.py` (+6) — import zip-bomb guard: `open_archive` rejects oversized uncompressed total / single entry / entry count before reading, still raises on bad zips, default ceiling 1 GiB (v2.1040.0 "The Bomb Squad"); prior line: as of v2.1039.0; `test_login_rate_limit.py` (+6) — main-app login brute-force throttle: pure-module unit tests (below/at threshold, window expiry, reset-on-success, per-IP isolation, config defaults/override) with injected clock+store (v2.1039.0 "The Cooldown"); prior line: as of v2.1038.0; `test_csrf_origin.py` (+7) — main-app CSRF origin check: 5 unit tests on `_origin_matches_host` (match/mismatch/referer-fallback/origin-precedence/no-headers) + 2 live integration tests (cross-origin POST → 403 CSRF, header-less POST not CSRF-blocked) (v2.1038.0 "The Bouncer"); prior line: as of v2.1037.0; `test_request_body_limit.py` (+5) — global request-body size limit: 5 in-process unit tests on `_request_body_too_large` (under/over/missing/disabled) + the resolved cap equals env-or-512-MiB-default (v2.1037.0 "The Weigh Station"); prior line: as of v2.1036.1; `test_safe_next_path.py` (+30) — post-login redirect scrubbing: both `safe_next_path` + `_safe_next_path` reject open-redirect targets (`//evil`, backslash `/\evil.com`, encoded `%5c`, CRLF smuggling, `javascript:`) and preserve legit same-origin paths (v2.1036.1 "The Straight Path"); prior line: as of v2.1036.0; `test_secret_key_guard.py` (+8) — APP_SECRET_KEY hardening: `_resolve_secret_key` uses a strong operator key verbatim, auto-generates a distinct strong ephemeral key for blank/placeholder/short values, and hard-fails under APP_SECRET_KEY_STRICT on a weak key while passing a strong one (v2.1036.0 "The Deadbolt"); prior line: as of v2.1035.0; `test_security_headers.py` (+3) — the HTTP security-headers middleware: asserts the full defensive header set (nosniff / X-Frame-Options DENY / Referrer-Policy / Permissions-Policy) on a public endpoint, the CSP's restrictive directives (`default-src 'self'`, `frame-ancestors 'none'`, `object-src 'none'`, `base-uri 'self'`), and that headers stamp HTML pages too (v2.1035.0 "The Hard Hat"); prior line: as of v2.1034.1; `test_demo_disable_uploads.py` (+1 → 8) — widened lockdown: `test_all_upload_endpoints_carry_the_guard` route-introspects the main app and asserts all 13 dependency-guarded upload endpoints (11 user-facing + 2 admin) list `require_uploads_enabled`, catching a future endpoint that forgets the guard (v2.1034.1 "The Closed Loophole"); prior line: as of v2.1034.0; `test_demo_disable_uploads.py` (+7) — the `DEMO_DISABLE_UPLOADS` toggle: 6 in-process unit tests on the `uploads_disabled_in_demo` predicate + `require_uploads_enabled` dependency (403 only when `DEMO_MODE` **and** `DEMO_DISABLE_UPLOADS`, silent no-op otherwise, default-true) and 1 adaptive integration test that proves the guard is wired to a real upload endpoint (campaign-background) over HTTP — 403 with the exact guard detail on a locked demo, 200 otherwise — so it passes whether the harness points at a dev stack or a live `DEMO_MODE=true` demo (v2.1034.0 "The Velvet Rope"); prior line: as of v2.1033.17; `test_hermetic_pcs.py` (+2 → 10) — B18 class 2: autouse `hermetic_battle` fixture restores the demo's seeded battle roster before any test whose predecessor overwrote it via `PUT /battle`, fixing the sphere/cone/line "got 0 seeded combatants" errors + the Drakkasha demo-spawn check; new guard leaks the seeded roster and asserts the next test sees it restored (v2.1033.17 "The Muster"); prior line: as of v2.1033.16; count unchanged — B18 class 6 cont'd: `test_defense_fighting_style` now asserts the Defense fighting-style AC **delta** (robust to Kael's Bracers of Defense +2), and the demon-slayer detune tests (`test_demon_slayer_rider` / `test_demon_slayer_frighten`) resolve the Rapier's inventory index **by name** (drifted 7→11) instead of a stale constant (v2.1033.16 "The Misplaced Rapier"); prior line: as of v2.1033.15; count unchanged — B18 class 6: fixed the four Garrik-item-showcase-interference tests — `test_potion_of_climbing` / `test_potion_of_diminution` now assert on the dice token (`_dice_part`, ignoring Garrik's Luckstone `+1`), and the two `test_potion_of_resistance_*` fixtures un-equip Garrik's Frost Brand (innate fire resistance) so the drunk potion is the sole resistance source (v2.1033.15 "The Naked Fighter"); prior line: as of v2.1033.12; `test_hermetic_pcs.py` (+2 → 8) — B18 class 3: `clean_pcs` generalized to clear every buff on each PC combatant (read from `GET /battle`), not just the hardcoded `_LEAKABLE_BUFF_KEYS`; new guard leaks a non-hardcoded `resistance-cold` buff and asserts the next test sees it cleared (v2.1033.12 "The Dispel"); prior line: as of v2.1033.11; `test_hermetic_pcs.py` (+3 → 6) — B18 class 1: `hermetic_pcs` extended to restore class-scoped `level`/`subclass` drift (with the PC's primary `class_slug`), closing the `assert 5 == 7` residual cluster + B9; new guard tests cover the class-scoped level leak/restore between tests (v2.1033.11 "The Second Seal"); prior line: as of v2.1033.7; `test_hermetic_pcs.py` (new, +3) — B17: the autouse `hermetic_pcs` conftest fixture snapshots each demo PC's pristine mutable sheet fields and restores drifted ones before every test so a sheet-mutating test's failed restore can't cascade through the serial run; the new file proves the fixture heals a leaked `spells` strip between tests + round-trips the snapshot/restore helpers (v2.1033.7 "The Airlock"); prior line: as of v2.1033.5; count unchanged — B16 final cluster: `test_cast_confusion_npc` resolves the Confusion spell index by name instead of the stale hardcoded `12` (drift, not RNG), no new tests (v2.1033.5 "The Right Spell"); prior line: as of v2.1033.4; count unchanged — B16 catalog-anchor fix flipped the stale `concentration:false` anchors to `true` in `test_cast_conjure_family` / `test_cast_polymorph` (the catalog was corrected to the RAW value) and renamed one polymorph test, no new tests (v2.1033.4 "Concentration Restored"); prior line: as of v2.1033.3; count unchanged — B16 derivation-helper fix corrected `_expected_attack_bonus` / `_expected_save_dc` in `test_spell_catalog_attack` / `_save` to resolve `class_spellcasting` (the demo caster is Lv7/INT, not the WIS the helper assumed), no new tests (v2.1033.3 "The Right Modifier"); prior line: as of v2.1033.2; count unchanged — B16 vision-cluster fix wired the `bright_map` conftest fixture into 5 existing roll-state gate tests (`test_cast_blur` / `holy_aura` / `mislead` / `true_seeing` / `foresight`) for order-independence against inherited `dark` ambient, no new tests (v2.1033.2 "Torches Lit"); prior line: as of v2.1033.0; `test_bonus_action_spell_pairing.py` (new, +9) — B15: the SRD bonus-action spell pairing rule gated in `/cast_spell` (409 `bonus_action_spell_pairing`), covering both orderings, the RAW cantrip exception, the **asymmetry** case that a symmetric implementation would fail, GM + `override` bypass, inert-with-no-battle, **survives a mid-turn client `pushBattle`** (the fail-open that drove the storage design), and marker expiry at turn advance (v2.1033.0 "One Spell Too Many"); prior line: as of v2.1032.0; `test_econ_rule_chip.py` (new, +3) + `test_econ_rule_chip_popover.py` (new UI, +4) — SRD Reference Phase 3 close-out: the 📖 rules chip in the init-tracker econ strip opens the shared SRD popover; every slug it lists is asserted to resolve via `/api/reference/entry`, and a back-compat test drives the legacy `data-cond-slugs` conditions path through the same generalized function (v2.1032.0 "The Rules Lawyer's Thumb"); prior line: as of v2.1031.0; `test_srd_reference.py` (+2) — SRD Reference Phase 3: the third `rules` batch (Objects, underwater/mounted combat, madness) — Underwater Combat findable, the Objects entry resolves with its material-AC text, and the full-text path reaches Madness by a body-only phrase (v2.1031.0 "The Salvager's Ledger"); prior line: as of v2.1030.0; `test_srd_reference.py` (+1) — SRD Reference Phase 3: the second `rules` batch (Adventuring + Environment) — Long Rest findable, the Falling entry resolves with its 1d6-per-10-feet text (v2.1030.0 "The Wanderer's Almanac"); prior line: as of v2.1029.0; `test_srd_reference.py` (+2) — SRD Reference Phase 3: the new `rules` reference type surfaces the non-content SRD combat rules (Cover findable with its degrees-of-cover text; the Grappling entry resolves via `/api/reference/entry`) (v2.1029.0 "The Combat Codex"); prior line: as of v2.1028.0; `test_mini_sheet_cond_warn.py` (+1) — SRD Reference Phase 3: the ⚠ Conditions pill on mini-sheet cards is a clickable button (`role="button"` + `data-cond-slugs`) that opens the offline SRD-rule popover (v2.1027.0 "The Table Ruling"); prior line: as of v2.1025.0; `test_srd_reference.py` (+4) — SRD Reference Phase 3 substrate: `GET /api/reference/entry?type=&slug=` returns one shipped-SRD record (Blinded found with desc text), 404 on unknown slug / unknown type / monster type (v2.1025.0 "The Marginal Gloss"); prior line: as of v2.1024.0; `test_bulk_map_upload.py` (new, +4) — bulk map upload: multiple images in one POST create a Map each (name from filename), bad files skipped not fatal, GM-gated, no-files rejected (v2.1024.0 "The Cartographer's Haul"); prior line: as of v2.1023.0; `test_pin_rule.py` (new, +6) — SRD Reference Phase 2b: a GM pins an SRD entry to the tabletop (`rule_pinned` broadcast + `GET /pinned_rule` persistence), unpin clears it, GM-gated, 404/400 guards, campaign reference page renders with pin UI (v2.1023.0 "The Pinned Page"); prior line: as of v2.1022.0; `test_srd_reference.py` (+1) — `test_reference_search_is_full_text`: the reference search matches description text, not just names, with name matches ranked first (v2.1022.0 "The Deeper Index"); prior line: as of v2.1021.0; `test_theme_burnt_manuscript.py` (new, +4) — the Burnt Manuscript theme: accepted by `POST /api/settings/theme` (unknown → 400), CSS block + picker card ship (v2.1021.0 "The Charred Page"); prior line: as of v2.1020.0; `test_srd_reference.py` (new, +6) — the in-app SRD 5.1 rules reference: `/reference` page render + `/api/reference/search` full-text over the six player-safe SRD types (Grappled/Fireball findable), monsters excluded (404), unknown type (404) (v2.1020.0 "The Open Compendium"); prior line: as of v2.1019.0; `test_overchannel_phase2.py` (new, +1) — Overchannel Phase 2 maxes the spell-attack-roll damage path (Guiding Bolt 4d6→24, or 48 on a crit) + fires the ⚡ card on a no-save cast (v2.1019.0 "The Complete Overload"); prior line: as of v2.1018.0; `test_thiefs_reflexes.py` (new, +7) — Thief's Reflexes (Thief Rogue Lv 17+) scoped v1: round-1 second-turn initiative (base−10) + broadcast, round/surprised/not-in-initiative/level/error gates; `test_wiki.py::test_wiki_doc_serves_thiefs_reflexes_plan` (+1) — the new plan doc serves via the wiki (v2.1018.0 "The Second Wind of Shadows"); prior line: as of v2.1017.0; `test_natures_sanctuary.py` (new, +6) — Nature's Sanctuary (Land Druid Lv 14+) end-to-end: wards the druid so a beast/plant attacker WIS-saves-or-misses in `/npc_attack` (beast blocked, humanoid ignored), install + level/error gates (v2.1017.0 "The Warded Grove"); prior line: as of v2.1016.0; `test_tranquility.py` (new, +4) — Tranquility (Open Hand Monk Lv 11+) end-to-end: installs the shared `sanctuary` buff at DC 8 + WIS + prof (enforced by the existing attacker-must-save gate), level/error gates (v2.1016.0 "The Aura of Peace"); prior line: as of v2.1015.0; `test_fiendish_resilience.py` (new, +6) — Fiendish Resilience (Fiend Warlock Lv 10+) end-to-end: grants resistance to a chosen damage type (buff carries `resistance_to`), re-pick replaces, bad-type/level/error gates (v2.1015.0 "The Infernal Ward"); prior line: as of v2.1014.0; `test_quivering_palm.py` (new, +8) — Quivering Palm (Open Hand Monk Lv 17+) two-mode end-to-end: setup spends 3 ki + installs the vibrations buff, trigger resolves the CON save → drop-to-0 or 10d10 necrotic + consumes the buff, not-set-up/out-of-ki/level/error gates (v2.1014.0 "The Lethal Vibration"); prior line: as of v2.1013.0; `test_peerless_skill.py` (new, +5) — Peerless Skill (College of Lore Bard Lv 14+) end-to-end: spends a Bardic Inspiration use for a +1d10 own-check bonus, resource decrement + broadcast, out-of-uses/level/error gates (v2.1013.0 "The Sure Hand"); prior line: as of v2.1012.0; `test_retaliation.py` (new, +7) — Retaliation (Berserker Barbarian Lv 14+) end-to-end: a reaction melee weapon counter-attack vs the damaging creature — hit-and-damage (loops until a hit lands), reaction-economy over_budget echo, level gate + error paths (v2.1012.0 "The Counterblow"); prior line: as of v2.1011.0; `test_hurl_through_hell.py` (new, +7) — Hurl Through Hell (Fiend Warlock Lv 14+) end-to-end: 10d10 psychic to a non-fiend + use consumed, fiend exemption (0 damage), once-per-long-rest 409, level gate + error paths (v2.1011.0 "The Lower Planes"); prior line: as of v2.1010.0; `test_overchannel.py` (new, +7) — Overchannel (Evocation Wizard Lv 14+) end-to-end: `/use_overchannel` arms a one-shot buff, the next damaging 1st-5th level spell is maxed (Fireball 8d6→48), first-use-free + second-use necrotic self-damage (6d12), one-shot consume, level gate + error paths (v2.1010.0 "The Overload"); `test_overchannel_self_damage.py` (new, +9) — unit tests of the `_overchannel_self_damage_expr` escalation formula; prior line: as of v2.1009.0; `test_champion_survivor_regen.py` (new, +12) — the Champion Fighter Survivor (Lv 18+) `5 + CON mod` start-of-turn regen, unit-tested on `_pc_champion_survivor_regen` across the Lv-17→None / Lv-18 boundary, CON-mod arithmetic, and multiclass + regression sheets (v2.1009.0 "The Last Breath"); `test_superior_critical_threshold.py` (new, +12) — the Champion Fighter Superior Critical (Lv 17+) crit-on-18 threshold, unit-tested on `_attacker_crit_threshold` across single-class + multiclass + regression sheets (v2.1008.0 "The Widened Edge"); `tests/harness_ui/test_map_editor_zoom.py::test_scroll_wheel_zooms` rewritten — bare wheel asserted as pan-not-zoom, zoom driven via Ctrl+wheel (v2.1007.0 "The Drafting Table", editor wheel parity); prior line: as of v2.1006.0, both per a fresh `--collect-only` — the harness_ui "299" figure had drifted from a real 310; `test_settings_map_controls.py` (new, +5) — the map-controls scheme endpoint + the reorganized /settings page + the ME.altControls bootstrap (v2.1006.0 "The Cartographer's Wheel"); `tests/harness_ui/test_map_pan.py` (+2 net) — `test_wheel_still_zooms` replaced by `test_wheel_pans_by_default` / `test_shift_wheel_pans_horizontally` / `test_ctrl_wheel_zooms`, and the drag-pan start point is now transform-derived so it can't land in the letterbox gutter; `test_summon_wildfire_spirit.py::test_ws_summon_lands_in_battle_state` (new, +1) — the Wildfire Spirit stands up as a real summon combatant (v2.1005.0 "The Kindled Ally"); `test_potent_spellcasting.py::test_ps_adds_wis_to_cantrip_damage_on_cast` (new, +1) — the Phase-2 read site adds +WIS to cleric-cantrip damage end-to-end (v2.1004.0 "The Cantrip's Edge"); `test_supreme_healing.py::test_sh_fires_on_heal_claim_path` (new, +1) — the `/apply_healing` heal-claim path maxes healing dice for a Lv 17+ Life cleric caster (v2.1003.0 "The Full Measure"); `test_combat_wild_shape.py::test_use_cws_heal_applies_hp` (new, +1) — the heal mode now applies HP server-side; state-asserted against the sheet (v2.1002.0 "The Wild Mend"); prior line: as of v2.1001.0, per a fresh `--collect-only` — the "4687" figure had drifted +6 from reality; `test_attack_prone_positional.py` (new, +4) — Phase 3 positional prone edge: within-5-ft advantage / beyond-5-ft disadvantage / off-grid fallback / attacker-prone-vs-target-prone cancel (v2.1001.0 "The Fallen Foe"); `test_backup_admin.py::test_offsite_remote_listing_grouping_and_pull_validation` (new, +1) — remote-listing grouping + traversal-proof pull requests; the live offsite-status test now expects the listing/pull keys (v2.1000.0 "The Salvage Run", offsite restore); `test_admin_center.py::test_backups_offsite_status_secret_free_shape` (new, +1) — the offsite-uploads status JSON is secrets-free + the /backups page renders the ☁️ card (v2.999.0 "The Courier's Desk", offsite web UI); `test_backup_admin.py` (+4) — offsite-upload engine: settings round-trip/preserve, S3 + OAuth rclone-conf generation (0600, secrets-free summary, newline-injection flattened), triggers/status/delete (v2.998.0 "The Cloud Courier"); the deep-dive test now also covers `grapple` + `legendary` (L18 dragon Legendary Resistance); the deep-dive test now also covers `reactions` + `saves`; `test_selftest.py::test_selftest_rest_deep_phase` + `test_wiki.py::test_wiki_doc_serves_self_test_srd_coverage` (new, +2) — the self-test **Rules deep-dive** tier framework + `rest` phase, and the SRD-coverage/demo-evaluation doc (v2.987.0 "The Deep End"); `test_selftest.py` (+2: `test_video_path_validation` name guard + `test_selftest_video_capture` end-to-end recorded run → served, traversal-guarded .webm) — self-test video capture (v2.985.0 "The Instant Replay", headless-Chromium recording); `test_selftest.py::test_selftest_slow_flag_and_stats_note` (new, +1) — a slow run flags scope.slow + step_delay and every run records a stat-purge note so self-tests don't skew campaign time stats (v2.981.0 "The Slow Waltz" — Run-slower button); `test_selftest.py::test_selftest_reseed_flagship_skipped_and_validation` (new, +1) — the ♻ reseed endpoint refuses the flagship per-campaign (non-destructive) + 400s an empty body (v2.980.0 "The Fresh Slate" — separate per-campaign reseed button); `test_selftest.py::test_selftest_doors_subset` (new, +1) — a doors-only subset opens/closes a door on each map (walls_update) with no combat; the full-run test also asserts a `door` check + the paced cinematic run (v2.979.0 "The Dress Rehearsal" — glide + doors + advance-to-melee); `test_selftest.py::test_selftest_movement_only_subset` (new, +1) — a phase subset runs just that phase (no combat rounds; setup limited to reachability + movement), scope recorded (v2.978.0 "The Focused Lens" — subset runs); `test_selftest.py` (+1: `test_load_run_rejects_bad_ids`) now also covers run-history persistence — a completed run is listed, reopenable, and the id guard rejects traversal (v2.977.0 "The Logbook") — plus negative-path `gate` checks (off-turn move 403 / over-budget attack 409, driven as a player) and a `spell_cast` check (v2.975.0 "The Incantation" — caster PCs cast a leveled spell or cantrip each round); `test_wiki.py::test_wiki_doc_serves_self_test` (+1) + `test_wiki_home_renders` asserts the `/wiki/doc/self-test` reference link (v2.974.0 "The Operator's Handbook", doc-only self-test guide); `test_selftest.py` (new, +5) — Admin-Center demo self-test: report roll-up recompute + node-status precedence + combatant construction + `run_status` JSON-safety (host-side), plus a live run-to-completion asserting every demo campaign reports error-free AND that combat rounds ran with PC/NPC actor nodes carrying pc_attack/npc_attack/turn_advance checks ("The Proving Ground" v2.972.0 setup + movement; "The Sparring Ring" v2.973.0 combat rounds); `test_wiki.py::test_wiki_home_renders` now asserts the new `/wiki/doors` guide link (v2.970.0 "The Doorkeeper's Manual", doc-only); `test_door_open_check.py` (+4) — GM's SELECTED NPC token rolls a door gate (via token_id): fails DC 999 → shut (not bypassed), passes DC 1, picks a DC-1 lock; GM with nothing selected still bypasses (v2.969.0 "The Monster's Hand"); `tests/harness_ui/test_token_click_select.py` (new, +1) — click-to-select a token (vttSelectedTokenId/vttActingTokenId); Escape clears (v2.968.0 "The Chosen Token"); `tests/harness_ui/test_door_gate_badges.py` (new, +1) — locked doors show 🔒 + check-to-open doors show 🎲 on the tabletop (v2.967.0 "The Signs on the Door"); `test_door_open_check.py` (+5) — LOCK enforcement: key holder unlocks+opens, no-key/no-pick stays locked, pick DC 1 opens, pick DC 999 stays shut, GM bypasses (v2.966.0 "The Turning Key"); `test_door_open_check.py` (+3) — door LOCK config {locked, key, lock_check, lock_dc} round-trips, persists while unlocked, incomplete pick gate dropped + DC clamp (v2.965.0 "The Iron Key"); `tests/harness_ui/test_map_editor_save_toast.py` (new, +1) — map-editor incremental saves fire the bottom-center `.me-toast` confirmation (v2.964.2 "The Saved Toast"); `tests/harness_ui/test_map_editor_door_check.py` (new, +1) — the map editor door menu exposes the “🎲 Open check” item (v2.964.1 "The Reachable Latch"); `test_door_open_check.py` (+5) — checked-door ENFORCEMENT: player passing (DC 1) opens, failing (DC 999) stays shut (verified server-side), GM bypasses, no-token → 400, closing needs no check (v2.964.0 "The Threshold Test"); `tests/harness/test_door_open_check.py` (new, +4) — per-door open-check {check, dc} round-trips on embedded + whole-segment doors, DC clamps to 1–40, incomplete gate dropped, plain door carries neither field (v2.963.0 "The Marked Door"); `test_movement_dash_modals.py` un-broken — the GM init tracker's “🎲 Prompt” owner lookup no longer throws a `charById is not defined` ReferenceError that tripped the tests' clean-console guard (v2.960.0 "The Name on the Prompt"); `tests/harness_ui/test_oa_hardblock_backstop.py` (new, +1) — OA hard-block backstop: when the pre-move preview misses an OA, the direct postMove() 409 snaps the token back + reopens the Continue/Stop modal instead of silently desyncing (v2.959.0 "No Silent Steps"); `tests/harness/test_enforce_gm_ranges.py` (new, +4) — the `enforce_gm_ranges` campaign toggle holds the GM to the same weapon/spell range gate players get: OFF = GM out-of-range attack succeeds (bypass), ON = 409 `out_of_range`, ON + override clears, ON in-range unaffected (v2.958.0 "The GM's Own Yardstick"); opportunity-attack reach is now edge-aware (large creatures threaten from their space) — `test_opportunity_attack_gridless.py` gained a size-2-watcher edge-reach test (+1) (v2.957.0 "The Big Reach"); `tests/harness/test_opportunity_attack_gridless.py` (new, +2) — opportunity attacks are enforced on gridless maps (Euclidean distance): preview flag + 409 gate + trigger list, and no OA when staying in reach (v2.956.1 "No Free Steps"); `tests/harness_ui/test_fog_hides_npcs_out_of_los.py` (new, +1) — dynamic fog hides non-owned tokens outside the player's current LOS (explored memory shows the room, not its occupants); fog grid halved again (FOG_SUBDIV 4) (v2.956.0 "Out of Sight"); `test_wall_lighting_shadow` de-flaked (\`__testDrawLighting\` now clears demo tokens/lights) + wall-shadow reach dialled back to *6 to avoid extreme fill coords (v2.955.1 "The Bounded Shadow"); `tests/harness_ui/test_gate_door_swing.py` (new, +1) — a gate's two door leaves swing arcs curve in opposite senses (sweep flags [0,1], not [1,1]) so they reflect the real opening direction (v2.955.0 "The Right Swing"); fog of war now tracks/renders on a finer FOG_CELL grid (half the 5-ft cell) — the fog cell-key tests compute keys via `__fogCellForTest` (v2.954.0 "The Finer Veil"); the player wall-fog mask now dilates seen cells by half a cell so boundary walls (the one you're standing against) render (v2.953.1 "The Wall at Hand"); `tests/harness_ui/test_wall_shadow_reach.py` (new, +1) — wall vision shadows reach fully past the map edge, so a PC near a wide wall no longer sees the far side (fog + lighting) (v2.953.0 "The Long Shadow"); `test_wall_player_visibility.py` gained a full-opacity wall-mask check (+1) — walls render at 100% where the player has seen, fixing the too-dim v2.950.0 veil-derived mask (v2.952.1 "The Clear Wall"); `tests/harness_ui/test_token_wall_collision.py` (new, +1) — player token moves clamp at solid walls / closed doors (open doors leave a gap; GM is free) (v2.952.0 "The Solid Wall"); `tests/harness_ui/test_gridless_drag_ruler.py` (new, +1) — a live movement ruler (line + ft chip) shows while dragging a token on a gridless map, green/red vs the active combatant's speed (v2.951.0 "The Measured Step"); `tests/harness_ui/test_wall_player_visibility.py` (new, +2) — walls/doors are visible to players fog-masked (only where the party can see) and unmasked for the GM; `test_token_layering.py` updated for the wall overlay moving above the veil (z8) (v2.950.0 "The Seen Walls"); `tests/harness_ui/test_wall_bootstrap_race.py` (new, +1) — the map bootstrap survives the parse-vs-fetch race: dropping the walls then re-applying the buffer restores wall occlusion (no more seeing past walls / lights through fog on load) (v2.949.0 "Load-Bearing"); `tests/harness_ui/test_light_glow_fog_mask.py` (new, +1) — the light-glow layer is masked by the fog veil, so torches the player can't see (behind fog / past walls) stop glowing (v2.948.0 "Snuffed"); Goblin Warrens fog of war turned back ON (dynamic); the fog-off anti-bypass test now forces fog off via the API + restores it, decoupled from the seed (v2.947.1 "The Fog Returns"); `test_vtt_selected_vision_veil.py::test_player_cannot_see_past_walls_with_fog_off` (new, +1) — a fog-OFF map no longer lets an untargeted player see past walls; their view is bounded to their own PCs' line of sight (v2.947.0 "No Peeking"); `test_token_visibility_perspective.py` gained the "⬆ My tokens on top" player toggle coverage (+2) — ON keeps controlled tokens always on top, OFF restores turn-gating; checkbox renders/persists for players, hidden for GM (v2.946.0 "The Raised Banner"); `test_map_fog_player.py::test_player_fog_reveals_only_own_tokens_vision` (new, +1) — a player's fog reveals only their OWN tokens' sight, not the whole party (v2.945.0 "Through Their Own Eyes"); `test_token_visibility_perspective.py` now asserts the GM's targeted token rides on top over its POV veil (others lower) + a new test that a player's controlled token is top-most only on its turn (v2.944.0 "The Turn to Shine"); `tests/harness_ui/test_vtt_selected_vision_veil.py` (new, +1) — targeting a token in the VTT veils what it cannot see (darkvision-LOS disk minus wall shadows), parity with the editor and independent of fog (v2.943.0 "The Adopted Eye"); the Goblin Warrens seed was re-authored again — fog of war OFF + gate CLOSED + tighter torch ring; `test_demo_map_elements.py::test_goblin_warrens_ships_walls_and_hotspots` now asserts fog disabled + the embedded gate ships closed (v2.942.0 "The Barred Gate"); `test_demo_map_elements.py::test_goblin_warrens_ships_walls_and_hotspots` now asserts all Goblin Warrens braziers flicker at 0.25× "very slow" and the map is weatherless (v2.941.1 "The Still Warren"); `test_map_lights_editor.py::test_editor_light_flicker_speed` now also covers the new "🐌 very slow" 0.25× flicker option (v2.941.0 "The Guttering Ember"); `tests/harness_ui/test_token_layering.py` now asserts the `#light-glow-canvas` (torch color) sits ABOVE the veil (z7>z6) — v2.861.0 had buried it under the darkness/fog veil so dark maps showed no torch color (v2.940.2 "The Torch Color"); `tests/harness_ui/test_token_visibility_perspective.py` now drives targeting through a real `Set` (was an array) so it guards `_anyTokenSelected` reading `.size` — the GM/player selected-token perspective (narrow-to-vision) that was dead since v2.882.0 because the Set had no `.length` (v2.940.1 "The Narrowed Eye"); the Goblin Warrens demo map was re-authored from a map-editor JSON export (v2.940.0 "The Rebuilt Warren") — `test_demo_map_elements.py::test_goblin_warrens_ships_walls_and_hotspots` now expects one wood wall with an embedded gate + dynamic fog (no secret door/hotspot/static reveal); `tests/harness_ui/test_editor_invisible_wall_visibility.py` (new, +1, v2.939.1 "The Visible Ghost") — an invisible wall renders as a glowing dashed ghost guide (bright 8-6 dashed core + wide halo) in the editor;  the three zone dividers became far-left toggle buttons that show/hide each zone independently, persisted per map (v2.939.0 "The Zone Toggles") — `test_map_editor_collapse.py` / `test_editor_group_collapse.py` / `test_editor_actions_zone.py` / `test_map_editor_toolbar.py` updated to the button-hide semantics; the Notes (Annotations) + Tokens groups moved from the Draw zone to the Map zone (v2.936.0 "The Map Super-Group") — `test_map_editor_toolbar.py` label order + `test_map_editor_collapse.py`/`test_editor_group_collapse.py` Draw-member lists updated; `tests/harness_ui/test_editor_light_bd_inline.py` (new, +1, v2.935.1 "The Inline B") — the B/D light labels read "B [field]" inline (fixing the global label{flex-direction:column} that stacked the letter above the field); the .me-capsel--stack caption variant stays column; flicker speed (v2.935.0 "The Flicker Speed"): `test_map_lights.py::test_light_flicker_round_trips_and_clamps` (+1) + `test_map_lights_editor.py::test_editor_light_flicker_speed` (+1) — the per-light flicker-speed multiplier persists (default 1, clamp 0..4) and the editor dropdown sets it; toolbar compaction guard relaxed 240→265; `tests/harness_ui/test_editor_light_recent.py` (new, +1, v2.934.0 "The Recent Palette") — the flicker pickers remember the last 5 colours; clicking a recent swatch fills the last-focused picker; persists across reload; `tests/harness_ui/test_editor_actions_zone.py` (new, +1, v2.932.0 "The Actions Zone") — View moved beside Tools and the Actions zone (File · Tools · View) collapses via its own divider; Walls opacity is a 0–100% (step 10) dropdown with the caption stacked above (v2.930.0 "The Opacity Dropdown") — `test_editor_window_wall.py` picks 0% via select_option; Environment regroup (v2.929.0 "The Gathered Environment"): ambient + fog + weather gathered into one Environment group (out of Lighting/Fog/Weather) — `test_map_editor_toolbar.py` / `test_map_editor_collapse.py` / `test_editor_group_collapse.py` group-name assertions updated; `tests/harness_ui/test_editor_view_stack.py` (new, +1, v2.928.0 "The Stacked View") — the View group's zoom controls stack vertically (zoom-in above zoom-out, same x); Theme C (v2.927.0 "The Shared Shape"): the three per-group ⬡ FreeForm buttons collapsed into one shared #me-freeform-btn — `test_map_editor_room.py` / `test_map_editor_terrain_polygon.py` / `test_map_editor_lair_zone.py` updated to arm the tool + toggle the shared FreeForm; `tests/harness_ui/test_editor_inspector.py` (new, +2, v2.926.0 "The Inspector", Theme B) — double-click-selecting a wall shows the floating #me-inspector; its material select / opacity slider / type buttons round-trip and Delete removes the wall + hides the panel; `tests/harness_ui/test_editor_context_labels.py` (new, +2, v2.925.0 "The Labelled Selection", Theme D) — the Select&move tool is renamed "Grab"; double-click-selecting a wall relabels its material caption to "✎ selected wall" (accented) and Esc reverts it; Theme A editor regroup (v2.924.0 "The Concern Groups"): `test_map_editor_toolbar.py`/`test_map_editor_collapse.py`/`test_editor_group_collapse.py` updated for the new Draw-zone group names (Walls · Terrain · Lighting · Fog · Weather · Notes · Lair · Tokens); every control keeps its element ID so no behaviour test changed; `tests/harness/test_wiki.py` gains `test_wiki_doc_serves_editor_controls_reorg_plan` (+1, v2.923.0 "The Drafting Table") — the map-editor control-reorg plan serves through /wiki/doc/plan-editor-controls-reorg; `tests/harness_ui/test_editor_map_import_export.py` (new, +2, v2.922.0 "The Portable Map") — the map editor Export builds a JSON layout object carrying every layer; Import replaces all placed elements (walls incl. windows + ambient) from a JSON payload; `tests/harness_ui/test_editor_window_wall.py` (new, +2, v2.921.0 "The Open Window") — 🪟 Window in the Walls material dropdown draws a window; the opacity slider takes it down to 0 (a fully open window); `tests/harness_ui/test_editor_dblclick_select.py` (new, +2, v2.920.0 "The Double-Click Select") — double-clicking a wall or door selects it into move+resize (two yellow handles); a door double-click does NOT toggle it open (deferred toggle cancelled); `test_token_context_menu.py` gains `test_player_right_click_gets_compact_menu` (+1, v2.919.0 "The Player's Menu") — a player right-click opens a compact menu (target only; no GM size/owner/team/delete controls); `test_token_context_menu.py` gains `test_gm_token_menu_sets_team` (+1, v2.918.0 "The Full Menu") — the token context menu's owner/team/colour rows; picking 🦸 Hero in the Team dropdown sets the token team; `tests/harness_ui/test_token_footprint_snap.py` (new, +2, v2.917.0 "The Squared Footprint") — a dropped token snaps so its N×N footprint aligns to cells (odd sizes on a grid line, even sizes half-offset), and the hit radius tracks the drawn radius (per-map scale); `tests/harness_ui/test_token_context_menu.py` (new, +1, v2.916.0 "The Conjured Menu") — right-clicking a token as GM opens the .tt-token-ctx action menu whose size control resizes the token (M/L/H/G) and closes on action; `tests/harness/test_token_size.py` (new, +4, v2.915.0 "The Bigger Beast") — per-token size override via PATCH /token/{id} {size}: round-trip + token_update broadcast, clamp to [1,4], 400 on non-int, 403 non-GM; `test_editor_token_scale.py` gains `test_editor_token_scale_presets` (+1, v2.914.0 "The Preset Row") — the S/M/L/XL token-size presets set the scale in one click, persist it, and highlight the active preset; `tests/harness/test_map_token_scale.py` (new, +4, v2.913.0 "The Sizing Dial") — the per-map default token-size multiplier: POST /settings/maps/{mid}/token_scale round-trips + broadcasts token_scale_update, clamps to [0.5, 3.0], 400 on non-numeric, 403 for non-GM; `tests/harness_ui/test_editor_token_scale.py` (new, +1, v2.913.0 "The Sizing Dial") — the editor Grid-group "tokens ×" dial grows the sample-token preview and persists token_scale to the map; `test_editor_group_collapse.py` gains `test_zone_collapse_persists_across_reload` (+1, v2.912.1 "The Remembered Fold") — a zone collapse survives a page reload via the per-map `me-collapsed-<MID>` localStorage store; `tests/harness_ui/test_editor_group_collapse.py` (new, +1, v2.912.0 "The Folded Lair") — clicking the Draw zone divider collapses the Lair group along with the rest of that zone, and a collapsed group's title computes to a vertical writing-mode; `test_map_editor_room.py` renames wall chaining to `test_freeform_wall_chaining` (via ⬡ FreeForm) and adds `test_basic_wall_is_two_point_no_chain` (v2.911.0 "The FreeForm") — 🧱 Wall now makes a single two-point wall; `test_invisible_wall_render.py` gains `test_draw_invisible_wall_from_material_dropdown` (v2.910.0 "The Drawn Ghost") — drawing with 👻 Invisible selected yields a wall with invisible:true; `tests/harness/test_invisible_wall.py` (new, +1) + `tests/harness_ui/test_invisible_wall_render.py` (new, +1, v2.909.0 "The Invisible Wall") — an invisible wall + per-door material round-trip, and the invisible wall shows a faint dashed guide to the GM but nothing to players; `tests/harness_ui/test_editor_sticky_tools.py` (new, +2, v2.908.0 "The Sticky Tool") — a left-clicked placeable tool auto-deselects after placing one item; a right-clicked tool stays armed (sticky, yellow ring); `tests/harness_ui/test_embedded_door_move_resize.py` (new, +1, v2.907.0 "The Sliding Door") — selecting a door (right-click → Move/resize) shows two edge handles; dragging a handle widens it (t1 grows) and dragging the body slides it (t0 shrinks); `tests/harness_ui/test_insert_door_menu.py` (new, +1, v2.906.0 "The Right-Click Door") — right-clicking a wall → Insert door here embeds one door and leaves the wall a single object; `test_map_editor_door_style.py` also asserts the Door tool un-presses after a placement (v2.905.0 "The Single Placement"); `tests/harness_ui/test_embedded_door_open_hit.py` (new, +1, v2.904.0 "The Swinging Leaf") — an open embedded door on a horizontal wall yields an interactive hit-line with vertical extent (follows the swung leaf, not the empty doorway);, 2026-07-04; `tests/harness_ui/test_embedded_door_render.py` (new, +1, v2.901.0 "The Hinged Wall") — a wall with an embedded door draws the door knob glyph and clicking its hit-line toggles the door open (composite id); `tests/harness_ui/test_embedded_door_vision.py` (new, +1, v2.900.0 "The Open Span") — the client `_wallSolidSpans` expands a wall into solid sub-spans: plain/closed-door → one span, window/legacy-open → none, OPEN embedded door → two spans with a gap at the door; `tests/harness/test_embedded_doors.py` (new, +5, v2.899.0 "The Door Within") — a wall's embedded door round-trips, toggling it via the composite `{wallId}:{doorId}` id opens/closes line of sight through the wall, an unknown embedded id 404s, and an inverted span is dropped; `test_map_editor_room.py`'s `test_free_polygon_walls_close_into_a_loop` (v2.898.0 "The Free Polygon") replaces the two-corner room test — clicking four vertices then the first dot closes a loop of 4 wall segments; `test_map_editor_door_style.py` rewritten for v2.902.0 "The Snug Doorway" — placing a door leaves ONE wall carrying an embedded doors[] entry (no split) and two doors placed close snap flush (test_place_door_embeds_in_wall_without_splitting + test_two_doors_on_one_wall_snap_flush + test_door_on_empty_map_is_rejected); a click on empty map places nothing (net +1: `test_place_door_in_wall` + `test_door_on_empty_map_is_rejected`); `tests/harness_ui/test_map_editor_delete_key.py` (new, +1, v2.896.0 "The Delete Key") — double-clicking a wall to select it then pressing Delete removes it server-side; `tests/harness_ui/test_map_editor_delete_all.py` (new, +2, v2.895.0 "The Clean Slate") — the editor's 🗑 Delete all confirm button stays disabled until `DELETE` is typed (lowercase doesn't count), then clears all seeded layers server-side; Cancel leaves them untouched; `tests/harness_ui/test_lair_panel_placed.py` (new, +1, v2.894.0 "The Zone Lair") — with an active battle whose only combatant has no lair actions, the GM init-tracker lair panel still lists the map-placed "Magma Erupts" pulled from a bound zone (a zone-driven lair with no creature owner); `test_trigger_lair_action.py`'s `test_set_in_lair_without_slug_is_a_zone_lair` (v2.893.0 "The Trigger Finger") replaces the old slug-required-400 test — a slug-less `in_lair=True` now returns 200 as a zone-driven lair; `tests/harness_ui/test_lair_actions_slideout.py` gains `test_gm_slideout_row_has_trigger_button_player_does_not` (+1) — the GM's placed-action rows carry a ⚡ Trigger button while a player's list-only rows do not (and show no mechanics); `tests/harness_ui/test_lair_actions_slideout.py` (new, +1, v2.892.0 "The Lair Ledger") — with a zone bound to magma-erupts the 📋 Lair actions button shows, its right slide-out lists "Magma Erupts" + the GM-only 6d6 mechanics, and hovering the row flashes the bound zone (fill-opacity 0.42); `tests/harness/test_lair_action_catalog.py` (new, +2, v2.891.0 "The Placed Strike") — `GET /api/lair_action_catalog` returns the id-deduped lair-action catalog with full mechanics (magma-erupts = DEX/6d6/fire), ids unique, readable by a player; `tests/harness/test_trigger_lair_action.py` gains `test_trigger_zone_placed_action_resolves_without_slug` (+1) — a map-placed action id fires with full mechanics even when the battle has no `lair_slug` (id-only catalog fallback); `tests/harness/test_terrain_hidden_per_region.py` (new, +1, v2.890.0 "The Hidden Ground") — a terrain region's per-region `hidden` flag round-trips through the terrain PUT/GET (set on one region, absent on a lean one); `tests/harness_ui/test_terrain_hidden_player.py` (new, +1) — with the map-level terrain toggle off, a player sees the visible region's label but not the per-region-hidden one, while the GM sees both (hidden flagged 🔒); `tests/harness_ui/test_map_editor_toolbar_compact.py` (new, +1, v2.889.0 "The Slim Ribbon") — the editor's floating toolbar stays < 240px tall (was ~284px) and all 9 always-expanded layer rows remain visible after the caption-select / File-group / Layers-row compaction; `test_map_editor_{door,label,measure,select}`, `test_map_light_types`, `test_map_lights_editor` now call `me_clear_toolbar` after opening the editor (the relaid-out toolbar moved controls over the full-bleed map); `tests/harness_ui/test_recap_history.py` (new, +1, v2.888.0 "The Adventure Log") — seeds a recap over HTTP, opens the GM "📜 Recaps" browser, and asserts the list row plus the detail view's editable GM-notes textarea + the player-notes section; `tests/harness_ui/test_session_recap_popout.py` (v2.886.0 "The Curtain Call" +1, v2.887.0 "The Table Talk" +1) — the GM's End-Session recap popout renders a session-name input + GM-notes textarea + End Session button and Cancel dismisses it without ending the session; `test_player_session_note_popout_renders` shows the player's per-session note popout (textarea + Save & Leave + Skip); `tests/harness/test_session_recap.py` (new, +5, v2.885.0 "The Session Ledger") — a GM sets a recap (nickname + GM notes) and reads it back; a player note is author-scoped and the GM sees every player note while the player receives neither the GM notes nor other players' notes; a player writing the GM recap gets 403; an over-long `session_key` gets 400; `POST /session/end` with `Accept: application/json` stamps the recap and returns the `session_key`; `tests/harness/test_login_demo_roster.py` (new, +2, v2.884.0 "The Cast List") `tests/harness/test_login_demo_roster.py` (new, +2, v2.884.0 "The Cast List") — the /login demo box lists all seven seeded demo accounts (each with a Fill button), the "Show campaigns" toggle (`#demo-show-campaigns`), and per-account campaign lists that start `hidden`; skips gracefully when the deployment isn't in demo mode; v2.883.0 "The Full Bleed" made the map editor full-bleed behind the floating toolbar (like the VTT), so a shared `conftest.me_clear_toolbar` helper pans the map out from under the ~280px toolbar before the map-editor tests interact with content near the top — `test_map_editor{,_door_style,_erase_menu,_gate,_gmpin,_interactions,_light_radius,_room,_save,_terrain,_undo,_zoom}` now call it after opening the editor (assertions unchanged); `tests/harness_ui/test_token_visibility_perspective.py` (new, +2, v2.882.0 "The Watchful Eye") — with no token selected the GM's `__tokenVisForTest.drawsOnTop` is true for every token and a player's is true only for tokens they control; selecting a token (`_targetingState.tokenIds` non-empty) suppresses the on-top redraw so the view narrows to that token's vision; `test_map_editor_layers` swaps the dropdown-collapse test for `test_layers_always_expanded` (9 always-visible rows + ⠿ handles) + `test_layer_reorder_changes_zorder` (dragging a handle changes the SVG `g[data-layer]` order + persists) for v2.881.0 "The Stacked Layers"; `test_map_editor_layers` gains `test_layers_dropdown_collapses` (v2.880.0 "The Layer Menu") — the layer checkboxes live in a collapsed-by-default `<details>` dropdown the summary reveals (its other layer tests open the dropdown first); `test_map_editor_lair_zone` gains `test_lair_zone_selected_shows_resize_handles` (v2.878.0 "The Reshaped Zone") — a selected zone shows corner resize handles + dragging one grows it; the Lair Zone tool also gained a colour picker/presets (v2.877.0) + a 6-line action description (v2.877.1); `test_map_lair_zones` also round-trips a per-zone `color` (valid #rrggbb lowercased, invalid dropped) for v2.877.0 "The Coloured Zone"; `test_map_editor_lair_zone` gains `test_lair_zone_selectable_and_right_clickable` (v2.876.0 "The Handled Zone") — a placed zone double-clicks to select (reloading its action into the dropdown) + right-clicks to a Move/Delete context menu in the no-tool state; `test_map_editor_lair_zone` now binds a zone via the "Lair Actions" dropdown (asserts the action description shows + label = action name) and `test_lair_zones_tabletop`'s zone label is the action name (v2.875.0 "The Chosen Action"); `test_lair_zones_tabletop` now also asserts a zone label names its bound action (v2.873.0 "The Lit Strike" — labels list bound actions + the struck zone flashes on trigger); `tests/harness_ui/test_lair_zone_targeting.py` (new, +1, v2.872.0 "The Struck Zone") — `_lairZoneTargetsForAction` resolves a token inside a zone bound to the fired action to its combatant id, and returns null for an unbound action (manual-picker fallback); `tests/harness_ui/test_lair_zones_tabletop.py` (new, +3, v2.871.0 "The Revealed Zone") — the tabletop 🎯 Zones button toggles the crimson `.tt-lairzone` overlay on/off for the GM, a player can toggle it too (targeting aid, not GM-only), and the button hides when the map has no zones; `tests/harness_ui/test_map_editor_lair_zone.py` (new, +2, v2.870.0 "The Painted Zone") — the 🎯 Lair Zone editor tool places a labelled rect with parsed action ids (renders `.me-lairzone`) + a 5-vertex free-polygon zone; `test_map_editor_toolbar` group-order gains "LAIR"; `tests/harness/test_map_lair_zones.py` (new, +4, v2.869.0 "The Marked Ground") — the lair-zone `GET`/`PUT /map/{id}/lair_zones` endpoints round-trip a labelled zone with an `actions` list + broadcast `lair_zones_update`, recompute a polygon's bbox (invalid points fall back to the rect), enforce GM-only writes (member reads, player 403), and 404 an unknown map; `tests/harness_ui/test_lair_action_ui.py` gains `test_lair_panel_is_collapsible` (+1, v2.868.0 "The Folded Lair") — the Battle-drawer lair-action panel is a native `<details>`, open by default, that the summary toggles closed (title in the summary, toggle/trigger buttons in the body); `tests/harness_ui/test_map_editor_hotkeys.py` gains `test_label_tool_hotkey` (+1, v2.866.0 "The Last Shortcut") — the map editor's Label tool now binds to the `a` key (the last placement tool that lacked a hotkey) and advertises it in the tooltip; `tests/harness/test_monster_spellcasting.py` (new, +2, v2.865.0 "The Unsealed Grimoire") — a caster monster's projected mini-sheet renders `.mini-cast-btn` cast buttons tagged `data-char-id="monster-template-N"` (the Archmage's parsed Globe of Invulnerability), un-gating the Spells tab now that `_parse_monster_spellcasting` turns the SRD Spellcasting prose into a structured spell list; plus a `/templates` contract-shape guard; `tests/harness_ui/test_lair_action_ui.py` gains three mini-sheet lair-strip tests (+3, v2.864.0 "The Stat-Block Roar") — the lair owner's init-tracker mini-sheet renders a `.lair-strip` with a `.lair-act-btn` Trigger per lair action while in-lair, the strip is absent out of lair, and buttons disable after the lair acts this round; `tests/harness_ui/test_lair_action_ui.py`'s two player regional-effects tests now open the Battle drawer + scope `#_regional_effects_panel` to `#players-drawer` (v2.863.0 "The Felt Presence" — the player atmosphere card relocated into the Battle drawer, matching the GM panel); and it gains `test_lair_panel_lives_in_battle_drawer` (+1, v2.862.0 "The Lair's Turn") — the 🌋 lair-action panel is now a descendant of the Battle drawer (`#players-drawer`), sits after `#gm-reactions-panel` in DOM order, and is no longer a `<body>`-appended floating card; `tests/harness_ui/test_token_layering.py` (new, +2, v2.861.0 "The Raised Piece") asserts tokens draw on `#token-veil-canvas` above every decoration layer (`wall-overlay`/`light-glow-canvas`/`weather-canvas` z-indexes), that the veil is `pointer-events:none` (drag/doors fall through) and carries drawn token pixels, and that a size-1 token's on-screen diameter stays at/above the `TOKEN_MIN_SCREEN_PX` floor when the camera is zoomed out; `tests/harness_ui/test_tabletop_fit.py` (new, +1, v2.860.0 "The Whole Board") asserts the tabletop fits the whole map into the viewport on load (map box ≤ pane, full vertical extent on-screen) — guarding the "tokens off-screen on a large natural-resolution map" regression; `test_demo_map_elements.py`'s Caldera test rewritten (v2.859.0 "The True Scale") for the live redesign — lava polygons, dark ambient, vertices within the natural-resolution bounds; `test_demo_campaigns.py`'s in-bounds token check now reads `width_px/height_px` from `/active-map` (dims = the image's natural size) instead of hardcoding; `tests/harness/test_terrain_visibility.py` (new, +3, v2.858.0 "The Hidden Ground") — the GM terrain-reveal toggle round-trips + surfaces on `/active-map` + broadcasts, is GM-only (player → 403), 404s an unknown map; `tests/harness_ui/test_terrain_visibility.py` (new, +1) — a player sees no terrain while hidden, the GM always does, and a GM reveal shows it to the player live; `tests/harness_ui/test_map_editor_light_radius.py` (new, +2, v2.857.0 "The Drawn Radius") — a placed light auto-selects and shows a radius handle, and dragging the dim handle outward grows `dim_ft` + flips the type to custom; `test_map_terrain.py` gains `test_terrain_polygon_points_round_trip` (+1, v2.855.0 "The Free Hand") — 3- and 6-vertex polygons round-trip with bbox recomputed, a 2-point set falls back to the rect; `tests/harness_ui/test_map_editor_terrain_polygon.py` (new, +2) places a 5-vertex region via the free-polygon toggle (close on the first dot) and confirms it renders as a `<polygon>` on the tabletop; `tests/harness_ui/test_map_editor_select_move.py` (new, +2, v2.854.0 "The Ready Hand") — the Select & move toggle is lit by default, double-clicking a light + dragging moves it and double-clicking a wall shows resize handles, and toggling off disables the drag; `tests/harness_ui/test_token_light_glow.py` (new, +1, v2.852.0 "The Carried Flame") — a lit token joins the glow set and draws a flickering glow at its centre, and turning the light off live drops it; `tests/harness_ui/test_map_editor_select_edit.py` (new, +2, v2.851.0 "The Chosen Stone") — double-clicking a wall/terrain region mirrors it into the material/type select and a change persists, Esc deselects; `tests/harness_ui/test_map_editor_light_select.py` (new, +2, v2.850.0 "The Chosen Flame") — double-click-selecting a light mirrors its values into the Markers toolbar, edits persist + flip type to custom, Esc deselects, and the right-click 🎨 pop-out edits `color2`; `test_map_lights.py` gains `test_light_color2_round_trips` (+1, v2.849.0 "The Twin Flame") and `tests/harness_ui/test_map_light_flicker.py` (new, +2) asserts the tabletop's `#light-glow-canvas` draws a coloured glow that changes across ~1s (torch flicker) and that a two-colour editor marker's fill animates; `test_map_terrain.py` gains `test_terrain_quad_points_round_trip` (+1, v2.848.0 "The Four Corners") — a 4-point free-form quad persists with x/y/w/h recomputed as its bbox, malformed `points` fall back to the rect or drop; `tests/harness_ui/test_map_editor_terrain_quad.py` (new, +1) four-dot-places a quad on a gridless map and asserts a corner handle drags independently; `test_demo_campaigns.py::test_demo_tokens_are_grid_aligned` reshaped (v2.847.0 "The Unruled Table") — the flagship keeps its 70 px grid-alignment guard while the five gridless leveled campaigns instead assert `grid_type: "none"` + `grid_size_px: 70` + every token in-bounds; `tests/harness_ui/test_map_fog_gm_perspective.py` (new, +2, v2.846.0 "The Watcher's Eye") asserts the GM sees no fog overlay with no target, and that targeting a token renders that entity's wall-occluded view (front clear / explored dim / unseen opaque); `tests/harness_ui/test_map_editor_fog_dynamic.py` (new, +1, v2.845.0 "The Lifting Veil") drives the editor's Fog group — entering Fog mode reveals the "explore (dynamic)" toggle, ticking it persists `fog_dynamic` + turns fog on, and the "Reset explored" button clears `fog_explored`; `tests/harness_ui/test_map_fog_dynamic.py` (new, +1, v2.844.0 "The Remembered Room") drives the `window.__testDrawFog` hook and samples the fog canvas to assert the three exploration-fog states — currently-visible = clear, wall-occluded-but-explored = dim memory, wall-occluded-never-seen = opaque; `tests/harness/test_fog_explore.py` (new, +5, v2.843.0 "The Explored Dark") covers the exploration-fog engine — monotonic explored-cell union, repeat no-op, member-open /explore vs GM-only /reset, and the `fog_update` broadcast carrying `fog_dynamic`+`fog_explored`; `test_map_fog.py` gains a `dynamic`-flag round-trip (+1) and `test_wiki.py` a `plan-exploration-fog` serve test (+1); the Props placement tests in test_map_editor_prop.py are skipped — feature parked). `tests/harness/test_demo_map_elements.py` (new, +3, v2.840.0 "The Furnished Board") discovers the leveled demo campaigns by name (`/api/user/gm-campaigns`) and asserts the Goblin Warrens map ships well-formed seeded walls (incl. a door + a secret door) + a hotspot, the Caldera Throne ships lava terrain + fire-glow lights + labels, and that a non-GM member sees the player-visible layers while GM pins stay out of the `/active-map` bootstrap — guarding the demo-seed literals (routed through the PUT sanitizers at seed time) against shape drift. `tests/harness_ui/test_map_editor_touch.py` (new, +1, v2.837.0 "The Two-Finger Table") dispatches synthetic two-finger TouchEvents and asserts spreading the fingers zooms in while pinching them together zooms out.. `tests/harness_ui/test_map_editor_terrain_resize.py` (new, +1, v2.832.0 "The Shifting Shore") right-clicks a terrain region, enters Resize (four corner handles), drags the SE handle, and asserts the region's width grew.. `tests/harness_ui/test_map_editor_weather.py` gains `test_editor_weather_previews` (+1, v2.831.0 "The Weather Eye") — the editor's #me-weather-canvas is blank with no weather, draws particles on rain, and clears when set back to none.. `tests/harness_ui/test_map_editor_zoom.py` gains `test_scroll_wheel_zooms` (+1, v2.830.0 "The Rolling Lens") — wheel-up over the stage raises the zoom %, wheel-down lowers it.. `tests/harness_ui/test_map_editor_fullbleed.py` (new, +1, v2.829.0 "The Open Table") asserts the toolbar is absolutely positioned overlaying the top of a full-screen stage and the footer is suppressed.. `tests/harness_ui/test_map_editor_toast.py` (new, +1, v2.825.0 "The Little Herald") asserts no toast on load and that clicking Save pops a visible toast reading "saved". `tests/harness_ui/test_map_editor_pan.py` (+2) drags the map and asserts the overlay's computed transform translate grows in both axes — for a plain left-drag with a `grab` cursor (v2.828.0 "The Free Camera", now a CSS-transform camera that pans even when the map fits), and via the middle button while the Wall tool is active (v2.824.0 "The Middle Grip"). `tests/harness_ui/test_map_editor_toolbar.py` now expects the 10-group order `FILE, TOOLS, WALLS, MARKERS, ENVIRONMENT, PROPS, TOKENS, GRID, LAYERS, VIEW` after v2.826.0 "The Filing Cabinet" renamed History→File and folded the standalone Tags group into it; it also asserts (v2.822.0 "The Frosted Bar") the edit bar's computed backdrop-filter carries a blur (the translucent ribbon). `tests/harness_ui/test_map_editor_hotkeys.py` (new, +2, v2.821.0 "The Quick Keys") verifies the single-key tool hotkeys — W activates Wall, D switches to Door then toggles off, the shortcut shows in the tooltip, and a keypress inside a numeric field does not switch tools. `tests/harness_ui/test_map_editor_collapse.py` gains `test_zone_divider_collapses_whole_zone` (+1, v2.820.0 "The Concertina") — clicking the Draw zone-divider label folds all five Draw groups (Map zone untouched), persists across reload, and re-expands on a second click. `tests/harness_ui/test_map_editor_layers.py` gains `test_layer_solo_shift_click` (+1, v2.819.0 "The Solo Spotlight") — shift-clicking a layer checkbox shows only that layer and a second solo-click restores all nine; and `test_layers_all_none_toggle` (+1, v2.818.0 "The Bare Map") — the Layers group's 👁 All / None button hides every overlay checkbox in one click and restores them on a second click. `tests/harness_ui/test_map_editor_toolbar.py` also asserts (v2.817.0 "The Painted Lines") the two zone dividers between the Actions · Draw · Map zones render their labels `DRAW`, `MAP`. `tests/harness_ui/test_map_editor_captions.py` (new, +1, v2.815.0 "The Captioned Rack") asserts every ribbon dropdown carries a non-empty caption above it (material / ambient / terrain / weather / prop / grid type). `tests/harness_ui/test_map_editor_topbar.py` (new, +1, v2.813.0 "The Raised Curtain") asserts the slim editor top bar — map title + rename in the nav, no version stamp, Back-to-settings before Wiki, Save in the History group, and the old header row removed. `tests/harness_ui/test_map_editor_toolbar.py` now expects the 11-group order after v2.812.0 "The Weathered Corner" folded the Weather dropdown into the Environment group. `tests/harness_ui/test_map_editor_collapse.py` (new, +1, v2.811.0 "The Folding Ruler") collapses the Grid toolbar group (controls hidden), confirms it persists across reload, and re-expands on a second label click. `tests/harness_ui/test_map_editor_toolbar.py` now expects the 12-group order after the v2.810.0 "The Tidier Bench" reorg (History far-left; Labels + GM-pin folded into Markers; Fog & Light + Terrain merged into Environment). `tests/harness_ui/test_tabletop_weather_control.py` (new, +1, v2.809.0 "The Storm Caller") clicks the GM live-weather Rain button and asserts the active map's weather is set + the button shows the active state. `tests/harness_ui/test_map_editor_weather.py` (new, +2, v2.808.0 "The Weather Turns") verifies the editor Weather selector persists the choice and that a `rain` `weather_update` draws particles on `#weather-canvas` (and clearing blanks it). `tests/harness/test_map_weather.py` (new, +4, v2.807.0 "The Gathering Clouds") covers the ambient-weather substrate (`POST /settings/maps/{id}/weather`) — set/round-trip (+ `weather_update` WS + active-map surfacing), `none`→`""`, bad-value 400, non-GM 403. `tests/harness_ui/test_map_editor_select.py` (new, +1, v2.806.0 "The Gathered Handful") marquee-selects two props (asserts the cyan highlight rings), group-moves them down (coords shift), and Delete removes both. `tests/harness_ui/test_map_editor_measure.py` (new, +1, v2.805.0 "The Measured Stride") arms the 📏 Measure tool, drags a ruler line, and asserts an `N ft` label (`text.me-measure`) shows and clears when another tool is armed. `tests/harness_ui/test_tabletop_label.py` (new, +1, v2.804.0 "The Sign Hung") pushes a `labels_update` over the WS path and asserts the label text renders as a `text.tt-label` in the tabletop overlay. `tests/harness_ui/test_map_editor_label.py` (new, +1, v2.803.0 "The Named Room") places a public text label in the map editor — asserts the text persists via the labels endpoint, renders `text.me-label`, and the Labels layer toggle hides it. `tests/harness/test_map_labels.py` (new, +4, v2.802.0 "The Labelled Map") covers the public text-label substrate (`GET`/`PUT /api/campaign/{cid}/map/{mid}/labels`) — set/get round-trip (clamps size 8..200, drops empty-text/no-coords, `labels_update` WS + active-map surfacing), defaults (size 24, invalid colour → white), player-PUT 403, unknown-map 404. `tests/harness_ui/test_map_editor_prop.py` (+1, v2.801.0 "The Snug Fit") asserts a prop placed with 🧲 Snap on lands on a grid-cell centre. `tests/harness/test_map_props.py` (+1, v2.800.0 "The Fading Prop") asserts a prop's `op` (opacity) round-trips, clamps to 0.1..1, and defaults to 1. `tests/harness/test_map_props.py` (+1, v2.799.0 "The Mirror Image") asserts a prop's `flip` (horizontal-mirror) flag round-trips and defaults to False. `tests/harness/test_map_props.py`'s serve-check grew to all 42 shipped prop SVGs (v2.798.0 "The Full Inventory" added portcullis/lever/trapdoor/lectern/cage/pit/obelisk/banner/bench/wardrobe). `tests/harness/test_map_props.py`'s serve-check grew to all 32 shipped prop SVGs (v2.797.0 "The Stocked Storeroom" added door/tent/signpost/bridge/ladder/stairs/sarcophagus/crystal/mushroom/pool). `tests/harness/test_map_props.py`'s serve-check grew to all 22 shipped prop SVGs (v2.796.0 "The Deeper Cupboard" added statue/altar/bones/weaponrack/anvil/cauldron/gravestone/throne/brazier/pillar/cart). `tests/harness_ui/test_map_editor_prop.py` + `tests/harness_ui/test_tabletop_prop.py` (+2, v2.795.0 "The Painted Scenery") each add an image-prop case — placing an `img:barrel` prop renders an `<image>` in the editor overlay, and an `img:chest` `props_update` renders an `image.tt-prop` on the tabletop. `tests/harness/test_map_props.py` (+2, v2.794.0 "The Prop Cupboard") also asserts an `img:bookshelf` prop kind round-trips un-truncated (cap widened 8→40) and that all 11 shipped `/static/props/*.svg` prop illustrations serve. `tests/harness_ui/test_tabletop_prop.py` (new, +1, v2.793.0 "The Dressed Stage") pushes a `props_update` over the WS path on the live tabletop and asserts the emoji glyph renders as a `text.tt-prop` in the overlay. `tests/harness_ui/test_map_editor_prop.py` (new, +1, v2.792.0 "The Stamp Set") places a decorative prop stamp in the map editor — picks a 🌲 glyph, arms the Prop tool, clicks the map, asserts the prop persists (kind/size/rot) via the props endpoint, renders an SVG `<text>`, and that the Props layer toggle hides it. `tests/harness/test_map_props.py` (new, +2, v2.791.0 "The Furnished Room") covers the decorative prop-stamp substrate (`GET`/`PUT /api/campaign/{cid}/map/{mid}/props`) — PUT/GET round-trip (clamps `size` to 8..400, wraps `rot` mod 360, defaults the glyph, drops non-numeric coords), `props_update` WS broadcast, `active-map` surfacing, plus the player-PUT 403 error path. `tests/harness/test_map_letterbox_color.py` (new, +3, v2.733.0 "The Painted Margin") covers the per-map average-colour surround toggle (`POST /settings/maps/{id}/letterbox_color`) — enable → 200 + `#rrggbb` + `map_letterbox_color` WS broadcast, disable → null + broadcast, plus 404 (unknown map) and 403 (non-GM) error paths. `tests/harness/test_suggestions.py` (new, +6, v2.716.0 "The Suggestion Box") covers the suggestion/issue reporting endpoints — create (+shape), missing-title 400, unknown-kind default, admin-gate 403 (list+update), and the admin triage flow (list/PATCH/DELETE/404) when the client is a site admin. `tests/harness_ui/test_tabletop_canvas.py::test_zoom_in_supersamples_canvas` (+1, v2.714.0 "The Sharper Lens") asserts zooming in grows the canvas backing store (supersample-on-zoom) so tokens stay crisp. `tests/harness/test_cast_daylight.py` (new, +7, v2.713.0 "The Bottled Sun") covers the new `/cast_daylight` endpoint auto-placing its non-concentration bright-light `daylight` emitter. `tests/harness/test_cast_darkness.py` (new, +7, v2.712.0 "The Smothered Torch") covers the new `/cast_darkness` endpoint auto-placing its magical-`darkness` emitter. `tests/harness/test_cast_fog_cloud.py` (+2, v2.711.0 "The Rolling Fog") covers casting Fog Cloud with a `center` auto-placing its `fog` vision-and-light emitter. Vision & Light Phase 5 (v2.710.0 "The Lantern's Reach") is presentation-only (the `drawLighting()` canvas overlay) — no new `tests/harness/` test; covered by `tests/harness_ui/test_tabletop_canvas.py` (no-console-errors load). `tests/harness/test_vision_light_phase4.py` (new, +8, v2.709.0 "The Held Breath") covers Vision & Light Phase 4 — Hide/Stealth (`POST /hide`): hidden target (Stealth 99) → unseen, poor hide → seen, truesight defeats hide, hidden attacker → `advantage_unseen_attacker` then revealed; + 404/400/403 error paths. `tests/harness/test_vision_light_phase3.py` (new, +9, v2.708.0 "The Conjured Night") covers Vision & Light Phase 3 — spell light/darkness/fog emitters (`POST`/`DELETE /light_emitter`): Darkness → `magical_dark`/unseen (darkvision can't pierce) but Devil's Sight does; Daylight on a dark map → bright/seen; Fog → unseen but truesight pierces; + bad-kind/bad-radius 400, unknown 404, non-GM 403. `tests/harness/test_vision_light_npc_attack.py` (new, +3, v2.707.0 "The Stalker's Edge") covers Vision & Light Phase 2 NPC mirror — `_visibility_between` wired into `/npc_attack` via `_npc_attack_vision_edges`: NPC-can't-see-target → `disadvantage_cant_see`, unseen-NPC → `advantage_unseen_attacker`, bright map → no edge. `tests/harness/test_vision_light_phase2.py` (new, +6, v2.706.0 "The Honest Dark") covers Vision & Light Phase 2 — `_visibility_between` wired into PC `/attack`: dark+target-unseen → `disadvantage_cant_see`, dark+unseen-attacker → `advantage_unseen_attacker`, mutual-blind → cancel, both-darkvision/bright-map → no edge, manual override still forces disadvantage. `tests/harness/test_vision_light_phase1.py` (new, +7, v2.705.0 "The Seeing Eye") covers Vision & Light Phase 1 — the `_visibility_between` resolver exposed read-only at `GET /visibility`: the ambient × senses × range matrix (bright→seen, dark+no-senses→unseen, dark+darkvision→obscured, beyond-range→unseen, truesight/Devil's-Sight→seen, target-lit→seen). `tests/harness/test_vision_light_phase0.py` (new, +4, v2.704.0 "The First Lantern") covers Vision & Light Phase 0 (docs/plans/vision-and-light.md) — the lighting data model: `Map.ambient_light` + token `light_bright_ft`/`light_dim_ft` (schema v82), GM set-endpoints, and `GET /tokens` surfacing. Defaults preserve status quo (token 0/0, ambient bright); PATCH clamps token light radii; `/settings/maps/{id}/ambient_light` persists + broadcasts + invalid→bright; non-GM → 403. `tests/harness/test_wiki.py::test_wiki_doc_serves_vision_plan` (+1, v2.703.0 "The Drawn Map") smoke-tests the new design-plan doc. `tests/harness/test_corona_of_light.py::test_col_imposes_disadvantage_on_fire_radiant_save` (+1, v2.702.0 "The Searing Sun") covers full-feature-automation Phase 8 — Light Cleric's Corona of Light installs a buff and a new save-disadvantage read (`_caster_corona_disadvantages_save`) swaps the enemy NPC saver's d20 → 2d20kl1 at the `/cast_spell` save sites for the cleric's fire/radiant spells: Tavik casts Sacred Flame at an NPC → control without corona rolls `1d20`, with corona active rolls `2d20kl1`. `tests/harness/test_master_of_tactics.py::test_mt_grants_ally_advantage_vs_target` (+1, v2.701.0 "The Tactician's Nod") covers full-feature-automation Phase 8 — Mastermind Rogue's Master of Tactics installs a target-keyed Help advantage buff on the aided ally (riding the True Strike / Vow of Enmity read): Pip helps Garrik vs a bandit → Garrik's `/attack` vs that bandit rolls `2d20kh1`. `tests/harness/test_emissary_of_redemption.py::test_er_reflects_radiant_on_attack` (+1, v2.700.0 "The Avatar of Peace") covers full-feature-automation Phase 8 — Emissary of Redemption (Redemption Paladin L20) fully mechanized: the endpoint installs a `resistance_to: ["all"]` buff and a new `_apply_damage_to_combatant` on-damage hook reflects half-damage radiant to attackers; a PC attacker hitting the Lv 20 Paladin (auto-apply on) triggers an `emissary-of-redemption` broadcast with `radiant_damage ≥ 1`. `tests/harness/test_misty_escape.py` (new, +4, v2.699.0 "The Vanishing Mist") covers full-feature-automation Phase 8 — the new `/use_misty_escape` endpoint (Archfey Warlock Lv 6+) composes the invisible attack-edge substrate + the v2.698.0 disengage substrate (once/short-rest teleport + invisible): `test_use_me_happy` (teleport_ft 60, both buffs installed, uses 1→0) + error paths (out-of-uses 409, wrong-subclass 409, level-gate 409). `tests/harness/test_disengage_oa.py` (new, +3, v2.698.0 "The Nimble Exit") covers full-feature-automation Phase 8 — `/token/move` now honors a mover's `effects.disengage: True` buff (Step of the Wind / Drunken Technique), suppressing OAs for the turn without consuming the flag: `test_disengage_suppresses_oa` (200 + `disengage_applied`, no triggers) + `test_disengage_not_consumed_second_move` (persists across moves) + `test_move_provokes_oa_without_disengage` (control → 409). `tests/harness/test_tempestuous_magic.py::test_tm_buff_rides_generalized_free_move_substrate` (+1, v2.697.0 "The Riding Storm") covers full-feature-automation Phase 8 — Storm Sorcery's Tempestuous Magic mechanized onto the generalized free-move substrate (v2.696.0): Zara at her 30-ft cap + the `tempestuous-magic-fly` buff → an over-cap +5-ft move returns 200 (`relentless_avenger_applied == True`); control without it 409s `over_speed_cap`. `tests/harness/test_skirmisher.py::test_sk_buff_rides_generalized_free_move_substrate` (+1, v2.696.0 "The Open Field") covers full-feature-automation Phase 8 — the Relentless Avenger `/token/move` free-move read site is generalized to be effect-keyed (any `oa_immune_during_move` buff), and Scout Rogue's Skirmisher mechanizes onto it: Pip at his 25-ft cap + the `skirmisher-bonus-move` buff → an over-cap +5-ft move returns 200 (`relentless_avenger_applied == True`); control without the buff 409s `over_speed_cap`. `tests/harness/test_tales_from_beyond.py` (+4, v2.695.0 "The Spirit's Tale") covers full-feature-automation Phase 8 — Spirits College Bard's Tales from Beyond now resolves the 4 mechanizable d6 tales server-side when a `target_combatant_id` is supplied (3/6 temp HP via `_grant_temp_hp`, 4 Brute STR-save → Prone + 2d10 force, 5 Tragic Romance WIS-save → Charmed via `_resolve_feature_save`): `test_tb_tale3_grants_temp_hp` + `test_tb_tale4_brute_save_and_damage` + `test_tb_tale5_charm_save` (force_tale, skip when TEST_MODE off) + `test_tb_no_target_announce_only`. `tests/harness/test_aura_of_the_guardian.py` (+2, v2.694.0 "The Shared Wound") covers full-feature-automation Phase 8 — Redemption Paladin's Aura of the Guardian now resolves the damage redirection server-side (heal the ally back + transfer the untyped/unreducible damage to the Paladin) when `ally_combatant_id` + `damage_amount` are supplied: `test_aotg_redirects_damage` (wounded ally → `ally_healed == 8`, `paladin_damage_applied == 8`) + `test_aotg_no_redirect_announce_only` (no args → `redirected is False`). `tests/harness/test_undying_sentinel.py::test_us_drops_to_one_hp_when_at_zero` (+1, v2.693.0 "The Refused Fall") covers full-feature-automation Phase 8 — Ancients Paladin's Undying Sentinel now applies the "drop to 1 HP instead of 0" mutation server-side via `_apply_heal_to_combatant` (Protective Spirit self-apply shape; flips dying → alive): sets Caelan to 0 HP, asserts `hp_after == 1` + `revived is True`. `tests/harness/test_disarming_attack.py` (+2, v2.692.0 "The Wrested Blade") covers full-feature-automation Phase 8 — Battle Master's Disarming Attack now resolves the maneuver on-target (trust-the-caller): the superiority die lands as bonus damage via `_apply_damage_to_combatant` + the STR save resolves via `_resolve_feature_save` (`condition_buff=None` — drop-object GM-narrated). `test_da_applies_damage_and_resolves_save` (templated bandit → `damage_applied == extra_damage`, `condition_installed is False`) + `test_da_no_target_announce_only` (no target → fields None). `tests/harness/test_pushing_attack.py::test_pa_applies_bonus_damage` (+1, v2.691.0 "The Driving Blow") covers full-feature-automation Phase 8 — Battle Master's Pushing Attack now applies the superiority die as bonus damage via `_apply_damage_to_combatant` (the STR save → 15-ft push was already wired in v2.99.433): templated bandit → `damage_applied == extra_damage`. `tests/harness/test_trip_attack.py` (+2, v2.690.0 "The Sweeping Leg") covers full-feature-automation Phase 8 — Battle Master Fighter's Trip Attack now resolves the maneuver on-target (trust-the-caller) when a `target_combatant_id` is supplied: the superiority die lands as bonus damage via `_apply_damage_to_combatant` + the STR save → Prone resolves via `_resolve_feature_save`. `test_ta_applies_damage_and_resolves_prone` (templated bandit → `damage_applied == extra_damage`, `condition_installed == (not passed)` with `condition_key == "prone"`) + `test_ta_no_target_announce_only` (no target → fields None). `tests/harness/test_berserker_path.py::test_ip_resolves_save_and_installs_frightened` (+1, v2.683.0 "The Menacing Glare") covers full-feature-automation Phase 8 — Berserker Barbarian's Intimidating Presence now resolves the target's WIS save via `_resolve_feature_save` (NPC inline, PC via RollRequest) + installs Frightened on a fail when a `target_combatant_id` is supplied: Krieger Lv 10 + a real bandit template, asserts `feature_save.resolved` + `condition_installed == (not passed)` with `condition_key == "frightened"`. `tests/harness/test_turn_the_faithless.py::test_ttf_resolves_saves_and_installs_turned` (+1, v2.682.0 "The Faithless Routed") covers full-feature-automation Phase 8 — Ancients Paladin's Turn the Faithless now resolves each target's WIS save via `_resolve_feature_save` (AoE loop; NPC inline, PC via RollRequest) + installs Turned on a fail: re-seeds two real bandit templates, asserts each `feature_saves` entry `resolved` + `condition_installed == (not passed)` with `condition_key == "turned"`. `tests/harness/test_abjure_enemy.py::test_ae_resolves_save_and_installs_frightened` (+1, v2.681.0 "The Holy Symbol") covers full-feature-automation Phase 8 — Vengeance Paladin's Abjure Enemy now resolves the target's WIS save via `_resolve_feature_save` (NPC inline, PC via RollRequest) + installs Frightened on a fail: re-seeds a real bandit template, asserts `feature_save.resolved` + `condition_installed == (not passed)` with `condition_key == "frightened"`. `tests/harness/test_natures_wrath.py::test_nw_resolves_save_and_installs_restrained` (+1, v2.680.0 "The Ensnaring Vines") covers full-feature-automation Phase 8 — Ancients Paladin's Nature's Wrath now resolves the target's STR/DEX save via `_resolve_feature_save` (NPC inline, PC via RollRequest) + installs Restrained (`_make_restrained_buff`, repeated save) on a fail: re-seeds a real bandit template, asserts `feature_save.resolved` + `condition_installed == (not passed)` with `condition_key == "restrained"`. `tests/harness/test_balm_of_the_summer_court.py` (+2, v2.679.0 "The Summer Balm") covers full-feature-automation Phase 8 — Dreams Druid's Balm of the Summer Court now applies both the rolled heal (`_apply_heal_to_combatant`) and the per-die temp HP (`_grant_temp_hp`) to the same ally when a `target_combatant_id` resolves: `test_bsc_applies_heal_and_temp_hp` (wounded NPC, 2 dice → `heal_applied == heal_amount` + `temp_hp_applied == temp_hp`) + `test_bsc_no_target_announce_only` (no target → fields None). `tests/harness/test_tentacle_of_the_deeps.py` (+2, v2.678.0 "The Grasping Deep") covers full-feature-automation Phase 8 — Fathomless Warlock's Tentacle of the Deeps now rolls the melee spell attack server-side vs the target's AC (`_read_target_ac`; nat 20 crits + doubles dice, nat 1 misses) + applies the cold on a hit via `_apply_damage_to_combatant` when a `target_combatant_id` resolves: `test_td_resolves_attack_and_applies_cold` (templated bandit → attack/damage fields consistent with the rolled outcome) + `test_td_no_target_announce_only` (no target → fields None). `tests/harness/test_storm_aura.py` (+3, v2.677.0 "The Roaming Tempest") covers full-feature-automation Phase 8 — Storm Herald Barbarian's Storm Aura now resolves the Sea (DEX save-for-half lightning) + Tundra (temp HP) single-target choices server-side with a `target_combatant_id`: `test_sa_sea_applies_lightning_to_target` (templated bandit → `damage_applied` full on fail / half on success), `test_sa_tundra_applies_temp_hp` (NPC → `temp_hp_applied == 2`), `test_sa_sea_no_target_announce_only` (no target → fields None). `tests/harness/test_halo_of_spores.py` (+2, v2.676.0 "The Necrotic Bloom") covers full-feature-automation Phase 8 — Spores Druid's Halo of Spores now rolls the target's CON save + applies the necrotic server-side (save-OR-NOTHING: full on fail, 0 on success) via `_apply_damage_to_combatant` when a `target_combatant_id` resolves: `test_hs_applies_damage_to_target` (templated bandit → `damage_applied` consistent with the outcome) + `test_hs_no_target_announce_only` (no target → fields stay None). `tests/harness/test_healing_light.py` (+2, v2.675.0 "The Celestial Mend") covers full-feature-automation Phase 8 — The Celestial Warlock's Healing Light now applies its rolled pooled-d6 HP to a `target_combatant_id` via `_apply_heal_to_combatant` (the Hands of Healing wire; caps at max HP, revives a dying PC): `test_hl_applies_heal_to_target` (wounded NPC at 10/50 → `heal_applied == heal_amount`, `revived == False`) + `test_hl_no_target_announce_only` (no target → fields stay None). `tests/harness/test_hands_of_healing.py` (+2, v2.674.0 "The Mending Touch") covers full-feature-automation Phase 8 — Way of Mercy Monk's Hands of Healing now applies the rolled HP to a `target_combatant_id` via `_apply_heal_to_combatant` (the heal-pipeline twin of `_apply_damage_to_combatant`; caps at max HP, revives a dying PC): `test_hh_applies_heal_to_target` (wounded NPC at 10/50 → `heal_applied == heal_amount`, `revived == False`) + `test_hh_no_target_announce_only` (no target → fields stay None). `tests/harness/test_wrath_of_the_storm.py` (+2, v2.673.0 "The Answering Thunder") covers full-feature-automation Phase 8 — Tempest Domain Cleric's Wrath of the Storm now rolls the attacker's DEX save + applies the 2d8 server-side (full on fail, half on success) via `_apply_damage_to_combatant` when an `attacker_combatant_id` resolves to a sheet: `test_wots_applies_damage_to_attacker` (templated bandit attacker → `save_total`/`save_passed`/`damage_applied` consistent with the outcome) + `test_wots_no_combatant_announce_only` (free-form `attacker_name` → fields stay None). `tests/harness/test_rebuke_the_violent.py::test_rtv_applies_psychic_damage` (+1, v2.672.0 "The Turned Blow") covers full-feature-automation Phase 8 — Redemption Paladin's Rebuke the Violent now rolls the attacker's WIS save + applies the reflected psychic server-side (full on fail, half on success) via `_apply_damage_to_combatant` when the attacker resolves to a sheet (NPC via template, PC via sheet): re-seeds a real bandit template attacker, asserts `save_total`/`save_passed`/`psychic_damage_applied` consistent with the rolled outcome. `tests/harness/test_mantle_of_inspiration.py` (+3, v2.671.0 "The Fey-Woven Shield") covers full-feature-automation Phase 8 — Glamour College Bard's Mantle of Inspiration now applies its `5 + bard_level` temp HP server-side via `_grant_temp_hp` to each `target_combatant_ids` ally (capped at CHA-mod), with the free reaction-move staying GM-narrated: `test_mi_applies_temp_hp_to_allies` (two fresh NPC allies each granted 11 temp HP) + `test_mi_caps_targets_at_cha_mod` (5 ids → only 3 buffed) + `test_mi_no_target_announce_only` (no ids → `targets_buffed == 0`). `tests/harness/test_mote_of_potential.py` (+2, v2.670.0 "The Mote Made Real") covers full-feature-automation Phase 8 — Mote of Potential's two applicable modes now apply server-side (was announce-only): `test_mp_attack_mode_applies_force_damage` (attack mode rolls 1d8 + lands force damage on an NPC bandit via `_apply_damage_to_combatant`) + `test_mp_save_mode_grants_temp_hp` (save mode grants `roll + CHA-mod` temp HP to a fresh NPC via `_grant_temp_hp` → granted == roll + 3). Check mode stays GM-narrated (die rolled + surfaced). `tests/harness/test_sculpt_spells.py` (new, +3, v2.669.0 "The Sheltered Blast") covers full-feature-automation Phase 8 — Evocation Wizard's Sculpt Spells now rides the Careful Spell auto-pass substrate (was announce-only): arming installs the `sculpt-spells-active` buff (`protected_combatant_ids` + `protection_label`); end-to-end a Sculpt-protected NPC auto-succeeds its Fireball save (the read substrate forces `1d20+99`) and the auto-pass card is labeled "Sculpt Spells" (the `protection_label` wire threaded through `_broadcast_careful_protected`); a non-wizard → 409. Regression: the existing `test_use_metamagic_careful.py` stays green (the shared helper/broadcast/cleanup changes are backward-compatible). `tests/harness/test_whispers_psychic_blades.py` (+2, v2.668.0 "The Whispered Wound") covers full-feature-automation Phase 8 — Whispers College Bard's Psychic Blades now rolls + applies the level-scaled NdN psychic damage server-side via `_apply_damage_to_combatant` when targeted (was announce-only): `test_wpb_applies_psychic_damage_to_target` (Lv 6 Lyra → 3d6 lands on an NPC bandit; rolled ∈ 3..18, applied lands accepting resistance halving) + `test_use_wpb_no_target_announce_only` (no target → `damage_rolled`/`damage_applied` stay None). `tests/harness/test_blade_flourish.py::test_bf_installs_speed_bonus_buff` (+1, v2.667.0 "The Quickened Blade") covers full-feature-automation Phase 8 — Blade Flourish's +10 ft walking-speed bonus (Swords College Bard) is now mechanized via the `speed_bonus_ft` substrate (was announce-only): a Mobile flourish installs a 1-round `blade-flourish-speed-active` buff with `effects.speed_bonus_ft: 10` (battle-seeded so `_install_buff` lands; flourish-agnostic; asserts the buff payload + 1-round/non-concentration shape; cleans up). (v2.667.1 "The Steady Roll" de-flaked the sibling `test_bf_damage_with_target_applies_bonus` — dropped the `bonus_applied > 0` requirement so a BI roll of 1 + resistance halving to 0 passes; the `applied in (rolled, rolled//2)` guard still catches genuine no-damage for `rolled > 1`.) `tests/harness/test_assassinate.py` (v2.665.0 "The Drop on Them" → **reverted v2.670.1**) — the v2.665.0 `assassinate-active` flag-buff + its `test_assassinate_buff_payload_carries_flags` test were reverted: Assassinate was already mechanized in `/attack` (v2.131.0–v2.132.0, auto-crit vs `target_surprised` + advantage vs not-yet-acted, gated on the subclass), so the flag-buff's keys were dead. The endpoint's 3 declaration/gate tests remain. `tests/harness/test_reaction_prompt.py::test_pc_defender_negate_then_answer_skips_on_hit_condition` (+1, v2.664.0 "The Missed Threat") covers pending-resolution Phase 3c-3 — the negate-then-answer ordering for a PC defender: Garrik (Battle Master, armed Menacing) hits Thalindra in the Shield band, she casts Shield (negate) FIRST, then answers the deferred on-hit WIS save and fails → Frightened does NOT install (the negate marks the attack id via `_revert_attack_buff_installs`, and `_resolve_save_failure` skips the install). Asserted only on a confirmed failed save (passing-save cycles retry). Completes the attack-flip arc (3c-1 undo + 3c-2 revert + 3c-3 skip). `tests/harness/test_arcane_charge_teleport.py` (new, +4, v2.662.0 "The Arcane Step") covers Eldritch Knight Arcane Charge Phase 4 — the actual teleport move (`POST /use_arcane_charge_teleport`): Garrik PATCHed to EK Lv 15 + arcane-charge buff installed, an in-range (≤30 ft) teleport returns 200 + broadcasts `token_move(teleport=True)` + the token's map position actually changes; a too-far destination → 409 `too_far` (with `max_ft`/`attempted_ft`, and `override` bypasses it); a PC without the buff (a Rogue) → 409 `no_arcane_charge`; missing dest coords → 400. Restore-safe fixture (snapshots subclass + level). `tests/harness/test_use_metamagic_empowered.py::test_empowered_reroll_aoe_multi_target_place_aoe` (+1, v2.661.0 "The Empowered Blast") covers Sorcery Phase 1.5 — the Empowered Spell reroll now integrates with the AoE multi-target `/place_aoe` placement path (previously only the single-target save-for-half + multi-beam attack paths honored it): Zara arms Empowered, casts Fireball with no target (→ pending AoE), then `/place_aoe` at two bandits → the reroll fires on the first damaged target (first-target-wins, once per cast) and the response + `spell_cast_aoe_resolved` broadcast carry the `empowered_spell` log (reroll count == CHA-mod, d6 dice); a second placement without re-arming carries no block (buff consumed once). `tests/harness/test_carry_weight.py` (+9, v2.660.0 "The Heavy Pack") covers carrying-capacity Phase 4 — the optional Encumbrance variant (PHB p.176): 8 pure-Python unit tests for `sheet_encumbrance()` (the `none`/`encumbered`/`heavily_encumbered` tiers at the STR×5 / STR×10 boundaries, strict-`>` edges so a load exactly at a threshold doesn't tip over, the `effective_str` override raising the threshold for Belt of Giant Strength, and `sheet_carry_summary` including the fields only when encumbered) + 1 `/sheet-json` integration test (a PC overloaded past STR×10 surfaces `encumbrance_tier: "heavily_encumbered"` + `speed_penalty_ft: 20` + `["STR","DEX","CON"]` disadvantage in the `derived.carry` block; restore-safe inventory snapshot). `tests/harness/test_reaction_prompt.py::test_npc_parry_flip_reverts_on_hit_condition` (+1, v2.659.0 "The Recalled Terror") covers pending-resolution Phase 3c-2 — an NPC Parry that flips a hit to a miss now reverts the on-hit condition too: Garrik (Battle Master) arms Menacing Attack, lands an in-band hit that frightens a Bandit Captain, the captain Parries → asserts the `monster-parry-negate` broadcast reports `conditions_reverted: ["frightened"]` (via the new `_revert_attack_buff_installs`, wired into all six flip-producer negate blocks) and the captain's live battle state is clean. Both this test's Garrik setup + the 3c-1 `garrik_battle_master` fixture are now restore-safe (snapshot/restore the original subclass + resources) so Garrik's demo Lucky `luck-point` survives for later tests. `tests/harness/test_weapon_hit_save_undo.py` (new, +2, v2.658.0 "The Undoable Fright") covers pending-resolution Phase 3c-1 — NPC weapon on-hit-*save* condition installs are now logged under the originating attack's id: Garrik (Battle Master) arms Menacing Attack, hits an NPC bandit until the WIS save fails (Frightened installs synchronously via `_fire_weapon_hit_saves` → `_resolve_feature_save`), then `/undo_attack_damage` reverts the Frightened install (asserts the `per_target` `buff_install` revert + the bandit's buffs are clean post-undo); plus a 404 error path for an unknown attack id. Before this commit the synchronous NPC install was un-logged, so the undo healed HP but left the condition stuck. `tests/harness/test_demo_features.py` (new, +5, v2.653.0 "The Featured Cast") covers `apply_srd_features` — the offline SRD backfill of demo PCs' subclass features + race traits at seed time: SRD combos fill, `variant-human` aliases to `human`, per-field partial coverage, non-SRD combos stay empty, a pre-set list is never overwritten, and (v2.653.1 "The Class Roll") class features parse from the SRD markdown blob into a level-filtered list (all 12 classes are SRD → every demo PC gets them; curated lists preserved). v2.654.0 "The Missing Kin" (+2) adds the 5 SRD 5.1 subraces missing from the shipped tier (Mountain Dwarf, Wood Elf, Forest Gnome, Stout Halfling, Drow) — they resolve from `local-srd` with their signature subrace traits, a Wood Elf demo PC now seeds race traits, and the provenance floor is bumped 9→14. v2.655.0 "The Celestial Loophole" (+2) adds Goliath + Aasimar from the 2024 SRD 5.2 (CC BY 4.0) — they resolve from `local-srd` with 2024 traits + an SRD-5.2 attribution, a Goliath demo PC seeds race traits, floor 14→16. `tests/harness/test_stats_api.py::test_stats_captures_lay_on_hands_healing` (+1, v2.652.1 "The Healer's Ledger") covers the stats heal-capture extension to Lay on Hands (a Paladin's `heal_done` now includes LoH healing). `tests/harness/test_stats_page.py` (new, +3, v2.652.0 "The Scoreboard") covers statistics Phase 3 — the `/campaign/{id}/stats` page: renders 200 with the roster switcher for the GM, 200 without it for a player, 404 for an unknown campaign (completing the feature end-to-end). `tests/harness/test_stats_api.py` (new, +3, v2.651.0 "The Tally Sheet") covers statistics Phase 2 — the read API `GET /api/campaign/{id}/stats`: a GM-scoped attack + cast bump the actor's `damage_dealt`/`attacks`/`spells_cast`/`top_spells` (capture Hooks B/C/D + the aggregation layer), a non-GM gets `scope="self"` and never another player's stats, and an unknown campaign → 404. `tests/harness/test_stats_capture.py` (new, +1, v2.650.0 "The First Tally") covers statistics-logging Phase 1: a PC's weapon hit on an NPC inserts a `damage_dealt` row into the new `campaign_stat_events` table (schema v81), verified by a direct DB query since the read API lands in Phase 2; also guards no attack-endpoint regression from the capture hook. `tests/harness/test_wiki.py` (+1, v2.649.1 "The Drafting Table") surfaces the new per-campaign statistics-logging design plan via `test_wiki_doc_serves_campaign_stats_plan` + the plans-page assertion. `tests/harness/test_use_metamagic_quickened.py` (new, +3, v2.649.0 "The Quickened Word") covers the 8th + final PHB metamagic: `/use_metamagic_quickened_spell` arms a `metamagic-quickened-pending` buff (2 SP), and the next `/cast_spell` of a 1-action spell retargets the economy slot to **bonus** + consumes the buff — completing the metamagic set. `tests/harness/test_war_magic.py` (+2, v2.648.8 "The Bonus Swing") covers the EK War Magic bonus-attack economy route: `/attack` with `as_war_magic_bonus: true` marks the **bonus** slot (not the action) for an EK Lv 7+, and a non-EK falls through to a normal action attack. `tests/harness/test_eldritch_strike_resolver.py` (+1, v2.648.7 "The Monster's Misfortune") covers Eldritch Strike against NPC targets: the marker now installs on NPC combatants and the NPC auto-save vs the EK's spell rolls at `2d20kl1` + consumes the marker (new `_remove_buff_from_combatant_id` helper) — the common "EK hits a monster then casts at it" case, previously PC-only. `tests/harness/test_war_magic.py` (+2, v2.648.6 "The War Mage's Cue") covers the EK War Magic `/cast_spell` advisory: an Eldritch Knight Lv 7+ casting a cantrip with their action now fires a `feature_used(source=war-magic-advisory)` surfacing the bonus-action weapon attack (`test_war_magic_advisory_on_ek_cantrip`); a Champion gets nothing (`test_war_magic_advisory_skips_non_ek`). `tests/harness/test_eldritch_strike.py` (+2, v2.648.5 "The Automatic Mark") covers Eldritch Strike auto-install: an EK Lv 10+ weapon hit now installs the `eldritch-strike-target` marker automatically in `/attack` post-resolution (shared `_install_eldritch_strike` helper) — `test_es_auto_installs_on_weapon_hit` asserts the marker lands on the target + the announce fires with no manual endpoint call; `test_es_does_not_auto_install_for_non_ek` asserts a Champion's hit installs nothing. `tests/harness/test_reaction_prompt.py` (+1, v2.648.0 "The Cancelled Luck") covers Lucky-vs-Lucky cancel (PHB p.167): resolving a defender's `use-lucky` with `params.cancel=true` when the attacker also has Lucky spends both luck points (shared `_spend_lucky_charge`), voids the reroll, and fires `feature_used(source=lucky-cancel)` + `reaction_prompt_resolved`. `tests/harness/test_gm_reactions_panel.py` (+1, v2.647.0 "The Spent Ring") covers reaction-item charge tracking: spending Lyra's Ring of Spell Turning (`cost_charges: 1`) via `/spend_reaction_manual` decrements the item's `charges`, surfaces `charges_remaining` on the response + `feature_used` broadcast, and refuses with `409 out_of_charges` once depleted (`_decrement_item_reaction_charge`, wired into both the `/use_reaction` item-* dispatch and the manual spend). `tests/harness/test_item_cloak_of_displacement.py` (+1, v2.645.0 "The Broken Mirage") covers the Cloak of Displacement suppression clause (RAW DMG p.158): taking damage now stamps `cloak_disp_suppressed_round` on the wearer + fires a `feature_used(source=cloak-displacement-suppressed)` card, and the attack-disadvantage read suppresses for the rest of that battle round — asserts a post-hit same-round attack rolls straight (no cloak label). `tests/harness/test_reaction_prompt.py` (+1, v2.644.0 "The Counter-Swing") covers Mage Slayer auto-attack: resolving the `spell_cast_near` `take-mage-slayer-strike` reaction with a chosen `attack_index` now runs that melee attack through the real `/attack` pipeline at the provoking caster, marking the **reaction** slot (not the action) via `/attack`'s new `as_reaction` flag — asserts the `weapon_attack` card + `feature_used(source=mage-slayer-autostrike)` fire and the action slot is untouched. `tests/harness/test_reaction_prompt.py` (+1, v2.643.0 "The Reaction Cantrip") covers War Caster auto-cast: resolving the OA `take-war-caster-cast` reaction with a chosen `spell_index` now runs that 1-action spell through the real `/cast_spell` pipeline at the provoker, marking the **reaction** slot (not the action) via cast_spell's new `as_reaction` flag — asserts the `spell_cast` card + `feature_used(source=war-caster-autocast)` fire and the action slot is untouched. `tests/harness/test_gm_reactions_panel.py` (+2, v2.642.0 "The Worn Cloak") covers the GM-panel item-reaction walk: the panel now reads equipped `sheet.inventory[*]._reactions[]`, so Lyra's Cloak of Displacement surfaces `item-cloak-displacement-advantage` in her catalog (`test_available_reactions_lists_item_reaction`) and the generic manual-spend dispatch accepts the item key — flipping the reaction chip + firing a `feature_used(source=manual-reaction)` card with no per-kind code path (`test_spend_item_reaction_manual`). `tests/harness/test_fail2ban_cloudflare_unban_parsing.py` (new, +4, v2.640.10 "The Clean Sweep") extracts the cloudflare-bouncer `actionunban` python program straight from the shipped action file and runs it against a pretty-printed Cloudflare access-rules response — asserting it deletes only the `mode=block` rule (never the whitelist or the nested `scope.id`). Guards the bug where `actionunban`'s compact-only sed (`"id":"`) never matched Cloudflare's real pretty JSON (`"id": "`), so `rule_id` came back empty, the DELETE never ran, and edge bans leaked at the Cloudflare edge on every unban. `tests/harness/test_campaign_wipe.py` (+1, v2.640.5 "The Ghost Battle") now seeds a `Battle` row and asserts `wipe_campaign_children` deletes it (`counts["battles"] == 1`), plus new `test_evict_battle_clears_hub_cache` asserting `CampaignHub.evict_battle` drops `_battle` + `_db_hydrated` (idempotent) — the two mechanisms that fix opportunity attacks + the Dash prompt silently breaking on a demo reseed (a stale hub battle cache served the previous cycle's combatants with dangling token ids + spent reactions). `tests/harness_ui/test_window_me_exposed.py` (new, +3, v2.640.1 "The Missing Identity") asserts `window.ME` is exposed on the tabletop (GM + player) + the character sheet — guarding the fix where the templates' lexical `const ME` was never attached to `window`, so `reaction_prompt.js` read `meId=null` and suppressed **every** reaction popup (the "GM moves a PC → no OA prompt" bug; the OA `reaction_prompt` broadcast correctly server-side but the browser never rendered it). HTTP/WS harness can't see this — it's a browser-JS defect, so it lives in the Playwright UI harness. `tests/harness/test_demo_campaigns.py` (+12, v2.640.0 "The Square Deal") adds `test_demo_campaign_has_encounter` + `test_demo_tokens_are_grid_aligned` (parametrized over all 6 demo campaigns): every demo campaign exposes ≥1 encounter via `/api/campaign/{id}/encounters`, and every token from `/api/campaign/{id}/tokens` has `x`/`y` divisible by the 70 px grid — guarding the `app/demo_campaigns.py` fix (off-grid 105 px token spacing → 140 px, villains row 640→630, + a seeded encounter per leveled campaign) and the Vault's Drakkasha token (y 200→210). `tests/harness/test_wiki.py::test_wiki_home_renders` (extended v2.639.0 "The Set Piece") also asserts the new illustrated `/wiki/building-an-encounter` GM guide is listed on the landing page (its 4 screenshots — the encounter library / token-template roster / SRD bestiary search / default-encounter wiring — land under `app/static/docs/encounters/`, captured by `tests/harness_ui/capture_encounters.py`). `tests/harness/test_wiki.py::test_wiki_home_renders` (extended v2.638.1 "The Card Catalog") now also asserts the guides table is kept in **alphabetical order** (parses the `id="guides-table"` link titles and checks they equal `sorted(...)` with a leading "The " ignored) — so a future out-of-order guide insertion fails the harness. `tests/harness/test_wiki.py::test_wiki_home_renders` (extended v2.638.0 "The Sorting Hat") also asserts the wiki landing page's new **audience filter** (the `class="aud-filter"` button bar with `data-aud-filter="players"`/`"gms"` + the `id="guides-table"` it filters) renders and that the **Format column was dropped** from the doc-tables (`<th>Format</th>` no longer present). `tests/harness/test_wiki.py::test_wiki_home_renders` (extended v2.637.0 "The Cartographer's Table") also asserts the new illustrated `/wiki/maps-grids-tokens` GM guide is listed on the landing page (its 4 screenshots — the Maps settings table / the tabletop board with grid + tokens / the token tracker / the Add Token modal — land under `app/static/docs/maps-tokens/`, captured by `tests/harness_ui/capture_maps_tokens.py`). `tests/harness/test_wiki.py::test_wiki_home_renders` (extended v2.636.0 "The Wardrobe") also asserts the new illustrated `/wiki/theming` guide is listed on the landing page (its 5 screenshots — the settings theme picker + the same character sheet under the Dark/Light/Fire/Hobbiton themes — land under `app/static/docs/theming/`, captured client-side via `data-theme` by `tests/harness_ui/capture_theming.py`). `tests/harness/test_wiki.py::test_wiki_home_renders` (extended v2.635.0 "The Open Door") also asserts the new illustrated `/wiki/inviting-players` GM guide is listed on the landing page (its 4 screenshots — People tab / add-member / roster / portrait — land under `app/static/docs/inviting-players/`, captured by `tests/harness_ui/capture_inviting_players.py`). `tests/harness/test_wiki.py::test_wiki_home_renders` (extended v2.634.0 "The Guided Tour") also asserts the new illustrated `/wiki/player-onboarding` guide is listed on the landing page (its 11 screenshots land in v2.633.1 "The Contact Sheet" under `app/static/docs/onboarding/`, captured by `tests/harness_ui/capture_onboarding.py`). `tests/harness/test_backup_admin.py::test_run_status` (+1, v2.633.0 "The Progress Bar") covers `backup_admin.run_status` backing the "Back up now" progress toast (`GET /backups/run-status`): no file → `idle`, a sidecar-written status reads back (`running`/`database`/20%), malformed → `idle`. `tests/harness/test_backup_admin.py::test_delete_backup` (+1, v2.632.0 "The Recycle Bin") covers `backup_admin.delete_backup` backing the new per-row 🗑 Delete button (`POST /backups/delete`): deletes a run's 3 artifacts + tag (returns 4), a re-delete removes nothing, and a bad bucket / traversal stem removes nothing. (v2.631.1 "The Full Clock" adds the hour to the backup filename stem — `YYYY-MM-DD_HHmmss_<tag>`; `test_backup_admin.py` stems updated to the 6-digit form.) (v2.631.0 "The Readable Name" renames backups to `YYYY-MM-DD_mmss_<tag>`; `test_backup_admin.py` grouping/`backup_files_for`/tag-restore tests now build the new stems and assert `g["ts"]` is the stem, while `test_artifact_path_safe_resolution` keeps an old `simplevtt-…` name to prove back-compat.) (v2.630.9 "The Trimmed Row" — Admin Center backups table drops the Contents column + the download is now a button; template-only, no harness change.) (v2.630.8 "The Labelled Runs" auto-tags backups by trigger source — `manual` for the Back-up-now button, `scheduled` for cron, `startup` for boot — shown in the Admin Center Tag column; shell-side wiring, the tag-surfacing is already covered by `test_backup_admin.py`.) (v2.630.7 "The Shared Inbox" fixes the "Back up now" 500 — the sidecar now `chmod 1777`s `/backups` so the appuser-run Admin Center can write its control files; a container volume-permission fix, verified manually end-to-end, no harness change.) `tests/harness/test_backup_admin.py::test_demo_override_active` (+1, v2.630.6 "The Live Demo Switch") covers the new `backup_admin.demo_override_active()` (true only when both `DEMO_MODE` and `DEMO_BACKUPS_ENABLED` are on; `demo_mode_active()` is then false) backing the "demo mode — backups enabled for testing" banner on the `/backups` page. Wiki SRD-coverage section (v2.630.5 "The Visible Numbers"): the per-category table is always visible; only the explainer prose below it is the collapsed `<details class="srd-coverage">` — `test_wiki.py::test_wiki_home_renders` still asserts the heading + `id="srd-coverage"` + the `<details>`/`<summary>`. `tests/harness/test_wiki.py::test_wiki_home_renders` (extended v2.630.4 "The Folded Coverage") also asserts the SRD automation-coverage breakdown is now a collapsed `<details class="srd-coverage">` with a `<summary>` (heading + `id="srd-coverage"` preserved). `tests/harness/test_wiki.py::test_wiki_home_renders` (extended v2.630.3 "The Tidy Column") also asserts the landing page's doc-listing tables use the compact emoji-only Status column — the `doc-table` layout class renders and a `status-shipped` span carries the full-text hover `title`. `tests/harness/test_wiki.py::test_wiki_backups_guide_renders` (extended v2.630.2 "The Folded Map") also asserts the guide's new "Inside the tarballs" layout breakdown (a media-bucket folder) + the cross-format "in-app import tools" section render. `tests/harness/test_wiki.py::test_wiki_backups_guide_renders` (extended v2.630.1 "The Full Inventory") also asserts the guide's new "What campaign content is captured" section calls out player characters + notes/handouts (the content the operator DB dump captures). `tests/harness/test_wiki.py::test_wiki_plans_page_renders` (+1, v2.630.0 "The Index Trimmed") covers the new `/wiki/plans` page the large Design-plans table moved to (nav + back-link + a representative spread of plan links + `status-shipped` badge + ≥30 plan entries); `test_wiki_home_renders` was trimmed to the sections that remain on the landing page (guides / references / repo-docs / banners) and now asserts the `/wiki/plans` link instead of every inline plan. `tests/harness/test_backup_admin.py::test_demo_mode_active` (extended v2.629.0 "The Test Switch") now also covers the `DEMO_BACKUPS_ENABLED` testing override — `DEMO_MODE=true` alone disables backups, but with the override on `demo_mode_active()` reports not-disabled and `backups_force_enabled()` is true. `tests/harness/test_wiki.py::test_wiki_backups_guide_renders` (+1, v2.628.0 "The Field Manual") asserts the new `/wiki/backups` guide renders (H1 + nav) and names the three operator backup-artifact suffixes (`.sql.gz` / `.homebrew.tar.gz` / `.uploads.tar.gz`) from its download-structure section; `test_wiki_home_renders` also asserts the `/wiki/backups` landing-page link. `tests/harness/test_backup_admin.py` (+2 in v2.627.0 "The Undo Button") covers the operator restore coordination: `test_tags_surface_and_restore_request` (a `simplevtt-<ts>.tag` surfaces on the grouped backup; `request_restore` writes the `.restore-request` trigger for a real backup and refuses a second while one is pending) and `test_request_restore_rejects_unknown_and_reads_result` (an unknown backup isn't queued; `last_restore_result` reads the sidecar's `.restore-result`). The destructive sidecar restore itself is demo-disabled + production-only (the safety-backup-first + DB/volume overwrite run in `entrypoint-backup.sh`), so it isn't harness-exercised. `tests/harness/test_backup_admin.py` (v2.626.0 "The Whole Picture") grouping + run-resolver tests now include the new `.uploads.tar.gz` artifact — a backup run groups into one backup of 3 files (db + homebrew + uploads) and `backup_files_for` resolves all three, backing the complete fresh-install restore set + the per-run download zip. `tests/harness/test_export_ui.py::test_campaign_settings_renders_backup_ui` (v2.625.1 "The Dedicated Drawer") also asserts the campaign-settings "Import & export" controls now live under their own top-level tab (`data-tab="import-export"`, `#custom-io` retargeted) rather than the Homebrew sub-tab. `tests/harness/test_backup_admin.py` (+2 in v2.625.0 "The Bundled Snapshot") adds `test_list_backups_groups_by_timestamp` (a backup run's `.sql.gz` + `.homebrew.tar.gz` collapse into one grouped backup with summed size; stray non-artifacts ignored) and `test_backup_files_for_resolves_run_and_is_safe` (a run resolves to its file paths; bad timestamp / bucket / missing run → empty, traversal-proof) — backing the per-run `.zip` download (`GET /backups/download?bucket=&ts=`, ZIP_STORED bundle of the run's already-gzipped artifacts). `tests/harness/test_backup_admin.py` (+1 in v2.624.0 "The Local Hour") adds `test_artifact_path_safe_resolution` covering the Admin Center backup-download path resolver `backup_admin.artifact_path` — a real `daily/` artifact resolves while unknown bucket, `..` traversal (bucket or name), sub-path names, wrong suffix, missing file, and wrong-bucket names all return `None` (traversal-proof). Backs the new `GET /backups/download` route (admin-center port 8015) + the per-row Download link; the same commit switches the artifacts table to local-time modified stamps (`| epoch`) instead of raw epoch. `tests/harness/test_export_ui.py` (+2 in v2.623.0 "The Two Buttons") now also covers Phase 9b — the PC-sheet + homebrew-item export buttons: `test_character_page_has_export_link` (the owner's `/character/{id}/sheet` renders the `/api/character/{id}/export` download link) and `test_homebrew_workshop_has_export_button` (the served `homebrew_workshop.js` carries the per-row `hbw-export` button wired to `/homebrew/{type}/{slug}/export`). Frontend-wiring guards; the endpoints are covered by `test_export_character.py` + `test_export_homebrew_item.py`. `tests/harness/test_import_campaign.py` (+2 in v2.622.0 "The Overwritten Page") now also covers Phase 7 campaign restore/overwrite (`POST /api/campaign/import?mode=restore&target_campaign_id=N`): `test_campaign_restore_replaces_in_place` clones a throwaway target (never the demo campaign), restores a *different* archive into it, and asserts the roster reflects the restored content while the old content is gone (wipe+repopulate, not append); `test_campaign_restore_errors` asserts restore without a target → 400, unknown target → 404, and a non-GM of the target → 403 — all resolving before any wipe, so shared state is safe. `tests/harness/test_export_ui.py` (new, +1, v2.621.0 "The Download Bell") covers Phase 9 of the backup/export-import arc — the campaign backup export/import UI. It fetches the campaign settings page as the GM and asserts the new controls render + are wired: `campaign-backup-btn` / `campaign-import-btn` / `campaign-import-file` present, and the JS references `/api/export-jobs/` (job-poll progress toast) + `/api/campaign/import` (clone) + the "Building backup" progress copy. A frontend-wiring regression guard; the endpoints themselves are covered by `test_export_campaign.py` + `test_import_campaign.py`. `tests/harness/test_backup_admin.py` (new, +7, v2.620.0 "The Operator's Dial") covers Phase 8 of the backup/export-import arc — the Admin Center's operator backup settings. The routes live on the Admin Center (port 8015, not the 8013 harness target), so it unit-tests the pure `app/admin_center/backup_admin.py` helper host-side: env-default vs file-backed `read_settings`, the write→read round-trip, `validate_cron` (5-field), bad-cron rejection (`ValueError`), retention clamping (0→1, 9999→365), `demo_mode_active`, and `trigger_run` + `list_artifacts` (only `.sql.gz` / `.homebrew.tar.gz` artifacts listed). The `/backups` page + sidecar watch-loop are the operator surface (Admin-Center-only). `tests/harness/test_import_campaign.py` (extended in v2.619.0 "The Grimoire Copied") now also embeds a `simplevtt-homebrew` pack (one feat) in the synthetic archive and asserts `counts.homebrew_created == 1` — proving the clone re-creates the campaign's homebrew via the shared `apply_homebrew_pack` factored out of the `/homebrew/import` route (the endpoint's own tests still pass against the thin wrapper). `tests/harness/test_import_campaign.py` (+2 since v2.617.0 "The Mirror Realm"; extended in v2.618.0 "The Sealed Letters") covers Phase 6b+6c of the backup/export-import arc — campaign-clone import `POST /api/campaign/import?mode=clone`: a cross-referencing synthetic campaign archive (map + character + NPC template + token referencing all three + playlist + track + encounter referencing map/playlist/payload + a handout + two notes) clones into a brand-new campaign, asserting each FK-remapped child count is 1 (token's map/char/template, track's playlist, encounter's refs all remapped to fresh ids), the cloned character lands in the new campaign's roster (cleaned up by archiving the clone), and (6c) `counts.handouts == 1` + `counts.notes == 1` (plaintext) + `counts.notes_skipped_encrypted == 1` (encrypted note skipped — per-campaign E2E keys don't round-trip). Error paths: non-zip → 400, wrong-level archive → 400, a non-GM-role player → 403. Synthetic archives keep it independent of the campaign export job. `tests/harness/test_import_character.py` (new, +4, v2.616.0 "The Returning Hero") covers Phase 6 of the backup/export-import arc — character import (clone): host-side unit tests for `app/import_bundle.py` (`read_manifest` rejects a wrong format + a level mismatch; `extract_media` writes bundled media to a fresh-uuid filename under a tmp uploads root and `rewrite_urls` threads the new url through the data — proving fresh names + no archive-name reuse) and live endpoint tests for `POST /api/campaign/{cid}/character/import?mode=clone` (GM imports a synthetic character archive → a new roster entry, detached via the settings delete in a `finally`; non-zip → 400, a wrong-level campaign archive → 400, non-GM → 403). Endpoint tests build synthetic archives so they don't depend on the export endpoint's cooldown. `tests/harness/test_export_character.py` (new, +3, v2.615.0 "The Adventurer's Folio") covers Phase 5 of the backup/export-import arc — the synchronous single-PC export `GET /api/character/{id}/export`: the owner exports their PC and the archive is character-scoped (asserts `level=character`, `data/character.json` matches the PC, and that `data/campaign.json` / `data/tokens.json` / `data/homebrew.json` / `data/maps/*` are absent — no campaign-wide leak); the campaign GM may export a different PC (distinct from the owner test so the per-character cooldown doesn't collide); a non-owner non-GM player is 403 and an unknown character 404. The two successful exports skip on a 429 so the live `EXPORT_COOLDOWN_CHARACTER_SECONDS` cooldown doesn't flake off TEST_MODE. `tests/harness/test_export_campaign.py` (new, +2, v2.614.0 "The Strongbox") covers Phase 4 of the backup/export-import arc — the job-based whole-campaign zip export (`POST /api/campaign/{cid}/export` → `GET /api/export-jobs/{id}` poll → `GET /api/export-jobs/{id}/download`): an unknown job id 404s (no POST, always runs); the round-trip test starts an export, asserts a non-owner gets 403 on both poll + download, polls to `done`, downloads the `application/zip` and inspects the archive (`manifest.json` with `level=campaign` + `source_campaign_id` + `counts.characters ≥ 1`, `data/campaign.json` is the campaign row, `data/homebrew.json` present). The POST-driven test skips on a 429 so the live `EXPORT_COOLDOWN_CAMPAIGN_SECONDS` cooldown doesn't flake off TEST_MODE (its pure logic is covered in `test_export_limit.py`). `tests/harness/test_export_homebrew_item.py` (new, +2, v2.613.0 "The Single Specimen") covers Phase 3 of the backup/export-import arc — the single-record homebrew export `GET /api/campaign/{cid}/homebrew/{type}/{slug}/export`: the happy path exports the demo-seeded `monsters/goblin-captain` as a one-row `simplevtt-homebrew` pack (200, envelope shape, `monster_slug=goblin-captain`); the error paths assert a non-GM 403, an unknown type 404, an unknown slug 404, and a shipped-SRD slug 404 (only the campaign's own homebrew is exportable — SRD content never leaks through the homebrew surface), ordered before the one successful export so the live per-campaign `EXPORT_COOLDOWN_HOMEBREW_SECONDS` cooldown never trips a 429 off TEST_MODE. `tests/harness/test_campaign_wipe.py` (new, +4, v2.612.6 "The Salvage Crew") + `tests/harness/test_export_bundle.py` (new, +5) cover Phase 2 of the backup/export-import arc: the reusable FK-safe campaign child-wipe `app/campaign_wipe.py::wipe_campaign_children` (extracted from `demo_seed.wipe()`; children deleted + campaign row + `active_map_id` nulled, memberships kept when `delete_memberships=False`, id-scoping leaves a bystander campaign intact, empty-list no-op) and the `simplevtt-export` zip primitives `app/export_bundle.py` (recursive/deduped `/static/uploads/...` media discovery, URL↔in-zip-path↔on-disk-source mapping, JSON-safe `row_to_dict` via SQLAlchemy column inspection, the `simplevtt-export` manifest envelope, and a zip write→read round-trip where a stale media source is skipped rather than fatal). Both exercised host-side against in-memory sqlite (`importorskip` sqlalchemy → run in the container / CI); the live demo reseed that now calls `wipe_campaign_children` stays covered by `tests/harness/test_demo_campaigns.py`. `tests/harness/test_export_limit.py` (new, +8, v2.612.5 "The Shared Hourglass") covers Phase 1 of the backup/export-import arc — the generalized per-(scope, id) export rate-limiter `app/export_limit.py`: the pure cooldown maths, per-scope window defaults + env overrides (the legacy `USER_DATA_EXPORT_COOLDOWN_SECONDS` for the `user` scope, `EXPORT_COOLDOWN_<SCOPE>_SECONDS` otherwise, non-integer fallback), the `(scope, id)`-keyed registry round-trip (separate buckets per id and per scope, re-allowed once the window elapses), and that the `app/user_export.py` back-compat shim still re-exports the legacy names. `tests/harness/test_wiki.py::test_wiki_doc_serves_backup_export_overhaul_plan` (+1, v2.612.4 "The Blueprint Vault") surfaces the new `docs/plans/backup-export-overhaul.md` design plan (Phase 0 of the backup/export-import arc: a `simplevtt-export` zip envelope at PC / campaign / homebrew-item levels, a clone-or-restore importer reusing the demo-loader's wipe/seed helpers, a generalized per-scope export rate-limiter, an in-memory export-job registry driving a progress toast, item-level homebrew export, and Admin Center backup settings via a `backup-settings.json` watch-file on the shared `backup_data` volume + a demo-mode backup skip) — asserts `/wiki/doc/plan-backup-export-overhaul` returns 200 with the plan H1 + nav, plus the `test_wiki_home_renders` landing-page assertion. `tests/harness/test_totem_spirit.py::test_ts_buff_payload_carries_totem_params` (+1, v2.612.2 "The Three Totems") covers full-feature-automation Phase 8: `use_totem_spirit` (Totem Warrior Barbarian Lv 3+) now installs a permanent `totem-spirit-active` flag-buff carrying the chosen totem's rage-gated params — seeds Krieger in a battle (`_install_buff` requires one), pins Bear's `totem_spirit_resistance_except: ["psychic"]` shape via `/buffs`, then switches to Eagle and asserts the params overwrite (Bear's flag gone). First multi-variant Phase-8 flag-buff (three payloads behind one endpoint). `tests/harness/test_potent_spellcasting.py::test_ps_buff_payload_carries_wis_mod_and_class_flags` (+1, v2.612.1 "The Lit Cantrip") covers full-feature-automation Phase 8: `use_potent_spellcasting` (Light/Knowledge/Grave/Peace Domain Cleric Lv 8+) now installs a permanent `potent-spellcasting-active` flag-buff carrying three `potent_spellcasting_*` effect keys (active/wis_mod=3 for Tavik's WIS 16/class=cleric) — the cleric/cantrip twin of v2.158.19 Empowered Evocation; the test pins the buff's flag shape via `/buffs`, pre-clearing the permanent buff so it's order-independent (Phase 2 deferred: `/cast_spell` reads the flags to auto-add WIS mod to cleric-cantrip damage). `tests/harness/test_reaction_prompt.py::test_silvery_barbs_applies_withheld_half_on_damage_flip` (+1, v2.612.0 "The Withheld Half") covers pending-resolution Phase 3a, the damage twin of Phase 2: Thalindra casts Fireball via the AoE path at [bandit, Caelan] with `auto_apply_damage` on; the scenario loops (original pass + reroll fail are both server-random) until a damage fail-flip lands, then asserts the `spell_cast_target_updated(silvery_barbs_flip=True)` update + the `silvery-barbs-reresolve` feature_used carrying `withheld_damage` + that Caelan's HP reflects **both** halves (full Fireball) — Silvery Barbs now converts a survived blast into full damage via `_resolve_save_for_half_flip`, instead of leaving the withheld half GM-narrated. `tests/harness/test_reaction_prompt.py::test_silvery_barbs_reresolves_save_or_suck_on_flip` (+1, v2.611.0 "The Word Made Real") covers pending-resolution Phase 2: an Archmage casts Hold Person at Caelan, the scenario loops until Silvery Barbs' reroll flips the save pass→fail, then asserts the `silvery-barbs-reresolve` broadcast names Paralyzed AND that Paralyzed is actually installed on Caelan (the shared `_resolve_save_failure` re-resolves the condition with all immunity gates). `tests/harness/test_wiki.py` (+1, v2.610.1) surfaces the pending-resolution-state-machine design plan. `tests/harness/test_reaction_prompt.py::test_cast_silvery_barbs_auto_rolls_reroll` (+1, v2.610.0 "The Reproachful Word") `tests/harness/test_reaction_prompt.py::test_cast_silvery_barbs_auto_rolls_reroll` (+1, v2.610.0 "The Reproachful Word") covers the second d20-reroll surface: Silvery Barbs now auto-rolls the forced reroll server-side (the save's natural + bonus are plumbed into the `save_resolved` context) and reports the new pass/fail verdict — asserts the card's reroll is internally consistent (`new_total == save_bonus + new_d20`, verdict matches `new_total` vs the DC). Applying a now-failed save's downstream effect stays GM-narrated. `tests/harness/test_reaction_prompt.py::test_use_lucky_auto_rolls_reroll_and_negates_on_miss` (+1, v2.609.0 "The Borrowed Luck") covers the first d20-reroll slice: the Lucky feat now auto-rolls the replacement d20 server-side (attacker takes the lower natural) and heals back the triggering hit when the reroll makes it miss. Asserts the reported reroll is internally consistent every run (`chosen == min(natural, new_d20)`, `new_total == bonus + chosen`, verdict vs AC) + the heal-back in whichever branch the server roll produced (never flaky). `tests/harness/test_reaction_prompt.py::test_npc_parry_auto_negates_exact_ac_hit` (+1, v2.608.0 "The Captain's Parry") extends the AC-bump auto-negation to monster reactions: Krieger swings on a Bandit Captain until a non-crit hit lands exactly at its AC, and using the monster Parry reaction heals the NPC combatant back by the full applied damage (the +2 AC turns the hit into a miss) — asserts `feature_used(source=monster-parry-negate)` with `heal_back == damage_applied` + a `battle_update` (the NPC HP path, via `_apply_heal_to_combatant`). `tests/harness/test_combat_inspiration.py::test_ci_ac_auto_negates_exact_ac_hit` (+1, v2.607.0 "The Trumpet's Reply") completes the AC-bump auto-negation family: Lyra (Valor) casts Bardic Inspiration on Garrik, Pip swings until a non-crit hit lands exactly at Garrik's AC, and using Combat Inspiration via the reaction prompt heals back the full applied damage (the +BI-die AC turns the hit into a miss) — asserts `feature_used(source=combat-inspiration-negate)` with `heal_back == damage_applied` + a matching `character_hp_update`. `tests/harness/test_admin_center.py::test_users_create_with_gm_pregrant` (+1, MFA-only, v2.606.1 "The One-Line Welcome") asserts that on an MFA-verified session, creating a user with `is_gm=1` in the same POST shows the GM pill on the new row without a separate role step (the condensed create form's inline GM/Admin pre-grant toggles). `tests/harness/test_admin_center.py::test_users_page_shows_last_login` (+1, v2.606.0 "The Last Seen") logs demo-alice into the main app to stamp `users.last_login_at`, then asserts the admin-center user table shows the new "Last login" column + a compact `YYYY-MM-DD HH:MM:SS` timestamp (not "never") in her row. `tests/harness/test_campaign_archive.py` + `tests/harness/test_pc_retirement.py` (+1 each, v2.605.3 "The Gentle Nudge") assert the archive/retire redirects carry a `?flash=<key>` param and the landing page renders the `#flash-toast` confirmation widget. `tests/harness/test_demo_campaigns.py` (+1 net, v2.605.0 "The Drowned Lighthouse") covers the demo reseed (Phase 4 of [`docs/plans/campaign-pc-archive.md`](plans/campaign-pc-archive.md)): the original "Sundered Vault" (id 1) is now seeded archived + a fresh "Demo L5: The Tide-Wracked Catacombs" joins the active leveled lineup — D7 asserts carol sees Goblin Warrens + Catacombs (and no Vault card), D6 (renamed `test_all_five_active_leveled_campaigns_seeded`) asserts the active lineup + the Vault in demo-gm's Archived section, and new `test_sundered_vault_seeded_archived` asserts the Vault renders with an Unarchive control in demo-gm's lobby. `tests/harness/test_pc_retirement.py` (new, +3, v2.604.0 "The Honored Retirement") covers the PC soft-retire (Phase 3 of [`docs/plans/campaign-pc-archive.md`](plans/campaign-pc-archive.md)): retiring Pip Quickfingers (owned by demo-alice) moves it out of the active `/characters` listing into the collapsed "Retired" section, unretire restores it (round-trip, restored in a `finally`), a non-owner non-admin (demo-bob) gets 403, and an unknown character 404s. `tests/harness/test_campaign_archive.py` (new, +3, v2.603.0 "The Quiet Shelf") covers the campaign soft-archive (Phase 2 of [`docs/plans/campaign-pc-archive.md`](plans/campaign-pc-archive.md)): archiving the demo campaign hides it from the active lobby + surfaces it in the collapsed "Archived" section, unarchive restores it (round-trip, restored in a `finally`), a non-GM member gets 403, and an unknown campaign 404s. `tests/harness/test_wiki.py` (+1, v2.602.1 "The Filing Cabinet") surfaces the new `docs/plans/campaign-pc-archive.md` design plan via `test_wiki_doc_serves_campaign_pc_archive_plan` + the `test_wiki_home_renders` landing-page assertion. `tests/harness/test_form_of_the_beast.py` (+1, v2.602.0 "The Lashing Tail") `tests/harness/test_form_of_the_beast.py` (+1, v2.602.0 "The Lashing Tail") adds `test_tail_reaction_auto_negates_exact_ac_hit`: the third slice of the reactions-automation v3 auto-resolution backlog — Path of the Beast's Tail Swat now retroactively heals back the triggering hit's full damage when its rolled +1d8 AC turns a non-crit hit into a miss (same recipe as the v2.600.0/v2.601.0 negations; the test forces determinism by probing for a hit landing exactly at AC, where any 1d8 roll negates regardless of the random tail die — Brakka Wildmane is the native demo fixture, HP patched high for the probe + restored). `tests/harness/test_reaction_prompt.py` (+1, v2.601.0 "The Finesse Parry") adds `test_defensive_duelist_auto_negates_in_band_hit`: the second slice of the reactions-automation v3 auto-resolution backlog — same recipe as the v2.600.0 Shield negation but a +PB AC bump instead of +5, so a non-crit hit in the `target_ac <= attack_total < target_ac + pb` band heals back the full applied damage. `tests/harness/test_reaction_prompt.py` (+1, v2.600.0 "The Returned Blow") adds `test_cast_shield_auto_negates_in_band_hit`: the first slice of the reactions-automation v3 auto-resolution backlog — casting Shield via the reaction prompt now retroactively heals back the triggering hit's full damage when its +5 AC turns a non-crit hit into a miss (reuses the v2.80.0 Uncanny Dodge heal-back recipe; `is_crit` is plumbed into the `attack_targeted` prompt context so a nat-20 is never negated). `tests/harness/test_ws_disabled_user.py` (new, +1, v2.599.10) is an end-to-end test that the campaign WebSocket rejects a user disabled mid-session: logs in as a demo player, confirms WS connect, flips `is_disabled` in the DB (via `docker compose exec db psql`) without touching the session, and confirms a fresh connect is refused (reverts in `finally`). `tests/harness/test_admin_center_csrf.py` (+4, v2.599.8; same-origin host-derivation test-fix v2.599.9) covers the Admin Center CSRF origin-check: a source guard plus live tests that a cross-origin POST to `/users/create` is 403-CSRF-rejected while header-less + same-origin POSTs fall through to the auth bounce. `tests/harness/test_session_cookie_hardening.py` (new, +3, v2.599.7) guards the session cookie hardening — both services plumb `SESSION_COOKIE_SECURE`, both middleware configs set `same_site="lax"` + env-driven `Secure`, and `.env.example` documents the flag. `tests/harness/test_js_xss_escaping.py` (+1, v2.599.6) extends the DOM-XSS source guards to the beast-picker CR-cap warning (a homebrew monster's `cr` must be `_esc`'d before `innerHTML` injection). `tests/harness/test_js_xss_escaping.py` (new, +1, v2.599.5) is a source guard for the DOM-XSS fix where the Dash/opportunity-attack modal injected a combatant's `watcher_name` into `innerHTML` raw — asserts it now passes through `escapeHTML()` and the raw form is gone. `tests/harness/test_upload_safety.py` (new, +6, v2.599.4) covers the `safe_ext` upload-extension allowlist (allowed exts pass + lowercase, no-suffix default, `.html`/`.svg`/`.js` rejected with 400, video-only set, audio set, + a wiring guard that the affected upload routes go through `safe_ext`/`IMAGE_VIDEO_EXTS`) — the stored-XSS fix where uploads took the on-disk extension from the user filename and served it from `/static`. `tests/harness/test_mini_sheet_xss.py` (new, +2, v2.599.3) renders `_tab_actions.html` with a malicious payload and asserts it is HTML-escaped, and guards `_tab_spells.html` at the source level — covers the stored-XSS fix where custom attack/item + homebrew-spell detail fields were concatenated raw into a `| safe` block. `tests/harness/test_visitor_log.py` (+1, v2.599.2) asserts docker-compose's `app` service plumbs `VISITOR_REQUEST_LOG_ENABLED` (it was never declared, so the feature silently never fired since v2.480.0). `tests/harness/test_visitor_log.py` (+1, v2.599.1) asserts `CF-Connecting-IP` opens the per-request visitor-log gate with `TRUSTED_PROXY_HOPS=0` (the pure-CF-Tunnel config), and the "no trustworthy IP source" test now disables `TRUST_CF_CONNECTING_IP` too. `tests/harness/test_fail2ban_ignoreip_wiring.py` (+1) asserts the published Cloudflare edge ranges (incl. `172.64.0.0/13` + an IPv6 block) are allowlisted in the jail `ignoreip`, so fail2ban can't ban the Cloudflare edge (it was self-banning `172.67.x` behind a CF Tunnel). Demo-rework arc (`tests/harness/test_demo_campaigns.py`): D3 (+3) — L3 "Goblin Warrens" in its GM's + members' lobbies (demo-gm/alice/carol); D4 (+5) — L9 "Storm Over Saltmarsh" in the second GM's (demo-gm2) + members' (bob/dave/erin) lobbies, and demo-gm2 owns *only* that campaign; D5 (+3) — L13 "Shadowfell Spire" in its GM's + members' (demo-gm/bob/dave) lobbies; D6 (+4) — L18 "Dragon's Apotheosis" in its GM's + members' (demo-gm/carol/erin) lobbies + a full-arc check that all five leveled campaigns seed; D7 (+1) — a shared player (carol) sees both the L3 and L5 campaigns in her lobby; D8 (+1, `test_wiki.py`) — `/wiki/demo-content` serves the demo catalog (all five campaign names + the collapsed generation-prompts section). `tests/harness/test_demo_gm_admin_gate.py` (+2) — the new `demo-gm2` (GM role, not site-admin) and the three new players (`demo-carol`/`demo-dave`/`demo-erin`) all get 403 on `/admin`; `tests/harness/test_demo_campaigns.py` (+7, new) — every one of the seven demo accounts logs in (grows as the five leveled campaigns land). `tests/harness/test_admin_center.py` (+6, Arc B2 of `docs/plans/app-wide-roles-and-storage.md`) covers per-user/per-campaign storage limits + enforcement: the `storage_quota.check_quota` unit (fast no-op when unlimited; blocks over the campaign + user limits; passes under; fails open when accounting is down — host-side sqlite + monkeypatched accounting); the storage-limit routes require auth + are refused for basic-auth header callers (parametrized over `/users/{id}` + `/campaigns/{id}`); and an MFA-on end-to-end — set a tiny campaign limit → over-limit Center map upload rejected (`err=`) → clear limit → upload succeeds. `tests/harness/test_admin_center.py` (+6, Arc B1 of `docs/plans/app-wide-roles-and-storage.md`) covers storage accounting: the pure `storage.aggregate` attributor (host-side, no DB — campaign→GM, standalone portrait→owner, unreferenced→unattributed, by-type sums, empty tree) and the live `/api/storage` (shape + requires-auth) + `/storage` page (redirects unauthenticated; renders By-user / By-campaign after login). `tests/harness/test_admin_center.py` (+3, Arc A4 of `docs/plans/app-wide-roles-and-storage.md`) covers Admin Center role assignment (`POST /users/{id}/role`): requires auth (unauth → 303), refused for basic-auth header callers (403/404), and an MFA-on grant-GM (pill shows) → revoke → grant/revoke admin → unknown-role err → delete round-trip on a throwaway user. `tests/harness/test_campaign_roles.py` (+3 A2 + 2 A3 = 5 total, Arc A2/A3 of `docs/plans/app-wide-roles-and-storage.md`) covers the role gates + caps: **A2** — a player (demo-alice) → 403 on `POST /campaigns` (the `require_gm` gate), a GM (demo-gm) creates a campaign (303 → campaign page), an unauthenticated POST is refused; the per-GM `GM_CAMPAIGN_LIMIT` cap is enforced inline (not live-tested — would need 10 creates). **A3** — a player creating standalone characters hits the `PLAYER_CHARACTER_LIMIT` cap → 403 with a "limit" message within a bounded number of creates, while a GM is uncapped (several creates all succeed). `tests/harness/test_wiki.py` (+1, v2.584.0 "The Three Keys") surfaces the new `docs/plans/app-wide-roles-and-storage.md` design plan (Arc A1 — the app-wide `User.is_gm` role substrate, schema v76, `require_gm` gate, `PLAYER_CHARACTER_LIMIT`/`GM_CAMPAIGN_LIMIT` config, demo-seed roles): `/wiki/doc/plan-app-wide-roles-and-storage` returns 200 with the plan H1 + nav, and the landing page lists it. The `require_gm` gate gets behavioral coverage in Arc A2 (campaign-create gating), where it becomes observable over HTTP. `tests/harness_ui/test_map_pan.py` (+2, v2.583.0, "The Unstuck Map") covers the tabletop map pan fix: a left-button drag over empty map area changes the `#map-transform` translate (the new cross-device pan gesture, added because right-button-only panning was eaten by the native context menu on macOS Safari / trackpads, leaving the map "stuck" while wheel-zoom kept working), and the wheel still zooms (guard against regressing the path that kept working). `tests/harness/test_admin_center.py` (+6) covers Phase 3b uploads (`docs/plans/admin-center-consolidation.md`) — the Center's campaign **map upload + activate** via the shared `uploads_data` volume: the `campaign_admin` `create_map`/`activate_map` round-trip (host-side sqlite — grid/dim clamping, first-map auto-activate, bad-grid fallback, cross-campaign reject); the upload + activate routes require auth (unauth → 303, parametrized) and are refused for basic-auth header callers (403/404, parametrized); and an MFA-on upload-a-tiny-PNG → listed → activate round-trip against a real campaign (skipped when MFA off). Phase 4 route-removal: **deleted** `test_admin_audit.py` (5) + `test_admin_user_audit_scrub.py` (5) + `test_admin_demo_reset.py` (2) + 3 in-app-mint tests from `test_demo_magic_link.py` — all exercised now-retired in-app `/admin` routes (user write surface v2.579.0; on-demand demo reset v2.580.0; demo magic-link mint v2.581.0); the moved behavior is covered Center-side in `test_admin_center.py`. `test_demo_magic_link.py`'s end-to-end happy path was **rewired** to mint container-side via the Admin Center (`_center_mint` — same SECRET_KEY → the token verifies at the app's `/demo-login`), exercising the real cross-service contract (Center mints → app redeems → replay 401); the mint/verify **unit** tests are unchanged. **Grew** `tests/harness/test_admin_routes_retired.py` (now +7): asserts each retired in-app route (`POST /admin/users`, `/disable`, `/reset_password`, `/delete`, `/scrub-audit-log`, `/admin/demo/reset`, `/admin/demo/mint-magic-link`) no longer succeeds (never 2xx/3xx — 404/405/401/403 only), i.e. no live duplicate write-path (`docs/plans/admin-center-consolidation.md` Phase 4). `tests/harness/test_admin_center.py` (+3) covers the Center's ported GDPR audit-log scrub (`POST /users/{id}/scrub-audit-log`, `docs/plans/admin-center-consolidation.md` Phase 4): requires auth (unauth → 303), refused for basic-auth header callers (403 gated / 404 off — never runs, so the shared log is untouched), and on an MFA-verified session a missing email → 400 + a never-logged-email scrub rewrites 0 lines with the JSON contract intact (safe no-op; skipped when MFA off). `tests/harness/test_admin_center.py` (+11) covers Phase 3b (no-upload subset) of `docs/plans/admin-center-consolidation.md` — the Center's MFA-gated campaign management: the `campaign_admin` member add/remove + system change + character create/assign/delete round-trip (host-side sqlite, `importorskip`, incl. idempotent add + wrong-campaign/unknown guards); all six management routes require auth (unauth → 303, parametrized); a representative subset (member-remove, character-delete, system) is refused for basic-auth header callers (403 gated / 404 off, parametrized); and a net-zero character create → assign → delete round-trip on a real campaign on an MFA-on stack (skipped when MFA off). `tests/harness/test_admin_center.py` (+6) covers Phase 3a of `docs/plans/admin-center-consolidation.md` — the Center's campaign-admin surface: the `campaign_admin` service list → detail → delete round-trip (host-side sqlite, `importorskip`, incl. unknown-id + re-delete raising); `/campaigns` auth-gated (unauth → 303), renders when `ADMIN_CENTER_ADMIN_TOOLS` is on + drills into the read-only detail (Overview/Members); an unknown campaign id redirects with an error (no 500); the MFA-gated delete requires auth (unauth → 303) and is refused for basic-auth header callers (403 gated / 404 off — never POSTed from a verified session, which would wipe a real demo campaign). `tests/harness/test_admin_center.py` (+6) covers Phase 2b of `docs/plans/admin-center-consolidation.md` — the MFA-gated destructive user ops: the disable/reset-password/delete routes require auth (unauth → 303, parametrized over all three), are refused for basic-auth header callers (403 gated / 404 when off — the MFA gate, since a header caller has no MFA-verified session), and a full disable → re-enable → reset-password → delete → re-delete round-trip on an MFA-on stack using a throwaway account (skipped when the stack has MFA off, since destructive ops are then always refused). `tests/harness/test_admin_center.py` (+8) covers Phase 2a of `docs/plans/admin-center-consolidation.md` — the opt-in `/users` user-admin page (read-only list + non-destructive create): the new `operator_audit` helper formats/appends operator-attributed audit lines (`actor=admin-center:<operator>`) that round-trip through the dashboard's own `audit_parse` (incl. the no-`AUDIT_LOG_PATH` path that must not raise); the `user_admin` service functions validate + reject duplicate/short-password + toggle-disable + delete (host-side in-memory sqlite, `importorskip`); and the live routes are auth-gated (unauth → 303), render when `ADMIN_CENTER_ADMIN_TOOLS` is on, reject a duplicate-email create (303 + `err=`), and require auth on create. `tests/harness/test_admin_center.py` (+4) covers the new opt-in `/tools` admin-tools page (Phase 1 of `docs/plans/admin-center-consolidation.md`): auth-gated (unauth → 303), renders when `ADMIN_CENTER_ADMIN_TOOLS` is on, the demo magic-link mint round-trips (303 + minted URL), and the destructive demo-reset is auth-gated (tested without ever firing the reseed). `tests/harness/test_wiki.py::test_wiki_doc_serves_admin_center_consolidation_plan` (+1) surfaces the new "Consolidate site-admin into the Admin Center" design plan. `tests/harness/test_admin_center.py` (+2) covers the new `/tests` admin-center dashboard: unauthenticated → 303 to /login (no basic-auth popup), and after session login the page renders the harness test-results dashboard (latest run or the empty-state). `tests/harness/test_homebrew_fork_srd.py` (→ 19) + `tests/harness_ui/test_homebrew_workshop.py` (→ 2) now also cover Phase 3: the `GET /api/campaign/{id}/srd_search` browse endpoint (finds shipped SRD by name, GM-gated, unknown-type 404) and the Workshop search→fork Playwright smoke (search "fireball" → Fork button → editor opens pre-filled). `tests/harness/test_homebrew_fork_srd.py` (→ 16) covers the fork / edit / revert / **list** endpoints (Phases 1–2 of `docs/plans/homebrew-fork-srd.md`): variant + override fork, GM-editor tweak (8d6→10d6) persisting with scope forced to campaign-N, the `/homebrew/custom` list (campaign's own records only, not shipped SRD), plus GM-gating + error paths. `tests/harness_ui/test_homebrew_workshop.py` (+1) is the Playwright smoke: GM opens Settings → Homebrew → Workshop, forks Fireball, the form editor pre-fills 8d6, the GM saves a 12d6 tweak, and it resolves server-side. `tests/harness/test_homebrew_fork_srd.py` (+9) covers the fork-SRD-as-homebrew endpoints (Phase 1 of `docs/plans/homebrew-fork-srd.md`): variant fork of an SRD spell (fresh slug, source:"custom", verbatim mechanics), override fork shadowing the SRD slug for the campaign only (no-campaign lookup still pristine SRD), 409 on double-override, type-generality (a monster), GM-gating (non-GM 403), unknown-slug/type/mode errors, and the revert (un-fork) delete + its 404. `tests/harness/test_srd_provenance.py` (+3) enforces the "SimpleVTT ships SRD 5.1 content only" rule: every shipped content record is `source:"srd"` + `scope:"global"` + SRD-attributed (the "only SRD" half), per-type counts meet a completeness floor so SRD mechanics can't silently shrink (the "all SRD present" half), and no homebrew-sourced record leaks into the shipped tree. `tests/harness/test_spell_catalog_concentration.py` (+2) now also gates the catalog `concentration` flag against each spell's duration in both directions (the v2.568.1 build-script fix that corrected 125 mis-flagged spells) + spot-checks fixed spells (Moonbeam, Bless, Fly, …). `tests/harness/test_aoe_enter_trigger.py` (+9) covers the persistent-AoE trigger (`docs/plans/aoe-enter-trigger.md`, Phases 1+3): an NPC moving into Spirit Guardians takes save-for-half damage; the trigger fires once per turn (out-and-back-in damages once); a move staying outside fires nothing; `override_barrier` skips it; the caster is immune to their own emanation; an NPC that *starts its turn* inside takes damage; an NPC *pushed in* via `/force_move` takes damage (forced-movement path); a PC moving in gets a WIS-save `roll_request`; and a *placed* sphere (Moonbeam) damages an NPC moving in (the no-save guard excludes Spike-Growth-style movement-damage AoEs). `tests/harness/test_fail2ban_discord_notify.py` (+7) covers the optional Discord webhook notify action (fail2ban Phase 4h): action file shape, ban/unban curl POSTs, the `${FAIL2BAN_DISCORD_*}` env placeholders + graceful no-op guard, and the render-script / compose / `.env.example` wiring. Recent UI tests: `tests/harness_ui/test_notes_crypto.py` (Phase 4b — browser PBKDF2+AES-GCM round-trip + full encrypt→store→fetch→decrypt, no plaintext at rest or on the wire) and `tests/harness_ui/test_notes_drawer.py` (Phase 5a GM prep-note CRUD; 5d a player writes/reloads-locked/unlocks a private note; 5b cross-context handout reveal — GM reveals → player sees it live over WS → GM hides → it disappears — zero console errors).
...
### `test_handout_file.py`
v2.1045.0 — handout **document attachments**, the "Resources" tier ([notes-and-handouts.md](../plans/notes-and-handouts.md)'s filed "handout media beyond images" follow-up). `handouts.file_url` / `file_name` / `file_size` (schema v104) + `POST …/handouts/upload_file`. The document rides the *existing* `revealed` / `reveal_to` gate rather than a second access-control surface, so these tests assert the gate as much as the upload. PDF only (browsers render it inline with no download step). Fixtures as `test_handouts.py`.

| Test | What it asserts |
|------|-----------------|
| `test_upload_handout_file` | GM `POST …/handouts/upload_file` (PDF) → 200 + a `/static/uploads/handouts/…pdf` URL, the *original* filename, and the real byte count. |
| `test_uploaded_file_is_served` | The returned URL resolves — GET returns bytes starting `%PDF-`, so the inline viewer isn't pointed at a 404. |
| `test_create_handout_with_file_round_trips` | The `file_url`/`file_name`/`file_size` trio survives create → GET by id. |
| `test_patch_attaches_and_detaches_file` | PATCH attaches a document to an existing handout; PATCH `file_url: ""` nulls the URL **and** drops name + size, so a stale filename never outlives the file. |
| `test_document_hidden_until_revealed` | **The access gate:** an un-revealed document is absent from a player's list + 404 by id (no `file_url` leak); after reveal-to-all the player reads `file_url` + `file_name`. |
| `test_document_scoped_reveal_excludes_other_player` | Revealed to alice only → alice's list has it, bob's does not. |
| `test_reveal_broadcast_carries_has_file` | The `handout_revealed` payload carries `has_file: true` so the client picks the 📄 toast without a second fetch. |
| `test_reveal_broadcast_has_file_false_without_document` | A text-only handout broadcasts `has_file: false` (present, not missing). |
| `test_unrevealed_document_not_in_player_list_after_hide` | The gate is not one-way — hiding a revealed document returns it to 404 for the player. |
| `test_upload_handout_file_player_403` | A non-GM member uploading a document → 403. |
| `test_upload_handout_file_bad_ext_400` | A non-PDF extension (`.docx`) → 400. |
| `test_upload_handout_file_bad_magic_400` | A `.pdf` extension over non-PDF bytes → 400 — the extension is a claim, and the viewer would render a blank frame. |
| `test_bad_file_size_degrades_to_null` | A junk `file_size` nulls out rather than 400-ing the save; the `file_url` still round-trips. |

### `test_ci_env_invariants.py`
v2.1047.1 (uploads) + v2.1047.5 (demo reset) — invariants the CI workflow's `.env` heredocs must satisfy. Both are the same shape of bug: an env value that silently voids a slice of the suite, and that reopens when a new job is copy-pasted from an old one. **(1) Uploads:** CI must exercise the upload endpoints rather than 403 past them. `docker-compose.yml` defaults `DEMO_DISABLE_UPLOADS` to `true` (`${DEMO_DISABLE_UPLOADS:-true}`) and the gate fires whenever that and `DEMO_MODE` are both on. The harness workflow needs `DEMO_MODE` for the seeded demo PCs, so before this fix all four CI jobs answered `403 "Uploads are disabled on this demo instance"` for every upload — silently voiding 9 upload-exercising harness files plus two admin-center storage tests. Config-parsing tests (no HTTP), in the same spirit as `test_fail2ban_compose_service.py`.

| Test | What it asserts |
|------|-----------------|
| `test_every_demo_mode_env_block_enables_uploads` | **The invariant:** any CI env block setting `DEMO_MODE=true` also sets `DEMO_DISABLE_UPLOADS=false`. Verified by negative control — removing one occurrence fails the test. |
| `test_demo_mode_env_blocks_exist` | **Guard the guard:** at least 4 heredoc blocks carry `DEMO_MODE=true`. The first draft's parser matched a bare trailing `EOF` and so found **zero** blocks against the workflow's quoted `<<'EOF'`, making the invariant above pass vacuously; this test is what caught it. |
| `test_workflow_is_valid_yaml` | The workflow parses and has jobs — a malformed workflow stops running silently rather than failing loudly. |
| `test_compose_still_defaults_uploads_disabled` | Compose still defaults the demo lockdown **on**, since that default is the only reason the CI override is needed; if it flips, the premise changed and wants re-review. |

**(2) Demo reset (v2.1047.5).** `DEMO_MODE=true` spawns an unconditional `while True: sleep(interval); reset_and_reseed()` scheduler (`app/demo_scheduler.py`) that **wipes and reseeds the dataset**. CI needs `DEMO_MODE` for the seeded PCs, but at the previous 60-minute interval the 272-minute harness job absorbed roughly four wipes while tests were running. `config.py` clamps the interval to `[5, 1440]`, so 1440 is the effective "never".

| Test | What it asserts |
|------|-----------------|
| `test_demo_reset_cannot_fire_during_a_ci_run` | **The invariant:** every `DEMO_MODE=true` block sets an interval ≥ 720 min. Verified by negative control — reverting one block to 60 fails the test. |
| `test_demo_reset_on_boot_still_enabled` | The fix doesn't throw out the seed with the scheduler: `DEMO_RESET_ON_BOOT=true` stays, so each job still starts from a freshly-seeded dataset. |
| `test_reset_interval_is_within_the_config_clamp` | The value can't exceed 1440 — `config.py`'s `max(5, min(1440, ...))` would silently clamp it back down, leaving the workflow *looking* fixed while the scheduler still fired. |

> **Why this doesn't weaken the lockdown's own coverage.** `test_demo_disable_uploads.py::test_all_upload_endpoints_carry_the_guard` is pure route introspection (config-independent), and its live HTTP test deliberately accepts either 200 or 403 — so the guard stays tested under either configuration.

### `test_media_gate.py`
v2.1047.0 — the same gate generalized to the rest of the uploads tree (`maps`, `thumbnails`, `tokens`, `portraits`, `token_templates`, `encounter_bg`, `audio`). `serve_upload` sits after the handouts route and before the `/static` mount; resolution is by **exact URL match across every column that can hold one**, not per-bucket, and orphans fail closed.

> **Demo note.** The seeded demo references *no* uploaded media — its art is bundled under `/static/demo/`, deliberately outside the gate — so these tests upload their own map through the real settings endpoint and delete it in teardown. The map list comes from the settings page's inline JS array, because `/api/campaign/{cid}/map-group` returns only the *active map's group*, which a freshly-uploaded map isn't in.

| Test | What it asserts |
|------|-----------------|
| `test_member_reads_uploaded_map` | A campaign member gets the exact uploaded bytes back. |
| `test_anonymous_cannot_read_uploaded_map` | **The core fix:** a session-less client holding the exact URL gets 404. |
| `test_campaign_player_can_read_campaign_media` | A *player* in the campaign can read its art — the gate is membership, not GM-only. |
| `test_uploaded_media_is_rendered_and_still_serves` | **Coverage guard:** every gated media URL the settings page actually renders still serves, so a DB column the resolver forgot surfaces as a concrete broken image. |
| `test_deleting_the_row_closes_the_gate` | Deleting the map revokes access to its file even though the bytes stay on disk (this caught a memo cache that broke revocation for 300 s). |
| `test_orphan_media_fails_closed` | A file referenced by no row → 404. |
| `test_traversal_and_nested_paths_rejected` | Percent-encoded traversal (so the client can't normalize it away first), a nested path, and a dotfile all → 404, and no stylesheet bytes leak. |
| `test_ungated_static_paths_still_served` | **Regression:** `/static/style.css` *and* the bundled `/static/demo/maps/tavern.png` are untouched by the interception. |
| `test_media_is_browser_cacheable_but_not_shared` | `private` but **not** `no-store`, plus an ETag — ordinary campaign art must stay revalidatable or the tabletop refetches every image. |
| `test_conditional_request_gets_304` | `If-None-Match` → bodyless 304 (this caught `FileResponse` stamping its ETag only at send time, so the check never fired). |
| `test_handout_bucket_keeps_its_stricter_handler` | The generic gate didn't swallow handouts — they keep `no-store` and per-player reveal rules. |

### `test_handout_media_gate.py`
v2.1046.0 — handout media (images **and** documents) is authorization-gated rather than served as public static bytes. `serve_handout_media` is registered ahead of the `/static` mount and authorizes every request through the same `_can_see_handout` rule the JSON endpoints use. Failures are a flat **404**, never 401/403 — a 403 would confirm that an un-revealed handout's media exists.

| Test | What it asserts |
|------|-----------------|
| `test_upload_url_is_campaign_prefixed` | Uploads are named `c<campaign_id>-<uuid>.<ext>` — the prefix is what authorizes a not-yet-attached file. |
| `test_anonymous_cannot_read_handout_media` | **The core fix:** a session-less client holding the exact URL gets 404. |
| `test_gm_can_read_own_handout_media` | The GM reads their own handout's image and gets the real bytes back. |
| `test_player_blocked_until_revealed_then_revoked_on_hide` | Un-revealed → 404; revealed → 200; **hidden again → 404**. The last leg is what the old static-bytes posture could not do. |
| `test_scoped_reveal_scopes_media` | A reveal to alice only → alice 200, bob 404 — media follows the body's scoping. |
| `test_document_media_is_gated_too` | The PDF path is gated identically to the image path (404 before reveal, `%PDF-` bytes after), and is served `Content-Disposition: inline` under the GM's original filename rather than the UUID on disk. |
| `test_unattached_upload_readable_by_gm_not_player` | The composer-preview case: a file with no referencing handout falls back to the filename's campaign id — GM yes, player no. |
| `test_deleting_handout_revokes_player_media_access` | Deleting the referencing handout leaves nothing to authorize a player against → 404. |
| `test_unknown_media_name_404` | An unknown filename → 404. |
| `test_media_path_traversal_rejected` | `..`, encoded traversal, a nested path, and a dotfile all → 404 — no escape from the handouts directory. |
| `test_other_static_paths_still_served` | **Regression:** the interception is scoped to the handouts prefix and doesn't shadow the rest of `/static` (`/static/style.css` → 200). |
| `test_served_media_is_not_shared_cacheable` | Served bytes carry `Cache-Control: private, no-store`, so a shared cache can't outlive a revoked reveal. |

> **Environment note.** The `upload_file` / `upload_image` tests need uploads enabled. A local stack running the shipped defaults (`DEMO_MODE=true` + `DEMO_DISABLE_UPLOADS` defaulting true) returns **403 "Uploads are disabled on this demo instance"** for every upload endpoint. Run these with `DEMO_DISABLE_UPLOADS=false docker compose up -d app`. This applies equally to the pre-existing `test_upload_handout_image*` tests.
...
### `test_presence_idle.py`
v2.1044.0 — the idle half of the `presence_update` broadcast. `CampaignHub` stamps each presence row with `idle_seconds` (age of that user's last inbound WS frame, relative so client clock skew doesn't matter) + `idle` (that age vs. `PRESENCE_IDLE_AFTER_SECONDS`), and puts the threshold on the payload as `idle_after_seconds`. The receive loop feeds `hub.mark_active` on every frame; the client pings on throttled real interaction and ages the clock forward locally between broadcasts, so no server sweeper task is needed.

**Not covered by design:** actual idle *expiry* (a seat going amber). The threshold is 300 s and no endpoint injects a clock, so an end-to-end assertion would mean a 5-minute sleep; the threshold itself is a one-line comparison in `get_presence`.

| Test | What it asserts |
|------|-----------------|
| `test_presence_update_carries_idle_fields` | Happy path — `presence_update.data` carries a numeric `idle_after_seconds > 0`; every user row has a bool `idle`, a numeric `idle_seconds >= 0`, and the pre-existing `user_id`/`display_name`/`is_gm`; a freshly-connected GM is `idle: false` with `idle_seconds < 60`. |
| `test_activity_frame_is_accepted_and_keeps_user_active` | The client's real `{"type":"activity"}` frame is consumed by `mark_active` without breaking the socket, and the sender still reads active on the next roster. |
| `test_multiple_tabs_yield_one_presence_row` | Two sockets for one user dedupe to a single row — guards the dedupe that the idle merge had to preserve when it replaced the old `continue`-on-duplicate-uid. |
| `test_garbage_frame_does_not_break_the_socket` | Error path — a non-JSON frame neither raises nor drops the connection from presence (the receive loop hands frames to `mark_active` unparsed). |

### `test_hermetic_pcs.py`
v2.1033.7 (B17) — regression guard for the `hermetic_pcs` autouse conftest fixture that heals a leaked sheet strip between tests, so a sheet-mutating test's failed restore can't cascade through the serial CI `harness` run. The fixture snapshots each demo PC's pristine top-level mutable sheet fields (`spells`/`spell_slots`/`resources`/`attacks`/`abilities`/defenses/…) on the first test and PATCHes back any that drifted before every subsequent test.

| Test | What it asserts |
|------|-----------------|
| `test_leak_strips_spells` | Empties Thalindra's `spells` and deliberately does NOT restore (simulates a forgotten/aborted restore). |
| `test_next_test_sees_restored_spells` | Runs after the leak test in file order → Thalindra's spells are back, proving `hermetic_pcs` restored them at setup. |
| `test_snapshot_restore_helper_round_trips` | `_snapshot_pristine` → strip → `_restore_pristine` returns the exact pristine `spells` list within one test. |
| `test_leak_bumps_level` | (B18 class 1) Drops Thalindra's class-scoped `level` to 3 and leaks it (no restore). |
| `test_next_test_sees_restored_level` | Runs after the level leak → Thalindra is back at Lv 7, proving the class-scoped (`class_slug`) restore heals level/subclass drift between tests. |
| `test_class_scoped_restore_round_trips` | `_snapshot_pristine` captures `cls.level` + `slug="wizard"`; a level→3 PATCH then `_restore_pristine` returns level to 7 within one test. |
| `test_leak_nonhardcoded_buff` | (B18 class 3) Drinks a Potion of Resistance on Garrik (installs `resistance-cold`, a key NOT in `_LEAKABLE_BUFF_KEYS`) in a seeded battle and leaks it. |
| `test_next_test_sees_cleared_buff` | Runs after the buff leak → Garrik's combatant has no `resistance-*` buff, proving `clean_pcs` clears any leaked buff by reading the live battle; tidies the leaked battle after. |
| `test_leak_battle_roster` | (B18 class 2) Overwrites the demo's seeded battle with one ad-hoc combatant (no `source_token_id`) and leaks it; asserts the seeded roster is gone. |
| `test_next_test_sees_restored_battle` | Runs after the battle leak → ≥4 seeded combatants (with `source_token_id`) are back, proving `hermetic_battle` restored the seeded roster at setup. |
...
### `test_attack_prone_positional.py`
Phase 3 of the advantage-disadvantage plan (v2.1001.0) — the positional prone edge. RAW PHB p.292: within 5 ft of a prone target → advantage; farther → disadvantage; off-grid → no edge (GM adjudicates). Tokens placed via `POST /character/{id}/place-token` in the proven (700, 700) demo-map region (70 px cell = 5 ft).

| Test | What it asserts |
|------|-----------------|
| `test_prone_target_within_5ft_grants_advantage` | Tavik prone + Pip's token 1 cell away → `2d20kh1` + `roll_state_applied = "advantage_target_prone_within_5ft"`. |
| `test_prone_target_beyond_5ft_imposes_disadvantage` | Tavik prone + Pip 4 cells away (20 ft) → `2d20kl1` + `"disadvantage_target_prone_beyond_5ft"`. |
| `test_prone_target_offgrid_no_edge` | Both tokens deleted → prone target alone yields a straight `1d20` with no roll state (the None fallback contract). |
| `test_prone_adv_cancels_attacker_prone_dis` | Pip also prone (Phase 2a attacker dis) while adjacent → PHB p.173 cancel: single `1d20` + `canceled_*`; cleanup reseeds the battle buff-free. |
...
| `test_polymorph_present_in_catalog` | Phase 4 — catalog anchor: 4th-level Transmutation, "Up to 1 hour" duration, WIS save, and `concentration: true` (RAW hour-long concentration; catalog corrected from the old `false`, B16). |
| `test_cast_binds_polymorph_concentration` | Phase 4 — `/cast_polymorph` returns `concentration: true` + installs the `concentration-polymorph` caster anchor (RAW hour-long concentration; catalog now agrees). Renamed from `test_cast_binds_concentration_despite_catalog_flag` in v2.1033.4 once the catalog `concentration:false` divergence was corrected (B16). |
...
### `test_disengage_oa.py`
v2.698.0 — `/token/move` now honors a mover's `effects.disengage: True` buff (Step of the Wind / Drunken Technique / Cunning Action): a disengaged mover provokes no OAs for the turn (triggers dropped, no 409); effect-keyed, NOT consumed (persists for the turn). Move response gains `disengage_applied`.

| Test | What it asserts |
|------|-----------------|
| `test_disengage_suppresses_oa` | Kael with a `disengage` buff leaves Tavik's reach (no `oa_confirmed`) → 200, `disengage_applied True`, empty `opportunity_attack_triggers`. |
| `test_disengage_not_consumed_second_move` | After a first out-of-reach move, re-position and move out again → still `disengage_applied True` (flag persists, not consumed). |
| `test_move_provokes_oa_without_disengage` | Control: same move without the buff → 409 `oa_confirmation_required`. |

### `test_misty_escape.py`
v2.699.0 — The Archfey Warlock (PHB p.109) Misty Escape Lv 6+ (new endpoint). Once/short-rest reaction: teleport 60 ft + turn invisible. Mechanized by composing the invisible attack-edge substrate (`effects.invisible` buff) + the v2.698.0 disengage substrate (`effects.disengage` buff, so the teleport drag is OA-free). Magnus Hexbinder PATCH'd to The Archfey Lv 6.

| Test | What it asserts |
|------|-----------------|
| `test_use_me_happy` | Battle-seeded Archfey Lv 6 → `teleport_ft == 60`, `invisible_installed`/`disengage_installed` True, `uses_remaining == 0`, broadcast (source `misty-escape`). |
| `test_use_me_out_of_uses` | Second back-to-back use → 409 `no_uses_left`. |
| `test_use_me_wrong_subclass` | Default Magnus (The Fiend) → 409 `wrong_subclass_or_level`. |
| `test_use_me_level_gate` | Archfey at Lv 5 (< 6) → 409 `wrong_subclass_or_level`. |
...
| `test_eldritch_strike_npc_save_disadvantage` | v2.648.7 Phase 3c — an NPC combatant (Bandit Captain, spawned from a real template so the NPC auto-save fires) carrying the mark naming Zara rolls its WIS auto-save vs Hold Person at `2d20kl1`, a `feature_used(source=eldritch-strike)` consume fires, and the marker is dropped from the NPC combatant (`GET /battle`). The NPC twin of the PC read. |
...
v2.99.249 — Oath of Redemption (Paladin subclass, XGE p.39) Rebuke the Violent reaction CD (Phase H.2 FIFTH + FINAL oath). Caelan PATCH'd to Redemption + CD 1/1 + Bandit attacker in battle. DC 14. **v2.672.0 (Phase 8):** the attacker's WIS save is rolled + the reflected psychic applied server-side (full on fail, half on success) when the attacker resolves to a sheet.
...
| `test_rtv_applies_psychic_damage` | Re-seeds a real bandit *template* attacker → `save_total` set, `save_passed` a bool, `psychic_damage_applied == 15` on a fail / `7` on a success (consistent with the rolled outcome). |
...
v2.99.281 — Redemption Paladin (XGE p.39) Aura of the Guardian reactive shield (H.2 depth). Lv 7+: when an ally within 10 ft (30 ft at Lv 18+) takes damage, you reaction-redirect the damage to yourself. Damage can't be reduced. **v2.694.0 (Phase 8):** with `ally_combatant_id` + `damage_amount`, the redirection is resolved server-side — ally healed back + Paladin takes the (untyped, unreducible) damage.

| Test | What it asserts |
|------|-----------------|
| `test_use_aotg_happy_lv7` | Lv 7 Caelan → `radius_ft == 10`, broadcast (source `aura-of-the-guardian`). |
| `test_aotg_redirects_damage` | Wounded NPC ally (10/50) + Caelan at 50 HP, redirect 8 → `ally_healed == 8`, `paladin_damage_applied == 8`, `redirected is True`. |
| `test_aotg_no_redirect_announce_only` | No redirect args → `redirected is False`, `ally_healed`/`paladin_damage_applied` None. |
...
v2.99.283 — Ancients Paladin (PHB p.87) Undying Sentinel "drop to 1 HP, not 0" (H.2 deeper, Lv 15). Once-per-long-rest. Auto-bootstraps an `undying-sentinel` resource if missing; refilled by the generic long-rest resource hook. **v2.693.0 (Phase 8):** the drop-to-1-HP mutation is applied server-side — the caster is brought to exactly 1 HP via `_apply_heal_to_combatant` (flips dying → alive).

| Test | What it asserts |
|------|-----------------|
| `test_use_us_happy_lv15` | Lv 15 Caelan → `uses_remaining == 0`, broadcast (source `undying-sentinel`). |
| `test_us_drops_to_one_hp_when_at_zero` | Caelan set to 0 HP via the sheet → endpoint returns `hp_after == 1`, `revived is True`. |
...
v2.99.292 — Redemption Paladin (XGE p.39) Emissary of Redemption Lv 20 passive capstone (H.2 deeper — CLOSES the H.2 Lv 20 batch). Resistance to all damage from creatures + half-damage radiant counter on hit. Both negated against a creature you attack/spell/damage until next long rest. **v2.700.0 (Phase 8):** both halves mechanical — endpoint installs a `resistance_to: ["all"]` buff; an on-damage hook reflects half-damage radiant to attackers (Redemption Lv 20). Per-target caveat GM-narrated.

| Test | What it asserts |
|------|-----------------|
| `test_use_er_happy_lv20` | Lv 20 Caelan (battle seeded) → `resistance_all_creature_damage == True`, `radiant_back_fraction == 0.5`, `resistance_installed is True`, broadcast (source `emissary-of-redemption`). |
| `test_er_reflects_radiant_on_attack` | Pip attacks the Lv 20 Paladin (auto-apply on) → an `emissary-of-redemption` broadcast with `radiant_damage ≥ 1` + `attacker_char_id == Pip` (the on-damage reflect hook). |
...
v2.99.293 — Light Domain Cleric (PHB p.61) Corona of Light Lv 17 capstone (H.1 deeper — opens H.1 Lv 17 batch). Action: bright sunlight 60 ft + dim 30 ft beyond, 1 min, enemies in bright light have disadvantage on saves vs your fire/radiant spells. Action chip. **v2.702.0 (Phase 8):** installs a `corona-of-light` buff; the cleric's fire/radiant spell saves impose disadvantage (2d20kl1) on enemy NPC savers (single-target + AoE `/cast_spell` sites). Distance gate + non-caster spells + light emission GM-narrated.

| Test | What it asserts |
|------|-----------------|
| `test_use_col_happy_lv17` | Lv 17 Tavik (battle seeded) → `bright_light_radius_ft == 60`, `dim_light_radius_ft == 90`, `duration_minutes == 1`, disadvantage types fire+radiant, `aura_installed is True`, broadcast (source `corona-of-light`). |
| `test_col_imposes_disadvantage_on_fire_radiant_save` | Tavik casts Sacred Flame (radiant, DEX save) at an NPC: control without corona → `1d20`; with corona active → the save roll breakdown contains `2d20kl1` (spell index via sheet-json; skips if absent). |
...
v2.99.295 — Life Domain Cleric (PHB p.61) Supreme Healing Lv 17 passive (H.1 deeper). When you would roll dice to restore HP with a spell, use max on each die instead. **v2.143.0** wired the max-dice substitution into the `/cast_spell` target-bound auto-heal path (`_max_dice_total`); **v2.1003.0** closed the last gap — the legacy `/apply_healing` heal-claim (chat-card) path maxes the dice too.
...
| `test_sh_fires_on_cast_spell_heal` | v2.143.0 — targeted Cure Wounds by Lv 17 Life Tavik → `spell_cast` carries `supreme_healing_applied: True` + `💗 Supreme Healing` / `[max:8]` in `auto_heal_breakdown`. |
| `test_sh_fires_on_heal_claim_path` | v2.1003.0 — target-less cast then `/apply_healing` claim → response `supreme_healing_applied: True`, breakdown carries `💗 Supreme Healing` + `[max:8]`, `rolled >= 8`. |
...
v2.99.308 — Mastermind Rogue (XGE p.46) Master of Tactics Lv 3+ (E.3 Rogue batch). Bonus action Help; when helping an ally attack, target can be within 30 ft of you (not 5 ft) if it can see/hear you. Costs bonus chip. **v2.701.0 (Phase 8):** with `ally_combatant_id` + `target_combatant_id`, installs a target-keyed Help advantage buff on the ally → the ally's next attack vs that target rolls 2d20kh1 (the True Strike / Vow read).

| Test | What it asserts |
|------|-----------------|
| `test_use_mt_happy_lv7` | PATCH Pip → Mastermind Lv 7 (no ids) → `help_action_economy == "bonus"`, `help_target_range_ft == 30`, `help_installed is False`, broadcast (source `master-of-tactics`). |
| `test_mt_grants_ally_advantage_vs_target` | Pip helps Garrik vs a templated bandit → `help_installed True`; Garrik's `/attack` vs the bandit has `2d20kh1` in `attack_breakdown`. |
| `test_use_mt_wrong_subclass` | Default Pip (Thief) → 409. |
| `test_use_mt_level_gate` | Mastermind at Lv 2 → 409. |

### `test_skirmisher.py`
v2.99.309 — Scout Rogue (XGE p.46) Skirmisher Lv 3+ (E.3 Rogue batch). Reaction to move up to half speed when enemy ends turn within 5 ft; no OA. Costs reaction chip. **v2.696.0 (Phase 8):** installs a `skirmisher-bonus-move` free-move buff that rides the generalized (effect-keyed) `/token/move` free-move read site — next move is cap-exempt + OA-free.

| Test | What it asserts |
|------|-----------------|
| `test_use_sk_happy_lv7` | PATCH Pip → Scout Lv 7 (halfling speed 25) → `bonus_move_ft == 12`, `no_oa == True`, `buff_installed is True`, broadcast (source `skirmisher`). |
| `test_sk_buff_rides_generalized_free_move_substrate` | Pip at 25-ft cap + skirmisher buff → +5-ft over-cap move returns 200 + `relentless_avenger_applied == True` (the generic free-move flag); control without the buff 409s `over_speed_cap`. Proves the read site is effect-keyed, not RA-specific. |
...
v2.99.317 — Spores Druid (TCE p.36) Halo of Spores Lv 2+ (E.4 Druid batch). Reaction when creature moves into or starts turn within 10 ft → necrotic damage on failed CON save. Die scales 1d4/1d6/1d8/1d10 at Lv 2/6/10/14. **v2.676.0 (Phase 8):** with `target_combatant_id`, the CON save is rolled + the necrotic applied server-side — save-OR-NOTHING (full on fail, 0 on success).
...
| `test_hs_applies_damage_to_target` | Templated bandit target → `save_total`/`save_passed` set, `damage_applied == damage_rolled` on a fail / `0` on a success. |
| `test_hs_no_target_announce_only` | No `target_combatant_id` → `save_total`/`save_passed`/`damage_applied` stay None. |
...
v2.99.318 — Wildfire Druid (TCE p.38) Summon Wildfire Spirit Lv 2+ (E.4 Druid batch). Action + Wild Shape (default) or Lv 2+ spell slot to summon Wildfire Spirit for 1 hour. **v2.1005.0 (Phase 8):** the spirit stands up as a REAL combatant via `_summon_companion` (token + battle-state entry, `is_summon`/`summoned_by` tags, HP = 5 + 5 × druid level, caster's init slot), surfacing `summon_combatant_id`/`summon_token_id`; happy-path tests dismiss the summon on teardown.

| Test | What it asserts |
|------|-----------------|
| `test_use_ws_happy_lv5_default` | PATCH Mira → Wildfire Lv 5 default → `resource_used == "wild-shape"`, `duration_minutes == 60`, broadcast (source `summon-wildfire-spirit`); dismisses the summon. |
| `test_use_ws_slot_variant` | slot_level 3 → `resource_used == "spell-slot"`, `slot_level == 3`; dismisses the summon. |
| `test_ws_summon_lands_in_battle_state` | v2.1005.0 Phase 9 state contract — the `battle_update` broadcast carries the summon: `is_summon`, `summoned_by == Mira`, name contains "Wildfire Spirit", `hp_max == 30` at Lv 5, init == the caster's 12. |
...
v2.99.319 — Dreams Druid (XGE p.23) Balm of the Summer Court Lv 2+ (E.4 Druid batch). Pool of d6 fey energy = druid_lv dice. Bonus action: spend up to half-druid-lv dice → ally within 120 ft heals total + 1 temp HP per die. Pool refills on long rest. **v2.679.0 (Phase 8):** with `target_combatant_id`, both the heal (`_apply_heal_to_combatant`) and the per-die temp HP (`_grant_temp_hp`) are applied to the ally server-side.
...
| `test_bsc_applies_heal_and_temp_hp` | Wounded NPC at 10/50 HP, 2 dice → `heal_applied == heal_amount`, `temp_hp_applied == temp_hp`, `revived == False`. |
| `test_bsc_no_target_announce_only` | No `target_combatant_id` → `heal_applied`/`temp_hp_applied`/`revived` stay None. |
...
### `test_combat_wild_shape.py`
v2.99.348 — Moon Druid (PHB p.69) Combat Wild Shape Lv 2+ (E.4 Druid batch close). Bonus-action Wild Shape announce + in-beast-form heal (`<slot_level>d8`). **v2.1002.0 (Phase 8):** the heal mode now applies the rolled HP to the druid via `_apply_heal_to_combatant` (self-apply; caps at max HP, revives), surfacing `heal_applied`/`hp_after`/`revived`; transform + slot spend + "while transformed" gate stay GM-tracked.

| Test | What it asserts |
|------|-----------------|
| `test_use_cws_transform_happy` | No `slot_level` → mode `transform`, `heal_amount == 0`, broadcast (source `combat-wild-shape`). |
| `test_use_cws_heal_happy` | `slot_level: 2` → mode `heal`, `heal_amount` in [2, 16], broadcast carries the roll. |
| `test_use_cws_heal_applies_hp` | v2.1002.0 Phase 9 state contract — Mira damaged to max−20, Lv-2-slot heal → `heal_applied == heal_amount`, and the sheet's `hp.current` rises by exactly `heal_applied` (`hp_after` echoes it). |
| `test_use_cws_wrong_subclass` | Circle of the Land → 409 `wrong_subclass_or_level`. |
| `test_use_cws_wrong_class` | Caelan (Paladin) → 409. |
...
v2.99.321 — Glamour College Bard (XGE p.16) Mantle of Inspiration Lv 3+ (F.1 Bard batch). Bonus action + 1 BI use → up to CHA-mod (min 1) allies within 60 ft each gain 5+bard_lv temp HP + immediate reaction-move at full speed without provoking OAs. **v2.671.0 (Phase 8):** temp HP now applied server-side via `_grant_temp_hp` to each `target_combatant_ids` ally (capped at CHA-mod); free reaction-move stays GM-narrated.
...
| `test_mi_applies_temp_hp_to_allies` | Two fresh NPC ally combatants named in `target_combatant_ids` → `targets_buffed == 2`, each `applied_targets[*].temp_hp_applied == 11`. |
| `test_mi_caps_targets_at_cha_mod` | 5 ids supplied but CHA-mod is 3 → `targets_buffed == 3` (extras dropped). |
| `test_mi_no_target_announce_only` | No `target_combatant_ids` → backward-compatible: `targets_buffed == 0`, `applied_targets == []`. |
...
v2.99.325 — Spirits College Bard (TCE p.30) Tales from Beyond Lv 3+ (F.1 Bard batch). Bonus action to roll 1d6 on Spirit Tales table; action to apply tale (6 tales: Clever Animal/Duelist/Beloved Friends/Brute/Tragic Romance/Traveler). `force_tale` body param (1-6) is a TEST_MODE escape hatch. **v2.695.0 (Phase 8):** with a `target_combatant_id`, the 4 mechanizable tales resolve server-side (3/6 temp HP, 4 Brute STR-save Prone + 2d10 force, 5 Tragic Romance WIS-save Charmed); tales 1/2 GM-narrated.
...
| `test_tb_tale3_grants_temp_hp` | force_tale 3 + target → `applied.kind == "temp_hp"`, `temp_hp_granted == temp_hp_amount` (∈ [8,18]). Skips if TEST_MODE off. |
| `test_tb_tale4_brute_save_and_damage` | force_tale 4 + templated target → STR `feature_save` resolved; on a fail 2d10 force applied + Prone, on a pass no damage. Skips if TEST_MODE off. |
| `test_tb_tale5_charm_save` | force_tale 5 + templated target → WIS `feature_save`; `condition_installed == (not passed)` with `condition_key == "charmed"`. Skips if TEST_MODE off. |
| `test_tb_no_target_announce_only` | No `target_combatant_id` → `applied` is None. |
...
v2.99.274 — Vengeance Paladin (PHB p.87) Abjure Enemy CD (H.2 depth). Sibling CD to Vow of Enmity (v2.99.246). Single target Wis save DC 8 + prof + CHA; fail Frightened + speed 0 or success speed halved (both end on damage). **v2.681.0 (Phase 8):** the WIS save is resolved via `_resolve_feature_save` (NPC inline, PC via RollRequest) + Frightened installed on a fail.

| Test | What it asserts |
|------|-----------------|
| `test_use_ae_happy` | Vengeance Caelan adjures Bandit → `save_dc == 14`, `uses_remaining == 0`, broadcast. |
| `test_ae_resolves_save_and_installs_frightened` | Templated bandit target → `feature_save.resolved is True`, `condition_installed == (not passed)`, `condition_key == "frightened"` on a fail. |
...
v2.99.272 — Ancients Paladin (PHB p.87) Turn the Faithless CD (H.2 Phase 2). Sibling CD to Nature's Wrath (v2.99.245). 30-ft AOE Wis save vs fey/fiend or Turned 1 minute. **v2.682.0 (Phase 8):** each target's WIS save is resolved via `_resolve_feature_save` (AoE loop; NPC inline, PC via RollRequest) + Turned installed on a fail.

| Test | What it asserts |
|------|-----------------|
| `test_use_ttf_happy` | Ancients Caelan vs Imp + Sprite → `save_dc == 14`, `uses_remaining == 0`, broadcast (source `turn-the-faithless`). |
| `test_ttf_resolves_saves_and_installs_turned` | Two templated bandit targets → each `feature_saves` entry `resolved`, `condition_installed == (not passed)`, `condition_key == "turned"` on a fail. |
...
v2.99.245 — Oath of the Ancients (Paladin subclass, PHB p.86) Nature's Wrath Channel Divinity (Phase H.2 Phase 1 of [docs/plans/paladin-oaths.md](../plans/paladin-oaths.md)). Sir Caelan Lightbringer is the demo fixture; tests PATCH his subclass to "Oath of the Ancients" and seed Bandit in battle. Caelan Lv 8 CHA 16: DC = 8 + prof 3 + CHA +3 = 14. **v2.680.0 (Phase 8):** the target's STR/DEX save is resolved via `_resolve_feature_save` (NPC inline, PC via RollRequest) + Restrained installed on a fail.
...
| `test_nw_resolves_save_and_installs_restrained` | Templated bandit target → `feature_save.resolved is True`, `passed` a bool, `condition_installed == (not passed)`, `condition_key == "restrained"` on a fail. |
...
v2.99.235 — Tempest Domain Cleric (PHB p.62) Wrath of the Storm reaction (Phase H.1 second domain). Brother Tavik Stonebrow is the demo fixture; tests PATCH his subclass to "Tempest Domain" + seed a `wrath-of-the-storm` resource. **v2.673.0 (Phase 8):** with `attacker_combatant_id`, the attacker's DEX save is rolled + the 2d8 applied server-side (full on fail, half on success).
...
| `test_wots_applies_damage_to_attacker` | Re-seeds a real bandit *template* attacker → `save_total`/`save_passed` set, `damage_applied == 2d8` on a fail / half on a success. |
| `test_wots_no_combatant_announce_only` | Free-form `attacker_name` (no combatant id) → `save_total`/`save_passed`/`damage_applied` stay None. |
...
v2.99.270 — Light Domain Cleric Lv 8 (PHB p.60) Potent Spellcasting (H.1 depth — first Lv 6/8/17 feature for an already-shipped Cleric domain). Tavik Lv 8 WIS 16 → +3 mod. **v2.612.1** turned the endpoint into a `potent-spellcasting-active` flag-buff install; **v2.1004.0** shipped the Phase-2 read site — `_potent_spellcasting_bonus` adds +WIS to one cantrip damage roll across the `/cast_spell` + `/place_aoe` damage paths.
...
| `test_ps_buff_payload_carries_wis_mod_and_class_flags` | v2.612.1 state contract — the endpoint installs the permanent buff with `potent_spellcasting_active/wis_mod/class` effect keys; cleaned up via `end_buff`. |
| `test_ps_adds_wis_to_cantrip_damage_on_cast` | v2.1004.0 read site — buff installed, Sacred Flame cast at an NPC bandit → `feature_used(source=potent-spellcasting-bonus)` fires with `+3` in the label. |
...
| `test_es_auto_installs_on_weapon_hit` | v2.648.5 — Garrik (EK Lv 10) attacks Pip via `/attack` (override) until a hit → the `eldritch-strike-target` marker auto-installs on Pip + a `feature_used(source=eldritch-strike)` fires, with **no** `/use_eldritch_strike` call (RAW auto-install via the shared `_install_eldritch_strike` helper). |
| `test_es_does_not_auto_install_for_non_ek` | v2.648.5 — default Champion Garrik's weapon hit installs **no** Eldritch Strike marker (gate is EK Lv 10+). |
...
| `test_war_magic_advisory_on_ek_cantrip` | v2.648.6 — an EK Lv 7+ casting a cantrip (Fire Bolt) via `/cast_spell` fires a `feature_used(source=war-magic-advisory)` (the missing proactive surface; resolution still via `/use_war_magic`). |
| `test_war_magic_advisory_skips_non_ek` | v2.648.6 — default Champion Garrik casting the same cantrip gets **no** War Magic advisory (gate is EK Lv 7+). |
| `test_war_magic_bonus_attack_marks_bonus_slot` | v2.648.8 — an EK Lv 7+ attacking with `as_war_magic_bonus: true` → an `economy_update` for the **bonus** slot, and **not** the action (the bonus-action weapon attack rides `/attack` directly). |
| `test_war_magic_bonus_attack_ignored_for_non_ek` | v2.648.8 — a Champion's `as_war_magic_bonus` does **not** mark the bonus slot (EK Lv 7+ gate holds). |
...
### `test_hands_of_healing.py`
v2.99.358 — Way of Mercy Monk (TCE p.51) Hands of Healing Lv 3+ (Phase G Monk Ways batch). Action + 1 ki → heal a touched creature for Martial Arts die + WIS mod. Kael Brightleaf PATCH'd to Way of Mercy Lv 7 (MA die 1d6). **v2.674.0 (Phase 8):** the heal is applied to a `target_combatant_id` via `_apply_heal_to_combatant` (caps at max HP, revives a dying PC); no target → announce-only.

| Test | What it asserts |
|------|-----------------|
| `test_use_hh_happy_lv7` | Lv 7 Way of Mercy → `martial_arts_die == "1d6"`, `die_roll` 1..6, `heal_amount == max(1, die_roll + wis_mod)`, `ki_spent == 1`, broadcast. |
| `test_hh_applies_heal_to_target` | Wounded NPC at 10/50 HP (40 headroom) → `heal_applied == heal_amount`, `revived == False`. |
| `test_hh_no_target_announce_only` | No `target_combatant_id` → `heal_applied`/`revived` stay None. |
| `test_use_hh_wrong_subclass` | Default Kael (Way of the Open Hand) → 409. |
| `test_use_hh_wrong_class` | Caelan (Paladin) → 409. |

### `test_tentacle_of_the_deeps.py`
v2.99.354 — The Fathomless Warlock (TCE p.70) Tentacle of the Deeps Lv 1+ (Phase G Warlock batch). Bonus action: summon a tentacle + make a melee spell attack vs a creature within 10 ft → 1d8 cold (2d8 at Lv 10) + speed -10 ft. PB summons per long rest (server-tracked via `tentacle_uses`). Magnus Hexbinder PATCH'd to The Fathomless Lv 5. **v2.678.0 (Phase 8):** with `target_combatant_id`, the melee spell attack is rolled server-side vs the target's AC (`_read_target_ac`; nat 20 crits, nat 1 misses) + the cold applied on a hit.

| Test | What it asserts |
|------|-----------------|
| `test_use_td_happy_lv5` | Lv 5 → `damage_dice == "1d8"`, `cold_damage` 1..8, `reach_ft == 10`, `summon_range_ft == 60`, `uses_max == 3` (PB), `uses_remaining == 2`. |
| `test_td_resolves_attack_and_applies_cold` | Templated bandit → `target_ac` set, `attack_total == attack_nat + attack_bonus`, `damage_applied == cold_damage` on a non-crit hit / ≥ on a crit / `0` on a miss. |
| `test_td_no_target_announce_only` | No `target_combatant_id` → `attack_nat`/`attack_total`/`hit`/`damage_applied` stay None. |
| `test_use_td_out_of_uses` | `tentacle_uses: 0` → 409 `out_of_uses`. |
| `test_td_long_rest_refill` | Exhaust → `/rest` long → refills to PB (3). |
| `test_use_td_wrong_subclass` | Default Magnus (The Fiend) → 409. |
| `test_use_td_wrong_class` | Caelan (Paladin) → 409. |

### `test_healing_light.py`
v2.99.353 — The Celestial Warlock (XGE p.54) Healing Light Lv 1+ (Phase G Warlock batch). Bonus action: spend up to CHA-mod d6s from a pool (= 1 + warlock level) to heal a creature within 60 ft. Magnus Hexbinder PATCH'd to The Celestial Lv 5 (pool 6). **v2.675.0 (Phase 8):** the rolled HP is applied to a `target_combatant_id` via `_apply_heal_to_combatant` (caps at max HP, revives a dying PC); no target → announce-only.

| Test | What it asserts |
|------|-----------------|
| `test_use_hl_happy_lv5` | Lv 5, 1 die → `heal_amount` 1..6, `max_dice == 6`, `dice_remaining == 5`, `max_range_ft == 60`, broadcast. |
| `test_hl_applies_heal_to_target` | Wounded NPC at 10/50 HP (40 headroom) → `heal_applied == heal_amount`, `revived == False`. |
| `test_hl_no_target_announce_only` | No `target_combatant_id` → `heal_applied`/`revived` stay None. |
| `test_use_hl_wrong_subclass` | Default Magnus (The Fiend) → 409. |
| `test_use_hl_wrong_class` | Caelan (Paladin) → 409. |
...
v2.99.254 — Battle Master maneuver 4 of 16 — Pushing Attack (PHB p.74). Save ability STR; on-fail effect "pushed up to 15 ft." **v2.99.433:** STR save → 15-ft push resolved server-side via `_force_move`. **v2.691.0 (Phase 8):** the superiority die also lands as bonus damage via `_apply_damage_to_combatant`.

| Test | What it asserts |
|------|-----------------|
| `test_use_pa_happy` | Lv 9 Garrik (d8) → `extra_damage` 1..8, `save_dc == 16`, `save_ability == "STR"`, `push_max_ft == 15`, dice 4 → 3, broadcast. |
| `test_pa_applies_bonus_damage` | Templated bandit target → `damage_applied == extra_damage`, `save_resolved is True`. |
| `test_pa_push_moves_target_on_failed_save` | Garrik above Pip on a grid → loops until a failed STR save → `push_applied is True` and Pip's token moved +210 px (15 ft / 3 cells); a passed save leaves the token in place. |
...
v2.99.252 — Battle Master maneuver 2 of 16 — Disarming Attack (PHB p.74). Mirrors Trip Attack's shape; on-fail effect is "drop one held object" vs Prone. **v2.692.0 (Phase 8):** with a `target_combatant_id` (trust-the-caller), the superiority die lands as bonus damage + the STR save resolves server-side (no condition — drop-object GM-narrated).
...
| `test_da_applies_damage_and_resolves_save` | Templated bandit target → `damage_applied == extra_damage`, `feature_save.resolved`, `condition_installed is False` (no engine condition for drop-object). |
| `test_da_no_target_announce_only` | No `target_combatant_id` → `damage_applied`/`feature_save` stay None. |
...
### `test_trip_attack.py`
v2.99.233 — Battle Master Fighter (PHB p.74) Combat Superiority pool + Trip Attack maneuver (Phase 1 of [docs/plans/battle-master.md](../plans/battle-master.md)). Garrik Ironside is the demo fixture; tests PATCH his subclass to "Battle Master" + seed a `superiority-dice` resource. **v2.690.0 (Phase 8):** with a `target_combatant_id` (trust-the-caller), the superiority die lands as bonus damage + the STR save → Prone resolves server-side.
...
| `test_ta_applies_damage_and_resolves_prone` | Templated bandit target → `damage_applied == extra_damage`, `feature_save.resolved`, `condition_installed == (not passed)`, `condition_key == "prone"` on a fail. |
| `test_ta_no_target_announce_only` | No `target_combatant_id` → `damage_applied`/`feature_save` stay None. |
...
### `test_tempestuous_magic.py`
v2.99.339 — Storm Sorcery Sorcerer (PHB p.137) Tempestuous Magic Lv 1+. Bonus action to fly 10 ft without provoking OAs after casting a Lv 1+ spell. Zara Emberfire PATCH'd to Storm Sorcery. **v2.697.0 (Phase 8):** installs a `tempestuous-magic-fly` free-move buff riding the generalized `/token/move` free-move read; next ≤10-ft move is cap-exempt + OA-free.

| Test | What it asserts |
|------|-----------------|
| `test_use_tm_happy_lv5` | Lv 5 Storm Sorcery (battle seeded) → `fly_distance == 10`, `oa_free is True`, `buff_installed is True`, broadcast (source `tempestuous-magic`). |
| `test_tm_buff_rides_generalized_free_move_substrate` | Zara at 30-ft cap + the buff → +5-ft over-cap move returns 200 + `relentless_avenger_applied == True`; control without the buff 409s `over_speed_cap`. |
| `test_use_tm_wrong_subclass` | Default Zara (Draconic Bloodline) → 409. |
| `test_use_tm_wrong_class` | Caelan (Paladin) → 409. |

### `test_storm_aura.py`
v2.99.367 — Path of the Storm Herald Barbarian (XGE p.10) Storm Aura Lv 3+. A 10-ft aura while raging: Desert (each other creature takes fire), Sea (one creature DEX-saves or takes lightning), Tundra (one creature gains temp HP). Krieger Stonefist PATCH'd to Storm Herald Lv 7. Desert installs an auto-tick aura (v2.99.426). **v2.677.0 (Phase 8):** Sea + Tundra single-target choices resolve server-side with a `target_combatant_id`.

| Test | What it asserts |
|------|-----------------|
| `test_use_sa_happy_desert` | Default → `environment == "desert"`, `fire_damage == 2`, `aura_radius_ft == 10`, broadcast. |
| `test_use_sa_happy_sea` | `environment: "sea"` → `lightning_dice == "1d6"`, `lightning_damage` 1..6, `save == "dex"`. |
| `test_sa_desert_installs_aura_and_ticks_fire` | v2.99.426 — desert installs the `storm-aura` aura buff; advancing the turn ticks 2 fire to each other creature (30 → 28). |
| `test_sa_sea_applies_lightning_to_target` | v2.677.0 — templated bandit target → `save_total`/`save_passed` set, `damage_applied` == full on fail / half on success. |
| `test_sa_tundra_applies_temp_hp` | v2.677.0 — NPC target, tundra → `temp_hp == 2`, `temp_hp_applied == 2`. |
| `test_sa_sea_no_target_announce_only` | v2.677.0 — sea, no target → `save_total`/`damage_applied`/`temp_hp_applied` stay None. |
| `test_use_sa_wrong_subclass` | Default Krieger (Berserker) → 409. |
| `test_use_sa_invalid_environment` | `environment: "jungle"` → 400. |

### `test_berserker_path.py`
v2.99.226 — Path of the Berserker (PHB p.49) Frenzy (Lv 3) + Intimidating Presence (Lv 10). Krieger Stonefist is the demo fixture; IP test PATCHes him Lv 7 → 10. **v2.683.0 (Phase 8):** Intimidating Presence resolves the target's WIS save via `_resolve_feature_save` (NPC inline, PC via RollRequest) + installs Frightened on a fail when a `target_combatant_id` is supplied.
...
| `test_ip_resolves_save_and_installs_frightened` | Krieger Lv 10 + templated bandit (with `target_combatant_id`) → `feature_save.resolved is True`, `condition_installed == (not passed)`, `condition_key == "frightened"` on a fail. |
...
| `test_cloak_suppressed_after_taking_damage` | v2.645.0 — RAW DMG p.158 suppression clause. Garrik attacks Lyra (AC 1 → hits land): attack #1 still rolls `disadvantage_cloak_of_displacement` (roll_state computed pre-damage), the hit fires `feature_used(source=cloak-displacement-suppressed)` + stamps `cloak_disp_suppressed_round`, and a later same-round attack rolls straight (no `cloak_of_displacement` in `roll_state_applied`). Lyra long-rested first so a leftover unconscious state can't add canceling advantage. |
...
| `test_perception_to_see_wearer_has_disadvantage` | v2.1055.0 — `POST /roll` (Perception, `vs_character_id` = the cloak wearer) → breakdown contains `2d20kl1` + `roll_state_applied` starts `auto_disadvantage_…cloak_of_elvenkind`. |
| `test_perception_without_target_is_straight` | Control: Perception with no `vs_character_id` → straight roll, no disadvantage. |
| `test_non_perception_vs_wearer_is_straight` | Control: an Investigation check vs the wearer does NOT pick up the cloak disadvantage (Perception-only gate). |
| `test_perception_vs_non_wearer_is_straight` | Control: Perception vs a non-wearer → straight roll. |
...
| Test | What it asserts |
|------|-----------------|
| `test_climbing_grants_advantage_on_str_check` | Garrik drinks Climbing (`buff_key: "climbing"`, `buff_installed: True`), then a `/roll` STR check broadcasts a `roll` whose dice token is `2d20kh1` (advantage). Asserts on the dice token (`_dice_part`, ignoring the `+1` from Garrik's Stone of Good Luck), not the full expression — B18 class 6. |
| `test_no_climbing_str_check_is_plain` | Control: without Climbing, Garrik's STR-check dice token is `1d20` (no advantage) — proving the advantage comes from the potion, not something innate. |
...
| Test | What it asserts |
|------|-----------------|
| `test_diminution_imposes_disadvantage_on_str_check` | Garrik drinks Diminution (`buff_key: "diminution"`, `buff_installed: True`), then a `/roll` STR-check dice token is `2d20kl1` (disadvantage). Asserts on the dice token (`_dice_part`, ignoring Garrik's Luckstone `+1`) — B18 class 6. |
| `test_no_diminution_str_check_is_plain` | Control: without Diminution, Garrik's STR-check dice token is `1d20` (no disadvantage) — proving the disadvantage comes from the potion, not something innate. |
...
### `test_stats_capture.py`
v2.650.0 — per-campaign statistics capture, Phase 1 (Hook A). The damage funnel `_apply_damage_to_combatant` logs `damage_dealt`/`damage_taken`/`ko` to the new `campaign_stat_events` table (schema v81). Verified by a direct DB query (the `docker compose exec db psql` precedent) since the read API lands in Phase 2.

| Test | What it asserts |
|------|-----------------|
| `test_pc_hit_on_npc_logs_damage_dealt` | Garrik hits an NPC dummy via `/attack` until a damaging hit → a new `damage_dealt` stat row for Garrik (baseline-delta DB count) + the attack endpoint still 200s (no regression from the capture hook). |

### `test_stats_api.py`
v2.651.0 — per-campaign statistics read API, Phase 2. `GET /api/campaign/{id}/stats` aggregates the event log; capture Hooks B (cast-heal), C (`spell_cast`), D (`attack`) feed it. Visibility: GM sees everyone; a non-GM sees only their own characters.

| Test | What it asserts |
|------|-----------------|
| `test_stats_reflects_attack_and_cast` | A GM-scoped attack (Garrik) + cast (Zara) bump `damage_dealt`/`attacks` and `spells_cast`/`top_spells` respectively (baseline-delta via the API). |
| `test_stats_player_sees_only_own` | A non-GM (Alice) gets `scope="self"` and never a GM-owned character's stats — even when passing that `character_id` (the visibility gate). |
| `test_stats_unknown_campaign_404` | Unknown campaign → 404. |
| `test_stats_captures_lay_on_hands_healing` | v2.652.1 — a Paladin's Lay on Hands heal feeds `heal_done` (Caelan long-rests, Pip damaged below max, LoH heal → Caelan's `heal_done` increases; Pip's HP restored). |

### `test_stats_page.py`
v2.652.0 — the `/campaign/{id}/stats` page (Phase 3): a shell that fetches the Phase-2 API client-side. Gated by `_user_can_view_campaign`; the GM gets a roster `<select>`, a player doesn't.

| Test | What it asserts |
|------|-----------------|
| `test_stats_page_renders_for_gm` | GM GET → 200 + the `id="stats-pc"` roster switcher. |
| `test_stats_page_renders_for_member_without_switcher` | A player (Alice) → 200 with **no** switcher. |
| `test_stats_page_unknown_campaign_404` | Unknown campaign → 404. |

### `test_use_metamagic_quickened.py`
v2.649.0 — Sorcerer Lv 3+ Quickened Spell metamagic (the 8th + final PHB metamagic). New endpoint `/use_metamagic_quickened_spell` spends 2 SP + installs a one-round `metamagic-quickened-pending` buff; the next `/cast_spell` of a 1-action spell reads it via `_caster_has_quickened_pending`, retargets the economy slot to `bonus` (so both the over-budget gate + the post-cast `_mark_battle_economy` consume the bonus slot, not the action), drops the buff, and broadcasts a `metamagic-quickened-spell-consumed` card. Built on the v2.643.0–v2.648.8 economy-slot-retarget plumbing. Demo subject: Zara.

| Test | What it asserts |
|------|-----------------|
| `test_quickened_arms_buff_and_decrements_sp` | `/use_metamagic_quickened_spell` → 200, `sp_cost==2`, `sp_remaining==sp_max-2`, a `metamagic-quickened-spell-armed` broadcast, and the `metamagic-quickened-pending` buff installed on Zara. |
| `test_quickened_cast_uses_bonus_slot_and_consumes` | Arm Quickened → cast Hold Person (1-action) → an `economy_update` for the **bonus** slot (and **not** the action), a `metamagic-quickened-spell-consumed` broadcast, and the pending buff dropped. |
| `test_quickened_wrong_class` | A Rogue (Pip) → 409 `wrong_class`. |
...
### `test_theme_burnt_manuscript.py`
v2.1021.0 — Burnt Manuscript theme (near-black + faded sepia; completes the "darker sepia themes" set alongside the existing `sepia` + `hearthstone`).

| Test | What it asserts |
|------|-----------------|
| `test_burnt_manuscript_theme_accepted` | `POST /api/settings/theme {burnt-manuscript}` → 200 (in `VALID_THEMES`); restores to dark. |
| `test_invalid_theme_rejected` | An unknown theme → 400. |
| `test_burnt_manuscript_css_block_present` | `/static/style-fantasy-themes.css` contains the `[data-theme="burnt-manuscript"]` block. |
| `test_burnt_manuscript_picker_card_present` | `/settings` renders the Burnt Manuscript picker card. |

### `test_bulk_map_upload.py`
v2.1024.0 — Bulk map upload. `POST /campaign/{cid}/settings/maps/bulk` creates a Map per uploaded file (name from filename), shared grid settings, bad files skipped-not-fatal. GM-only.

| Test | What it asserts |
|------|-----------------|
| `test_bulk_upload_creates_multiple` | 3 PNGs → `created` 3, names derived (`goblin-cave.png` → "Goblin Cave"); cleaned up. |
| `test_bulk_upload_skips_bad_files` | PNG + a text file → `created` 1 / `skipped` 1, the bad file named in `errors`. |
| `test_bulk_upload_gm_only` | A player → 403. |
| `test_bulk_upload_no_files` | No images → 400/422. |

### `test_generate_map.py`
v2.1048.0 — Procedural map generator. `POST /campaign/{cid}/settings/maps/generate` draws a seeded map server-side with Pillow and creates a Map row from it. Returns `{ok, map: {id, name, image_url, walls, doors}}`. GM-only. v2.1049.0 added functional walls: the generated map populates `Map.walls` (Maps 2.0 LOS format) with solid segments + toggleable door segments. v2.1050.0 added a `biome` picker (dungeon / cave / wilderness / tavern).

| Test | What it asserts |
|------|-----------------|
| `test_generate_creates_map` | 200 + `{ok, map}` shape, `image_url` under `/static/uploads/maps/…png`, `walls`/`doors` counts > 0, and the PNG is actually served (`\x89PNG` magic); cleaned up. |
| `test_generate_populates_functional_walls` | The generated map exposes walls via `GET …/map/{id}/walls` (each segment has `id`+`x1/y1/x2/y2`), and a generated door (`door:true, open:false`) flips to `open:true` via `POST …/door/{id}/toggle`. |
| `test_generate_all_biomes` | Each of dungeon/cave/wilderness/tavern → 200, `walls` > 0, name `"Generated {Biome}"`. |
| `test_generate_bad_biome` | Unknown `biome` → 400. |
| `test_generate_density_changes_map` | Sparse vs dense (same biome+seed) → different served images; out-of-range `density` clamps to 200. |
| `test_generate_custom_size` | `size=custom` cols=50/rows=40 → served PNG is exactly `50*70×40*70` px (parsed from PNG IHDR). |
| `test_generate_default_name` | No name → `"Generated Dungeon"`. |
| `test_generate_is_deterministic` | Same `seed` twice → byte-identical served images. |
| `test_generate_bad_size` | Unknown `size` → 400. |
| `test_generate_gm_only` | A player → 403. |

### `test_npc_death_saves.py`
v2.1053.0 — NPC death saves (Phase 4a). `POST /api/campaign/{cid}/set_npc_death_saves {combatant_id, rolls_death_saves}` (GM) flags a boss NPC to roll death saves at 0 HP; the NPC branch of `_apply_damage_to_combatant` runs `_npc_death_save_transition` and broadcasts `npc_death_save`. State lives on the combatant dict, carried forward across the `/battle` PUT. v2.1054.0 (Phase 4b): a dying flagged boss auto-rolls its death save on turn-start (`_npc_death_save_roll` in `update_battle`).

| Test | What it asserts |
|------|-----------------|
| `test_toggle_sets_and_clears` | Toggle on → `rolls_death_saves` + `death_saves: alive` on the combatant (GET `/battle`); toggle off clears both. |
| `test_toggle_gm_only` | A player → 403. |
| `test_toggle_rejects_pc` | Toggling a PC combatant → 400. |
| `test_toggle_unknown_combatant` | Unknown combatant id → 404. |
| `test_boss_enters_dying_at_zero` | A flagged boss dropped to 0 (Magic Missile) → `death_saves.status == "dying"`, hp 0, and an `npc_death_save` WS event with `status: dying`. |
| `test_unflagged_npc_drops_inert` | Control: an unflagged NPC at 0 HP has no `death_saves` (unchanged behavior). |
| `test_damage_while_dying_ticks_failure` | Damage to a dying boss → `failures == 1`, still `dying`. |
| `test_turn_start_auto_rolls_death_save` | Advancing the turn to a dying boss → `npc_death_save` (`source: turn_start`, `raw`); counters match the rolled d20 (≥10 success / nat 20 wake / nat 1 double-fail / else fail). |
| `test_turn_start_no_roll_for_alive_boss` | A flagged-but-alive boss on its turn → no `npc_death_save`. |

### `test_set_surprised.py`
v2.1056.0 — Surprise-state model. `POST /api/campaign/{cid}/set_surprised {combatant_ids, surprised}` (GM) sets a `surprised` flag on combatants; `/attack` reads it via `_target_is_surprised` for the Assassin auto-crit; the turn-advance hook clears it on the target's first turn.

| Test | What it asserts |
|------|-----------------|
| `test_set_surprised_marks_and_clears` | Toggle on → `surprised: true` on the combatant (GET `/battle`); off → cleared. |
| `test_set_surprised_gm_only` | A player → 403. |
| `test_set_surprised_no_ids` | No `combatant_ids` → 400. |
| `test_set_surprised_unknown_skipped` | Unknown id → 200, reported in `skipped`. |
| `test_assassinate_auto_crit_from_server_flag` | An Assassin hitting a server-flagged surprised target auto-crits (`is_crit: true`) with no client `target_surprised`. |
| `test_surprise_clears_on_first_turn` | Advancing the turn to the surprised target → its `surprised` flag clears. |

### `test_detect_surprise.py`
v2.1057.0 — Auto-detect surprise. `POST /api/campaign/{cid}/detect_surprise {ambusher_combatant_ids}` (GM) rolls each ambusher's Stealth vs every other combatant's passive Perception; defenders that notice no ambusher get `surprised: true`.

| Test | What it asserts |
|------|-----------------|
| `test_detect_flags_low_perception_defender` | Seeded rogue Stealth beats a bandit's passive Perception (10) → bandit in `surprised` + flag set; ambusher in neither list. |
| `test_detect_consistency` | `threshold == min` ambusher roll; surprised ∪ not_surprised == all non-ambushers; the two are disjoint. |
| `test_detect_gm_only` | A player → 403. |
| `test_detect_no_ambushers` | Empty ambusher list → 400. |

### `test_pin_rule.py`
v2.1023.0 — SRD Reference Phase 2b: pin a rule to the tabletop. `POST /api/campaign/{cid}/pin_rule {slug, type}` (GM) broadcasts `rule_pinned` (name+desc) + stores an ephemeral per-campaign pin; `/unpin_rule` broadcasts `rule_unpinned`; `/pinned_rule` re-fetches for late joiners. `GET /campaign/{cid}/reference` is the campaign-scoped page with the pin UI.

| Test | What it asserts |
|------|-----------------|
| `test_pin_rule_broadcasts_and_persists` | GM pin → 200 + `rule_pinned` broadcast (slug/desc) + `/pinned_rule` echoes it. |
| `test_unpin_rule_clears` | Unpin → 200 + `rule_unpinned` broadcast + `/pinned_rule` null. |
| `test_pin_rule_gm_only` | A player pinning → 403. |
| `test_pin_rule_unknown_slug` | Unknown slug → 404. |
| `test_pin_rule_bad_type` | `type=monsters` (not player-safe) → 400. |
| `test_campaign_reference_page_renders` | `/campaign/{cid}/reference` (GM) → 200 with `CAN_PIN = true` + campaign id. |

### `test_srd_reference.py`
v2.1020.0 — In-app SRD 5.1 rules reference. `GET /reference` (page) + `GET /api/reference/search?type=&q=` over the six player-safe shipped-SRD types (conditions/spells/items/feats/races/backgrounds); shipped-SRD tier only, monsters excluded. Public (SRD is CC-BY).

| Test | What it asserts |
|------|-----------------|
| `test_reference_page_renders` | `/reference` → 200 HTML with the search input + wiki nav. |
| `test_reference_search_all_returns_records` | `type=all` → full-text records (`slug/name/type/type_label/desc/source`), all player-safe types, `source == local-srd`. |
| `test_reference_search_conditions_finds_grappled` | `type=conditions&q=grappled` → the Grappled condition with desc. |
| `test_reference_search_spell_fireball` | `type=spells&q=fireball` → Fireball with desc. |
| `test_reference_search_excludes_monsters` | `type=monsters` → 404 (GM-visibility data). |
| `test_reference_search_unknown_type` | `type=bogus` → 404. |

### `test_thiefs_reflexes.py`
v2.1018.0 — Thief's Reflexes (Thief Rogue Lv 17+, PHB p.98), scoped v1. `POST /use_thiefs_reflexes` validates Thief Lv 17+ + active round-1 battle + not-surprised + in-initiative, then broadcasts the second-turn initiative (base − 10). Pip Quickfingers (demo Rogue) is PATCHed to Lv 17. The full initiative-engine turn is filed in `docs/plans/thiefs-reflexes.md`.

| Test | What it asserts |
|------|-----------------|
| `test_thiefs_reflexes_second_turn_initiative` | Round 1, base init 18 → `second_turn_initiative` 8 + a `thiefs-reflexes` broadcast. |
| `test_thiefs_reflexes_not_first_round` | Round 2 → 409 not_first_round. |
| `test_thiefs_reflexes_surprised` | `surprised: true` → 409. |
| `test_thiefs_reflexes_not_in_initiative` | Thief absent from initiative → 409 not_in_initiative. |
| `test_thiefs_reflexes_level_gate` | Pip at Lv 7 → 409 wrong_subclass_or_level. |
| `test_thiefs_reflexes_missing_character_id` | No character_id → 400. |
| `test_thiefs_reflexes_unknown_character` | Unknown char → 404. |

### `test_natures_sanctuary.py`
v2.1017.0 — Nature's Sanctuary (Circle of the Land Druid Lv 14+, PHB p.69). `POST /use_natures_sanctuary` installs a `natures-sanctuary` buff (`effects.dc` = druid spell save DC); the `/npc_attack` gate makes a beast/plant attacker WIS-save-or-auto-miss. Mira Greenleaf (demo Druid) is PATCHed to Circle of the Land Lv 14.

| Test | What it asserts |
|------|-----------------|
| `test_natures_sanctuary_installs_buff` | Lv 14 → a `natures-sanctuary` buff with `effects.dc` = the returned DC. |
| `test_natures_sanctuary_blocks_beast_attack` | A beast attacker (loop until a failed save) → `natures_sanctuary_blocked` True + `hit` False. |
| `test_natures_sanctuary_ignores_humanoid` | A humanoid attacker is never blocked (beast/plant restriction). |
| `test_natures_sanctuary_level_gate` | Mira as Moon Lv 5 → 409 wrong_subclass_or_level. |
| `test_natures_sanctuary_missing_character_id` | No character_id → 400. |
| `test_natures_sanctuary_unknown_character` | Unknown char → 404. |

### `test_tranquility.py`
v2.1016.0 — Tranquility (Way of the Open Hand Monk Lv 11+, PHB p.79). `POST /use_tranquility` installs the shared `sanctuary` buff carrying `effects.dc = 8 + WIS + prof` (so the existing attacker-must-Wis-save gate enforces it). Kael Brightleaf (demo Open Hand Monk) is PATCHed to Lv 11.

| Test | What it asserts |
|------|-----------------|
| `test_tranquility_installs_sanctuary` | Lv 11 → a `sanctuary` buff with `effects.dc` = 8 + WIS mod + prof and `sanctuary_attacker_must_save` True. |
| `test_tranquility_level_gate` | Kael at Lv 5 → 409 wrong_subclass_or_level. |
| `test_tranquility_missing_character_id` | No character_id → 400. |
| `test_tranquility_unknown_character` | Unknown char → 404. |

### `test_fiendish_resilience.py`
v2.1015.0 — Fiendish Resilience (The Fiend Warlock Lv 10+, PHB p.110). `POST /use_fiendish_resilience` installs a stable-key `fiendish-resilience` buff carrying `effects.resistance_to: [damage_type]` (the shared `_resistance_halve` substrate), re-pickable. Magnus Hexbinder (demo Fiend Warlock) is PATCHed to Lv 10.

| Test | What it asserts |
|------|-----------------|
| `test_fiendish_resilience_grants_resistance` | Pick fire → buff with `resistance_to: ["fire"]`, `buff_installed` True. |
| `test_fiendish_resilience_repick_replaces` | fire → cold flips `resistance_to` (single buff by stable key). |
| `test_fiendish_resilience_bad_damage_type` | Non-RAW damage type → 400. |
| `test_fiendish_resilience_level_gate` | Magnus at Lv 5 → 409 wrong_subclass_or_level. |
| `test_fiendish_resilience_missing_character_id` | No character_id → 400. |
| `test_fiendish_resilience_unknown_character` | Unknown char → 404. |

### `test_quivering_palm.py`
v2.1014.0 — Quivering Palm (Way of the Open Hand Monk Lv 17+, PHB p.79). `POST /use_quivering_palm` two-mode: `setup` spends 3 ki + installs a `quivering-palm` buff on the target; `trigger` resolves the target's CON save → drop-to-0 on fail / 10d10 necrotic on success, consuming the buff. Kael Brightleaf (demo Open Hand Monk) is PATCHed to Lv 17; trigger passes `override` for the action chip.

| Test | What it asserts |
|------|-----------------|
| `test_quivering_palm_setup_spends_ki_and_marks_target` | Setup → 3 ki spent (`ki_remaining` 7 of 10) + the `quivering-palm` buff on the target. |
| `test_quivering_palm_trigger_resolves_save` | Trigger resolves the CON save: fail → target at 0 HP / removed; success → 10d10 necrotic; buff consumed either way. |
| `test_quivering_palm_trigger_without_setup` | Trigger with no vibrations → 409 not_set_up. |
| `test_quivering_palm_setup_out_of_ki` | Setup with < 3 ki → 409 out_of_uses. |
| `test_quivering_palm_level_gate` | Kael at Lv 5 → 409 wrong_subclass_or_level. |
| `test_quivering_palm_bad_mode` | mode not in {setup, trigger} → 400. |
| `test_quivering_palm_missing_target` | No target_combatant_id → 400. |
| `test_quivering_palm_unknown_character` | Unknown char → 404. |

### `test_peerless_skill.py`
v2.1013.0 — Peerless Skill (College of Lore Bard Lv 14+, PHB p.55). `POST /use_peerless_skill` spends a Bardic Inspiration use and rolls 1d{BI die} to add to the caster's own ability check (no economy chip). Lyra Sunstrider (demo Lore Bard) is PATCHed to Lv 14.

| Test | What it asserts |
|------|-----------------|
| `test_peerless_skill_adds_bonus` | Lv 14 → `die_size` 10, `bonus` 1-10, resource drops by one, `peerless-skill` broadcast carries the rolled bonus. |
| `test_peerless_skill_out_of_uses` | 0 Bardic Inspiration → 409 out_of_uses. |
| `test_peerless_skill_level_gate` | Lyra at Lv 6 → 409 wrong_subclass_or_level. |
| `test_peerless_skill_missing_character_id` | No character_id → 400. |
| `test_peerless_skill_unknown_character` | Unknown char → 404. |

### `test_retaliation.py`
v2.1012.0 — Retaliation (Path of the Berserker Barbarian Lv 14+, PHB p.49). `POST /use_retaliation` resolves a reaction melee weapon attack vs a creature that damaged you (roll vs AC, apply weapon damage on a hit, crit-doubling). Krieger Stonefist (demo Berserker) is PATCHed to Lv 14; the reaction chip is bypassed with `override`.

| Test | What it asserts |
|------|-----------------|
| `test_retaliation_hits_and_damages` | Loops (reaction overridden) until a hit lands; a hit applies damage; `target_ac` resolved. |
| `test_retaliation_reaction_economy` | A first override call marks the reaction chip → a second call echoes `over_budget` True. |
| `test_retaliation_level_gate` | Krieger at Lv 7 → 409 wrong_subclass_or_level. |
| `test_retaliation_missing_character_id` | No character_id → 400. |
| `test_retaliation_missing_target` | No target_combatant_id → 400. |
| `test_retaliation_attack_index_out_of_range` | Bad attack_index past the gates → 400. |
| `test_retaliation_unknown_character` | Unknown char → 404. |

### `test_hurl_through_hell.py`
v2.1011.0 — Hurl Through Hell (The Fiend Warlock Lv 14+, PHB p.110). `POST /use_hurl_through_hell` applies 10d10 psychic to a hit target once per long rest (auto-bootstrapped resource), exempting fiends. Magnus Hexbinder (demo Fiend Warlock) is PATCHed to Lv 14.

| Test | What it asserts |
|------|-----------------|
| `test_hurl_through_hell_damages_non_fiend` | Humanoid target → 10-100 psychic applied, `target_is_fiend` False, `uses_remaining` 0. |
| `test_hurl_through_hell_exempts_fiend` | Target flagged `creature_type=fiend` → `damage_rolled`/`damage_applied` 0. |
| `test_hurl_through_hell_once_per_long_rest` | A second hurl before a long rest → 409 out_of_uses. |
| `test_hurl_through_hell_level_gate` | Magnus at Lv 5 → 409 wrong_subclass_or_level. |
| `test_hurl_through_hell_missing_character_id` | No character_id → 400. |
| `test_hurl_through_hell_missing_target` | No target_combatant_id → 400. |
| `test_hurl_through_hell_unknown_character` | Unknown char → 404. |

### `test_overchannel_phase2.py`
v2.1019.0 — Overchannel Phase 2 (Evocation Wizard Lv 14+). Extends the max-damage read to the `/cast_spell` spell-attack-roll damage path (+ `/place_aoe`). Thalindra@Lv14 is given Guiding Bolt (4d6 ranged spell attack).

| Test | What it asserts |
|------|-----------------|
| `test_overchannel_maxes_attack_roll_spell` | Armed cast of Guiding Bolt → on a hit `auto_attack_damage_rolled` == 24 (4d6 maxed; 48 on a nat-20 crit) + a ⚡ Overchannel card fires. |

### `test_overchannel.py`
v2.1010.0 — Overchannel (Evocation Wizard Lv 14+, PHB p.117). `POST /use_overchannel` arms a one-shot `overchannel-armed` buff; the next damaging 1st-5th level spell has every damage roll maxed by the `/cast_spell` NPC-auto-damage sites (via `_max_dice_total`), then the buff drops and the escalating necrotic self-damage (`use_number × spell_level` d12, ignoring resistance) hits the caster on the 2nd+ use since a long rest. Thalindra (demo Evocation Wizard) is PATCHed to Lv 14 for the happy paths.

| Test | What it asserts |
|------|-----------------|
| `test_overchannel_maxes_fireball_and_is_free_first_use` | Arm (use #1) → Fireball's `auto_save_damage_rolled` is maxed to 48 (8d6); `overchannel` payload reports maxed + use #1 + zero self-damage; ⚡ card broadcast. |
| `test_overchannel_second_use_applies_self_damage` | Use #2 → `self_damage_expr` = 6d12 (2×3), `self_damage_applied` > 0, caster HP drops. |
| `test_overchannel_buff_is_one_shot` | The armed buff is consumed by one cast — a second cast without re-arming has no `overchannel` payload. |
| `test_overchannel_level_gate` | Thalindra at Lv 7 → 409 wrong_subclass_or_level. |
| `test_overchannel_missing_character_id` | No character_id → 400. |
| `test_overchannel_unknown_character` | Unknown char → 404. |
| `test_overchannel_wrong_class` | Krieger (Barbarian) → 409. |

### `test_overchannel_self_damage.py`
v2.1010.0 — unit tests of `_overchannel_self_damage_expr(use_number, spell_level)`: the first overchannel since a long rest is free (`""`); the n-th use (n≥2) returns `n × spell_level` d12. Guarded with a fastapi import-skip; runs in CI / docker.

| Test | What it asserts |
|------|-----------------|
| `test_first_use_is_free` | use 1 → `""`. |
| `test_second_use_level_1` | use 2, L1 → 2d12. |
| `test_second_use_level_3_fireball` | use 2, L3 → 6d12. |
| `test_third_use_level_3` | use 3, L3 → 9d12. |
| `test_second_use_level_5_max` | use 2, L5 → 10d12. |
| `test_third_use_level_5` | use 3, L5 → 15d12. |
| `test_zero_use_number_is_free` / `test_zero_spell_level_is_free` / `test_bad_inputs_are_free` | 0 / bad inputs → `""`. |

### `test_champion_survivor_regen.py`
v2.1009.0 — Champion Fighter Survivor (Lv 18+): `_pc_champion_survivor_regen(sheet)` returns `5 + CON mod` for a Lv 18+ Champion (single-class or multiclass), `None` otherwise. The turn-advance start-of-turn hook applies the regen when the active Champion is at `0 < current ≤ max // 2` HP. In-process unit tests of the pure helper (no Lv-18 PC exists in the demo). Guarded with a fastapi import-skip; runs in CI / docker.

| Test | What it asserts |
|------|-----------------|
| `test_champion_below_18_no_regen` | Lv 17 Champion → None (has Superior Critical, not yet Survivor). |
| `test_champion_lv18_regen_5_plus_con_mod` | Lv 18, CON 14 (+2) → 7. |
| `test_champion_lv20_high_con` | Lv 20, CON 20 (+5) → 10. |
| `test_champion_default_con_is_plus_zero` | CON 10 (+0) → 5. |
| `test_champion_low_con_reduces_regen` | CON 8 (-1) → 4. |
| `test_non_champion_fighter_no_regen` | Lv 20 Battle Master → None (Champion-only). |
| `test_rogue_no_regen` | Lv 20 Rogue → None. |
| `test_none_sheet_no_regen` | `None` sheet → None. |
| `test_bad_level_no_regen` | Non-int level → None (defensive parse). |
| `test_missing_con_defaults_to_plus_zero` | No abilities block → CON 10 (+0) → 5. |
| `test_multiclass_champion_lv18_regen` | Wizard 2 / Fighter-Champion 18, CON 16 (+3) → 8. |
| `test_multiclass_champion_lv17_no_regen` | Wizard 3 / Fighter-Champion 17 → None. |

### `test_superior_critical_threshold.py`
v2.1008.0 — Champion Fighter Superior Critical (Lv 17+): `_attacker_crit_threshold(sheet)` returns 18 (crit on 18-20) at Lv 17+, 19 (Improved Critical) at Lv 3-16, 20 otherwise. In-process unit tests of the pure helper (no Lv-17 PC exists in the demo, so the crit-on-18 floor can't ride the roster like Improved Critical does). Guarded with a fastapi import-skip; runs in CI / docker.

| Test | What it asserts |
|------|-----------------|
| `test_champion_below_3_stays_at_20` | Lv 2 Champion → threshold 20 (pre-Improved-Critical). |
| `test_champion_lv3_improved_critical_19` | Lv 3 Champion → 19. |
| `test_champion_lv16_still_improved_critical_19` | Lv 16 Champion → 19 (just below the Superior Critical floor). |
| `test_champion_lv17_superior_critical_18` | Lv 17 boundary → 18. |
| `test_champion_lv20_superior_critical_18` | Lv 20 Champion → 18. |
| `test_non_champion_fighter_stays_at_20` | Lv 17 Battle Master → 20 (Superior Critical is Champion-only). |
| `test_rogue_stays_at_20` | Lv 17 Rogue → 20. |
| `test_none_sheet_defaults_to_20` | `None` sheet → 20. |
| `test_bad_level_defaults_to_20` | Non-int level → 20 (defensive parse). |
| `test_multiclass_champion_lv17_superior_critical_18` | Wizard 3 / Fighter-Champion 17 multiclass → 18. |
| `test_multiclass_champion_lv3_improved_critical_19` | Wizard 5 / Fighter-Champion 3 → 19. |
| `test_multiclass_champion_lv16_stays_19` | Wizard 2 / Fighter-Champion 16 → 19. |
...
### `test_settings_map_controls.py`
v2.1006.0 — per-user tabletop map-control scheme. `POST /api/settings/map_controls` flips between the default scroll-to-pan scheme (bare wheel pans X/Y, Shift+wheel pans X, Ctrl+wheel zooms at the cursor; trackpad pinch = ctrlKey wheel) and "Alternative controls" (the pre-v2.1006.0 bare-wheel-zooms behavior). The tabletop reads `ME.altControls`; right-drag / left-drag-on-empty panning works in both schemes. Companion Playwright coverage: `tests/harness_ui/test_map_pan.py` (wheel pans / Shift+wheel pans X / Ctrl+wheel zooms) + the supersample test now zooms via Ctrl+wheel.

| Test | What it asserts |
|------|-----------------|
| `test_map_controls_alternative_on_then_off` | POST `{"alternative": true}` → 200 + `alternative_controls == true`; flipping back persists. |
| `test_map_controls_persists_for_player` | Per-user isolation — Alice's scheme is independent of the GM's; cleanup restores the default. |
| `test_map_controls_missing_field_422` | Error path — a body without `alternative` → 422. |
| `test_settings_page_renders_map_controls_and_groups` | `/settings` renders the Map controls section + the v2.1006.0 group reorganization (Appearance / Tabletop & controls / Gameplay / Audio headers). |
| `test_tabletop_bootstraps_alt_controls_flag` | The tabletop page's `ME` bootstrap carries `altControls`. |
...
| `test_war_caster_auto_casts_at_provoker` | v2.643.0 — resolving `take-war-caster-cast` with a chosen `spell_index` (Sacred Flame, discovered via `sheet-json`) auto-casts that 1-action spell at the provoker through the real `/cast_spell` pipeline: a `spell_cast` card for Sacred Flame fires, a `feature_used(source=war-caster-autocast)` trail fires, Tavik's **reaction** slot flips True (via cast_spell's `as_reaction` flag), and his **action** slot is NOT consumed. |
| `test_mage_slayer_auto_attacks_caster` | v2.644.0 — Magnus casts within 5 ft of Krieger (Mage Slayer); resolving `take-mage-slayer-strike` with a chosen melee `attack_index` (discovered via `sheet-json`) auto-attacks the caster through the real `/attack` pipeline: a `weapon_attack` card from Krieger fires, a `feature_used(source=mage-slayer-autostrike)` trail fires, Krieger's **reaction** slot flips True (via /attack's `as_reaction` flag), and his **action** slot is NOT consumed. |
| `test_lucky_vs_lucky_cancel` | v2.648.0 — Lucky vs Lucky (PHB p.167). Garrik (native Lucky) attacked by Krieger (PATCHed to also have Lucky); resolving Garrik's `use-lucky` with `params.cancel=true` → `lucky_cancel=true`, both `charges_after` + `attacker_charges_after` = 2 (3→2 each), `feature_used(source=lucky-cancel)`, and `reaction_prompt_resolved`. Restores Krieger's feats/resources on teardown. |
...
| `test_cast_shield_auto_negates_in_band_hit` | v2.600.0 — v3 auto-resolution. With `auto_apply_damage` on, Krieger swings on Thalindra until a non-crit hit lands in the negation band (`target_ac <= attack_total < target_ac + 5`, restoring full HP between probes); casting Shield via `/use_reaction` then retroactively restores the full applied damage — asserts `feature_used(source=shield-negate)` with `heal_back == damage_applied` + `character_hp_update(source=shield-negate)` whose `delta == damage_applied`. A crit or an at-or-above-AC+5 hit is left advisory (unchanged). |
...
| `test_defensive_duelist_auto_negates_in_band_hit` | v2.601.0 — v3 auto-resolution (same recipe as the v2.600.0 Shield negation, +PB AC instead of +5). With `auto_apply_damage` on, Krieger swings on Lyra until a non-crit hit lands in her +3 negation band (`target_ac <= attack_total < target_ac + 3`, healing to full between probes); using Defensive Duelist then restores the full applied damage — asserts `feature_used(source=defensive-duelist-negate)` with `heal_back == damage_applied` + `character_hp_update(source=defensive-duelist-negate)` whose `delta == damage_applied`. |
...
v2.68.0 — GM Reactions Panel (see [`docs/plans/reactions-automation.md`](plans/reactions-automation.md)). New `GET /available_reactions` + `POST /spend_reaction_manual` endpoints surface every combatant's reaction catalog to the GM and let the GM flip any reaction chip with one click. PC class features (Uncanny Dodge / Cutting Words / Indomitable), PC feats (Sentinel / Polearm Master / etc.), PC reaction spells (Shield / Counterspell / etc. via `casting_time` scan), NPC monster reactions (Parry / etc. via `category == "reaction"` walk), and (v2.642.0) PC item reactions via an equipped-`sheet.inventory[*]._reactions[]` walk.

| Test | What it asserts |
|------|-----------------|
| `test_available_reactions_lists_pc_class_features` | Pip (Rogue Lv 7) catalog contains `uncanny-dodge` + `reaction_used: false`. |
| `test_available_reactions_lists_item_reaction` | v2.642.0 — Lyra's equipped Cloak of Displacement surfaces `item-cloak-displacement-advantage` (kind=`item`, source names the item) via the new inventory `_reactions` walk. |
| `test_spend_item_reaction_manual` | v2.642.0 — POST `/spend_reaction_manual` with the item-reaction key → 200, `economy_update` flips Lyra's reaction, `feature_used(source=manual-reaction)` fires (generic dispatch, no per-kind path). |
| `test_item_reaction_charge_decrement` | v2.647.0 — reaction-item charge tracking. Lyra's Ring of Spell Turning (`cost_charges: 1`) pinned to 2 charges, then spent via `/spend_reaction_manual` (battle re-seeded between spends to reset the slot): `charges_remaining` 2→1→0 on response + a `feature_used(charges_remaining=1)` broadcast, and a 4th spend → `409 out_of_charges`. |
...
| `test_fog_cloud_with_center_places_fog_emitter` | v2.711.0 — cast with `center: {x,y}` → an `emitter_id` + a `fog` light emitter at that point with the scaled radius + `caster_char_id` (auto-place-on-cast). |
| `test_fog_cloud_no_center_no_emitter` | Cast without `center` → `emitter_id` None + no emitter (backward compat). |

### `test_cast_daylight.py`
v2.713.0 — new `/cast_daylight` endpoint (RAW L3 Evocation, 60 ft, 1 hour non-concentration, fixed 60-ft-radius bright-light sphere) + auto-placed Phase-3 `daylight` emitter. Brother Tavik Stonebrow (Cleric) armed with Daylight + an L1-L9 slot table.

| Test | What it asserts |
|------|-----------------|
| `test_daylight_happy` | Cast → 200; `radius_ft == 60`, `range_ft == 60`, `concentration is False`. |
| `test_daylight_with_center_places_emitter` | Cast with `center` → a non-concentration `daylight` emitter (radius 60) at that point. |
| `test_daylight_no_center_no_emitter` | Cast without `center` → no emitter. |
| `test_daylight_missing_character_id_400` | Missing character_id → 400. |
| `test_daylight_low_slot_400` | slot_level=2 → 400 (Daylight is L3). |
| `test_daylight_wrong_class_409` | class_slug=wizard → 409 `wrong_class`. |
| `test_daylight_spell_not_known_409` | Cleric without Daylight on his list → 409 `spell_not_known`. |

### `test_cast_darkness.py`
v2.712.0 — new `/cast_darkness` endpoint (RAW L2 Evocation, 60 ft, concentration, no save, fixed 15-ft-radius sphere of magical darkness) + auto-placed Phase-3 `darkness` emitter. Thalindra Moonwhisper (Wizard) armed with Darkness + an L1-L9 slot table (snapshot + restored).

| Test | What it asserts |
|------|-----------------|
| `test_darkness_happy` | Cast → 200; `radius_ft == 15`, `range_ft == 60`, `concentration is True`. |
| `test_darkness_with_center_places_emitter` | Cast with `center` → an `emitter_id` + a `darkness` emitter (radius 15) at that point with `caster_char_id`. |
| `test_darkness_no_center_no_emitter` | Cast without `center` → `emitter_id` None + no emitter. |
| `test_darkness_missing_character_id_400` | Missing character_id → 400. |
| `test_darkness_low_slot_400` | slot_level=1 → 400 (Darkness is L2). |
| `test_darkness_wrong_class_409` | class_slug=cleric → 409 `wrong_class`. |
| `test_darkness_spell_not_known_409` | Wizard without Darkness on her list → 409 `spell_not_known`. |
...
### `test_vision_light_phase0.py`
v2.704.0 — Vision & Light Phase 0 (docs/plans/vision-and-light.md): the lighting data model (schema v82). `Map.ambient_light` (bright/dim/dark, default bright) + token `light_bright_ft`/`light_dim_ft` (default 0/0), GM set-endpoints, surfaced on `GET /tokens`. No combat behavior yet.

| Test | What it asserts |
|------|-----------------|
| `test_defaults_preserve_status_quo` | A freshly-placed token reports `light_bright_ft`/`light_dim_ft` == 0 and `GET /tokens` `ambient_light` == "bright". |
| `test_patch_token_light_radii` | `PATCH /token/{id}` with `light_bright_ft: 20`, `light_dim_ft: 999` → response + `GET /tokens` show 20 / 240 (clamped to [0, 240]). |
| `test_set_map_ambient_light` | `POST /settings/maps/{id}/ambient_light` `"dark"` → 200, persisted, `map_ambient_light` broadcast, reflected in `GET /tokens`; invalid `"neon"` → falls back to `"bright"`. |
| `test_non_gm_cannot_set_ambient_light` | Alice (non-GM) → 403. |

### `test_vision_light_phase1.py`
v2.705.0 — Vision & Light Phase 1: the `_visibility_between` resolver, exposed read-only at `GET /api/campaign/{cid}/visibility`. Attacker Pip + target Garrik 5 ft apart; senses ride combatant buffs. No `/attack` wiring yet.

| Test | What it asserts |
|------|-----------------|
| `test_bright_ambient_seen` | Default bright map → `illumination == "bright"`, `visibility == "seen"`, `range_ft == 5`. |
| `test_dark_no_senses_unseen` | Dark ambient + no senses → `visibility == "unseen"`. |
| `test_dark_with_darkvision_obscured` | Dark + `darkvision_ft: 60` buff (≥ range) → `illumination` reads as `"dim"`, `visibility == "obscured"`. |
| `test_darkvision_beyond_range_unseen` | Dark + no usable darkvision → still `unseen`. |
| `test_dark_with_truesight_seen` | Dark + `truesight_ft: 60` → `seen` regardless of light. |
| `test_dark_with_devils_sight_seen` | Dark + `sees_in_darkness` (Devil's Sight) → dark reads as bright → `seen`. |
| `test_dark_target_lit_seen` | Dark map but the target carries a `light_bright_ft: 20` torch → its square is bright → `seen` to a senseless attacker. |

### `test_vision_light_phase2.py`
v2.706.0 — Vision & Light Phase 2: `_visibility_between` wired into the PC `/attack` pipeline via `_attack_vision_edges`. Attacker Pip + target Garrik 5 ft apart; senses ride combatant buffs.

| Test | What it asserts |
|------|-----------------|
| `test_dark_target_unseen_disadvantage` | Dark map, Pip (no senses) can't see Garrik (darkvision) → `roll_state_applied == "disadvantage_cant_see"` (no manual flag). |
| `test_dark_unseen_attacker_advantage` | Dark map, Pip (darkvision) sees Garrik but Garrik (no senses) can't see Pip → `advantage_unseen_attacker`. |
| `test_dark_mutual_blind_cancels` | Dark map, neither has senses → adv + dis cancel → `canceled_…unseen_attacker…cant_see`. |
| `test_dark_both_darkvision_no_edge` | Dark map, both have darkvision (≥ range) → both obscured → no `cant_see`/`unseen_attacker` edge. |
| `test_bright_map_no_edge` | Default bright map → the helper short-circuits → no lighting edge. |
| `test_manual_override_still_forces_disadvantage` | Bright map + manual `attacker_cant_see_target` flag → `disadvantage_cant_see` still applies. |

### `test_vision_light_npc_attack.py`
v2.707.0 — Vision & Light Phase 2 NPC mirror: `_visibility_between` wired into `/npc_attack` via `_npc_attack_vision_edges`. NPC attacker carries a real created token for its position; target Garrik; senses ride combatant buffs.

| Test | What it asserts |
|------|-----------------|
| `test_npc_cant_see_target_disadvantage` | Dark map, NPC (no senses) can't see Garrik (darkvision) → `disadvantage_cant_see`. |
| `test_npc_unseen_attacker_advantage` | Dark map, NPC (darkvision) sees Garrik but Garrik (no senses) can't see the NPC → `advantage_unseen_attacker`. |
| `test_npc_bright_map_no_edge` | Default bright map → the helper short-circuits → no lighting edge. |

### `test_vision_light_phase3.py`
v2.708.0 — Vision & Light Phase 3: spell-placed light/darkness/fog emitters read by `_illumination_at_point`. Validated via `/visibility`. Attacker Pip, target Garrik; the emitter is centered on Garrik.

| Test | What it asserts |
|------|-----------------|
| `test_darkness_blocks_darkvision` | A `darkness` emitter → `illumination == "magical_dark"`, a darkvision attacker is `unseen` (darkvision doesn't pierce). |
| `test_darkness_pierced_by_devils_sight` | Devil's Sight (`sees_in_darkness`) → `seen` through magical darkness. |
| `test_daylight_lights_dark_map` | A `daylight` emitter on a dark map → `illumination == "bright"`, `seen`. |
| `test_fog_blocks_darkvision` | A `fog` emitter → `illumination == "fog"`, a darkvision attacker is `unseen`. |
| `test_fog_pierced_by_truesight` | Truesight → `seen` through fog. |
| `test_place_bad_kind_400` | `POST /light_emitter` with an unknown kind → 400. |
| `test_place_bad_radius_400` | `radius_ft: 0` → 400. |
| `test_remove_unknown_404` | `DELETE /light_emitter/{unknown}` → 404. |
| `test_non_gm_cannot_place` | Alice (non-GM) placing an emitter → 403. |

### `test_vision_light_phase4.py`
v2.709.0 — Vision & Light Phase 4: Hide/Stealth via `POST /hide` + the unseen-attacker advantage + reveal-on-attack. Bright map; attacker Pip, target Garrik 5 ft apart in an active battle.

| Test | What it asserts |
|------|-----------------|
| `test_hide_installs_score` | `POST /hide` returns the installed `stealth_score`. |
| `test_hidden_target_unseen_to_low_perception` | Hidden target (Stealth 99) → `/visibility` `unseen` + `hidden: true`. |
| `test_hidden_target_seen_when_perception_beats_stealth` | Poor hide (Stealth 1) beaten by Pip's passive Perception → `seen`. |
| `test_truesight_defeats_hide` | Truesight → `seen` regardless of Stealth. |
| `test_hidden_attacker_advantage_then_revealed` | Hidden Pip attacks → `advantage_unseen_attacker`; revealed after (Garrik then sees Pip; 2nd swing has no unseen edge). |
| `test_hide_unknown_combatant_404` | `/hide` unknown combatant → 404. |
| `test_hide_missing_combatant_id_400` | `/hide` without combatant_id → 400. |
| `test_hide_non_gm_403` | Alice (non-GM) → 403. |
...
### `test_gif_token_src_stable.py`
v2.1047.2 — an animated-GIF token's `<img>` `src` must not be reassigned on every render. `_updateGifOverlay` guarded the write with `img.src !== t.image_url`, but the `src` *property* getter returns the resolved absolute URL (`http://host/static/…`) while `t.image_url` is root-relative — so the comparison was **always true** and `src` was rewritten at the tail of every `render()` (60+ call sites: drag, pan, zoom, every inbound token WS update), which re-runs the image-update algorithm and restarts the animation. The fix compares `getAttribute('src')`.

Needs uploads enabled (attaching a GIF to a token only happens through the upload endpoint); skips otherwise. Places and deletes its own token for **Garrik Ironside** — deliberately outside the demo's tokenized-six lineup — so the map the visual-regression snapshots capture is untouched.

**Two traps this file hit, both of which produced a vacuously-passing test:** driving renders with synthetic wheel events never reached `render()` at all (the negative control passed against the *unfixed* code), so the test now drives six real token-move WS updates instead; and `locator("#vtt-canvas").hover()` fails actionability because the Maps 2.0 `#wall-overlay` SVG sits above the canvas. The test therefore also asserts the overlay actually **repositioned**, so a zero-write result can only mean the guard held rather than that the function never ran.

| Test | What it asserts |
|------|-----------------|
| `test_gif_token_src_not_rewritten_on_every_render` | The overlay `<img>` carries the root-relative URL as its *attribute*; its `src` *property* resolves to a different (absolute) string — the premise the old guard broke on; and after six token-move renders the instrumented `src` setter recorded **zero** writes. Verified in both directions: 6 writes against the unfixed guard, 0 against the fix. |

### `test_init_monster_row_sheet.py`
v2.1044.1 — opening a monster's full stat block from its init-tracker row. Adds a **double-click** affordance (matching the map token's) *without* taking the primary click, which still expands the inline mini-sheet; and makes an **orphan** row (whose expanded card is only the "Template missing — open the full monster sheet via the 📋 button above" placeholder) open the sheet on the first click instead. The real risk guarded here is the interaction between the two: the dblclick handler has to undo the toggle that the first click of the pair already applied, or rows end up randomly expanded.

**Two fixture traps worth knowing** (both cost a debugging round when this file was written): the seeded battle uses `active: False`, because an active battle makes the GM auto-expand-on-turn-change open the row during render; and clicks target `.mini-header-name`, not `.mini-header`, whose centre lands on the econ-chip `<button>`s that both handlers deliberately ignore. Also, `#monster-sheet-drawer`'s inline style sets both `display:none` and `display:flex` (the later wins), so it is *always* "visible" to Playwright — the drawer's real open signal is `#monster-sheet-backdrop` plus the iframe `src`, which is what `_expect_drawer_open` asserts.

| Test | What it asserts |
|------|-----------------|
| `test_monster_row_is_stamped_with_template_id` | The row renders with `data-monster-tid` and carries the `a.monster-sheet-link` anchor the handlers synthesize a click on. |
| `test_single_click_still_expands_inline_and_does_not_open_drawer` | One click adds `.open` to `.init-card-sheet` and leaves the drawer closed — the pre-existing behavior survives. |
| `test_double_click_opens_sheet_drawer_and_preserves_expand_state` | Double-click opens the drawer (backdrop visible + iframe pointed at `/monster-template/{id}/sheet`) and leaves the expand state exactly as it was. |
| `test_orphan_row_opens_sheet_on_first_click` | With `data-monster-orphan="1"` set (read at click time by the real handler), one click opens the drawer and does *not* expand the placeholder card. |
...
> **v2.1033.7 (B17) — the `hermetic_pcs` autouse fixture heals the sheet-pollution cascade.** CI run `29867270217` (post-fix) confirmed the full-serial `harness` job dropped from **4715/4829 failing (~97%)** to **51 failed / 4754 passed** — the cascade is gone. The `hermetic_pcs` fixture in `conftest.py` snapshots each demo PC's pristine mutable sheet fields on the first test and restores any that drifted before every subsequent test, so a leaked `spells`/`resources`/`spell_slots`/`attacks` strip no longer cascades. It does **not** cover class-scoped `level`/`subclass` drift, battle-state leaks, or buff-key leaks — those are the residual ~51, triaged into five classes in [`BUGS.md` B18](../../BUGS.md). Re-run the rows below before assuming they still flake; remove any that no longer reproduce.
