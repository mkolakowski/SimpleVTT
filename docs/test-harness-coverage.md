...
v2.1048.0 — Procedural map generator. `POST /campaign/{cid}/settings/maps/generate` draws a seeded dungeon (rooms + corridors + doors) server-side with Pillow and creates a Map row from it. Returns `{ok, map: {id, name, image_url}}`. GM-only.

| Test | What it asserts |
|------|-----------------|
| `test_generate_creates_map` | 200 + `{ok, map}` shape, `image_url` under `/static/uploads/maps/…png`, and the PNG is actually served (`\x89PNG` magic); cleaned up. |
