...
v2.1048.0 — Procedural map generator. `POST /campaign/{cid}/settings/maps/generate` draws a seeded dungeon (rooms + corridors + doors) server-side with Pillow and creates a Map row from it. Returns `{ok, map: {id, name, image_url, walls, doors}}`. GM-only. v2.1049.0 added functional walls: the generated map populates `Map.walls` (Maps 2.0 LOS format) with solid segments + toggleable door segments.
...
| `test_generate_default_name` | No name → `"Generated Medium Dungeon"`. |
