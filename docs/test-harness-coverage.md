...
> **v2.1033.7 (B17) — the `hermetic_pcs` autouse fixture now heals most sheet-pollution flakes.** Several rows below are "PC sheet polluted — lost a class feature / spells / resource / level." The `hermetic_pcs` fixture in `conftest.py` snapshots each demo PC's pristine mutable sheet fields on the first test and restores any that drifted before every subsequent test — so a leaked `spells`/`resources`/`spell_slots`/`attacks` strip no longer cascades. It does **not** cover class-scoped `level`/`subclass` drift (left to those tests' restore-in-finally). Re-run the rows below against a full-serial CI run before assuming they still flake; remove any that no longer reproduce.
