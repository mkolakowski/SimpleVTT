...
### `test_ci_uploads_enabled.py`
v2.1047.1 — CI must exercise the upload endpoints rather than 403 past them. `docker-compose.yml` defaults `DEMO_DISABLE_UPLOADS` to `true` (`${DEMO_DISABLE_UPLOADS:-true}`) and the gate fires whenever that and `DEMO_MODE` are both on. The harness workflow needs `DEMO_MODE` for the seeded demo PCs, so before this fix all four CI jobs answered `403 "Uploads are disabled on this demo instance"` for every upload — silently voiding 9 upload-exercising harness files plus two admin-center storage tests. Config-parsing tests (no HTTP), in the same spirit as `test_fail2ban_compose_service.py`.
