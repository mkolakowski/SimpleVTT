...
**Total tests:** 2472 in `tests/harness/` + 74 in `tests/harness_ui/` (as of v2.183.6, 2026-06-12).
...
Phase 3a — buff *effect* validation (not just install). Where Phases 2F / 2F-2 prove a buff lands on the target, this proves the installed buff's mechanical effect is actually applied during play — by the exact amount RAW declares. Covers the two auto-applied attack-roll uplifts: Bless (+1d4, attacker key `bless`) and Bane (−1d4, attacker key `baned`), wired into `/attack`'s d20 expression via `_attacker_has_bless` / `_attacker_has_bane`. The check is exact, not "token appears": the attacker (Krieger) is dropped into a fresh battle twice under one dice seed — once with the buff pre-seeded into its combatant (PUT /battle), once without — and the attack-total delta (which isolates the buff die, since the same seed holds the d20 + flat bonus constant) must equal exactly the d4 value the engine printed in the buffed breakdown, with the registry sign. Pre-seeding the buff avoids any save-fail loop. The save-side Bless/Bane and AC/speed buffs are filed for later Phase 3 slices.

| Test | What it asserts |
|------|-----------------|
| `test_bless_bane_attack_uplift_contribution_is_exact` | Bless (+1d4) and Bane (−1d4): the with-buff vs. without-buff attack-total delta equals exactly the printed d4 value × the registry sign (same-seed pair), and the breakdown shows the d4 with the correct sign. |
