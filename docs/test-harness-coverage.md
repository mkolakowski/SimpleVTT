...
**Total tests:** 2681 in `tests/harness/` + 83 in `tests/harness_ui/` (as of v2.246.0, 2026-06-13).
...
| `test_ioun_variant_exposes_effective_ability` (×3, v2.225.0; v2.245.0 dropped STR, v2.246.0 dropped CON) | Parametrized over three ability variants — Dexterity (Caelan 10→12), Wisdom (Krieger 13→15), Charisma (Rowan 8→10) — asserts each PC's `derived.effective_abilities.<ABILITY>` reports the pure-additive `base + 2` with an "Ioun Stone" source. (The STR variant on Lyra was dropped in v2.245.0 and the CON variant on Brakka in v2.246.0, each when the stone was detuned to free a 3rd attunement slot for a ring; STR stays covered by three Belts of Giant Strength + Gauntlets of Ogre Power, CON by the Amulet of Health.) Together with the INT primary test, proves the single `ioun-stone` slug + per-item `_ability_bonus` covers all six variants. |
