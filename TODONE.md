...
- ✅ **B1 — Skull-overlay test skipped in CI (missing emoji font)** · was 🟡 P2 · **FIXED (v2.1005.1).** Applied fix path 1: the encounter-sim workflow job installs `fonts-noto-color-emoji` before `playwright install` (deliberately NOT the harness_ui job — its Linux-baked visual baselines were rendered without an emoji font), and the skull-overlay test is un-skipped. Passes locally 3×. (Closed out of BUGS.md v2.1005.1.)
- ✅ **B2 — Two encounter-sim tests skipped (Garrik not tokenized)** · was 🟡 P2 · **FIXED (v2.1005.1).** Neither fix path in the filing was taken — instead of changing the demo seed (visual/roster blast radius), the tests now tokenize Garrik themselves via new `place_token` / `delete_token` helpers in `tests/encounter_sim/helpers/battle.py` (POST `place-token`, cleanup on teardown), so the orphan-cleanup keeps his combatant and the public demo map is untouched. Un-skipping the brawl test also surfaced that its `#battle-round-label` assertions targeted an element removed at v2.49.102 — replaced with `window.battle.round` structural checks. Both tests pass locally 3×. (Closed out of BUGS.md v2.1005.1.)
...
- ✅ **DONE (v2.97.12; reconciled v2.666.1)** — **More pills in the roll log for spells.** The filed P3 ("move spell type, range, action type and details to pills; details as an expanding pill; distinct color from damage pills") shipped end-to-end at v2.97.12: `appendSpellCast` in `app/static/tabletop.js` renders a `.spell-meta-pills` row with pills for school (type), range, casting time (action type), concentration/ritual flags, and an expanding `<details class="spell-meta-pill">` for the description; the server sends `spell_school`/`spell_range`/`spell_casting_time`/`spell_desc`/… (`tabletop_routes.py:~24223`); the CSS (`tabletop.html` inline `<style>`) uses the accent-purple family, distinct from `.result-pill`'s semantic damage/heal colors. Stale-in-backlog only — caught + removed during the v2.658.0–v2.666.x drift-reconciliation sweep.
...
- ✅ **DONE (v2.744.0 "The Pip Counter")** — **Class Resource Tracking in Mini-Sheet.** The tabletop mini-sheet now renders a compact resource row below HP for every countable class/subclass resource (Rage, Ki, Sorcery points, Bardic Inspiration, Channel Divinity, Second Wind, Action Surge, Lay on Hands pool, Wild Shape, Arcane Recovery, …) — auto-populated from the sheet's already-seeded `resources` list, persisted server-side, with +/- steppers wired to the existing `POST /resource` endpoint and the `resource_update` WS so consumption shows live to every client. `_mini_sheet_card.html` + `tabletop.html` + `tabletop.js`. Harness: `test_mini_sheet_resources.py`. (Reconciled v2.744.1: the Rage-specific "active" mechanic the original TODO asked for is **also already shipped** — the mini-sheet's Features tab Rage button routes to `/use_rage`, which installs the active rage buff (bludgeoning/piercing/slashing resistance + damage bonus + STR-check/save advantage) and decrements the use counter; the player ends it via the buff badge × (`/end_buff`), and it auto-ends on KO / a turn with no attack. Covered by `test_use_rage.py` + `test_persistent_rage.py` / `test_mindless_rage.py` / `test_relentless_rage.py` / `test_frenzy_exhaustion.py`. No separate toggle work needed.)

- ✅ **DONE (v2.741.0–v2.742.0)** — **Ability Score Generation** (both methods). **Dice rolling** (v2.741.0 "The Rolled Hero"): a 🎲 Roll 4d6 (drop lowest) button rolls six scores server-side via the dice engine (`POST /api/campaign/{cid}/character/{id}/roll-abilities`), shows each score + its dice, and fills STR→CHA. **Point buy** (v2.742.0 "The Measured Hero"): a 📊 Point buy button in the abilities edit view — 27-point budget, scores 8–15, live remaining + over-15/over-budget clamp (pure client math). Harness: `test_roll_abilities.py` + `test_pointbuy.py`. The optional GM lock/unlock-rerolls toggle also shipped (v2.743.0 "The Locked Dice": `Campaign.ability_rolls_locked` + `POST /ability-rolls-lock`; `test_ability_rolls_lock.py`).
...
- ✅ **DONE (v2.745.0 "The Stat-Block Whisperer")** — **Homebrew Monster "Parse from description" helper** (the optional bonus on the v2.3.8 homebrew-monster-attack-fields item). The structured-action editor (`features_editor.js`) now has a **✨ Parse from description** button on each attack row: it sends the row's pasted SRD-style desc to `POST /api/parse-monster-action`, which regex-extracts `+N to hit` → To-hit, `(NdM + K) <type> damage` → Damage + Type, `DC N <ability> saving throw` → Save DC/ability, plus reach/range, and pre-fills the form (review-before-save). Stateless text parsing. Harness: `test_parse_monster_action.py` (melee, save-based breath weapon, ranged range field, empty → 400).

- ✅ **DONE (v2.730.0 "The After-Action Report")** — **Reporting Page.** A GM-only campaign **activity** report at `/campaign/{cid}/report` (📈 Report Quick Links pill) summarizing session count, event mix, per-session activity timeline, and most-active actors — derived from the `campaign_stat_events` log via `stats_service.campaign_activity_report` (+ a JSON `/api/campaign/{cid}/report`). Complements the per-character combat stats page. Token-move history from the original ask is omitted (moves aren't logged as events). Harness: `test_campaign_report.py`.

- ✅ **DONE (already shipped v2.3.37; verified + regression-guarded v2.729.1)** — **Homebrew Clone.** All six file-based homebrew types (feats / backgrounds / races / subclasses / monsters / classes) have a `POST /campaign/{cid}/custom-<type>/{slug}/clone` endpoint + a 📋 Clone button in the campaign-settings homebrew menu; `_clone_homebrew_record` + `_unique_clone_slug` write a fresh `copy-of-<slug>` record with a "Copy of …" name. Locked in by `tests/harness/test_homebrew_clone.py` (create → clone → the `copy-of-…` record exists with a "Copy of" name + the source's action preserved; unknown source → 404). Stale-in-backlog only.

- ✅ **DONE (v2.729.0 "The Called Initiative")** — **Initiative Tracker Roll Prompt.** Each PC combatant in the init tracker without an initiative gets a GM-only 🎲 Prompt button that sends the owner a roll-request (`stat_key: "initiative"` → DEX mod + `initiative_bonus`) carrying `RollRequest.initiative_combatant_id` (schema v84); the player's response writes the total back as that combatant's initiative, re-sorts the tracker, and broadcasts `battle_update` (the button then disappears). Harness: `test_initiative_prompt.py`.

- ✅ **DONE (already shipped since v2.3.8; verified + regression-guarded v2.728.1)** — **Homebrew Monster Attack Fields → Rollable Attack Buttons.** The homebrew-monster Actions editor (`campaign_settings.html` `data-row-mode="action"` + `features_editor.js`) exposes structured attack fields (`attack_roll` / `attack_bonus` / `damage` / `damage_type` / `save_ability` / `save_dc` / `charges_max`); `_coalesce_monster_actions` persists them; and the shared monster stat-block read-view (`_monster_template_to_sheet` → `sheet["attacks"]` → `_tab_actions.html` → `renderActionButtons`) renders them as clickable 🎯/🎲/📋 buttons firing `/npc_attack` — identical to shipped SRD monsters. Locked in by `tests/harness/test_homebrew_monster_attack_fields.py` (attack + save actions round-trip through `/custom-monsters` → `/api/content/monsters/{slug}` with fields intact). Only the *optional* "Parse from description" auto-fill bonus remains (kept in TODO.md). Stale-in-backlog only.
...
- ✅ **DONE (v2.49.139 → v2.759.2)** — **Frosted-glass treatment across the whole tabletop interface.** The iOS-style frosted-glass look (translucent `color-mix` backing + `backdrop-filter: blur(...) saturate(...)`, with a per-user `--glass-alpha` knob) started on roll-log cards (v2.49.139) and now covers the tabletop — topbar cards, drawer cards, `.init-row`/`.init-entry`, `.gm-panel`, roll/spell cards, the targeting chip, and (v2.759.2) the ruler + AoE-picker hints. All backings use theme-coherent `color-mix` + `var(--accent)` so they adapt across the 9 themes. Optional unbuilt ops follow-up: a "low-detail" toggle to cap compositor layers on very long sessions.

## Maps & Map Editor — shipped

- ✅ **DONE (v2.761.0–v2.763.0)** — **Multi-map encounters** (Map Editor Framework). An encounter groups extra maps via `Encounter.linked_map_ids` (schema v89); the tabletop GM quick-switcher (`GET /map-group` + `POST /switch-map/{id}`, broadcasting `map_change`) flips the active map among them mid-session; the encounter editor in campaign settings has a linked-maps multi-select. Tests: `test_multi_map_encounter.py`, `test_map_switcher.py`, `test_encounter_linked_maps.py`.

- ✅ **DONE (v2.752.0–v2.759.0)** — **Maps 2.0 — Advanced Map Features.** The full advanced-map set: **combat movement locking** (move-endpoint speed cap), **fog of war** (per-user hidden filter + Vision & Light LOS), **walls & doors** — `Map.walls` JSON (v2.752.0), server-side vision occlusion in `_visibility_between` (v2.753.0), the GM `🧱 Walls` editor with draw/delete (v2.754.0) + segment **drag-to-move** (v2.759.0), interactive **door open/close** toggling that updates LOS live (v2.755.0), and **wall-shadow** rendering in the client lighting overlay (v2.757.0) — plus **clickable hotspots** (`Map.hotspots`, the `📍 Spots` editor, description popups + an optional 🎲 roll prompt, v2.756.0/v2.758.0). Tests: `test_map_walls.py`, `test_vision_wall_occlusion.py`, `test_door_toggle.py`, `test_map_hotspots.py`, `test_wall_editor.py`, `test_hotspot_editor.py`, `test_wall_lighting_shadow.py`, `test_wall_drag.py`.
...
- ✅ **DONE (v2.1045.0 "The Sealed Envelope")** — **Resources.** The filed ask (GM uploads documents; players view them inline in the browser with no download; access control over who sees each one) shipped as **document attachments on handouts** rather than a separate `Resource` model: `Handout.file_url` / `file_name` / `file_size` (schema v104) + `POST /api/campaign/{cid}/handouts/upload_file` (GM-only, **PDF only**, ≤ 25 MB, `%PDF-` magic-byte checked, storage-quota checked), rendered by an inline `<iframe>` viewer in the Notes drawer with a "⤢ Full page" escape hatch. Access control is the handout's existing `revealed` / `reveal_to` gate — un-revealed = GM-only, `"all"` = every player, or reveal to named players — so there is one `_can_see_handout` decision, not two. Harness: `test_handout_file.py` (13). ✅ *The access gate closed at v2.1046.0* ("The Locked Reading Room"): `serve_handout_media`, registered ahead of the `/static` mount, authorizes every byte through `_can_see_handout`, so hiding a handout revokes its media and a leaked URL is useless without a session. The URL shape was kept on purpose — the bytes stay on the `uploads_data` volume, so backups and admin-center storage accounting keep working unchanged, and every pre-existing handout is retroactively protected. Harness: `test_handout_media_gate.py` (12). *Not shipped:* non-PDF document types (the inline-rendering guarantee is PDF-specific) and handout **audio**, still filed on the plan.
...
- ✅ **DONE (v2.746.0–v2.751.0)** — **Summon Spells — Creature Picker + Auto-Control + Init Placement.** The full cast-flow UX arc for summon spells: **(#1 picker)** `GET /summon-options` + the `window.openSummonPicker` modal cover all six conjure spells (count-based Animals/Woodland/Minor-Elementals + slot-scaled single Elemental/Fey/Celestial) and Find Familiar's fixed RAW `form` list, wired into the mini-sheet cast flow; **(#2 auto-control)** `_summon_companion` stamps the summon token's `controller_user_id` with the casting character's owner, so the player moves/acts with their summon (no GM hand-off); **(#3 auto-init)** `_summon_initiative_for_body` defaults a summon's initiative to the caster's, so it acts on the caster's turn (RAW PHB p.193). Plan: [`movement-and-summons.md`](docs/plans/movement-and-summons.md). Tests: `test_summon_options.py`, `test_summon_picker.py`, `test_cast_conjure_animals.py` (init + control).

- ✅ **DONE (reconciled v2.745.1)** — **Advantage & Disadvantage Tracking.** All three TODO phases shipped: (1) manual per-character roll-state toggle (`POST /api/campaign/{cid}/character/{id}/roll-state` + `_apply_roll_state` + `_roll_state_pill.html`, Phase 1 v2.2.0); (2) condition automation (Phase 2a–2e, v2.152.0–v2.156.0 — attacker/target condition adv/dis on `/attack`, `/npc_attack`, `/roll`, NPC saves, plus auto-fail STR/DEX from Paralyzed/Stunned/etc. per RAW p.173 cancel logic); (3) context-aware rolls (the one-shot manual adv/dis buttons compose with the toggle + conditions). Plan: [`advantage-disadvantage.md`](docs/plans/advantage-disadvantage.md). Tests: `test_ability_disadvantage_generalized.py`, `test_dodging_disadvantage.py`, `test_exhaustion_disadvantage.py`, `test_race_save_advantage.py`, `test_attack_invisible_target_disadvantage.py`, `test_buff_save_advantage.py`, et al.

- ✅ **DONE (reconciled v2.745.1)** — **Death Saving Throws.** Core shipped v2.1.0 (state machine in `_apply_hp_change`; `/death-save`, `/death-save/override`, `/stabilize` endpoints; `_death_saves_tracker.html` pips on mini + full sheet; "Roll Death Save" through the roll pipeline so it honors adv/dis; healing wakes; massive-damage instant-kill; GM override). Phase 3a turn-start auto-prompt v2.150.0; Phase 3b Medicine-check stabilize v2.151.0. Plan: [`death-saves.md`](docs/plans/death-saves.md). Tests: `test_death_save.py`, `test_death_save_prompt.py`, `test_death_save_drops_concentration.py`. Deferred-by-design (tracked in the plan doc): Phase 3c stable countdown + Phase 4 NPC death saves.

- ✅ **DONE (reconciled v2.745.1)** — **Combat 2.0 — Action Economy Tracking.** The `.econ-chips` strip in each initiative-card mini-header (v2.4.31+) tracks Action / Bonus action / Reaction (boolean, click to mark spent/greyed) + Movement (numeric ft, consumed by grid moves, near/over styling). Casts + attacks auto-mark the right slot via `_mark_battle_economy` / `_markCombatantEconomy` (`_casting_time_to_economy` maps the spell's casting time); state syncs over WebSocket via `pushBattle`; GM can manually toggle any chip; turn advance resets. Substrate: `_mark_battle_economy`, `_is_slot_used`, `_feature_economy_slot`. (Movement-vs-grid distance leans on the shipped grid awareness.)
...
- ✅ **DONE (reconciled v2.751.1)** — **Paladin Aura of Courage (Lv 10).** `_ally_has_aura_of_courage(db, campaign_id, saving_char_id)` walks initiative for a Paladin Lv 10+ and gates the frightened-condition install (skip + `feature_used(source=aura-of-courage)` broadcast), matching the Aura of Devotion pattern. Test: `test_aura_of_courage.py`. (The original TODO bundled a Caelan 7→10 roster bump; the *feature automation* is what shipped here.)
- ✅ **DONE (reconciled v2.751.1)** — **Fighter Indomitable (Lv 9+).** `/use_indomitable` installs a single-use `indomitable`-armed self-buff; the save-roll hook swaps `1d20 → 2d20kh1` and consumes it (RAW-bent v1: advantage rather than post-roll reroll, per B10). `_reroll_detect_indomitable` + the `indomitable` resource. Tests: `test_use_indomitable.py`, `test_use_indomitable_reroll.py`, `test_indomitable_might.py`. (The "IN PROGRESS as v2.56.0" marker was stale.)
